...
\begin{notation}\label{STAU NOT}
	...
	We write
	$S^{\tau} = \phi S \vee J_{\overline{\phi S}}$,
	where the join is 
...
&
	NW(\tau)_{\phi}(S)
&
	NW(S^{\tau} \to \overline{\phi T})_{\phi}(S)
	\ar{l}[swap]{\simeq}
\end{tikzcd}
\end{equation}
induced by the natural maps
$S \to S^{\tau}$ between trees and 
$(S^{\tau} \to \overline{\phi T}) \to \tau$
between necklaces.
\end{remark}
...
\begin{proof}
	...
	we now define the map $NW(\tau) \to X$ via 
	(where $S^{\tau}$ is as in Notation \ref{STAU NOT})
...
\end{equation}
	...
	$
	which sends
	$\boldsymbol{E}(S) \to \Omega[J \xrightarrow{\tau} T] \to X$
	to
	$n (\tau) = 
	(J_{\overline{\varphi S}} \vee \varphi S \to
	\overline{\varphi S})
	=
	(S^{\tau} \to \overline{\varphi S})
	$
	(cf. Notation \ref{STAU NOT}).
	Recall (cf. Remark \ref{STAU REM}) 
	that the natural map
	$n( \tau) \to \tau$ in $\mathsf{Nec}$
	induces isomorphisms
	$NW(n(\tau))_{\mathfrak{s}}(S) 
	\to
	NW(\tau)_{\mathfrak{s}}(S)$.
	...
	$\mathsf{Nec}_{\vect{S}//X}$
	and 
	$NW(\tau)_{\mathfrak{s}}(S) = \emptyset$
	whenever $\tau$ is not in 
	$\mathsf{Nec}_{\vect{S}//X}^{\Omega}$
	one can replace 
	$\mathsf{Nec}_{\vect{S}//X}$
	with 
...
\\
\simeq &
	\underset{(\boldsymbol{E}(S_v) \to \Omega[\tau_{T_v}] \to X)
		\in 
		\prod_{v \in \boldsymbol{V}(S)} \mathsf{Nec}^{\Omega,nor}_{\vect{S_v}//X}}{\colim}
	\left(\prod_{v \in \boldsymbol{V}(S)}
	NW(\tau_{T_v})_{\mathfrak{s}_v}(S_v)\right)
\\
\simeq &
	\prod_{v \in \boldsymbol{V}(S)}
	\left(
	\underset{(\boldsymbol{E}(S_v) \to \tau_{T_v} \to X)
		\in \mathsf{Nec}^{\Omega,nor}_{\vect{S_v}//X}}{\colim}
	NW(\tau_{T_v})_{\mathfrak{s}_v}(S_v)
	\right)
\\
\simeq &
	\prod_{v \in \boldsymbol{V}(S)} 
...
\begin{corollary}[{cf. \cite[Cor. 4.4]{DS11}}]
	...
	$(\tau, S \xrightarrow{\phi} T, \Omega[\tau] \xrightarrow{x} X, J_{\bullet})$ 
	where:
	\begin{enumerate}[label=(\roman*)]
		\item $(J \xrightarrow{\tau} T) \in \mathsf{Nec}$ is a necklace; 
		...
		i.e.
		$J \supseteq \phi(S)$;		
		\item $\Omega[ \tau] \to X$ is a map in $\mathsf{dSet}$
		such that the induced composite
		$\boldsymbol{E}(S) \to 
		\boldsymbol{E}(T) \to X(\eta)$
		is the coloring $\mathfrak{s}$;
		\item $J_{\bullet}$ denotes a tall simplex in $NW(\tau)_{n,\phi}$, i.e.
...
$(\tau,\phi,x,J_{\bullet})$ and
$(\tau^{\**},\phi^{\**},x^{\**},J_{\bullet}^{\**})$
to be equivalent if there is
a map
$\varphi \colon \Omega[\tau] \to \Omega[\tau^{\**}]$
such that
$\phi^{\**} = \varphi \phi$,
$x = x^{\**} \varphi $
and
...
the maps
$\Omega[\tau] \xrightarrow{x} X$ up to degeneracy.
...
\begin{definition}[{cf. \cite[\S 4]{DS11}}]
	Let $J \xrightarrow{\tau} T$ be a necklace and 
	$X \in \mathsf{dSet}$.
	A map $\Omega[\tau] \to X$
	is called \emph{totally non-degenerate}
	if for all beads $T_v$ of $\tau$
	the induced dendrex
	$\Omega[T_v] \to X$
	is non-degenerate.
\end{definition}
...
\[
	\Omega[J \xrightarrow{\tau} T] \to 
	\Omega[J^{\**} \xrightarrow{\tau^{\**}} T^{\**}] \to X
\]
...
\begin{proof}
	...
	$\Omega[\tau_{R}] \to \Omega[\tau_R'] \to X$,
	$\Omega[\tau_S] \to \Omega[\tau_{S}'] \to X$.
	Writing 
	$\tau_R' = (J'_R \to R')$
	and 
	$\tau_S' = (J'_S \to S')$,
...
\begin{remark}
	Suppose $(\tau,\phi,x,J_{\bullet})$
	is a flanked quadruple
	and set $\tau_k = (J_k \to T)$.
...
\begin{lemma}[{cf. \cite[Lemma 4.5]{DS11}}]
	\label{FLANKING LEM}
	\begin{enumerate}[label=(\roman*)]
		\item Any quadruple $(\tau,\phi,x,J_{\bullet})$
...
\begin{corollary}[{cf. \cite[Cor. 4.8]{DS11}}]
	Each quadruple $(\tau,\phi,x,J_{\bullet})$ as in Corollary \ref{NWXREPS COR}
...
\begin{proof}
	...
	$Sc[T] \to \upsilon_{\**}X \xrightarrow{q} \tau_G(X)$.
	...
	of the form $F \colon \upsilon_{\**}X \to Y$ with $Y$ a genuine equivariant operad must also enforce the $\sim$ relation.
...
\begin{proof}
      ...
      \(
      \tau \iota^{\**} \upsilon_{\**}(f) =
      \tau \iota_G^{\**}\upsilon_{G,\**}(f)
      \)
      is levelwise an isofibration in $\Cat$.
\[
\begin{tikzcd}
	\dSet^G \arrow[r, "\upsilon_{\**}"] \arrow[d, "\upsilon_{G,\**}"']
&
	\dSet^{\mathsf O_G^{op}} \arrow[d, "\iota^{\**}"]
\\
	\dSet_G \arrow[r, "\iota_G^{\**}"]
&
...
\end{tikzcd}
\]
      We see that $\iota_G^{\**} \upsilon_{\**} X \to \iota_G^{\**} ho(X)$ is again a universal map to a (levelwise) strict Segal object,
      but then the universal property of the $(\tau, N)$-adjunction implies
\[
	N \tau \iota_{\**} \upsilon_{\**} X \simeq \iota_G^{\**} ho(X).
\]
      The result follows.
\end{proof}
...
\[
	i_G^{\**} ho(hcN(f)) = i_G^{\**} \pi_0(\upsilon_{G,\**}(N(f))) =
	N \iota^{\**} \pi_0 \upsilon_{\**}(f)
\]
      is so,
