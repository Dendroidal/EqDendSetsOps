...
\begin{lemma}\label{FACEINNECK LEM}
	...
	and let $E = \boldsymbol{E}^{\mathfrak{i}}(J)$. Then
	\begin{enumerate}[label=(\roman*)]
		\item a face $U \hookrightarrow T$
		is in $\Omega[\tau]$
		iff its outer closure $\bar{U}$ is; 
		\item an outer face 
		$U = \bar{U} \hookrightarrow T$
		is in $\Omega[\tau]$ iff $E_U = \emptyset$.
	\end{enumerate}
\end{lemma}
...
\begin{proof}
	...
	The argument for (ii) is by induction on the number of edges in $E$,
	with the base case where $E = \emptyset$  being obvious.
%	
	Otherwise, letting $e\in E$, since $e$ is an inner edge of
	both $J$ and $T$
...
\begin{proposition}
	Let $\tau\colon J \to T$ and $\tau' \colon J' \to T'$ be necklaces. 
	Any map $\tau \to \tau'$ in $\mathsf{Nec}$
	is uniquely determined by some map 
	$T \to T'$ in $\Omega$. 
	More precisely, there exists an unique dashed arrow in 
\begin{equation}\label{NECKMAP EQ}
\begin{tikzcd}
	\Omega[\tau] 
	\ar[hookrightarrow]{r} 
