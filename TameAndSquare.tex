...
\begin{notation}
        ...
        we write
        $T_v = \coprod_{[u] = v} T_u$
        for the $G$-corolla given by
        the union of subcorollas $T_u$ of the expanded representation
        % i.e. planar outer faces of $T$ with a single vertex $u \in \boldsymbol{V}(T)$,
        for those $u \in \boldsymbol{V}(T)$ such that $[u] = v \in \boldsymbol{V}_G(T)$.
\end{notation}
...
will require a notion where the source is a \textit{non-equivariant} tree.
\begin{definition}
...
We can give a general description of $\tau_G(Z)$ for any $Z \in \dSet_G$,
