      ...
      Here, $J = T \setminus \set{b,f}$, and the two beads are
           \begin{equation}
            \begin{tikzpicture}[auto,grow=up, level distance = 2.2em,
                  ...
                  \tikzstyle{level 2}=[sibling distance=4em]%
                  \tikzstyle{level 3}=[sibling distance=2.25em]%
                  \node at (-5,0) [font=\normalsize] {$\bar{\mathfrak n J_u} = T_u$}
                  child{node [dummy] {}
                    child{node [dummy] {}
                  ...
                  \node [font=\normalsize] {$\bar{\mathfrak n J_v} = T_v$}
                  child{node [dummy] {}
                    child{node [dummy] {}
                      child{edge from parent node [swap] {$e$}}
                      edge from parent node [swap] {$f$}
...
\begin{remark}
      ...
      an inner face $J \to U$ is isomorphic to some $[k]$,
      and each bead of the associated necklace $\mathfrak n$
      is some $[m_i]$ such that $m_1 + \dots + m_k = m$.
      Then $\Omega[\mathfrak n] = \iota_!(\Delta[m_1} \wedge \dots \Delta[m_k])$ is a necklace in the sense of Dugger-Spivak \cite{DS11},
    and the natural map $\Omega[\mathfrak n] \to \Omega[U]$ is equal to $\iota_!\left(\Delta[\mathfrak n] \to \Delta[m])$ from \cite[\S 3]{DS11}.
\end{remark}
