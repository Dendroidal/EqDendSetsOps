...
\begin{theorem}[{cf. \cite[Thm. III, Prop. 4.78]{BP_HGOP}}] % ISOFIBHARD PROP
      The cateogry $\sOp^G$ is equipped with a model structure where the weak equivalences (resp. fibrations) are those maps
...
\begin{definition}
      We say $(\mathfrak C, \O) \in \sOp^G$ is \textit{$\Sigma$-cofibrant} if the underlying symmetric sequence of $\O$
...
\begin{theorem}[{cf. \cite[Prop. 3.63]{BP_HGOP}}] % SIGMAG_COF PROP
      ...
      then the underlying map of $\mathfrak C$-colored symmetric sequences is a $G$-graph cofibration.
\end{theorem}
