...
\begin{definition}
	...
	\end{tikzcd}
	\]
	is defined by $(X \otimes_{\mathsf{Set}} K)(T) = X(T) \times K$
...
\begin{remark}
	...
	and $X(T) = \emptyset$ if $T$ is non-linear.
\end{remark}
...
\begin{itemize}
	...
	\item[(TA2)] $\Omega[C] \otimes_{\mathsf{Set}}\left(\Lambda^i[n] \to \Delta[n]\right)$ for $C \in \Sigma_G$, $0 \leq i \leq n$;
	\item[(TA3)] 
	$\left( Sc[T] \to \Omega[T] \right) 
	\square_{\mathsf{Set}} 
	\left(\partial \Delta[n] \to \Delta[n]\right)$ for $T \in \Omega_G$, $n \geq 0$.
...
\begin{itemize}
	...
	(TA1),(TA2),(TA3);
	\item a map $X \to Y$ between fibrant object is a fibration iff
...
\begin{proof}[Proof of Theorem \ref{TAMEMS_THM}]
	...
	We set $I$ to consist of the maps in (TC1),(TC2),(TC3)
	and $J$ to consist of the maps in (TA1),(TA2),(TA3).
	
	That the weak equivalences are accessible follows since they are the preimage by $\gamma^{\**}$ of the weak equivalences in 
	$\mathsf{sdSet}^G$ 
...
We next check nc1. 
Note that the maps in $I\text{-cof} \cap W$
