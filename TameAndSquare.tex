...
\begin{definition}
      A subcategory $\mathcal D \subseteq \mathcal C$ is a \textit{sieve} if
      whenever $B \in \mathcal D$ and there exists an arrow $A \to B$ in $\mathcal C$, $A \in \mathcal D$.
\end{definition}

\begin{definition}
      \label{WIS_DEF}
      A full subcategory $\Omega_\F \subseteq \Omega_G$ is called a \textit{weak indexing system} if
      \begin{enumerate}
...
\begin{remark}
      ...
      this plus the Segal condition impose additional operadic closure conditions on these families,
...
\item the normal model structure $\PreOp^G_{normal}$ is replaced with the $\F$-normal model structure $\PreOp^G_{normal,\F}$ (on the same category)
      from \cite[\S 6]{BP_edss}.
\item the Tame model structure $\PreOp^G_{tame}$ is replaced with
      the \textit{$\F$-tame model structure} $\PreOp^G_{tame, \F}$
      where $C \in \Sigma_G$ and $T \in \Omega_G$ from the generating (anodyne) cofibrations (TC2),(TC3),(TA2),(TA3)
      are restricted to $\Sigma_\F$ and $\Omega_\F$ respectively.
      % ----------
\item The model structure $\sOp^G$ from Theorem \ref{SOPG_THM} is replaced with the $\F$-model structure from \cite[Theorem III]{BP_HGOP}.
      This model structure has generating (trivial) cofibrations as in Remark \ref{GENCOF_SGOP_REM},
      restricting to $C \in \Sigma_\F$.
...
All results from \S \ref{TAMEDEFEX SEC} extend to general weak indexing systems $\F$:
\begin{enumerate}
...
\item Lemma \ref{SLIMOD LEM} follows from Definition \ref{WIS_DEF}(i) and Remark \ref{ETAF_REM}.
      As a warning, we note that we need that for any $\F$-Segal operad $X$, $X^H$ is a Segal \textit{space};
      however, in general, $X^H$ is \textit{not} a Segal operad.
\end{enumerate}

\subsection{$\sOp^G_\F$ and $\F$-tame}

Modifications:
\begin{enumerate}
\item For $Y \in \PreOp^G$, we say a (non-degenerate) dendrix $y \colon \Omega[U] \to Y$ is \textit{$\Sigma_\F$-free} if
...
All results from \S \ref{GSOP_SEC} extend to general weak indexing systems $\F$:
\begin{enumerate}
...
\item Lemma \ref{KEYPRVAR LEM} follows since if $Y$ is $\Sigma_\F$-free, then
      the fact that $\Omega_\F$ is a sieve implies
      the filtration of \eqref{ANODYNE_MAP} produced by the Characterizstic Edge Lemma \ref{CHAREDGE LEM}
...
All results from \S \ref{PFMNTHM SEC} extend to general weak indexing systems $\F$,
using the above modifications and the given proofs.
