...
Moreover, we again assume $\Phi$ contains exactly one representative of each planar structure,
so that the only planar isomorphisms are again the identities.
However, we caution that in order to pullback a planar structure
along a map $\varphi \colon F \to F'$
one needs to assume that 
$\varphi$ is an \emph{independent map}
in the sense of \cite[Def. 5.28]{Per18}.


{\color{red} HERE}

As such, only independent maps can be planar
