...
%
The term \emph{graph subgroup} is motivated by a description of such $\Gamma$:
...
(for details, see \cite[\S 4]{CM13b}).
...
using the abstract framework from \cite{Ste16}.
...
some previous notions that are used throughout.
Namely, 
...
\item a \textit{face map} if it is injective on edges;
      ...
      an \textit{outer face} if it does not
...
\begin{remark}\label{TREEFACT_REM}
      ...
      $\boldsymbol{E}(\varphi S) = \phi (\boldsymbol{E}(S))$,
      while the 
...
\begin{remark}
	...
	(not necessarily planar) inner face maps.
...
Moreover, we again assume $\Phi$ contains exactly one representative of each planarization,
...
along a map $\varphi \colon F \to \tilde{F}$,
one needs to assume  
...
\cite[Def. 3.19]{BP_geo}.
In particular, the unique factorization
$F \xrightarrow{\simeq} F'
\to \tilde{F}$
of a map $\varphi$
as an isomorphism followed by a planar map
...
As was the case with the categories
$\Omega$ of trees and 
$\Phi$ of forests,
each $G$-tree $T \in \Omega_G$
comes with a chosen planarization.

As in $\Omega$,
maps $\varphi \colon S \to T$ between $G$-trees
are built from a few basic types of maps.
...
and called a 
\emph{shuffling map} if
$\varphi \colon I \to J$
is an isomorphism of sets.
Further, a shuffling map
is called a
\emph{shuffling degeneracy/tall/face/inner/outer}
if each of the component maps is a
degeneracy/tall/face/inner/outer 
map in $\Omega$.
...
\tikzstyle{level 2}=[sibling distance=1.8em]%
	...
	child{node [dummy] {}
		child{
		edge from parent node [swap, near end] {$\bar{\alpha}$}}
		child[level distance=2.5em]{
		edge from parent node [swap, very near end] {$\beta$}}
		child{
		edge from parent node [near end] {$\alpha$}}
	...
	child{node [dummy] {}
		...
		child[level distance=2.5em]{
		edge from parent node [swap, very near end] {$b$}}
		child{
...
\begin{corollary}[{cf. Prop. \ref{TREEFACT_PROP}, \cite[Rem. 5.49]{Per18}}]
	...
	unique up to unique 
	isomorphisms, as
	a shuffling degeneracy followed by
	a shuffling inner face,
	a shuffling outer face,
	and a quotient map.
\end{corollary}
...
\begin{corollary}[{cf. Prop. \ref{TREEFACT_PROP}, \cite[Rem. 5.49]{Per18}}]
      A map $\varphi \colon S \to T$ in $\Omega_G$ has a strictly unique factorization
\[
	S \xrightarrow{d,r} 
	\varphi S \xrightarrow{i,p} 
	\overline{\varphi S} \xrightarrow{o,p} 
	\varphi^{\**}T \xrightarrow{q} T
\]
      as indicated:
...
Finally, contrasting with the above,
we define the faces of a $G$-tree non-equivariantly.
...
In the following, recall that a class of maps is called \textit{saturated} if it is closed under pushouts, retracts, and transfinite compositions.
...
Informally,
$G$-$\infty$-operads can be viewed as
...
\begin{proof}
	...
	$\mathsf{PreOp}^G$.
	Since Reedy fibrant preoperads \todo{do you mean Reedy fibrant Segal preoperads?} are in particular
...
\begin{proof}[Proof of Lemma \ref{UNITEQUIV LEM}]
	...
	by \cite[Prop. 3.63]{BP_HGOP} % \ref{SIGMAG_COF PROP}.
...
(given by the top horizontal map in \eqref{PUSHOUTPROP EQ}).
...
\begin{definition}
	...
	\begin{itemize}
		...
		$ \partial_{U_v} p \in \mathcal{O}$ or $\partial_{U_v} p \in \Sigma_{\mathfrak{C}}[\vect{C}]$; % \O \amalg_{\mathfrak{C}}B$;
...
Weak indexing systems can be thought of as the operadic generalization of a family of subgroups (cf. Remark \ref{WIS_FAMILY_REM}).
The set of families $\F$ of a subgroup $G$ are in bijective correspondence with \textit{sieves} $O_\F \subseteq O_G$ of the orbit category of $G$;
we extend and generalize this observation.

\begin{definition}
      A \textit{sieve} of a category $\mathcal C$ is a full subcategory $\mathcal D \subseteq \mathcal C$ such that
      for all arrows $A \to B$ in $\mathcal C$, $B \in \mathcal D$ implies $A \in \mathcal D$.
\end{definition}
...
\begin{definition}
      ...
      \item For $T \in \Omega_G$, $T \in \Omega_\F$ iff $T_v \in \Omega_\F$ for all $v \in \boldsymbol{V}_G(T)$.
      \end{enumerate}
\end{definition}

\begin{remark}
      \label{ETAF_REM}
      In particular, (ii) vacuously implies that $G/H \cdot \eta \in \Omega_\F$ for all $H \leq G$ and any weak indexing system.
\end{remark}
...
\begin{remark}
      ...
      made explicit in \cite[Defn. 3.22]{BH15}, \cite[Defn. 2.12]{Rub_nf}.
...
These modifications only effect the following two proofs:
\begin{itemize}
...
\begin{definition}
      ...
      \item For $Y \in \PreOp^G$, we say a (non-degenerate) dendrex $y \colon \Omega[U] \to Y$ is \textit{$\Sigma_\F$-free} if
            ...
            This implies $y$ is $\Sigma_\F$, which is equivalent to the condition that $\Stab(y)$ is a graph subgroup.
...
These modifications only effect the following three proofs:
\begin{enumerate}
