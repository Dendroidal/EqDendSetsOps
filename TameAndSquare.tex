...
\begin{proof}[Proof of Theorem \ref{TAMEMS_THM}]
	...
	the ``moreover'' claim
	follows from the fact that weak equivalences in the two model structures coincide together with  
	Lemma \ref{TAMECOFCOF_LEM},
	which says that tame cofibrations are normal cofibrations.
...
but in this paper we need only a brief overview of that perspective.
%
...
$\mathsf{sOp}^G$
(cf. \cite[Def. \ref{AC-VINTER DEF}]{BP_ACOP}).
Compare with the pseudo-intervals in $\mathsf{PreCat}$,
cf. Definition \ref{PSEUINT DEF}.
...
\begin{remark}
	...
	for $\mathsf{sSet}$, we can rewrite the notation therein as
\[
	\mathbb{F}
