...
Our goal in this appendix is to discuss the 
``genuine operadification''functor
$\tau_G \colon \mathsf{dSet}_G \to \mathsf{Op}_G$
first mentioned in
\eqref{TAUFUNCTS EQ},
culminating in the proof
of Proposition \ref{HOOPID_PROP},
which is the full equivariant generalization of
\cite[Prop. 4.8]{CM13b}
(cf. Remark \ref{TWOHOMOP REM}).


In general, and for an arbitrary 
$X \in \mathsf{dSet}_G$,
one can describe 
$\tau_G X \in \mathsf{Op}_G$
by suitably extending the machinery in Appendix \ref{WCONS AP}
and applying the observations
in Remark \ref{TAUFUNEX REM} as follows.
