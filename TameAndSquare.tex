...
In this section, we prove our main result, Theorem \ref{QE THM}, modulo a key lemma to be proved in Section \ref{KEYRES SEC}.

We do so in two main steps.
First, after recalling the Dwyer-Kan model structure on equivariant simplicial operads,
we prove that the nerve functor is a right Quillen equivalence to 
the tame model on $\PreOp^G$ from the previous section.
This yields two Quillen equivalences between $\sOp^G$ and the joint Reedy model structure on $\sdSet^G$.
...
We begin with $\Omega(T)$; more details can be found in \cite[\S 2.3.1 and \S 4.2]{BP_HGOP}). % {REPFUN_SEC}
