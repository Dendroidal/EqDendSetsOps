...
Objects are given by tree diagrams $T$ as below\footnote{Formally, we use the model of \textit{broad posets} as introduced in \cite{Wei12}.}.
\begin{equation}
      \label{eq:TREE}
...
The sets of edges, inner edges, and vertices of a tree $T$ are denoted $\boldsymbol{E}(T)$, $\boldsymbol{E}^i(T)$, and $\boldsymbol{V}(T)$ respectively.
If an edge $t$ is pictorally above (or equal to) an edge $s$, we write $t \leq_d s$\footnote{While this notation may seem backwards, $\leq_d$ flows in the direction of composition.}.

If $|\boldsymbol{V}(T)| = 1$ and $|\boldsymbol{L}(T)|=n$, we call $T$ an \textit{$n$-corolla}.
Additionally, we write $\eta$ for the \textit{stick} tree; i.e. the unique tree with $|\boldsymbol{V}(\eta)| = 0$.

Given any $v \in |\boldsymbol{V}(T)|$, we write $T_v$ for the sub-diagram of $T$ consisting of the vertex $v$.

\begin{convention}
      For technical ease, we will assume that all trees $U$ are equipped with a \textit{planar} structure,
      a total order on $\boldsymbol{E}(T)$ which extends $\leq d$.
\end{convention}

An arrow $\phi \colon S \to T$ between trees is given by
...
If $S$ is a corolla with leaves $\set{e_1,\dots, e_n}$ and root $e_0$,
we first require that $\phi(e_i) \leq_d \phi(e_0)$ and $\phi(e_i) \not\leq_d \phi(e_j)$ for all $i \geq j > 0$.
Moreover, consider the sub-diagram of $T$ where we chop of all vertices and edges above $\phi(e_i)$ and below $\phi(e_0)$;
$\phi$ is a map of trees iff the set of leaves of this sub-diagram is precisely $\set{\phi(e_1), \dots, \phi(e_n)}$.
For general $S \in \Omega$, $\phi$ is a map of tree iff every restricted map $S_v \to T$ is a map of trees.
...
\begin{example}
      ...
      \begin{equation}
            \begin{tikzpicture}[auto, grow=up, level distance = 2.2em,
                  ...
                  \tikzstyle{level 3}=[sibling distance=2.25em]%
                  \node at (0,0) {$\cmark$} % {$T \setminus \set{e}$}
                  child{node [dummy] {}
                    child{edge from parent node [swap] {$d$}}
                    child{node [dummy] {}
                      child{edge from parent node [swap] {$b$}}
                  ...
                  \node at (2.5,0) {$\cmark$} % {$\partial_{u,v}T$}
                  child{node [dummy] {}
                  ...
                  \node at (5.5,0) {$\cmark$} % {$\partial_v(T \setminus \set{c})$}
                  child{node [dummy] {}
                    child{node [dummy] {}
                      edge from parent node [swap] {$e$}
                    }
...
An arrow $S \to T$ is called a \textit{tall map} if
$\phi(\boldsymbol{L}(S)) = \boldsymbol{L}(T)$ and $\phi(r_S) = r_T$
for $r_{(-)}$ denoting the root edge.
An arrow $S \to T$ is called a \textit{face map} if it is injective on edges,
...
Heuristically, inner face maps split vertices to create new inner edges in $T$,
...
\begin{proposition}[{cf. \cite[Prop. 2.2]{BP_edss}}]
      A map $\phi \colon S \to T$ in $\Omega$ has a factorization, unique up to unique isomorphisms,
      \[
            S \xrightarrow{\phi^-} U \xrightarrow{\phi^i} V \xrightarrow{\phi^o} T
      \]
...
\begin{definition}
      ...
      maps $F_i \to F'_{\phi(i)}$ for all $i \in I$.
...
with the \textit{expanded representation} given by the two trees on the left below, and
the \textit{orbital representation} given by the single tree on the right.
\begin{equation}
      \label{GTREE_EQ}
      \begin{tikzpicture}[auto,grow=up, level distance = 2.2em,
            every node/.style={font=\scriptsize,inner sep = 2pt}]%
...
\item A functor $T: G \ltimes I \to \Omega$.
\item An induction $T \simeq G \cdot_H T_{\**}$ where $T_{\**}$ is a fixed component of $T$ and $H \leq G$ is the stablizier of that component.
\item A quotient $T \simeq G \cdot U / \Gamma$ for some $U \in \Omega$ and graph subgroup $\Gamma \leq G \times \Aut(U)$.
      % \item The quotient $T \simeq (G \cdot T_{\**}) / N$ for $N$ the graph of the homomorphism $H \to \Aut(T_{\**})$ encoding the $H$-action.
\end{enumerate}
Moreover, we have that $\Omega_G$ is fibered over the category $\mathsf O_G$ of transitive $G$-sets and $G$-maps.
...
\begin{remark}
      ...
      conversely, the orbital representation compactly displays the relevant data for composing norm maps of operads.
\end{remark}

We can again describe maps between $G$-trees as being built out of several types of maps.
First, on each fiber over $\mathsf O_G$, we have map which are faces or degeneracies on one (and thus all) components.
There is a new type of type of map, \textit{quotients}, which corresponding to the cartesian arrows of $\mathsf O_G$,
which behave like twisted fold maps on the components.
\todo[inline]{example}
...
\[
      ...
      \qquad
      \Omega[T] = \coprod \Omega[T_i] \simeq G \cdot_H \Omega[T_{\**}].
...
To begin, we make the following definitions (see \cite[\S 6]{Per18}), generalizing the standard presheaf constructions,
with one equivariant clarification.
...
\begin{definition}
      ...
      \begin{equation}
            \partial \Omega[T]  = \colim_{U \in \mathsf{Face}(T), U \neq T} \Omega[U] = \bigcup_{U \in \mathsf{Face}(T), U \neq T} \Omega[U]
      \end{equation}
      and \textit{boundary inclusions} the maps in $\dSet^G$ of the form
      $\partial \Omega[T] \to \Omega[T]$.

      Next, given any $e \in E(T)$, the associated \textit{$G$-inner horn} is the sub-presheaf
      \begin{equation}
            \label{GINNERHORN_EQ}
            \Lambda^{Ge}[T] \into \partial \Omega[T] \into \Omega[T],
            \qquad
            \Lambda^{Ge}[T] = \colim_{U \in \mathsf{Face}(T), (T_i \setminus Ge) \not\into U} \Omega[U]
      ...
      \begin{equation}
            \label{eq:SC}
            Sc[T] = \colim_{U \in \mathsf{Face}_{SC}(T)} \Omega[U] = \bigcup_{U \in \mathsf{Face}_{SC}(T)} \Omega[U],
      \end{equation}
...
\begin{remark}
      ...
      \[
            ...
            \qquad
            \Lambda^{Ge}[T] \simeq G \cdot_H \Lambad^{E_{\**}}[T_{\**}],
            \qquad
...
\todo[inline]{$G$-$\infty$-operads are combinatorial operads with weak composition}
...
We now consider the induced diagram between homotopy categories
on the right,
which consists of derived functors on homotopy categories
(since the left Quillen functors $\gamma^{\**},c_!$ preserve all weak equivalences, and thus need not be derived). 
