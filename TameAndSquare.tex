...
\begin{remark}
	...
	$S_i \to T_{\varphi(i)}$
	being identities.
	A quotient map with this additional property is called 
	a \emph{pullback map}.
\end{remark}
...
Finally, in contrast to the notion of sorted face map above,
...
The latter are an extension of the notion of operad
...
\begin{remark}
	...
	$\mathsf{sdSet}^G$
	can alternatively be described as a further localization of the 
...
This section recalls the normal model structure on preoperads
$\mathsf{PreOp}^G$
%introduced and 
...
\begin{remark}\label{PRIEX REM}
	Our primary instances of Notation \eqref{XUDECALT NOT}
...
\begin{remark}
	...
	notations,
	reserving the $\O(\vect{C})$ style notation for the context of operads (see \S \ref{GSOP_SEC}).
\end{remark}
...
as well as its obvious simplicial generalization
...
\begin{enumerate}[label = (\roman*)]
	\item \textit{fully-faithful} if for all $G$-corollas $C$ and  $G$-equivariant colorings
...
\cite[Rem. 4.41, Thms. 5.51 and 5.48]{BP_edss}
