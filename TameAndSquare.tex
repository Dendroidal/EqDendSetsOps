...
\todo{trees $T \in \Omega$, vs $U \in \Omega$ and $T \in \Omega_G$?}
...
\begin{notation}
        \label{TV_NOT}
        For $T \in \Omega$ and any $v \in \boldsymbol{V}(T)$, let
        $T_v \in \Sigma$ denote the planar outer face $T_v \hookrightarrow T$ whose only vertex is $v$.
\end{notation}
...
\begin{notation}
        Following Notation \ref{TV_NOT}, for any $\mathfrak C$-tree $\vect T = (T, \mathfrak c)$ and any vertex $v \in \boldsymbol{V}(T)$,
        we write $\vect T_v = (T_v, \mathfrak c_v)$ for the associated $\mathfrak C$-corollas, where
        $\mathfrak c_v$ is the restriction of $\mathfrak c$ to $\boldsymbol{E}(T_v) \subseteq \boldsymbol{E}(T)$.        
\end{notation}
...
$\mathsf{dSet}_G$
of those objects satisfying the following strict Segal condition.
\begin{definition}
        \label{OPG_DEF}
        A genuine equivariant dendroidal set $Z \in \dSet_G$ is a \textit{genuine $G$-operad} if
        ...
        The category $\Op_G$ is the full subcategory of $\dSet_G$ spanned by genuine $G$-operads.
\end{definition}
...
\todo[inline]{need $T_v$ for $v \in \boldsymbol{V}_G(T)$ here. Maybe move orbital faces (and the following notation) into \S 2?}

\textcolor{blue}{
  \begin{notation}
          For each $G$-vertex $v$ of $T \in \Omega_G$, we write 
          $T_v \hookrightarrow T$ for the planar orbital outer face whose only
          $G$-vertex is $v$.
  \end{notation}
}
...
\begin{remark}\label{NERTENID REM}
	...
	\eqref{STRSEGCON EQ} for $N\O$ \todo{different categories}
...
\todo{$T \in \Omega$ or $T \in \Omega_G$?}
...
that is a planar orbital inner face (Definition \ref{ORBITALFACE_DEF}).
...
\begin{remark}
        We note that a genuine $G$-$\infty$-operad is a ``weak'' genuine $G$-operad (Definition \ref{OPG_DEF})
        paralleling the comparison between a quasicategory and (the nerve of) a category.
\end{remark}
