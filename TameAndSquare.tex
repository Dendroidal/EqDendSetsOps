...
The free operad adjunction $\mathbb F$ factors
\begin{equation}
      \label{F_FAC_EQ}
      \begin{tikzcd}
            \Sym_{\bullet}\V) \arrow[r, shift left, "{(-) \amalg \eta_\bullet}"]
            &
            \Sym_{\bullet,\eta}(\V) \arrow[l, shift left] \arrow[r, shift left, "\mathbb F_{\bullet, \eta}"]
            &
            \Op_{\bullet}(\V) \arrow[l, shift left, "U_\eta"]
      \end{tikzcd}
\end{equation}
where
\begin{itemize}
\item $(-) \amalg \eta_\bullet$ sends $(\mathfrak C, X)$ to $(\mathfrak C, X \amalg \eta_{\mathfrak C}, \eta_{\mathfrak C} \hookrightarrow X \amalg \eta_{\mathfrak C})$,
\item the map $\eta_{\mathfrak C} \to U_\eta(\O)$ selects the identity operations for each color,
      and
\item
      $\mathbb F_{\bullet, \eta}$ is the ``free operad with prescribed units'' monad.
\end{itemize}

In order to give a complete description of $\mathbb F_{\bullet, \eta}$,
we apply the results of Appendix \ref{AMALGMON_SEC}
to a factorization of $\mathbb F_{\bullet}$ orthogonal to \eqref{F_FAC_EQ},
which passes through
the category $\Op_{\bullet, nu}(\V)$ of non-unital operads.
...
Now, we have a factorization of $\mathbb F$
\begin{equation}
      \label{F_FAC_NU_EQ}
      \begin{tikzcd}
            \Sym_{\bullet}(\V) \arrow[r, shift left, "{\bar{\mathbb F}}"]
            &
            \Op_{\bullet,nu}(\V) \arrow[l, shift left, "U_{nu}"] \arrow[r, shift left, "\mathbb P"]
            &
            \Op_{\bullet}(\V) \arrow[l, shift left, "\hat U_\eta"]
      \end{tikzcd}
\end{equation}
through the category $\Op_{\bullet, nu}$.
$U_{nu}$ and $\hat U_\1$ are forgetful functors with left adjoints $\bar{\mathbb F}$ and $\mathbb P$.
The ``adds units'' functor $\mathbb P$ is easy to describe:
on underlying symmetric sequences, it sends $(\mathfrak C, \O)$ to $(\mathfrak C, \O \amalg \eta_{\mathfrak C})$,
and the composition law may be canonically extended so that the new operations act as units.

The functor $\bar{\mathbb F}$ realizes the category $\Op_{\mathfrak C, nu}$ as the category algebras over a monad on $\Sym_\bullet(\V)$.
We construct this monad analogously to $\mathbb F$ as in \cite[Appendix A]{BP_HGOP}, % {MONAD_APDX}
...
\begin{proposition}
      The free non-unital operad monad $\bar{\mathbb F}$ sends a symmetric sequence $X \colon \Sigma_{\mathfrak C}^{op} \to \V$ in $\Sym_{\bullet}(\V)$ to the left Kan extension below.
      \[
            \begin{tikzcd}
            ...
            \end{tikzcd}
      \]
      More generally, $\bar{\mathbb F}^{\circ n}X$ is given by the the following left Kan extension.
      \begin{equation}
            \label{FREENUOP_EQ}
            \begin{tikzcd}
...
the left adjoint $F_p = \mathbb F_\eta$ to the forgetful functor $\mathsf{fgt} \colon \Op_{\bullet} \to \Sym_{\bullet,\eta}(\V)$,
i.e. the ``free operad with prescribed units'' functor, exists,
and following
Corollary \ref{ALGLEFTEX_COR} and \eqref{FRMONDES EQ}
and sends a pointed symmetric sequence $Y \colon \Sigma_{\mathfrak C}^{op} \to \V$ in $\Sym_{\bullet, \eta}(\V)$ to the coequalizer of the following in $\Sym_{\mathfrak C}(\V)$
\begin{equation}
      \begin{tikzcd}
...
This can be described compactly as followed.
\begin{lemma}
      \label{FP_FETA_LEM}
      For $Y \in \Sym_{\mathfrak C, \eta}(\V)$,
...
as noted in \S \ref{REVMS_SEC} and \cite{BP_HGOP, BP_edss, Per18}.
