...
\begin{remark}
      We can describe $G$-trees $T \in \Omega_G$ by
      ...
      $\Gamma = \sets{\left(h,\phi(h)\right)}{h \in H}$ associated to the homomorphism $\phi \colon H \to \Aut(T_{\**})$ encoding the $H$-action on $T_{\**}$,
...
\tikzstyle{level 2}=[sibling distance=1.8em]%
	...
	child{node [dummy] {}
		...
		child{
		edge from parent node [near end] {$\phantom{\bar{\alpha}}\alpha$}}
