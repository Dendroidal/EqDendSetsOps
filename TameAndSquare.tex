...
\begin{lemma}\label{TAUOTIMES_LEM}
	...
	with the second $\otimes_{\mathfrak{C}_{\bullet}}$
	the (fiberwise) simplicial tensoring of $\sOp^G$.
\end{lemma}
...
\begin{proof}
	The identification
	$\tau(X \otimes_{\mathfrak{C}_{\bullet}} K) 
	\simeq 
	\tau(X) \otimes_{\mathfrak{C}_{\bullet}} K$
...
\begin{equation}
\begin{split}
	\left(N \{K,\O\}_{\mathfrak{C}_{\bullet}}\right)_{\mathfrak{c}}(U)
&
\simeq
	\prod_{v \in \boldsymbol{V}(U)} 
	\{K,\O\}_{\mathfrak{C}_{\bullet}}(U_v,\mathfrak{c}_v)
\simeq
	\prod_{v \in \boldsymbol{V}(U)}
...
\simeq
	\left(\{K,N \O\}_{\mathfrak{C}_{\bullet}}\right)_{\mathfrak{c}}(U)
\end{split}
\end{equation}
...
\begin{remark}
	In \cite[\S 7.1]{CM13b}, $\Omega(T) \otimes_{\mathfrak{C}_{\bullet}} K \in \sOp$ and 
	$\Omega[T] \otimes_{\mathfrak{C}_{\bullet}} K \in \mathsf{PreOp}$ were denoted $T[K]$, $\Omega[K,T]$ respectively, and were constructed by hand.
\end{remark}
...
We denote by $\mathsf{sOp}^G$ the category of \textit{$G$-equivariant colored simplicial operads}.
These can be built as algebras over a certain monad on the category of symmetric sequences.
...
\begin{notation}
	Given $\O \in \sOp^G$, we write
	$\mathfrak C_\O \in \Set^G$ for the associated $G$-set of colors of $\O$.
\end{notation}
...
\begin{definition}
	A map $\phi \colon X \to Y$ in $\sSym^G_{\mathfrak C}$ is called a \textit{$\Sigma$-cofibration} if
	...
	$\phi(\vect C) \colon X(\vect C) \to Y(\vect C)$ is an $\F^\Gamma_{\vect C}$-cofibration in $\sSet^{\Aut_{G \ltimes \Sigma_{\mathfrak C}^{op}}(\vect C)}$.
      ...
      if the underlying map of $\mathfrak C$-colored symmetric sequences is a $\Sigma$-cofibration in $\sSym_{\mathfrak C}^G$. 
\end{definition}
...
\begin{proposition}\label{SGS_COF_PROP}
	$X \to Y$ in $\mathsf{sSym}^G$ is a $\Sigma$-cofibration iff
	it forgets to a $\Sigma$-cofibration in $\mathsf{sSym}$.
...
in $\Sym_{\mathfrak C}$ (rather than in $\Sym_\bullet$).
...
Using the standard coequalizer description of algebras over a monad, one has
\[
...
\begin{remark}
      In \cite[\S 7.1]{CM13b}, $\Omega(T) \otimes_{\mathfrak{C}_{\bullet}} K$ was denoted $T[K]$.
\end{remark}

The tensoring and cotensoring induce analogous operations on the categories of $G$-objects by postcomposition.
{\color{OliveGreen} % ----------
  They are computed as before,
  with conjugation and diagonal $G$-actions on $[K,\O(-)]$ and $\O(-) \times K$:
  \begin{equation}
        \label{DIAGTENS_EQ}
...
Via direct compution, we have the following.
\begin{lemma}
      \label{OPTENS_LEM}
      ...
      %       \).
      % \end{enumerate}
      Then the two lifting problems below are equivalent,
      \begin{equation}
            % \resizebox{\textwidth}{!}{%
            \begin{tikzcd}[column sep = small, ampersand replacement = \&]
                  ...
                  \&
                  N\O_{\mathfrak c}(C)
                  % \O(\vect C)^{\Aut_{G \ltimes \Sigma_{\mathfrak C}}(\vect C)}
                  \arrow[d, "{F_{\mathfrak c}(C)}"]
                  % \& % ----------
    ...
    \end{equation}
    where
    $\mathfrak c = \mathfrak C_\phi \colon \boldsymbol{E}(C) \to \mathfrak C_\O$
    is induced by $\phi$.
\end{lemma}

\begin{remark}
      We do not require that $\O \to \P$ is in $\sOp^G_{\boldsymbol{E}(C)}$ in the previous lemma.
      However, this is specific to $\Omega(C)$.
      In general, following Remark \ref{PREOP2VAR_REM},
      we must fix some $G$-set of colors $\mathfrak C$ in order to have
      tensoring and cotensoring become part of a two-variable adjunction.
      \[
            \sSet^{G,op} \times \sOp^G_{\mathfrak C} \xrightarrow{\set{(-),(-)}_\Set} \sOp^G_{\mathfrak C}
            \qquad
            \sOp^G_{\mathfrak C} \times \sSet^G \xrightarrow{(-) \otimes_\Set (-)} \sOp^G_{\mathfrak C}
            \qquad
            \sOp^{G,op}_{\mathfrak C} \times \sOp^G_{\mathfrak C} \xrightarrow{[-,-]} \sSet^G
      \]      
\end{remark}
...
\begin{remark}
      ...
      We note that $\sOp^G(G \cdot_H^{\mathfrak C}\O, \P)$ embeds in ${\sOp^H}(\O,\P)$
...
\end{lemma}
\begin{proof}
      Since $\Omega(C) \otimes (-) \colon \sSet^G \to \sOp^G_{\boldsymbol{E}(T)}$ preserves colimits, it suffices to check when
      $(A \to B) = G/H \cdot (K \to L)$ for $K \to L$ a cofibration in $\sSet$.    
      Then, as cofibrations in the fiber $\sOp^G_{\mathfrak C}$ are cofibrations in the total category $\sOp^G$ by \cite[Prop. 4.11]{BP_HGOP}, % FIBERGLMOD PROP
...
\begin{remark}
      ...
      {\color{OliveGreen} % ----------
        \[
              \begin{tikzcd}
                    \PreOp^G \arrow[d, "\upsilon_{\**}"'] \arrow[r, equal]
                    &                    
                    \PreOp^G \arrow[d, "i_{\**}"']
                    &
                    \sOp^G \arrow[l, "N"'] \arrow[d, "i_{\**}"]
                    \\
                    \PreOp_G \arrow[d, "\pi_0"'] \arrow[r, "j_G^{\**}"]
                    &
                    \PreOp^{\mathsf O_G^{op}} \arrow[d, "\pi_0"']
                    &
                    \sOp^{\mathsf O_G^{op}} \arrow[l, "N"'] \arrow[d, "\pi_0"]
                    \\
                    \dSet_G \arrow[d, "i_G^{\**}"'] \arrow[r, "j_G^{\**}"]
                    &
                    \dSet^{\mathsf O_G^{op}} \arrow[d, "j^{\**}"']
                    &
                    \Op^{\mathsf O_G^{op}} \arrow[d, "j^{\**}"] \arrow[l, "N"']
                    \\
                    \sSet^{\mathsf O_G^{op}} \arrow[r, equal]
                    &
                    \sSet^{\mathsf O_G^{op}}
                    &
                    \Cat^{\mathsf O_G^{op}} \arrow[l, "N"']
              \end{tikzcd}
        \]
      } % ----------
\end{remark}
