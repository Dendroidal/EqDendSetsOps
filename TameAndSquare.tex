...
\begin{remark}\label{GENDENSETADJ REM}
	...
	$\dSet_G$, 
	and at some points in this paper \todo{add examples} we will need to deal with the fact that many natural constructions in the non-equivariant setting generalize to objects in 
...
and the family of {\color{blue} graph subgroups} \todo{graph subgroup has not been defined yet} is
...
\begin{theorem}\label{JB_THM}
	...
	which we call the \emph{joint Bousfield localization},
        with \textit{joint equivalences} and \textit{jount fibrations/fibrant objects}.
	Moreover:
...
We give an explicit construction for the rigidification of a dendroidal set into a simplicial operad,
extending the work of Dugger-Spivak \cite{DS11} from the categorical to the operadic context.
This will be used to give a concrete description of the key adjunction $\dSet^G \rightleftarrows \sOp^G$.

The fundamental object for this discussion is a necklase.
First, we recall three concepts from $\Omega$ and $\dSet$.
\begin{notation}
      \begin{enumerate}
      \item Given a map $U \xrightarrow{\phi} T$, we let $\overline{\phi U}$
            % Given any face map $U \into T$, we let $\overline{U}^T = \overline{U}$
            denote the \textit{outer closure} of $U$ in $T$,
            the smallest planar outer face of $T$ such that $U \into \overline{U}$
            (equivalently, $\overline{U}$ is the unique planar outer face $U''$ in the factorization \eqref{OMEGAFACT_EQ},
            {cf. \cite[Notation 2.14]{BP_edss}}).
            
            Moreover, for any vertex $v \in \boldsymbol{V}(U)$, we write $T_v$ for the outer closure $\overline{\phi U_v}$.
            \todo[inline]{we are writing outer closures as $\overline{\phi U}$ as opposed to $\overline{U}^T$, yes? I agree that this is better for iterated closures.}
      \item Let $E \subseteq \boldsymbol{E}(T)$
            ...
            \]
      \item Given $T \in \Omega$ and any inner edge $e \in \boldsymbol{E}^i(T)$, we have a \textit{grafting decomposition} 
            $T = R \amalg_e S$ where $R,S$ are outer faces of $T$ such that $e$ is a leaf of $R$ and the root of $S$.
      \end{enumerate}
\end{notation}
...
\begin{definition}
      ...
      The category $\mathsf{Nec}$ of necklaces is the full subcategory of $\mathsf{dSet}$
      spanned by the $\Omega[T,E]$.
\end{definition}

\begin{remark}
      The presheaves $\Omega[T,E]$ interpolate between the Segal cores
      $Sc[T] = \Omega[T, \boldsymbol{E}^i(T)]$ and the whole presheaf
      $\Omega[T] = \Omega[T, \varnothing]$.

      Moreover, we have a natural inclusion $\Omega \into \mathsf{Nec}$ sending $T \mapsto (T, \varnothing)$.
\end{remark}
...
\begin{proof}
	...
	$\overline{\iota U} \to T'$
	in $\Omega$
        for all $U \in \mathsf{Face}_{sc}(J)$, $J = T - E^c$.
	Hence, the result follows from the identification
	$T
	\simeq  
	\underset{U \in \mathsf{Face}_{sc}(J)}{\colim}
	\overline{\iota U}$, where the colimit is now in $\Omega \subseteq \Op$,
...
\[
      \begin{tikzcd}
            ...
            T
            \\            
            S' \arrow[r,dashed, "tall"] \ar{u}
            &
            F'_0 \arrow[r,dashed, "inn"] \ar{u}{out}
            &
            F'_1 \arrow[r, dashed, "inn"] \ar{u}{out}
            &
            \cdots \arrow[r, dashed, "inn"]
            &
            F'_n \ar{r}{face} \ar{u}{out}
            &
            T \ar[equal]{u}
      \end{tikzcd}
\]
...
\todo[inline]{confirming: $\phi$ is the map $S \to T$, yes?}
...
\]
\todo[inline]{the second equality uses that $F_0' \to F_0$ is outer, yes?}
\end{definition}
...
\\
	S \arrow[r, "tall"]
&
	F^{\**}_0 \arrow[r, dashed, "inn"]
&
	F^{\**}_1 \arrow[r, dashed, "inn"]
&
	\cdots \arrow[r, dashed, "inn"]
&
	F^{\**}_n \arrow[r, dashed, "face"]
&
	T^{\**}
\end{tikzcd}
\end{equation}
...
&&%%%
	S \ar{r}{tall} \ar[equal]{d}
&
	F_i \ar{r}{face} \arrow[d, "deg"]
&
	T \ar{d}
\\
	S' \ar{u} \arrow[r, "tall"] \ar[equal]{d}
&
	F'_i \ar{r} \arrow[u, "out"'] \arrow[d, "deg"]
&
	T \ar{d}
&&%%%
	S \ar{r}
...
\\
	S' \ar{r}
&
	\left(F_i'\right)^{\**} \arrow[r, dashed, "face"]
&
	T^{\**}
&&%%%
	S' \arrow[r, "tall"] \ar{u}
&
	\left(F_i^{\**}\right)' \ar{r} \arrow[u, "out"']
&
	T^{\**}
\end{tikzcd}
\end{equation}
...
(as $B(\overline{\phi S}, E_{\overline{\phi S}}) = \boldsymbol{V}(S^E)$)
...
$\boldsymbol{E}(S) \to A_i(\eta)$.
\todo[inline]{is there a quick formal proof of this? The one I found uses that $\PreOp$ is a presheaf category on $\Set$, and is not hard but annoying to write up (which is why I imagine you did not)}
\end{remark}
...
%
	In particular, $N$ fully-faithful then implies that
...
\todo[inline]{we initially just have a map $\boldsymbol{E}(S) \to \boldsymbol{E}(T) \setminus E^c = \boldsymbol{E}(J)$, yes?}
...
on
\eqref{NWKANEXEV EQ}.
Moreover, one can further replace it with the retractive subcategory
$\mathsf{Nec}_{\vect{S}//X}^{\Omega,nor}$.
...
Further, two pieces of such data represent the same simplex iff they are equivalent
under the relation generated by the following identifications:
there exists a map of necklaces $(T,E) \to (T^{\**},E^{\**})$
such that the maps 
$\Omega[T,E] \to X$,
$\Omega[T^{\**},E^{\**}] \to X$
...
\begin{proof}
	...
	The base case is when $E = \emptyset$
