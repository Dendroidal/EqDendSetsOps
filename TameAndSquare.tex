...
\begin{abstract}
	...
	between the 
	the model category for 
	colored $G$-equivariant simplicial operads with norm maps
	and
	the model category for
	$G$-$\infty$-operads in $G$-equivariant dendroidal sets.
\end{abstract}
...
\begin{proof}
	...
	%
	But since $N \mathcal{P}$ is $\Sigma$-free due to Proposition \ref{COPOFSIGCOF PROP},
...
\begin{example}
      ...
      \begin{equation}
            ...
            \end{tikzpicture}
      \end{equation}
      In particular, note that $\boldsymbol{E}(J)$ and $\boldsymbol{E}(J_F)$ need not be comparable.
\end{example}
