...
\todo[inline]{this ``i.e.'' makes it seem like we are restricting the morphisms, not the objects}
...
\todo[inline]{in this description, the descriptions of the face maps are by an operation on the target diagram, while the description of a degeneracy is an operation on the source}
\begin{example}
...
\begin{remark}\label{CNVXM REM}
	...
	is an isomorphism.
	Equivalently, if % $\psi$ is convex precisely if
...
Before constructing the new model structure on pre-operads, we recall some definitions and discussion from \cite[\S 4 and \S 5]{BP_edss}.

\begin{definition}
      The category of \textit{(equivariant) preoperads} $\mathsf{PreOp}^G$ is the full subcategory of $\mathsf{sdSet}^G$ spanned by
      those $X$ such that
...
\begin{definition}
      A preoperad $X \in \mathsf{PreOp}^G$ is called a \textit{$G$-Segal operad} if
      $X\left( \Omega[T] \right) \to 
      X \left( Sc[T] \right)$
      is a Kan equivalence for each $T \in \Omega_G$.
      
      A $G$-Segal operad $X$ is additionally called \textit{Reedy fibrant} if $\gamma^{\**}X$ is dendroidally Reedy fibrant in $\sdSet^G$,
      or equivalently (\cite[Remark 4.41]{BP_edss}), if $\gamma^{\**}X$ is fibrant in the dendroidal Segal space model structure on $\sdSet^G$.
\end{definition}

We say a map $f$ in $\PreOp^G$ is a \textit{joint equivalence} (resp. \textit{normal monomorphism}) if
$\gamma^{\**}f$ is a joint equivalence (resp. cofibration) in the joint model structure on $\sdSet^G$.

There is a characterization of joint weak equivalences between $G$-Segal operads
analogous to the Dwyer-Kan equivalences of simplicial operads (cf. Theorem \ref{SOPG_THM}).

\begin{definition}%[{cf. \cite[Defn. 5.6,5.7]{BP_edss}}]
      Given a $G$-Segal operad $X$ and a $G$-corolla $C$,
      a \textit{$C$-signature}
      is a map $\partial \Omega[C] \to X$;
      explicitly, this is a tuple
      \[
            (x_1, \dots, x_n; x_0) \in \prod_{G/H_j \cdot e_j \in \boldsymbol{E}_G(C)} X(\eta)^{H_j}
      \]
      Given a $C$-signature $(x_1, \dots, x_n; x_0)$ of $X$,
      the \textit{mapping space} $X(x_1,\dots,x_n; x_0)$ is the pullback
      \begin{equation}
            \label{MAPPINGX_EQ}
            \begin{tikzcd}
...
\begin{definition}
      Given an $G$-Segal operad $X$, define $ho(X) \in \dSet_G$ by
      \[
...
\begin{definition}
      ...
      \begin{enumerate}[label = (\roman*)]
      \item \textit{fully-faithful} if for all $G$-corollas $C$ and all $C$-signatures $(x_1,\dots, x_n;x_0)$ of $X$, the induced map
            \[
...
\begin{theorem}
      \label{PREOPMS_THM}
      There is a model structure on $\PreOp^G$, called the \textit{normal model structure}, such that weak equivalences (resp. cofibrations) are the joint equivalences (resp. normal monomorphisms).
      Moreover:
      \begin{enumerate}
      \item the normal model structure is left proper;
      \item the fibrant objects are the Reedy fibrant $G$-Segal operads;
      \item the adjunction $\gamma^{\**} \colon \PreOp^G \rightleftarrows \sdSet^G \colon \gamma_{\**}$ is a Quillen equivalence
            between the normal model structure and the joint model structure on $\sdSet^G$.
      \item weak equivalences in the normal model structure are closed under filtered colimits.
      \item a map between Reedy fibrant $G$-Segal operads is a joint equivalence iff it is a Dwyer-Kan equivalence.
      \end{enumerate}
\end{theorem}
\begin{proof}
      The existence of the normal model structure follows from \cite[Thm. 4.39]{BP_edss}.
      (i) follows immediately from Theorem \ref{JB_THM}\ref{PROPER_LBL}.
      (ii) follows from \cite[Thm. 5.51]{BP_edss} (see \cite[Remark 4.48]{BP_edss}).
      (iii) is \cite[Thm. 4.42]{BP_edss}.
      (iv) follows from Lemma \ref{FCOLIM_WE_LEM}.
      (v) is \cite[Theorem 5.48]{BP_edss} after (ii).
\end{proof}

Conidition (v) holds more generally.
\begin{proposition}
      A map between $G$-Segal operads is a joint equivalence iff it is a Dwyer-Kan equivalence.
\end{proposition}
\begin{proof}
      ...
      \[
            ...
            \end{tikzcd}
      \]
      for $Z \in \PreOp^G$ and $\mathfrak C = Z(\eta)$,
      ...
      The result then follows from Theorem \ref{PREOPMS_THM}(v).
\end{proof}




We give an alternate {\color{red} back!}
\begin{notation}
...
{\color{OliveGreen} % ----------
  \begin{remark}
        Given a $G$-Segal space $X$ and a $G$-corolla $C = (C_i)_{i \in I}$,
        a $C$-profile $(x_1, \dots, x_n; x_0)$ on $X$ is equivalent to a $G$-map
        $\boldsymbol{E}(C) \to X(\eta)$,
        which in particular splits into maps $\boldsymbol{E}(C_i) \xrightarrow{\mathfrak c_i} X(\eta)$,
        and \eqref{MAPPINGX_EQ} and \eqref{XUDEC EQ} combine to get
        \[
              X(x_1, \dots, x_n; x_0) \simeq \left( \prod_i X_{\mathfrak c_i}(C_i) \right)^G.
        \]
        In particular, a map of $G$-Segal spaces $f \colon X \to Y$ is fully-faithful iff
        for each $n$, graph subgroup $\Gamma \leq G \times \Sigma_n$ supported on $H \leq G$,
        and $H$-map $\boldsymbol{E}(C_\Gamma) \to X(\eta)$,
        $X_{\mathfrak c}(C_\Gamma)^H \to Y_{f\mathfrak c}(C_\Gamma)^H$
        is a Kan equivalence,
        where $C_\Gamma \in \Omega^H$ is $C_n$ equipped with the $H$-action induced by $\Gamma$.
  \end{remark}
} % ----------
...
\]
	...
	and is defined by the following pushout when $U=[n]$ is linear.
\[
	\begin{tikzcd}
...
\section{Simplicial operads and the Quillen equivalences}
\label{QE_SEC}
...
more details can be found in \cite[\S 2.3.1 and \S 4.2]{BP_HGOP}). % {REPFUN_SEC}
...
\begin{equation}
      ...
      \begin{cases}
            ...
            % \mbox{there exists $C \xrightarrow{\phi} T$ in $\Omega$ such that $\partial \phi = \vect C$}
            \\
            \varnothing & \text{else,}
      \end{cases}
\end{equation}
...
%(in fact, this inclusion has been used as an alternative \textit{definition} of the category $\Omega$).
...
As with the tame model structure on $\PreOp^G$,
the generating cofibrations of $\sOp^G$ are built out of a fibered simplicial tensoring,
parallel to that from \S \ref{FIBTENS_SEC} and \cite[Remark 4.13]{BP_edss}.
...
\todo[inline]{this pushout is very wrong}
{\color{OliveGreen} % ----------
        Explicitly, we have the following pushout diagram in $\sOp$.
\begin{equation}
      \label{OPTENS_EQ}
      \begin{tikzcd}
            \mathbb F_{\mathfrak C_\O}(\varnothing \cdot K) \arrow[d] \arrow[r]
            &
            \mathbb F_{\mathfrak C_\O}(\varnothing) \arrow[d]
            \\
            {\color{red} \O \times K}
            \arrow[r]
            &
...
\]
by postcomposition.
\todo[inline]{update the following}
{\color{OliveGreen} % ----------
...
\begin{lemma}
      ...
      % \item \( 
      %       \Hom_{\sOp^G}(\Omega(C) \otimes_\Set K, \O) \simeq
      %       \left(
      %             \displaystyle\prod_{i \in I} \displaystyle\coprod_{\mbox{\scriptsize $C_i$-profiles $\vect C_i$}} \Hom_\sSet(K, \O(\vect C_i))
      %       \right)^{G}
      %       \simeq
      %       \displaystyle\coprod_{\vect C_{\**}} \Hom_\sSet(K, \O(\vect C_i)^{\Aut_{G \ltimes \Sigma_{\mathfrak C}}(\vect C_i)}).
      %       \).
      % \end{enumerate}
...
\begin{proof}
      ...
      and by \cite[Prop. 4.11]{BP_HGOP} cofibrations in the fiber $\sOp^G_{\mathfrak C}$ are cofibrations in the total category $\sOp^G$, % FIBERGLMOD PROP
...
\begin{proposition}
      ...
      are the maps
      \begin{itemize}
      \item[(C1)] $\emptyset \to G/H \cdot \Omega(\eta)$ % for $H \in \F_1$,
...
\begin{proof}
      ...
      % \]
      % while for general $T$, the grafting decomposition $T = C \amalg_{\underline{l}_{C}}(T_{Ge})$ induces the following pushing in $\sOp^G$.
      % \[
      %       \begin{tikzcd}
      %             \displaystyle{
...
%       When $|V(T)| \geq 1$, following Proposition \ref{WLEFTQPUSH PROP} and Lemma \ref{OPTENS_LEM},
...
\begin{lemma}
      ...
      % that is,
      % a map of $G$-$\infty$-operads $X \xrightarrow{f} Y$ is an isofibration iff $i_G^{\**}ho(f)$ is a levelwise an isofibration of (nerves of) categories.
\end{lemma}
\begin{proof}
...
\begin{remark}
      ...
      \todo[inline]{this isn't stated in \cite{BP_edss}, and needs to be proved}.
\end{remark}
