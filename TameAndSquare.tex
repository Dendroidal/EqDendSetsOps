...
\begin{abstract}
	We establish a Quillen equivalence 
	between the homotopy theory of colored equivariant simplicial operads with norm maps
...
%the $G$-equivariaqnt structure maps on the left below
%\[
%      \O(n) \longto \Hom(X^n,X)
%      \qquad
%      \O(n)^\Gamma \longto \Hom^H(N_\Gamma X, X)
%\]
%induce the maps on the right,
where 
...
equivariant simplicial categories and equivariant quasicategories by Bergner \cite{Ber17}.
However, Bergner's results rely upon the axiomatic framework from \cite{Ste16},
and as noted e.g. in the intro to \cite{BP_HGOP},
the operadic case is intrisically outside the bounds of this formalism.
...
Section 2 mostly recalls 
some previous notions that are use throughout.
Namely, 
Sections \ref{FORESTS_SEC} and \ref{GTREES SEC}
...
The goal of Section \ref{QE_SEC} is to prove 
Theorem \ref{QE THM}
modulo a key lemma, Lemma \ref{KEYPRVAR LEM},
whose proof is postponed to Section{} \ref{KEYRES SEC}.
First,
Section \ref{GSOP_SEC} recalls the model structure on
$\mathsf{sOp}^G$ from \cite{BP_HGOP}.
Then, in 
Section \ref{PREOPOPEQUIV SEC}
we establish Theorem \ref{PREQUIEQUIV THM},
showing that the top functor $N$ in 
\eqref{ADJSQ EQ}
induces an equivalence of homotopy categories.
Lastly, Section \ref{PFMNTHM SEC}
...
% in the prequels to this paper many of the key technical properties, including Proposition \ref{TREEFACT_PROP}, %{\color{red} }
% of $\Omega$
% are proven using 
...
We briefly give a more formal, algebraically-flavored description of a tree,
following Weiss \cite{Wei12}.
For each edge $t$, let $t^\uparrow$ denote the tuple of edges immediately above $t$.
In our example, $c^\uparrow = ab$, $a^\uparrow$ is undefined, and $e^\uparrow$ is empty.
Each vertex can by then specified symbolically
as a \textit{generating broad relation} $t^\uparrow \leq t$;
the above relations then are equivalent to saying that
$a$ is a leaf and $e$ is a \textit{stump}.
These relations satisfy a natural form of transitivity,
and the resulting closed set of broad relations form the \textit{broad poset} description of $T$.
Equivalently, these relations are generating \textit{operations} in the colored set-operad $\Omega(T)$ generated by $T$
(see e.g. \cite[\S 3]{MW07}, \cite[Rem. 4.4, Ex. 4.6]{Per18}, or \eqref{OMEGADEF_EQ}).
...
Further discussion and more rigorous definitions can be found in e.g. \cite[\S 2.1]{BP_edss}.\\

We will find it convenient to assume that 
...
\begin{remark}\label{TODF REM}
	...
	the component labeled $o$ (resp. $d$) % $\psi^o$ (resp. $\psi^-$)
	is an isomorphism.
...
\begin{remark}\label{CNVXM REM}
	...
	the factor labled $i$ in \eqref{NP_TREEFACT_EQ}
	is an isomorphism;
	Equivalently, those maps $\psi$ such that
...
\begin{remark}
	...
	the factors labeled $d,p$ and $o,p$ % $\varphi^{-},\varphi^o$
	are identities are the (not necessarily planar) inner face maps;
	both of these are closed under composition.	
\end{remark}
...
as they give a planar isomorphism of $G$-sets on the set of tree components.

A map $\varphi$ of $G$-trees is called a
...
and $\Omega(T)$ the colored set-operad generated by $T$
(cf. \eqref{TAUNER EQ}). %\cite[\S 3]{MW07}, \cite[Rem. 4.4, Ex. 4.6]{Per18}, \eqref{OMEGADEF_EQ}).
...
\todo[inline]{using $S$ and $T$ for regular trees, vs $U$ and $V$ as has typically been the case}
...
\begin{example}
      \label{NECKLACE_EX}
      Consider the trees $J$ and $U$ below.
      The labeling of edges indicates a map $\boldsymbol{E}(J) \to \boldsymbol{E}(U)$ which
      extends to an inner face map.
      \begin{equation}
            \begin{tikzpicture}[auto,grow=up, level distance = 2.2em,
                  every node/.style={font=\scriptsize,inner sep = 2pt}]%
                  \tikzstyle{level 2}=[sibling distance=4em]%
            ...
            \end{tikzpicture}
      \end{equation}
      Here, $J = T \setminus \set{b,f}$, and the two beads are
           \begin{equation}
            \begin{tikzpicture}[auto,grow=up, level distance = 2.2em,
                  every node/.style={font=\scriptsize,inner sep = 2pt}]%
                  \tikzstyle{level 2}=[sibling distance=4em]%
...
\begin{remark}
      ...
      Moreover, 
      % Given $T \in \Omega$,
      % and writing $T_{\mathfrak{n}} \colon \mathsf{lr}(T) \to T$,
      % one has 
...
\begin{remark}
      % \todo[inline]{why this matches dugger-spivak}
      In the case where $U = [m]$ is linear,
      an inner face $\mathfrak n \colon J \to U$ is isomorphic to some $[k]$ with $1 \leq k \leq m$,
      and each bead of the necklace $\mathfrak n$
      is some $[m_i]$, such that $m_1 + \dots + m_k = m$.
      Then the associated presheaf
      \( \Omega[\mathfrak n] = \iota_!(\Delta[m_1] \vee \dots \vee \Delta[m_k]) \)
      is a necklace in the sense of Dugger-Spivak \cite{DS11},
      and the natural map $\Omega[\mathfrak n] \to \Omega[U]$ is equal to
      \( \iota_!\left(\Delta[\mathfrak n] \to \Delta[m]\right) \)
      from \cite[\S 3]{DS11}.
\end{remark}
...
\begin{example}
      Let $\mathfrak n \colon J \to U$ be the necklace from Example \ref{NECKLACE_EX},
      and consider the outer faces $F$ and $F'$ of $U$.
      ...
      \begin{equation}
            ...
            \end{tikzpicture}
      \end{equation}
      In particular, we note that $\boldsymbol{E}(J)$ and $\boldsymbol{E}(J_F)$ need not be comparable.
\end{example}
...
\begin{definition}\label{XTENDSIM DEF}
	...
	Further, we define $\tau_G(Z)(T) = Z(T)/\sim$.
\end{definition}
...
Let $Z \in \mathsf{dSet}_G$ be a genuine $G$-$\infty$-operad. Then
	\begin{itemize}
	\item[(a)] $\tau_G(Z) \in \mathsf{dSet}_G$ is a genuine equivariant operad, i.e. it satisfies the strict right lifting condition against the Segal core inclusions
