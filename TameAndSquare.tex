...
\begin{definition}
	...
	Moreover, we call $X(\eta)$ the 
	\emph{color set} of $X$
	and denote the associated \emph{color set functor} as below.
...
&
	X \left(\amalg_{A(\eta)} \Omega[\eta]\right)
	\arrow[lu, phantom, "\lrcorner", very near start]
&%%
	\** \ar{r}[swap]{\mathfrak{c}} 
&
...
\begin{remark}
	...
	be the fiber subcategory of the color set functor
	\eqref{COLORSET EQ}
	over $\mathfrak{C}$,
...
\end{proposition}
\begin{proof}
      Given a Segal operad $X$, consider a dendroidal Reedy fibrant replacement $X \to \tilde{X}$ such that $X(\eta) \simeq \tilde{X}(\eta)$. 
...
\begin{definition}
	...
	we define their
	\emph{fiber cotensor}
	$\left\{ K, X \right\}_{\mathsf{\mathfrak{C}_{\bullet}}} \in \mathsf{Preop}$
	via the pullback
