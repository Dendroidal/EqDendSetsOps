...
The sets of edges, inner edges, leaves, and vertices of a tree $T$ are denoted $\boldsymbol{E}(U)$, $\boldsymbol{E}^i(U)$, $\boldsymbol{V}(T)$, and $\boldsymbol{V}(U)$ respectively.
...
\begin{example}
      \label{TREEMAP_EX}
      The labels on the edges of the tree diagrams below indicate a map to $\boldsymbol{E}(T)$ from \eqref{eq:TREE}.
...
We denote objects in $\Omega_G$ by $T = (T_i)_{i \in I}$.
...
Thus, presheaves $X \in \dSet^G$ may be probed by $G$-trees through these $\Omega[T]$,
which allows us to detect norm map information (see \cite[Example 4.9]{Per18}, {\color{blue} Example}).
...
Equivariant Simplicial operads are encoded as algebras over a monad $\mathbb F$ on the category of equivariant symmetric sequences.
\begin{definition}
      ...
      If $\mathfrak C$ is a $G$-set, we write $G \ltimes \Sigma_{\mathfrak C}$ for the Grothendieck construction associated to this action.
      In this case, the automorphism group of $\vect C = (\mathfrak c_1, \dots, \mathfrak c_n; \mathfrak c_0) \in G \ltimes \Sigma_{\mathfrak C}$ is a subgroup of $G \times \Sigma_n$.
      For any $\Gamma \leq G \times \Sigma_n$, we say \textit{$\Gamma$ stabilizes $\vect C$} if $\Gamma \leq \Aut(\vect C)$. 
\end{definition}

\begin{definition}
      Fix a $G$-set of colors $\mathfrak C$.
      The category of \textit{$\mathfrak C$-colored simplicial symmetric sequences} is the functor category
      $\Fun(G \ltimes \Sigma_{\mathfrak C}^{op}, \sSet)$.
\end{definition}
\todo[inline]{if we don't use $W$-construction stuff, this can be much shorter}
\begin{definition}
      The category $\mathsf{sSym}^G$ of
      \textit{equivariant simplicial symmetric sequences} 
      (of all colors) is the category with:
      \begin{itemize}
      \item objects given by pairs $(\mathfrak C, X)$ with
            $\mathfrak{C} \in \mathsf{Set}^G$ a $G$-set of colors and
            $X \colon G \ltimes \Sigma_{\mathfrak{C}}^{op} \to \sSet$ a functor;
      \item arrows $(\mathfrak C, X) \to (\mathfrak D, Y)$ given by a $G$-map 
            ...
            \begin{equation}
                  ...
                  \begin{tikzcd}[row sep = tiny, column sep = 45pt]
                        G \ltimes \Sigma_{\mathfrak{C}}^{op} \arrow[dr, "X"{name=U}] 
                        \arrow{dd}[swap]{\varphi}
                        \\
                        & \sSet
                        \\
                        |[alias=V]| G \ltimes \Sigma_{\mathfrak{D}}^{op} \arrow[ur, "Y"']
                        \arrow[Leftarrow, from=V, to=U,shorten >=0.25cm,shorten <=0.25cm]
                  \end{tikzcd}
            \end{equation}
      \end{itemize}
\end{definition}
$\sSym^G$ and $\sOp^G$ are both natural fibered over the category $\Set^G$ by selecting the set of colors,
and the free operad monad and associated adjunction
\[
      \mathbb F \colon \sSym^G \rightleftarrows \sOp^G \colon U
\]
are fibered over $\Set^G$ as well.
We denote the fibers and monad on the fibers by $\mathbb F_{\mathfrak C} \colon \sSym^G_{\mathfrak C} \to \sOp_{\mathfrak C}^G$.
For further discussion on fibered adjunctions, see \cite{BP_HGOP}.

\begin{convention}
      For convenience, we will often just write $\O \in \sOp^G$, and denote by $\mathfrak C_\O \in \Set^G$ the $G$-set of colors of $\O$.
\end{convention}
...
where weak equivalences are hoomotopically essentially surjective maps which have the same homotopy types for every space of norm maps.
First, the fibers $\sOp^G_{\mathfrak C}$ have a model structure transfered from $\sSym^G_{\mathfrak C}$ by \cite[Thm. I]{BP_HGOP}:
a map $X \to Y$ is a weak equivalence (resp. fibration) iff
...
To describe the generators, we first recall the following definition.
\begin{definition}
      Let $\tilde[1]$ denote the walking isomorphism category,
      with two objects $0,1$ and a unique isomorphism between them.
      
      A \textit{$\sSet$-interval} is a cofibrant simplicial category $\mathbb I \in \mathsf{sCat}_{\set{0,1}}$ equivalent to $\tilde[1]$.
\end{definition}
The canonical example is $W_!N\tilde[1]$, with the equivalence given by the counit of the adjunction.

Then the generating cofibrations in $\mathsf{sOp}^G$
are the maps
\begin{itemize}
\item[(OC1)] $\emptyset \to G/H \cdot \eta$ % for $H \in \F_1$,
\item[(OC2)] $\mathbb{F} \left( \Sigma_{\tau}[G \cdot C_n]/\Lambda \cdot i\right)$
      for $n \geq 0$,
...
\item[(OA2)] 
      ...
      and $j$ a generating trivial cofibration in $\sSet$, $\Lambda^k[m] \to \Delta[m]$ for some $0 \leq k \leq m$ % $ \in \mathcal{J}$.
\end{itemize}
