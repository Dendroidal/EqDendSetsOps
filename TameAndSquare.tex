...
which preserves  
the order relations on edges %  {\color{blue} the broad poset/operadic structure}
...
Let $X \in \mathsf{dSet}_G$ be a genuine $G$-$\infty$-operad. Then
	\begin{itemize}
	\item[(a)] $\tau_G(X) \in \mathsf{dSet}_G$ is a genuine equivariant operad, i.e. it satisfies the strict right lifting condition against the Segal core inclusions
	$Sc[T] \to \Omega[T]$ for $T \in \Omega_G$;
	\item[(b)] the quotient map
	$X \to \tau_G(X)$ is the universal map from $X$ to a genuine equivariant operad;
	\end{itemize}
	In particular, $\tau_G(X)$ is the genuine operadification of $X$.
\end{corollary}
