...
To begin, we extend any $(G,\Sigma)$-family to a family of subgroups of $G \ltimes \Omega_{\mathfrak C}^{op}$.

\todo[inline]{GO THROUGH AND FIX THE OPS}
We write
\begin{equation}
      \label{VC_EQ}
...
\end{equation}
for the natural composite from \eqref{FGC_EQ}
(we note that the diagonal inclusion $\Delta$ does have the correct variance as written:
a map $(x_i) \to (y_i)$ in the source is given by a tuple $(g, \sigma^{op}, (y_i \to g x_{\sigma i})_i)$
while a map in the target is given by a tuple $(\sigma^{op}, (g_i, y_i \to g_i x_{\sigma i}))_i$,
with compositition as expected with $g \circ h = gh$ and $\sigma^{op} \circ \tau^{op} = (\tau \sigma)^{op}$).
...
\begin{definition}
      \label{HATFC_DEF}
      Given a $(G,\Sigma)$-family $\F$ and a $G$-set $\mathfrak C$,
      we define the family $\hat \F_{\mathfrak C}$ of subgroups of $G \ltimes \Omega_{\mathfrak C}^{op}$ to be the pullback
      \[
            \hat \F_{\mathfrak C}  =
            \boldsymbol{V}_{\mathfrak C}^{\**} \left(\F_{\mathfrak C}^\ltimes\right) =
            \boldsymbol{V}_{\mathfrak C}^{\**} \left(\pi_{\mathfrak C}^{\**} \F \right)^\ltimes
      \]
      for $\boldsymbol{V}_{\mathfrak C} \colon: G \ltimes \Omega_{\mathfrak C}^{0,op} \to \Sigma \wr (G \ltimes \Sigma_{\mathfrak C}^{op})$
      and $\pi_{\mathfrak C} \colon G \ltimes \Sigma_{\mathfrak C}^{op} \to G \times \Sigma^{op}$
      as in \eqref{VC_EQ} and Definition \ref{GSFAM_DEF}.     
\end{definition}
...
\begin{remark}
      ...
      \begin{equation}
            \label{FVT_EQ}
            \hat \F_{\mathfrak C} =
            % \boldsymbol{V}_{\mathfrak C}^{\**} \pi_{\mathfrak C}^{\**} (\F^{\ltimes}) =
...
% ------------------------------ MORE EXPLICIT DESCRIPTION FOR GRAPH SUBGROUPS ------------------------------
% \begin{remark}
...
\begin{remark}
      When $\mathfrak C = \**$ and $\F$ is a $(G,\Sigma)$-graph family (Definition \ref{GRAPHSUB_DEF}),
      % a \textit{family of $G$-corollas} \cite[Defn. 4.44]{BP_geo},
...
\begin{remark}
      % Similarly to Definition \ref{HATFC_DEF}, a $(G,\Sigma)$-family $\F$ induces familes
      % $\F_{\mathfrak C}^{\underline{m}}$ and $\hat \F_{\mathfrak C}^{\underline{m}}$ on
      % $G \ltimes \Sigma_{\mathfrak C}^{\underline{m},op}$ and $G \ltimes \Omega_{\mathfrak C}^{\underline{m},op}$, respectively.
      ...
      %             \Sigma \wr \left( G \ltimes \Sigma_{\mathfrak C}^{op} \right)
      %             &
      %             G \ltimes \Omega_{\mathfrak C}^{0,op} \arrow[l, "\boldsymbol{V}_{\mathfrak C}"']
      %             % & % ----------
      %             % \V^{\Sigma \wr \left( G \ltimes \Sigma_{\mathfrak C}^{op} \right)}_{\F_{\mathfrak C}^\ltimes} \arrow[r, "\boldsymbol{V}_{\mathfrak C}^{\**}"] \arrow[d, "\nabla_m^{\**}"']
      %             % &
      %             % \V^{G \ltimes \Omega_{\mathfrak C}^{0,op}}_{\hat \F_{\mathfrak C}} \arrow[d, "\nabla_m^{\**}"]
      %             \\
      %             \displaystyle{
      %               \Sigma \wr \left( \coprod_m G \ltimes \Sigma_{\mathfrak C}^{op} \right)
      %             }
      %             \arrow[u, "\nabla_m"]
      %             &
      %             G \ltimes \Omega_{\mathfrak C}^{\underline{m},op} \arrow[u, "\nabla_m"'] \arrow[l, "\boldsymbol{V}^{\underline{m}}_{\mathfrak C}"]
      %             % & % ----------
      %             % \V^{\Sigma \wr \left( \coprod_m G \ltimes \Sigma_{\mathfrak C}^{op} \right)}_{\left(\F_{\mathfrak C}^{\underline{m}} \right)^{\ltimes}}
      %             % \arrow[r, "{\left(\boldsymbol{V}^{\underline{m}}_{\mathfrak C}\right)^{\**}}"]
      %             % &
      ...
      %       \qquad
      %       \hat \F_{\mathfrak C}^{\underline{m}} = \nabla_m^{\**}\hat \F_{\mathfrak C}.
      % \]
      % However, since \eqref{FCM_EQ} commutes, we have
      % \[
      %       \hat \F_{\mathfrak C}^{\underline{m}}
      %       = \nabla_m^{\**} \hat \F_{\mathfrak C}
      %       = \nabla_m^{\**} \boldsymbol{V}_{\mathfrak C}^{\**} \F_{\mathfrak C}^\ltimes
      %       = \left(\boldsymbol{V}_{\mathfrak C}^{\underline{m}}\right)^{\**} \nabla_m^{\**} \F_{\mathfrak C}^{\ltimes}
      ...
      and $\boldsymbol{V}_{\mathfrak C}^{\underline{m}}$ the functor defined analogously to \eqref{LC_EQ}.
      ...
      However, \eqref{PHIFLTIMES_EQ} implies these induced families agree with those previously defined on the groupoids in the bottom square
...
\begin{proof}
      ...
      \]
      By Propositions \ref{SIGMAWRGF PROP}(ii) and \ref{EQQUILADJ PROP} and \eqref{VCM_LQ_EQ},
      this is a $\left(U^{a}_{\underline{2}}\right)^{\**} \hat \F_{\mathfrak C}^{\underline{2}}$-cofibration.
      
      Since $\F$ is a pseudo indexing system, \eqref{PIS_EQ} and \eqref{FCM_EQ} imply
      \[
            \hat \F_{\mathfrak C}^{\underline{2}}
      ...
      and hence
      \[
            \mathsf{lr}_! = \left((U^{a}_{\underline{2}})^{\**} \nabla_2^{\**} \mathsf{lr}_{\mathfrak C}^{\**} \right)_!
            \colon
            \V^{G \ltimes \Sigma-{\mathfrak C}^{a,op}}_{(U^a_{\underline{2}})^{\**}\hat \F_{\mathfrak C}^{\underline{2}}}
            \longto
            \V^{G \ltimes \Sigma_{\mathfrak C}^{op}}_{\F_{\mathfrak C}}
      \]
      is left Quillen by Proposition \ref{EQQUILADJ PROP}.
      Thus $(\O_{k-1} \to \O_k) = \mathsf{lr}_! n_k^{\O, X,Y}$ is a cofibration in $\V^{G \ltimes \Sigma_{\mathfrak C}^{op}}_{\F_{\mathfrak C}} = \Sym^G_{\mathfrak C, \F}(\V)$, as desired.
\end{proof}
