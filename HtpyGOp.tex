...
is an $\Aut_{G \ltimes \Sigma_{\mathfrak C}}(\vect C)$-$\otimes$-trivial cofibration in $\V^{\Aut_{G \ltimes \Sigma_{\mathfrak C}}(\vect C)}$, 
...
\begin{definition}
      \label{SOTIMES_DEF}
      We say that a map $F \colon \O \to \P$ in $\Op^G_{\mathfrak C}(\V)$ is a \textit{$\Sigma^{\otimes}$-trivial cofibration} if
      for each $\mathfrak C$-signature $\vect C$,
...
\begin{remark}
      \label{GOTC_REM}      
      We highlight that the above proof shows that $\F$-trivial cofibrations in $\Op^G_{\mathfrak C}(\V)$ are
      $\Sigma^\otimes$-trivial cofibrations.
\end{remark}
...
This model structure will be built in Sections \ref{MAPSOPG_SEC}--\ref{HMTYEQ SEC}.
