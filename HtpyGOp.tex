...
% \footnote{Thm 3.1 is non-formal wrt \cite{Ste16} or \cite{BM03}, but \textit{is} formal in the sense that the transfer argument from $\Sym^G_{\**}(\Top^G)$ is an easy application of Quillen's path object argument. The difficulty comes in addressing the analogue of THMII.}
...
Alternatively, one may say that Theorem \ref{THMIII} extends the well known Dwyer-Kan model structures on all colored operads (and maps that change colors) in
...
and the right adjoint is an operadic generalization of the homotopy coherent nerve of \cite{Cor82} (cf. \cite{Joy}, \cite[\S 1.1.5]{Lur09}).
...
\begin{remark}\label{SEMI_REM}
	...
	performing cofibrant and bifibrant replacements.
        % \todo[inline]{fibrant replacements aren't perfect, in that we only have a zig-zag connecting $x$ with $x_f$.}
...
we fill in some technical work that was postponed in \S \ref{PRE SEC},\ref{FIXCOL SEC},
...
Every operad has an underlying symmetric sequence by forgetting the algebraic structure.
In this subsection, we describe this underlying categorical data 
in a way which integrates smoothly with the fibered monad perspective (cf. Remark \ref{COLCHADJ REM}).

We begin in the non-equivariant setting.
In this case, objects in $\Sym_\bullet(\V)$ can be described as presheaves on natural generalizations of the symmetric category.
...
Specifically, our goal in this section is the prove Theorems \ref{THMI} and \ref{THMII};
that is, to build compatible model structures on the fiber categories $\Op_{\mathfrak C}^G(\V)$ of operads with a fixed $G$-set of colors.
As noted in the introduction, this cannot be achieved via an application of the formal machinery of e.g. \cite{Ste16} or \cite{BM03}, as
\[
      \Op_{\mathfrak C}(\V^G) \neq \Op_{\mathfrak C}^G(\V) \neq \left(\Op_{\mathfrak C}(\V)\right)^G
\]
unless $\mathfrak C$ has a trivial $G$-action.
Instead, we will transfer the model structure from the category $\Sym_{\mathfrak C}^G(\V) = \Fun(G \ltimes \Sigma_{\mathfrak C}^{op}, \V)$ of $\mathfrak C$-colored symmetric sequences.
However, we will not be using the projective model structures on these functor categories, as this will not provide the desired weak equivalences on operads.
To build the correct model structures on $\Sym_{\mathfrak C}^G(\V)$, we will
extend the definition and analysis of the various model structures on the category $\V^G$ of $G$-objects in $\V$ determined by a family of subgroups of $G$
from a group $G$ to an arbitrary groupoid $\G$.
This analysis will further streamline our analysis of the naturality of these model structures, as well as
their homotopical interactions with each other and tensor products.

This section is organized as follows.
In \S \ref{EHT_SEC} we introduce the global monoid axiom,
and conduct our analysis of the homotopy theory of $\G$-objects in $\V$, culminating in Proposition \ref{SIGMAWRGF PROP}.
We also provide sufficient conditions for the existence of these model structure and the satisfaction of the global monoid axiom and other suitably nice properties in Propositions \ref{WEAKCELL PROP} and \ref{ALLEQ PROP}.

Next, in \S \ref{SYMC_MS_SEC}, we apply \S \ref{EHT_SEC} to the categories $G \ltimes \Sigma_{\mathfrak C}^{op}$, 
in such a way that the model structures on the fibers $\Sym_{\mathfrak C}^G(\V)$ are compatible with the fibration $\Sym_\bullet^G(\V) \to \Set^G$.
In \S \ref{OPC_MS_SEC}, we apply the results of Appendix \ref{MONAD_APDX}, given in Lemma \ref{OURE LEM},
to transfer these model structures to $\Op_{\mathfrak C}^G(\V)$ and prove Theorem \ref{THMI}.
We end with a proof of Theorem \ref{THMII} in \S \ref{MS_SEC} by a careful synthesis of the proof of Theorem \ref{THMI} with the theory from \S \ref{EHT_SEC}. 

\subsection{Equivariant homotopy theory}
\label{EHT_SEC}

We begin by recalling the genuine model structure on the category $\V^G$ of $G$-objects in $\V$.
...
we conflate a $(G,\Sigma)$-family $\F$,
...
\begin{proof}[Proof of Proposition \ref{SIGMAG_COF PROP}]
	...
	then each 
	of the filtration pieces 
	$\O_{k-1} \to \O_k$ from \eqref{OUFILRE EQ}
...
\begin{example}
	...
	$W_! \colon \sSet \to \Cat_{\bullet}(\sSet)$ is the left adjoint to the homotopy coherent nerve
        of \cite{Cor82}
        (see e.g. \cite[\S 1]{Joy02}). % \cite{Rie11}, \cite{Rie} 
\end{example}
...
\end{equation}
      ...
      says that $G/H \cdot (-) \colon \mathsf{Cat}_{\{0\}}(\V) \to \mathsf{Cat}^G_{G/H,\F^{all}}(\V)$\todo{check this formatting} is left Quillen,
