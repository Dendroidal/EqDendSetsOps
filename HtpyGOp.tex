...
\begin{remark}
      ...
      We note that we have a homomorphism $\pi_{T_v} \colon \Aut_v(T) \to \Aut(T_v)$ that does not extend to all of $\Aut(T)$.

      Now, given a $(G,\Sigma)$-family $\F$ and a subgroup $ \Gamma \leq G \times \Aut(T)$, we see that
      $\Gamma \in \F_T = (\boldsymbol{V}^{\**}\F^\ltimes)_T$ iff
...
\begin{remark}
	...
	More precisely, a weak indexing system is precisely a pseudo indexing system consisting of $G$-graph subgroups (e.g. \cite[Defn. 6.34]{BP_geo}).
...
\begin{remark}
	Theorem \ref{SIGMAG_COF_THM} is the colored analogue to {\color{red} \cite[{Remark 6.64 with Theorem 6.59}]{BP_geo}}.
...
\begin{proof}
	...
	whenever $\O$ is $\Sigma_\F$-cofibrant and $u \colon X \to Y$ is an $\F$-cofibration in $\Sym^G_{\mathfrak C,\F}(\V)$.
      ...
      $\left(\mathsf{lr}_{\mathfrak C} \pi_{\mathfrak C, a} \right)_!$
      sends
      ...
      a $\nabla_2^{\**}\F_{\mathfrak C}$-cofibration
      ...
      $(\O_{k-1} \to \O_k) =  \left(\pi_{\mathfrak C, a} \mathsf{lr}_{\mathfrak C} \right)_! n_k^{\O, X,Y}$
