...
the $n$-th operadic level $\O(n)$ has both a $\Sigma_n$-action and $G$-action, commuting with each other, or, equivalently, 
...
Alternatively, one may say that Theorem \ref{THMIII} extends the well known model structures of on all colored operads (and maps that change colors) in
...
Hence, our desired model structure on 
$\mathsf{sOp}^G_{\bullet}$
produces a model structure on 
$\mathsf{sCat}^G_{\bullet}$
which \emph{does} coincide with the model structure obtained by applying
\cite[Prop. 2.6]{Ste16}
...
our first step towards building model structures on the category
$\mathsf{sOp}^G_{\bullet}$
of equivariant operads with all colors in Theorem \ref{THMIII}
...
in $\mathfrak{C}$, called \emph{$\mathfrak{C}$-signatures}.
...
\begin{enumerate}[label = (\roman*)]
	...
	\item $(\V, \otimes)$ has cofibranty symmetric pushout powers (Definition \ref{CSPP_DEF});
...
a similar $\F$-model structure on a simpler category
...
For certain special $(G,\Sigma)$-families, we will be able to show that cofibrant objects in 
$\Op^G_{\mathfrak C}(\V)$
...
\begin{customthm}{II}\label{THMII}
	...
	Then if $\O \to \mathcal{P}$ is a cofibration between cofibrant objects for the $\F$-model structure in $\Op^G_{\mathfrak{C}}(\V)$,
...
as a particular case of Theorem \ref{SIGMAG_COF_THM}.
...
a ``category of components'' $\pi_0 \mathcal{C}$.
...
%
Informally, this ensures the spaces 
$\O(\vect{C})^{\Lambda}$
...
\subsubsection*{Remainders (equivariant operads explicitly)}
