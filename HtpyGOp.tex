...
Finally, a desire to understand and control the interaction of algebra and homotopy theory has fueled a wave of papers analyzing
model structures and other homotopical features of the categories of enriched operads and their algebras.

We bring this expansion to the equivariant setting.
...
\begin{center}
      \begin{tabular}[h]{c | c | c}
        ...
        \cite{Ber07b},\cite{CM13b} & $\V = \sSet$ & \begin{tabular}[t]{@{}c@{}} weak equivalences: DK-equivalences \\ fibrations: isofibrations \end{tabular}
        \\
        \cite{BM13},\cite{Cav} & ``adequate'' plus technical & \begin{tabular}[t]{@{}c@{}} fibrant objects: locally fibrant objects \\ trivial fibrations: local trivial fibrations which are surjective on objects \\ \textit{weak equivalences: DK-equivalences} \end{tabular}
        \\
        \cite{Mur15} & monoid axiom & \begin{tabular}[t]{@{}c@{}} weak equivalences: DK-equivalences \\ trivial fibrations: local trivial fibrations which are surjective on objects \end{tabular}
        \\
        \cite{Sta14} & monoid axiom, simplicial, plus weak equivalences closed under transfinite (lazy) \begin{tabular}[t]{@{}c@{}} weak equivalences: DK-equivalences \\ fibrations: isofibrations \end{tabular}
      \end{tabular}
\end{center}
...
in Theorem \ref{THMII} on the category
...
Then, for any tree $\vect{T}$ one has
\[\F_{\vect{T}} = 
\left(\boldsymbol{V}_{\mathfrak{C}}^{\**}\F^{\ltimes}\right)_{\vect{T}}\]

\begin{definition}
Let $\F$ be a $(G,\Sigma)$-family.
We say $\F$ is a \emph{pseudo indexing system} if
\[
\boldsymbol{V}_{\mathfrak{C}}^{\**}\F^{\ltimes}
	\subseteq
\mathsf{lr}_{\mathfrak{C}}^{\**}\F^{\ltimes}
\]
\end{definition}


\todo[inline]{the definition given below also does not depend on $\F$ being a $(G,\Sigma)$-graph family. this is a good synthesis}
...
Specifically, this result is a special case of Theorem \ref{SIGMAG_COF_THM},
with Proposition \ref{BOXVT_PROP} the key technical step in the proof.
...
First, we extend any $(G,\Sigma)$-family to a family of subgroups of $G \ltimes \Omega_{\mathfrak C}^{op}$ for any $G$-set $\mathfrak C$.
...
\begin{definition}
      Fix a $(G,\Sigma)$-family $\F = \set{\F_n}$.
      Define the family $\hat \F = \hat \F_{\mathfrak C}$ of subgroups of $G \ltimes \Omega_{\mathfrak C}^{op}$ to be the pullback
      \[
            \hat \F_{\mathfrak C} = \boldsymbol{V}_{\mathfrak C}^{\**} \phi_{\mathfrak C}^{\**} \F^\ltimes
      \]
      for $\phi_{\mathfrak C} \colon G \ltimes \Sigma_{\mathfrak C}^{op} \to G \times \Sigma^{op}$
      the natural map.
\end{definition}

{\color{red} Luis: the $\phi_{\mathfrak{C}}$ notation you are using here clashes with the $\pi_{\mathfrak{C}}$ notation from Remark \ref{FAMC_DEF_REM}. Fix this.}

Unpacking, we see that for any $\mathfrak C$-tree $\vect T$,
$\F_{\vect T} = \hat \F_{\vect T}$ is the family of subgroups of $\Aut_{G \ltimes \Omega_{\mathfrak C}^{op}}(\vect T)$ consisting of
those $\Gamma$ such that, for all vertices $v \in V(\vect T)$,
$\pi_{\vect T_v}(\Gamma) \in \F_{\vect T_v} = \phi_{\vect T_v}^{\**}\F_{T_v}$.
% that is,
% \[
%       \phi_{\vect T_v} \pi_{\vect T_v}(\Gamma) = \pi_{\vect T} \phi_{T_v}(\Gamma) \in \F_{T_v}
% \]
% using the commuting square
% \begin{equation}
%       \label{AUTT_EQ}
%       \begin{tikzcd}
%             \Aut_{G \ltimes \Omega_{\mathfrak C}^{op}}(\vect T) \arrow[d, "\pi_{\vect T_v}"'] \arrow[r, "\phi_{\vect T}"]
%             &
%             \Aut_{G \times \Omega^{op}}(T) \arrow[d, "\pi_{T_v}"]
%             \\
%             \Aut_{G \ltimes \Omega_{\mathfrak C}^{op}}(\vect T_v) \arrow[r, "\phi_{\vect T_v}"]
%             &
%             \Aut_{G \times \Omega^{op}}(T_v)
%       \end{tikzcd}
% \end{equation}


{\color{red} Luis: What does 
	$\pi_{\vect T_v}(\Gamma)$ mean? 
	I have a strong suspicion that you are not referring to the right thing.
}
...
\begin{remark}
      We can recover each $\hat \F_{\mathfrak C}$ from $\hat \F_{\set{\**}}$.
      As $\F_{\vect C} = \phi_{\vect C}^{\**} \F_C$ for all $\mathfrak C$-signatures $\vect C$ by \eqref{PHISTAR_EQ},
      we have
      \begin{equation}
            \label{FVT_EQ}
            \hat \F_{\mathfrak C} = \phi_{\mathfrak C}^{\**}\hat \F_{\set{\**}},
            \qquad
            \F_{\vect T} = \phi_{\vect T}^{\**}(\F_T)
      \end{equation}
      for
      $ \phi_{\mathfrak C}  \colon G \ltimes \Omega_{\mathfrak C}^{op} \to G \times \Omega^{op}$.
      % \[
      %       G \ltimes \Omega_{\mathfrak C}^{op} \xrightarrow{\ \phi_{\mathfrak C} \ } G \times \Omega^{op},
      %       \qquad
      %       \Aut_{G \ltimes \Omega_{\mathfrak C}^{op}}(\vect T) \xrightarrow{\ \phi_{\vect T} \ } \Aut_{G \times \Omega^{op}}(T).
      % \]
\end{remark}
...
We can use this inductive decomposition to detect cofibrancy.
%
For full generality, given any $m \geq 0$, we define a \textit{coherent $\underline{m}$-labeling} on a $\mathfrak C$-tree $\vect T \in \Omega_{\mathfrak C}$
to be an assignment
\[
      \boldsymbol{E}(\vect T) / \Aut_{G \ltimes \Omega_{\mathfrak C}^{op}}(T) \longto \underline{m} = \set{1,2,\dots,m}.
\]
Denote the category of coherently $\underline{m}$-labeled $\mathfrak C$-trees and label-preserving isomorphisms by $\Omega^{\underline{m}}_{\mathfrak C}$.

{\color{red} Luis: Surely $\boldsymbol{E}(\vect T)$ should be $\boldsymbol{V}(\vect T)$
}


{\color{red} Luis: This is a very poor choice. 
	There is no reason to restrict the labeling according to the automorphisms, when you can instead restrict the automorphisms according to the labelings, which gives a more general result.
}



{\color{red}Luis:
	$\Omega^{\underline{m}}_{\mathfrak{C}}$ will admit (either as is currently or using the more general definition in the previous comment)
	a vertex functor
	\[
	\Omega^{\underline{m}}_{\mathfrak{C}}
	\xrightarrow{\boldsymbol{V}}
	\Sigma \wr \left(\coprod_m \Sigma_{\mathfrak{C}} \right)
	\]
	and analogously for when there are $G$-actions.
	Using this alternate vertex functor one should be able to completely sidestep the need to do induction over the trees at all.
	I.e. Remark \ref{FT_REM} should not be needed 
	for anything.
	Fix the argument by removing all the unnecessary discussion related to Remark \ref{FT_REM}.
}
...
\begin{proposition}
      \label{BOXVT_PROP}
      Suppose $f_a: A_a \to B_a$, $1 \leq a \leq m$, are $\F$-cofibrations in $\Sym_{\mathfrak C}^G(\V)$.
      ...
      Then $\hat f$ is an $\F$-cofibration in $\V^{G \ltimes \Omega_{\mathfrak C}^{\underline{m},op}}_{\F}$,     
      %
      ...
      Moreover, $\hat f(\vect T)$ is a trivial cofibration if any $f_a$ is for any label $1 \leq a \leq m$ present in $\vect T$.
\end{proposition}
\begin{proof}
      ...
      The moreover follows by an analogous induction.
      % Let $\vect T \in G \ltimes \Omega_{\mathfrak C}^{\underline{m},op}$ with $\vect T = \vect R \amalg (\vect T_i)$ and $|V(\vect T)| = n+1$.
      % By Proposition \ref{EQQUILADJ PROP}, \eqref{FVECTT2_EQ}, and Remark \ref{COHMLBL_REM}, we have left Quillen functors
      % \[
      %       \V^{\Sigma_r \wr \left(G \ltimes \left(\Omega_{\mathfrak C, \leq w}^{\underline{m}}\right)^{op}\right)}_{\F^{\ltimes r}}
      %       \xrightarrow{\quad \Delta_r^{\**} \quad}
      %       \V^{G \ltimes \Sigma_r \wr \left(\Omega_{\mathfrak C, \leq w}^{\underline{m}}\right)^{op}}_{\Delta_r^{\**}\F^{\ltimes r}}
      %       \xrightarrow{\quad ev_{(\vect T_i)} \quad}
      %       \V^{\Aut(\vect T)}_{\F_{\vect T}},
      % \]
      % thus Proposition \ref{SIGMAWRGF PROP}(i) implies
      % $\hat f^{\square r}\left((\vect T_i)_{1 \leq i \leq r}\right)$ is a $\F_{\vect T}$-cofibration in $\V^{\Aut(\vect T)}_{\F_{\vect T}}$.
      % More easily, $f(\vect R)$ is a $\F_{\vect T}$-cofibration in $\V^{\Aut(\vect T)}_{\F_{\vect T}}$.

      % Proposition \ref{RESGEN PROP} then implies that
      % $\hat f(\vect T) = f(\vect R) \square f^{\square r}\left( (\vect T_i)_{1 \leq i \leq r} \right)$ is as well, finishing the proof.
\end{proof}
...
For the last step of the proof of Theorem \ref{THMIII}, we  will need to restrict our $(G,\Sigma)$-families significantly.

\begin{definition}
      \label{PIS_DEF}
      A $(G,\Sigma)$-family $\F$ is called a \textit{pseudo indexing system} if
      \[
            \hat \F_{\set{\**}} \subseteq \pi_{\mathsf{lr}}^{\**}\F,
      \]
      for $\pi_{\mathsf{lr}} \colon G \times \Omega^{op} \to G \times \Sigma^{op}$.
\end{definition}

Equivalently,
\begin{equation}
      \label{PIS_EQ}
      \F_{T} \subseteq \pi_{\mathsf{lr}(T)}^{\**} \F_{\mathsf{lr}(T)}
\end{equation}
for all $T \in \Omega$,
$\pi_{\mathsf{lr}(T)} \colon \Aut_{G \times \Omega^{op}}(T) \to \Aut_{G \times \Omega^{op}}(\mathsf{lr}(T))$.



% \begin{definition}
%       \label{GRAPHSUB_DEF}
%       A subgroup $\Gamma \leq G \times \Sigma_n^{op}$ is a \textit{$G$-graph subgroup} if
%       $\Gamma \cap \Sigma_n^{op} = \**$;
%       that is, if $\Gamma$ consists of elements of the from $(h, \phi(h)^{-1})$
%       for $h \in H \leq G$ and $H \xrightarrow{\phi} \Sigma_n$.
      
%       We write $\mathcal{F}^{\Gamma} = \set{\mathcal{F}_n^{\Gamma}}$
%       for the $(G,\Sigma)$-family of $G$-graph subgroups.

%       A \textit{$(G,\Sigma)$-graph family} is any $(G,\Sigma)$-family $\mathcal F = \set{\F_n}$ consisting of subfamilies of $\F^\Gamma$.
% \end{definition}

% We consider a very special class of $(G,\Sigma)$-graph families.

% \begin{definition}
%       \label{WIS_DEF}
%       A $(G,\Sigma)$-graph family $\F = \set{\F_n}$ is a \textit{weak indexing system} if
%       for all $T \in \Omega = \Omega_{\set{\**}}$ with $\mathsf{lr}(T) = C$,
%       \begin{equation}
%             \label{WIS_EQ}
%             \F_T \subseteq \pi_{\mathsf{lr}(T)}^{\**} \F_C
%       \end{equation}
%       for $\pi_{\mathsf{lr}(T)} \colon G \times \Aut_{\Omega^{op}}(T) \to G \times \Aut_{\Omega^{op}}(C)$ the natural map.
% \end{definition}

% \begin{example}
%       For any group $G$, $\F^\Gamma$ is a weak indexing system.
% \end{example}
...
%       additional context and motivation can be found at each of these locations.
      
%       The descriptions as written previously involve the combinatorics of a certain category $\Omega_G$ of \textit{$G$-trees} or certain closure conditions under operations of finite sets,
%       neither of which will be needed for this article.
%       However, by \cite[Remark 6.45]{BP_geo}, the above characterization is the same as those already given.
% \end{remark}
...
\begin{remark}
      ...
      (\cite[Defn. 9.5]{Per18}, \cite[Defn. 4.49]{BP_geo}, \cite[Defn. 6.2]{BP_edss});
      the two notions are in agreement by \cite[Remark 6.45]{BP_geo} when each $\F_n$ is a collection of \textit{$G$-graph subgroups} of $G \times \Sigma_n$.
...
\begin{lemma}
      ...
      \item For all $G$-sets $\mathfrak C$,
            $\hat \F \subseteq \pi_{\mathfrak C, \mathsf{lr}}^{\**} \phi_{\mathfrak C}^{\**} \F$
            for $\pi_{\mathfrak C, \mathsf{lr}} \colon G \ltimes \Omega_{\mathfrak C}^{op} \to G \ltimes \Sigma_{\mathfrak C}^{op}$.
      \end{enumerate}
\end{lemma}

\begin{remark}
      (ii) is equivalent to these statement that
      for each $\mathfrak C$-trees $\vect T$ with $\mathsf{lr}(\vect T) = \vect C$,
      $\F_{\vect T} \subseteq \pi_{\mathsf{lr}(\vect T)}^{\**}\F_{\vect C}$
      for $\pi_{\mathsf{lr}(\vect T)} \colon \Aut_{G \ltimes \Omega_{\mathfrak C}^{op}}(\vect T) \to \Aut_{G \ltimes \Sigma_{\mathfrak C}^{op}}(\vect C)$.
\end{remark}
...
\begin{proof}
      ...
      \[
            \hat \F_{\mathfrak C} =
            \phi_{\mathfrak C}^{\**} \hat \F_{\set{\**}} \subseteq
            \phi_{\mathfrak C}^{\**} \pi_{\mathsf{lr}}^{\**} \F =
            \pi_{\mathsf{lr}, \mathfrak C}^{\**} \phi_{\mathfrak C}^{\**} \F
      \]
      by \eqref{FVT_EQ} and \eqref{PIS_EQ}.
      % \[
      %       \F_{\vect T} = 
...
\end{theorem}
\begin{proof}
      Using the notation from \eqref{NKOXY EQ},
      $n_k^{\O,X,Y}(\vect U)$ is a cofibration in $\V^{\Aut(\vect U)}_{\F_{\vect U}}$
      by Proposition \ref{BOXVT_PROP}.
      The result then follows from
      the filtration \eqref{NKOXY EQ}/\eqref{FILTPUSHG EQ}, Proposition \ref{EQQUILADJ PROP}, and Corollary \ref{PIVECTT_COR}.
\end{proof}
...
in the formulation of Theorem \ref{THMII},
...
T \downarrow \Omega^0_{\mathfrak{C}} 
	\ar{r} & 
\left( T_v \right)_{\boldsymbol{V}(T)} \downarrow \Sigma \wr \Sigma_{\mathfrak{C}}
	\ar{r} & 
\left( T_v \right)_{\boldsymbol{V}(T)} \downarrow_{\Sigma} \Sigma \wr \Sigma_{\mathfrak{C}}
\end{tikzcd}
\end{equation}
\begin{equation}\label{COMPISO EQ}
T \downarrow \Omega^0_{\mathfrak{C}} 
