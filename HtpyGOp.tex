...
=
\coprod_{[\vect{T}] \in \mathsf{Iso}(\Omega_{\mathfrak{C}}\downarrow \vect{C})}
\left(
\left(
...
Adapting the work in {\color{red} \cite{BP_geo}} by keeping track of colors one obtains two key functors
\[
\Omega_{\mathfrak{C}}^0 \xrightarrow{\mathsf{lr}} \Sigma_{\mathfrak{C}}
\qquad \qquad
...
where by ``tall map'' we mean a map
which sends leaves to leaves and the root to the root.
As for the ``vertex'' functor 
$\boldsymbol{V}$,
we have
$\boldsymbol{V}(\vect{T}) = 
(\vect{T}_v )_{v \in \boldsymbol{V}(T)}$,
i.e. 
$\boldsymbol{V}(\vect{T})$ is the tuple of colored corollas indexed by the vertices of $T$. Note that while as an object this is essentially the same as the set of vertices of $T$,
by regarding it as an object in $\Sigma \wr \Sigma_{\mathfrak{C}}$ rather than just a set we can keep track of extra automorphism data.
...
\section{Colored operads}\label{FIXCOL SEC}
...
however it is clear that they are equivalent.


{\color{Orange} % ----------
  If we want Lemma \ref{BOXVT_LEM}, I don't think we can give a complete argument in less time than {\color{OliveGreen} Olive Green work}.
  I do not know of a way to conclude that $n_k^{\O,X,Y}(\vect U)$ is a (trivial) cofibration in $\V^{\Aut(\vect U)}_{\F_{\vect U}}$
  without Lemma \ref{BOXVT_LEM} in general.
...
\begin{proof}
      ...
      this filtration, Proposition \ref{EQQUILADJ PROP}, and Corollary \ref{PIVECTT_COR}. % from Proposition \ref{BOXVT_PROP}
\end{proof}
...
This model structure will be built in Sections \ref{MAPSOG_SEC}--\ref{HMTYEQ SEC}.
\S \ref{MAPSOPG_SEC} introduces several classes of maps of operads,
\S \ref{GENCOF SEC} produces a generating set of (trivial) cofibrations,
and \S \ref{TRIVCOF_SEC} concludes in Proposition \ref{J-CELL_PROP} that trivial cofibrations are in fact weak equivalences.
...
The conditions on $\V$ are significant;
...
\begin{notation}
      ...
      More simply, we write $\eta$ for the $\V$-category which represents objects,
...
\item \textit{path-lifting}
	...
	$\eta \xrightarrow{0} \I$, $\eta \xrightarrow{1} \I$
...
Definition \ref{MODEL_DEFN} describes a model structure on 
...
Moreover, Lemma \ref{POINT_4_LEMMA} and Proposition \ref{J-CELL_PROP} establishes (4).
...
that relative $\mathcal J_\F$-cells are $\F$-weak equivalences.
...
\label{INTCOF THM}
Suppose $\V$ is satisfies the monoid axiom and has cofibrant unit.
...
\todo[inline]{after Main Results, include a thm-review; e.g. monoid axiom implies $\Cat_{\mathfrak C}(\V)$ has the canonical model structure}
...
\begin{proof}
      By the global monoid axiom \eqref{GLOMO_EQ} and Lemma \ref{TRANSCOMP_ES_LEM}, it suffices to show that every pushout
      ...
      the map $\O \to \P$ is a levelwise global $\otimes$-trivial cofibration and essentially surjective.
      ...
      so that the claim follows by Remarks \ref{FIBERGLMOD REM} and \ref{GOTC_REM}.
      ...
      an $\F$-weak equivalence and a levelwise global $\otimes$-trivial cofibration.
      ...
      is itself a local isomorphism, and thus certainly a local $\F$-weak equivalence and a levelwise global $\otimes$-trivial cofibration.
      % 
...
that we can assume the map is a fibration in $\mathsf{Cat}_{\{0,1\}}(\mathcal )$
...
which depends on a careful and technical analysis of the $W$-construction applied to 
$\widetilde{\mathbbm{1}}$.
...
and $2$-out-of-$3$ for $\Cat_\bullet(\V)$ % (unenriched categories)
implies 
...
it is also this case which requires that $\F$ respects units.
...
$i\mathfrak{a} = -\mathfrak a = \mathfrak{a}$,
$-\mathfrak{b} = \mathfrak{b}$,
$i\mathfrak{c} = -\mathfrak c = \mathfrak{c}$.
...
and suppose 
$\Lambda \leq G \times \Sigma_n^{op}$
stablizes both $\vect{B},\vect{C}$, i.e. 
