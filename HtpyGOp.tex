...
\mathsf{Set}
\]
are represented by (some choice of) $f^{\**}c$.

{\color{red} HERE}

However, the universal condition in \eqref{CARTARDEF EQ}
is stronger
\end{remark}
...
\begin{notation}
Given a functor $\mathcal{B} \xrightarrow{\mathcal{C}_{\bullet}} \mathsf{Cat}$
we write 
...
$\mathcal{C}_f(c) \to c'$ in $\mathcal{C}_{b'}$.
...
together with equivariance data (cf. \cite[Lemma \ref{TAS-REDUCELAN LEM}]{BP_TAS}), % (this strengthens Lemma \ref{REDUCELAN LEM} a little),
