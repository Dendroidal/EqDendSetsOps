...
\begin{center}
      \begin{tabular}[h]{c | c | c}
        ...
        \\
        \cite{BB13} & monoid axiom & tame polynomial monads are admissible
        \\
        \cite{PS14} & (symmetric) $h$-monoidal plus technical & (symmetric) colored operads are admissible
      \end{tabular}
\end{center}
...
\begin{center}
      \begin{tabular}[h]{c | c | c}
        ...
        \\
        \cite{BM12},\cite{Cav} & ``adequate'' plus technical & \begin{tabular}[t]{@{}c@{}} fibrant objects: locally fibrant objects \\ trivial fibrations: local trivial fibrations which are surjective on objects \\ \textit{weak equivalences: DK-equivalences} \end{tabular}
        \\
...
\begin{definition}
      Fix a $(G,\Sigma)$-family $\F = \set{\F_n}$, and let $\vect T \in G \ltimes \Omega_{\mathfrak C}^{op}$.
      Define the family $\F_{\vect T}$ of $\Aut_{G \ltimes \Omega_{\mathfrak C}^{op}}(\vect T)$ to be
      ...
      Denote by $\hat \F = \hat \F_{\mathfrak C}$ the family $\set{\F_{\vect T}}$ of subgroups of $G \ltimes \Omega_{\mathfrak C}^{op}$.
\end{definition}
...
\begin{remark}
      ...
      \begin{equation}
            \label{FVT_EQ}
            \hat \F_{\mathfrak C} = \phi_{\mathfrak C}^{\**}\hat \F,
            \qquad
...
\begin{remark}
      \label{FT_REM}
      We can additionally recover $\F_{\vect T}$ using an inductive definition of $\hat \F$.
      For $n \geq 0$, let $\Omega_{\mathfrak C, \leq n} \subseteq \Omega_{\mathfrak C}$ denote
      the subcategory spanned by those trees $\vect T$ with $|V(\vect T)| \leq n$.
      We extend $\F$ to a families $\F_{\leq n}$ of subgroups of $G \ltimes \Omega_{\mathfrak C, \leq n}^{op}$,
      with $\F_{\leq \infty} = \hat \F$. 
      
      Define $\F_{\leq 0} = \phi_0^{\**}\F$ for $\phi_{\leq 0} \colon G \to G \times \Sigma^{op}$, $\phi_{\leq 0}(\**) = (\**,1)$,
      and $\F_{\leq 1} = \F$.
      %
      Now suppose $\F_{\leq n}$ has been defined as a family of subgroups of $G \times \Omega_{\mathfrak C, \leq n}^{op}$,
      and suppose $\vect T \in \Omega_{\mathfrak C}$ has $|V(T)| = n+1$.
      $\vect T$ has a grafting decomposition $\vect T = \vect R \amalg (\vect T_i)_{1 \leq i \leq r}$
      (cf. \cite[Example 3.34]{BP_geo})
      with $\vect R \in \Sigma_{\mathfrak C}$ and $\vect T_i \in \Omega_{\mathfrak C, \leq n}$.
      The automorphism group of $T$ is detected by this tuple $(T_i)$:
      \[
            \Aut_{G \times \Omega_{\mathfrak C}^{op}}(\vect T) = \Aut_{G \ltimes \Sigma_r \wr \Omega_{\mathfrak C, \leq n}^{op})}\left((\vect T_i)_{1 \leq i \leq r}\right),
      \]
      and moreover the family $\F_{\vect T}$ is detected by this decomposition:
      \begin{equation}
            \label{FVECTT2_EQ}
            \F_{\vect T} =
            \left( \Delta_r^{\**} \left( \F_{\leq n}^{\ltimes r} \right) \right)_{(\vect T_i)_{1 \leq i \leq r}}
            \ \cap \
            \pi_{\vect T_R}^{\**} \F_{\vect R}
      \end{equation}
      for
      \[
            G \times \Sigma_r \wr \Omega_{\mathfrak C, \leq n}^{op} \xrightarrow{\ \Delta_r \ }
            \Sigma_r \wr \left(G \ltimes \Omega_{\mathfrak C, \leq n}^{op}\right),
            \qquad
...
\begin{proposition}
      ...
      Moreover, $\hat f(\vect T)$ is a trivial cofibration if any of the $f_a(\vect T_v)$ are.
\end{proposition}
...
\begin{proof}
      ...
      For induction, suppose $\hat f$ is a $\F$-cofibration in $\V^{G \ltimes \Omega_{\mathfrak C, \leq n}^{op}}_{\F}$,
      and that $\vect T \in G \ltimes \Omega_{\mathfrak C}^{\underline{m},op}$ with $\vect T = \vect R \amalg (\vect T_i)$ and $|V(\vect T)| = n+1$.
...
\begin{lemma}
      ...
      \item For all $G$-sets $\mathfrak C$ and $\mathfrak C$-trees $\vect T$ with $\mathsf{lr}(\vect T) = \vect C$, 
            $\F_{\vect T} \subseteq \pi_{\mathsf{lr}(\vect T)}^{\**}\F_{\vect C}$
            for $\pi_{\mathsf{lr}(\vect T)} \colon \Aut(\vect T) \to \Aut(\vect C)$.
      \end{enumerate}
...
The following result is then immediate from Proposition \ref{EQQUILADJ PROP}.
\begin{corollary}
      \label{PIVECTT_COR}
