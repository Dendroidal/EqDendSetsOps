...
any $(G,\Sigma)$-family $\F$ yields $\F$-model structures $\Cat_{\mathfrak C,\F}^G(\V)$, $\Cat_{\bullet,\F}^G(\V)$.

Unpacking, from \eqref{THMI_EQ}, \eqref{THMIII1ST EQ}, and \eqref{THMIII2ND EQ},
it is clear that these categorical model structures only depend on $\F_1$ in $\F$.
Moreover, it is straightforward that these equations yield that
...
We can also recover these model structures directly from our analysis in this paper, replacing
\begin{itemize}
...
\item $\boldsymbol{V}_{\mathfrak C} \colon \Omega_{\mathfrak C} \to \Sigma \wr \Sigma_{\mathfrak C}$ with
      $\boldsymbol{V}_{\mathfrak C} \colon \mathbb{N}_{\matfrak C} \to \mathbb{N} \wr (\mathfrak C^{\times 2}) = \coprod_{n \geq 0} (\mathfrak C^{\times 2})^{\times n}$.
\end{itemize}
In this case, we see in particular that the proof of Theorem \ref{THMI} no longer depends upon cofibrant symmetric pushout powers
(as expected, as we have removed all traces of the symmetric groups),
and thus this hypothesis can be stricken from both Theorems \ref{THMI} and \ref{THMIII}.

Moreover, the pseudo indexing system condition in the linear setting becomes trivial,
as both sides of the equation simply become the family $\coprod_{n \geq 0} \F_1$ of the groupoid $G \times \mathbb{N}$,
and thus Theorem \ref{THMII} follows for any family $\F_1$ of $G$.
...
\begin{example}
      ...
      \todo[inline]{does top satisfy global monoid axiom?}
