...
then its $n$-the level $\O(n)$ has both a $\Sigma_n$-action and $G$-action, commuting with each other, or, equivalently, 
...
Alternatively, Theorem \ref{THMIII} extends the well known model structures of on all colored operads (and maps that change colors) in
...
Similarly, the project this paper belongs to is based in the identification by the authors of a category
...
the correct notion of equivalences as suggested by graph subgroups,
but it is worth nothing that this here is intrinsically operadic
and does not occur when working with categories.
Indeed, the inclusion
$\mathsf{sCat}^G_{\bullet} \subset 
\mathsf{sOp}^G_{\bullet}$
of $G$-equivariant colored categories into 
$G$-equivariant colored operads
identifies
$\mathsf{sCat}^G_{\bullet} \subset 
