...
\begin{remark}
	As $\Cat^{G}_{\mathfrak C}(\V) = \Op^G_{\mathfrak C}(\V) \downarrow \eta$,
...
For technical assistance, we will use a specific model of ``the category of finite sets'' throughout this paper.
\begin{definition}
      Let $\Fin$ denote a (fixed) full subcategory of \textit{ordered finite sets} and unordered set maps,
      such that the wide subcategory of order-preserving maps is skeletal.
      Moreover, let $\Sigma$ denote the core of $\Fin$, the wide subcategory with only the isomorphisms.
      
      A natural model for $\Fin$ is the category with objects $\underline{n} = \set{1,2,\dots, n}$ for $n \geq 0$ and all maps of sets,
      while a natural model for $\Sigma$ is $\coprod_{n \geq 0} \Sigma_n$.
\end{definition}

\begin{remark}
      Given a contravariant functor $\mathcal B^{op} \xrightarrow{\mathcal C_\bullet} \Cat$, we note that
      \[
            \left( \mathcal B^{op} \ltimes \mathcal C_\bullet \right)^{op}
      \]
      is the associated contravariant Grothendieck construction,
      and moreover the natural projection map to $\mathcal B$ is a (split) Grothendieck fibration.
\end{remark}
...
$j_!$ is the inclusion $\Cat^G(\V) = \Op^G_\bullet(\V) \downarrow \1 \into \Op^G_\bullet(\V)$,
...
the category with objects pairs 
...
$\varphi_! (c) \to c'$ in $\mathcal{C}_{b'}$.

Note that the natural projection
$\mathcal{B} \ltimes \mathcal{C}_{\bullet} \to \mathcal{B}$
...
so that the functor $T_{b'} \Rightarrow \check{\varphi}_! T_{b}$
...
\item[(ii)] $T^I$ is a fibered monad with respect to $\pi^I\colon \mathcal{C}^I \to \mathcal{B}^I$;
...
$\mathsf{Op}_{\mathfrak{C}}(\mathcal{V})$ may often be equipped with a model structured via transfer across the associated monadic adjunction,
which assemble (\cite{BM13,Cav,Mur15}) into a Dwyer-Kan model structure structure on $\mathsf{Op}_\bullet(\mathcal{V})$.
...
Additionally, the above cited formal approaches fail to see the automorphisms of $G$-equivariant symmetric sequences, % objects in $\Sym^G_\bullet(\V)$,
...
\begin{definition}\label{CSYM DEF}
	Denote by $\mathfrak {C} \in \mathsf{F}$ a fixed set of \textit{colors} (or \textit{objects}).
	A tuple
        $\vect C = (\mathfrak c_1, \dots, \mathfrak c_n; \mathfrak c_0) \in \mathfrak C^{\times n} \times \mathfrak C$
	is called a \textit{$\mathfrak {C}$-signature} of \textit{arity} $n$.
	The \textit{$\mathfrak C$-symmetric category} $\Sigma_{\mathfrak C}$ is the category whose objects are the $\mathfrak{C}$-signatures and morphisms action maps
\[
...
\]
	for each permutation $\sigma \in \Sigma_n$, with the natural notion of composition.
        ...
        Alternatively, we will find it useful to visualize signatures as \textit{corollas with edges decorated by colors in $\mathfrak{C}$}, as depicted below, so that the map labeled $\sigma$
        is the unique map of trees indicated such that teh coloring of an edge equals the coloring of its image.
        % interchange the edges of the leftmost tree in such a way that one obtains the colored corolla on the left.
\[
\begin{tikzpicture}
...
\end{tikzpicture}
\]
Given any map $f \colon \mathfrak{C} \to \mathfrak{D}$ of colors, there is a functor
$f_{\**} \colon \Sigma_{\mathfrak{C}} \to \Sigma_{\mathfrak{D}}$
given by $f_{\**} (\mathfrak c_1, \dots, \mathfrak c_n; \mathfrak c_0) = (f(\mathfrak c_1),\cdots,f(\mathfrak c_n);f(\mathfrak c_0))$. 
\end{definition}
...
\begin{itemize}
\item objects given by a pair $(\mathfrak C, X)$ with
      $\mathfrak{C} \in \mathsf{F}$ a set of colors and
...
\item arrows $(\mathfrak C, X) \to (\mathfrak D, Y)$ given by a map 
      ...
      \begin{equation}
            \begin{tikzcd}[row sep = tiny, column sep = 35pt]
                  \Sigma_{\mathfrak{C}}^{op} \arrow[dr, "X"{name=U}] \arrow{dd}[swap]{f_{\**}}
                  \\
                  & \mathcal{V}
                  \\
...
It is clear that the natural projection map
$\mathsf{Sym}_\bullet(\mathcal{V}) \to \mathsf{F}$
is a (split) Grothendieck fibration with fibers the categories
$\Sym_{\mathfrak C}\mathsf{Fun}(\Sigma_{\mathfrak{C}}^{op},\mathcal{V})$.


\begin{remark}\label{SUBCATDOWNL REM}
      Given a category $\V$, let $\Cat \downarrow^l \V$ denote the category with
      objects given by functors $\mathcal B \to \V$,
      and arrows given by pairs $(f,\phi)$ with $f \colon \mathcal B \to \mathcal C$ a functor and $\phi$ a natural transformation
      \begin{equation}
            \begin{tikzcd}[row sep = tiny, column sep = 35pt]
                  \mathcal B \arrow{dr}[name=U]{} \arrow{dd}[swap]{f}
                  \\
                  & \mathcal{V}
                  \\
                  |[alias=V]| \mathcal C \arrow{ur}[swap]{}
                  \arrow[Leftarrow, from=V, to=U,shorten >=0.25cm,shorten <=0.25cm
                  ,swap
                  ]
            \end{tikzcd}
      ...
      Then $\mathsf{Sym}(\mathcal{V})$ is naturally a (neither wide nor full) subcategory of the $\mathsf{Cat}\downarrow^l \mathcal{V}$.
\end{remark}
...
\todo[inline]{discuss $\Omega$, $\Omega^0$, $\Phi$; leaf-root in preamble}
...
together with equivariance data (cf. \cite[Lemma \ref{TAS-REDUCELAN_LEM}]{BP_TAS}), % (this strengthens Lemma \ref{REDUCELAN LEM} a little),
...
\begin{lemma}
      Suppose $\V$ is a strongly cofibrantly generated model category with fixed points which preserve trivial cofibrations.
...
\begin{proof}
      ...
      the strongness of cofibrant generation ensures that $\mathcal J_G$-cells in $\V^G$ forget to $\mathcal J$-cells in $\V$.
...
%       A consequence of cofibrantly-generated $\V$ having cellular fixed points
%       (cf. \cite[Lemma A.3]{Ste16})
%       (cf. the proof of \cite[Prop. 6.3(i)]{BP_geo})
...
%       then the $\F$-model structure $\V^\G_\F$ exists.
% \end{theorem}      
...
naturally acts on the set $\Sigma_{\mathfrak{C},n}$
...
we say that \textit{$\Lambda$ stabilizes $\vect C$}. % \emph{$\vect{C}$ and $\Lambda$ are compatible}.
\end{definition}



\begin{remark}\label{CHOOSESIGN REM}
Given $\Lambda \leq G \times \Sigma_n^{op}$ 
the projection to $\Sigma_n^{op}$ yields a
right action of $\Lambda$ on $\underline{n}_+$.
...
\todo[inline]{we only need graph subgroups and indexing systems for $\Sigma_G$-cofibrancy. Do we need to address all three of these at this point? Do we want to move the Example below to such a section? Or to the next paper?}
...
Second, we wish to define $\Sigma_{\mathfrak C}[\vect T]$ for $\mathfrak C$ and $\vect T$ with compatible $G$-action.
\todo[inline]{Move blue definitions to the preliminaries}
{\color{blue}
  \begin{definition}
        Let $\mathcal C \in \Cat^G$ be a category with $G$-action.
        Denote by $\Fin^G \wr \mathcal C \to \Fin^G$ the Grothendieck fibration associated to the functor
        \[
              \Fin^G \longto \Cat,
              \qquad
              A \longmapsto \Hom^G(A, \mathcal C).
        \]
  \end{definition}

\begin{definition}
      Let $\Phi^G$ denote the category of forests with $G$-action and $G$-equivariant maps, and fix a $G$-set $\mathfrak C$ of colors.
      Define the category of \textit{$\mathfrak C$-colored $G$-forests} $\Phi^G_{\mathfrak C}$ to be
      the category of $G$-forests equipped with a $G$-equivariant edge coloring map $E(\vect T) \to \mathfrak C$,
      or formally the pullback
      \[
            \begin{tikzcd}
                  \Phi^G_{\mathfrak C} \arrow[r] \arrow[d]
                  &
                  \Fin^G \wr \mathfrak C \arrow[d]
                  \\
                  \Phi^G \arrow[r, "E"]
                  &
                  \Fin^G
...
$\vect{F_i} \xrightarrow{g} \vect{F}_{gi}$ (acting by $g$ on the coloring of the edges) over the action arrow
$\{i\} \xrightarrow{g} \{i\}$.
...
as on the left, inducing a bijection on the right.
\begin{equation}
\begin{tikzcd}
	\vect{D}  
	\arrow{d}
...
&
	\vect{F}_{gi}
  \end{tikzcd}
  \Sigma_{\mathfrak C}[\vect F_i](\vect D) \xrightarrow{g} \Sigma_{\mathfrak C}[\vect F_{gi}](g \vect D).
\end{equation}
{\color{red} this diagram takes place in 
$G\ltimes \Omega_{\mathfrak{C}}$ rather than in 
$G^{op} \ltimes \Omega_{\mathfrak{C}}$ - I'm starting to think that this is an intrinsic feature, not poor organization}


\begin{remark}
Alternatively, the representables
$\Sigma_{\mathfrak{C}}[-]$
assemble into a functor fibered over $\Fin$
\[
\Sigma_{\bullet}[-] \colon \Phi_{\bullet} \to \mathsf{Sym}_{\bullet}
\]
and thus induces a functor fibered over $\Fin^G$
\[
...
\end{proposition}
\begin{proof}
      Fix $X \in \mathsf{Sym}^G_{\mathfrak{C}} = \mathsf{Set}^{G \ltimes \Sigma_{\mathfrak{C}}^{op}}$.
...
\begin{remark}\label{SYMCOCHADJ REM}
...
\begin{remark}\label{FIXSETCHGR REM}
Given a homomorphism $\phi \colon G \to \bar{G}$ we again write $\phi\colon G \times \Sigma^{op} \to \bar{G} \times \Sigma^{op}$
for the induces homorphism. 
Following Proposition \ref{EQUILADJ PROP}, any $(\bar{G},\Sigma)$-family $\bar{\F}$ then induces a $(G,\Sigma)$-family $\phi^{\**} \bar{\F}$
such that, for each $\bar{G}$-set of colors $\bar{\mathfrak{C}}$
the adjunction 
\[
...
Remarks \ref{SYMCOCHADJ REM} and \ref{FIXSETCHGR REM} can be combined.
If $\phi \colon G \to \bar{G}$ and a $G$-set $\mathfrak{C}$,
by applying Remark \ref{SYMCOCHADJ REM} to the map of $G$-sets $\mathfrak{C} \to \bar{G} \cdot_G \mathfrak{C}$
and applying Remark \ref{FIXSETCHGR REM} to the $\bar{G}$-set $\bar{G} \cdot_G \mathfrak{C}$
...
n_k^{(\O,X,Y)}(\vect{U})=
	...
	\otimes
		\mathop{\mathlarger{\mathlarger{\mathlarger{\square}}}}\limits_{v \in V^{in}(\vect{U})} u(\vect{U}_v)
          \right)
          % ----------------------------------------------------------------------------------------------
          % -------------------- ONLY NEED THIS SECOND HALF FOR $\SIGMA_G$-COFIBRANCY --------------------
          % =
          % \left(
...
\]
for $m = |V^{in}(\vect U)|$.
By Proposition \ref{EQUILADJ PROP}, this is a genuine trivial cofibration in $\V^{\Aut_{G \ltimes \Sigma_{\mathfrak C}^{op}}(\vect U)}$,
and thus we have 
\[
...
Hence the global monoid axiom implies that each filtration piece $\O_{k-1}(\vect C) \to \O_k(\vect C)$ as well as their composite
$\O(\vect C) \to \O[u](\vect C)$ are
genuine weak equivalences in $\V^{\Aut_{G \ltimes \Sigma_{\mathfrak C}}(\vect C)}$.
The result follows.
...
% By the filtration \eqref{OUFILRE EQ}
% and the fact that level genuine trivial cofibrations are closed under transfinite composites and pushouts,
% % as colimits are computed levelwise and the restriction map is left Quillen
% it thus suffices to check that
% each map $\O_{k-1} \to \O_k$
% is a level genuine trivial cofibration in $\mathsf{Sym}^G_{\mathfrak{C}}$,
% i.e. $\O_{k-1}(\vect C) \to \O_k(\vect C)$ is a genuine trivial cofibration in $\V^{\Aut(\vect C)}$ for all $\mathfrak C$-signatures $\vect C$.
% From the pushout \eqref{OUPUSHRE EQ}
% and Proposition \ref{EQQUILADJ PROP},
% it is enough to show that the map
% $n_k^{(\O,X,Y)}(\vect U)$ in \eqref{NKOXY EQ}
% is a genuine trivial cofibration in $\V^{\Aut(\vect U)}$
% for all generating trivial cofibrations $u \in \Sym_{\mathfrak C}$.
...
%       \mathop{\mathlarger{\mathlarger{\mathlarger{\square}}}}\limits_{v \in V^{in}(\vect{U})} u(\vect{U}_v)
% \end{equation}
% in \eqref{NKOXY EQ} are precisely the evaluations of 
% \begin{equation}
%       \label{NKOXY3 EQ}
%       \Big(\varnothing \to \O\Big)^{\square m_a}
%       \mbox{\quad and \quad}
%       u^{\square m_i},
%       \qquad
%       \qquad
...
%       m_i = |V^{in}(\vect U)|,
% \end{equation}
% as defined by \eqref{FSQNDEF EQ},
% at $\Big( (\vect U_v)_{ac} \Big)$ and $\Big( (\vect U_v)_{in} \Big)$ respectively,
% % $(\emptyset \to \O)^{\square m_a}$ and 
% % $u^{\square m_i}$ 

% Thus, \eqref{NKOXY EQ} is the image of \eqref{NKOXY3 EQ} under the composite of left Quillen functors between genuine model structures
% % \[
% %       \V^{\Sigma_{m_a} \wr G \ltimes \Sigma_{\mathfrak C}^{op}}_{gen} \times
% %       \V^{\Sigma_{m_i} \wr G \ltimes \Sigma_{\mathfrak C}^{op}}_{gen}
% %       \to
% %       \V^{\Aut((\vect U_V)_{ac})}_{gen} \times \V^{\Aut( (\vect U_v)_{in} )}_{gen} \xrightarrow{\otimes}
% %       \V^{\Aut((\vect U_V)_{ac}) \times \Aut( (\vect U_v)_{in} )}_{gen} \to
% %       \V^{\Aut(\vect U)}
% % \]
% \[
%       \V^{\Sigma_{m_a} \wr G \ltimes \Sigma_{\mathfrak C}^{op}} \times
%       \V^{\Sigma_{m_i} \wr G \ltimes \Sigma_{\mathfrak C}^{op}}
%       \to
%       \V^{\Aut((\vect U_V)_{ac})} \times \V^{\Aut( (\vect U_v)_{in} )} \to
%       \V^{\Aut(\vect U)} \times \V^{\Aut(\vect U)} \xrightarrow{\otimes}
%       \V^{\Aut(\vect U)}
% \]
% from Propositions \ref{EQQUILADJ PROP} and \ref{RESGEN PROP}.
% Since the maps in \eqref{NKOXY3 EQ} are a genuine cofibration and a genuine trivial cofibration (as $k \geq 1$),
% % or that, by Proposition \ref{SIGMAWRGF PROP},
...
% % Propositions \ref{EQQUILADJ PROP} and \ref{RESGEN PROP}
% % now yields that
% \eqref{NKOXY EQ} is indeed a genuine trivial cofibration in $\V^{\Aut_{G \ltimes \Sigma_{\mathfrak C}}(\vect U)}$,
% finishing the proof of the 
% case where $\V$ is $\mathsf{sSet}$ or $\mathsf{sSet}_{\**}$.

% ------------------------------ SEMI-MODEL STRUCTURE PROOF ------------------------------
...
\begin{remark}\label{OPCOCHADJ REM}
Since the operadic forgetful functor $f^{\**}$ is given by the underlying 
forgetful functor on sequences, it follows immediately from Remark \ref{SYMCOCHADJ REM}
that the induced adjunction
\[
...
\begin{remark}\label{OPFIXSETCHGR REM}
Given a homomorphism $\phi \colon G \to \bar{G}$ and $\bar{G}$-set of colors $\bar{\mathfrak{C}}$,
Remark \ref{OPFIXSETCHGR REM} generalizes to give a Quillen adjunction
\[
...
\begin{remark}\label{OPCOMBADJ REM}
Given a homomorphism $\phi \colon G \to \bar{G}$ and $G$-set of colors $\mathfrak{C}$,
Remark \ref{OPCOMBADJ REM} generalizes to give a Quillen adjunction
\[
...
\subsection{$\Sigma_G$-cofibrations}
...
\begin{definition}
      ...
      A \textit{$G$-graph family} is any $(G,\Sigma)$-family $\mathcal F = \set{\F_n}$ consisting of subfamilies of $\F^\Gamma$.
\end{definition}

\begin{definition}
      We say a map $F \colon \O \to \P$ in $\Op^G(\V)$ is a \textit{$\Sigma_G$-cofibration} if
      the map $U(F) \colon \O \to F^{\**}\P$ on underlying symmetric sequences is a cofibration in $\Sym_{\mathfrak C_\O, \F^\Gamma}$.
      
      More generally, if $\F$ is any $G$-graph family, we say $F$ is a \textit{$\Sigma_\F$-cofibration}
      if the map $U(F)$ is a cofibration in $\Sym_{\mathfrak C_\O, \F}$.
\end{definition}
...
\begin{remark}
      ...
      for all $\mathfrak C_\O$-signatures $\vect C$.
\end{remark}      

\begin{example}
      All generating cofibrations of $\Op^G(\V)$ are $\Sigma_G$-cofibrations,
...
\begin{theorem}
      ...
      If $\O \in \Op^G(\V)$ is $\F$-cofibrant, then $\O$ is $\Sigma_\F$-cofibrant.
\end{theorem}
This will be a special case of Corollary \ref{SIGMAG_COF_COR} below.
...
\begin{definition}
      We generalize the constructions from Equations \eqref{FCAPBARF_EQ}, \eqref{FWRNXI EQ} and \cite[6.13, 6.19, 6.37]{BP_geo}.
      ...
      the inclusion of the fiber product over the projections $\Delta, \bar \Delta \to G$,
      and $\Sigma_n \wr_G \Lambda$ the semi-direct product $\Sigma_n \ltimes \Lambda^{\times_G^\lrcorner n}$.
      
      Now let $\mathcal F$, $\bar{\mathcal F}$ be $G$-graph families of $\Lambda$, $\bar \Lambda$ respectively.
...
% \begin{proposition}
%       [{cf. \cite[Prop. 6.40]{BP_geo}}]
%       \label{640_LEM}
%       We have left Quillen functors as below by Proposition s\ref{RESGEN PROP}, \cite[Prop. 6.6, 6.14, 6.6]{BP_geo}.
%       \[
...
\begin{proof}
      ...
      Second, unpacking \cite[Equation (6.51)]{BP_geo} we have that $\Gamma \leq G \times \Aut(T)$ is in
      ...
      \[
            ...
            \Big) \in \F_{T_l},
      \]
      where $\lambda \setminus \set{l}$ is the induction partition on $\boldsymbol{L}(C) \setminus \set{l}$ and
      $\pi_{G \times \Aut(T_l)}$ is the observable projection;
...
\begin{proposition}[{cf. \cite[Prop. 6.52]{BP_geo}}]
      \label{BOXVT_PROP}
      Suppose $f_s: A_s \to B_s$, $1 \leq s \leq l$, are $\F$-(trivial)-cofibrations in $\Sym(\V)$
      % level $\F$ (trivial) cofibrations in $\Sym(\V)$, i.e. $f_s(\vect C)$ is a (trivial) cofibration in $\V^{\Aut(\vect C)}_{\F_{\vect C}}$ for all signatures $\vect C$.
...
\begin{proof}
      ...
      \]
      Propositions \ref{EQQUILADJ PROP} and \ref{RESGEN PROP} and Lemma \ref{FVECTT_LEM} % 640_LEM
      ...
      The result now follows from Propositions \ref{SIGMAWRGF PROP}(i) and \ref{EQQUILADJ PROP} with the induction hypothesis. % 641_LEM
\end{proof}

We recall a key definition from \cite{Per18,BP_geo}.
\begin{definition}
      \label{WIS_DEF}
      A \textit{weak indexing system}...

      In particular, the induction map
      \[
            \phi_! \colon \V^{G \times \Aut(T)}_{\F_T} \to \V^{G \times \Aut(C)}_{\F_C},
            \qquad
            \phi_! \colon \V^{\Aut(\vect T)}_{\F_{\vect T}} \to \V^{\Aut(\vect C)}_{\F_{\vect C}}
      \]
are left Quillen.      
\end{definition}
\todo[inline]{weak indexing system}

\begin{corollary}
      \label{SIGMAG_COF_COR}
      Fix a weak indexing system $\F$
      {\color{blue} plus assumptions and stuff on $\V$}.
...
\begin{proof}
      ...
      this follows from Proposition \ref{BOXVT_PROP} and Lemma \ref{RESGEN PROP}.
\end{proof}
...
\todo[inline]{here or later?}

We consider two conditions on $(\V,\otimes)$ necessary for our first main result, Theorem \ref{THMI}:
\begin{itemize}
\item \mbox{$\V$ admits equivariant model structures.}
\item \mbox{$\V$ satisfies the global monoid axiom.}
\end{itemize}

In this short section, we identify several sufficient conditions which are easier to check in practice.
...
\begin{definition}
      We say $\V$ has \textit{weak acyclic cellular fixed points} (resp. \textit{monoidal weak acyclic cellular fixed points}) if for all finite groups $G$ and subgroups $H$, the functor $(-)^H \colon \V^G \o \V$
      \begin{enumerate}[label = (\roman*)]
      ...
      \item sends $\mathcal J_G$ to $\mathcal J$-cof (resp. $\mathcal J \otimes \V^G$ to $(\mathcal J \otimes \V^G)$-cof).
      \end{enumerate}
\end{definition}

The following are straightforward.
\begin{lemma}
      Suppose $\V$ is a strongly cofibrantly generated model category.
      \begin{enumerate}[label=(\roman*)]
      \item If $\V$ has weak acyclic cellular fixed points, then $\V$ has fixed points which preserve trivial cofibrations and hence
            admits all equivariant model structures.
...
\begin{lemma}
      ...
      \item $\V$ has weak acyuclic cellular fixed points iff $\V$ has monoidal weak acyclic cellular fixed points.
      \end{enumerate}
\end{lemma}
\begin{proof}
...
\begin{remark}
      If all objects in $\V^G$ are cofibrant, Propositions \ref{EQQUILADJ PROP} and \ref{RESGEN PROP} imply
      $\mathcal J \otimes \V^G \subseteq \mathcal J_G$-cell.
\end{remark}
...
(2),(3) follow since colimits in $\mathsf{Op}^G(\V)$ are created in $\Op(\V)$, and it holds non-equivariantly.
...
\label{TRANSCOMP_ES_LEM}
      Transfinite composition of $\F$-essentially surjective maps in $\Op^G(\V)$ is $\F$-essentially surjective.
\end{lemma}
...
But then since $a_!$ is left Quillen (cf. Remark \ref{OPCOCHADJ REM}) this is the pushout of a trivial cofibration in the fiber model structure
...
so that since Remark \ref{OPCOMBADJ REM} says that 
$G/H \cdot (-) \colon \mathsf{Cat}_{\{0\}}(\V) \to \mathsf{Cat}^G_{G/H,\F}(\V)$ is left Quillen
...
% \begin{lemma}\label{TRANSCOMP_LGC_LEM}
% 	Transfinite compositions of local genuine trivial cofibrations in $\Op^G(\V)$ are local genuine weak equivalences. 
% \end{lemma}

% \begin{proof}
% 	Fix a $\mathfrak{C}$-signature
...
%\begin{proposition}[{cf. \cite[Lemma 4.10]{Cav}}]
%      \label{CAV_4.10_PROP}
%      Equivalence and virtual equivalence define equivalence relations on $\mathfrak C(\O)^H$.
%\end{proposition}

%\begin{proof}        
%Only transitivity requires explanation, following from the amalgamation of interval objects \cite[Cor. 1.16]{BM13}
...
By \eqref{BASCOMP EQ}
one has that $\F$-weak equivalences in $\mathsf{Op}^G(\V)$
...
Then:
\begin{enumerate}[label=(\roman*)]
\item $\F$-weak equivalences between fibrant objects in $\mathsf{Op}^G(\V)$
satisfy $2$-out-of-$3$;
\item if $\V$ is right proper, $\F$-weak equivalences in $\mathsf{Op}^G(\V)$
satisfy $2$-out-of-$3$;
\item $\F$-Dwyer-Kan equivalences in $\mathsf{Op}^G(\V)$
satisfy $2$-out-of-$3$.
\end{enumerate}
\end{proposition}
...
in $\mathsf{Op}^G(\V)$, and the induced maps on the right for each
...
Moreover, if $\V$ is right proper, Lemma \ref{RIGHTPROPER_LEM}
...
Fix a $G$-set of colors $\mathfrak{C}$.
Let $\P \in \mathsf{Op}^G(\mathfrak{C})$ be an operad,
...
%the maps $\kappa_i, 1\leq i \leq n$ induce
one has
...
a lift $B \to X$ %in the left square
exists iff
a lift $B' \to X$ %in the total square 
exists;
\item in the right diagram, 
a lift $B \to X$ %in the right square 
exists iff
a lift $B \to X'$ %in the total square 
exists.
\end{enumerate}
...
%
Moreover, let 
\[\beta \colon 1_{\V} \to \mathcal{C}(1,0), \qquad
\bar{\beta} \colon 1_{\V} \to \bar{\mathcal{C}}(1,0),\qquad
H \colon \mathbb{C} \to \mathcal{C}(0,0), \qquad
...
provided that:
\begin{inparaenum}
\item[(i)] $\mathcal{D}$ is fibrant;
\item[(ii)] $\V$ is right proper.
\end{inparaenum}
\end{lemma}
...
\begin{inparaenum}
\item[(i)] $\P$ is fibrant or;
\item[(ii)] $\V$ is right proper and
...
\begin{remark}
      ...
      \item Transfinite composites of $\F$-essentially surjective maps in $\Op^G(\V)$ are $\F$-essentially surjective.
...
\begin{theorem}
      ...
      Then there exists a semi-model structure on $\Op^G(\V)$, denoted $\Op^G_\F(\V)$,
