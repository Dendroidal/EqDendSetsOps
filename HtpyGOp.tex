...
\todo[inline]{talk about families for categories - only $\F_1$-matters, and so we are recovering Caviglia (equivariantly or not - say what the trivial family is for $G = \**$)}
...
%      Explicitly, for
%      $a_0,a_1 \in \C(\O)^H$, and $F: \O \to \P$ in $\mathsf{Op}^G_\bullet(\V)$ a local weak $\F$-equivalence,
...
\end{tikzcd}
\end{equation}
\todo[inline]{how is that pushout map a cofibration?}
