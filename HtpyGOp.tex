...
Alternatively, one may say that Theorem \ref{THMIII} extends the well known model structures on all colored operads (and maps that change colors) in
...
%
It is worth nothing that the issue here is intrinsically operadic
...
\emph{does} coincide with the model structure obtained by applying
\cite[Prop. 2.6]{Ste16}
...
\begin{remark}
	...
	which can be identified as a family of subgroups of $G$ itself.
...
\begin{remark}\label{SEMI_REM}
	When working with operads, some authors (e.g. \cite{Spi,Whi_op,WY18})
	...
	performing (co)fibrant replacements.
...
\begin{remark}\label{GTRIV REM}
	It may be tempting to think that if $G=\**$ is trivial
...
(the ``hardest'' step is the observation that fixed points of isomorphism are isomorphisms).
...
\begin{remark}\label{CPPWHY REM}
	...
	In our opinion, the main obstacle towards generalizing our results is then that of identifying 
...
We end this section with a noteworthy 
...
\begin{remark}
	...
	As such, we do believe that the symmetric spectra
...
\begin{remark}\label{OP_MAP REM}
      ...
      Moreover, the two $\varphi^{\**}$ functors fit into a square of adjunctions 
...
{\color{red} HERE}
      ...
      In general, we can not give an explicit description of $\check{\varphi}_!$.
      However, when $\varphi$ is injective, $\varphi_!X$ is simply 
      {\color{red} $X \amalg \varnothing_{\mathfrak D \setminus \mathfrak C}$
      this is wrong},
      and as in this case $\mathbb F_{\mathfrak D} \varphi_! = \varphi_! \mathbb F_{\mathfrak C}$ as endofunctors on $\Sym^G_\bullet(\V)$,
...
\begin{proposition}\label{WEAKCELL PROP}
	...
	\begin{enumerate}[label=(\roman*)]
		...
		Moreover, fixed points $(-)^H \colon \V^G \to \G$
...
$\F^{\ltimes n}$-cofibration in $\V^{\Sigma \wr \G}$;
...
between $\F^{\ltimes n}$-cofibrant objects in $\V^{\Sigma \wr \G}$.
\end{enumerate}
\end{proposition}
...
\begin{remark}\label{THMISM REM}
	...
	is known to be underlying genuine cofibrant in 
	...
	We notice that conditions (i),(ii),(iii),(iv) in Theorem \ref{THMI}
...
In the following 
we slightly abuse notation by conflating a $(G,\Sigma)$-family $\F$,
...
\begin{remark}
      We unpack $\boldsymbol{V}^{\**}\F^\ltimes$.
      For any $T \in \Omega$ and $v \in \boldsymbol{V}(T)$, let $\Aut_{v}(T) \leq \Aut(T)$ denote the subgroup which fixes the root of $T_v$.
      Then we have homomorphism $\pi_{T_v} \colon \Aut_v(T) \to \Aut(T_v)$
      that does not extend over the inclusion $i_v \colon \Aut_v(T) \to \Aut(T)$.
      Now, given a $(G,\Sigma)$-family $\F$ and a subgroup $ \Gamma \leq G \times \Aut(T)$, 
      $\Gamma \in (\boldsymbol{V}^{\**}\F^\ltimes)_T$ iff
      $\pi_{T_v}\left( \Gamma \cap (G \times \Aut_{v}(T)) \right) \in \F_{T_v}$.
      % Equivalently, \mbox{$\F_T = \bigcap_{v \in \boldsymbol{V}} (G \times \pi_{T_v})^{\**}(G \times i_v)_! \F_{T_v}$}
      % where we note that since $i_v$ is injective, $(i_v)_! = (i_v)_{\**}$.
      
      Following \cite[Remark 6.46]{BP_geo}, we note that
...
\begin{remark}
      \label{PIS_REM}
      The opposite inclusion to \eqref{PIS_EQ} always holds:
      Letting $i \colon G^{op} \times \Sigma \to \Sigma \wr (G^{op} \times \Sigma)$ denote the inclusion of the unary tuples,
      it is clear that $\F \subseteq i^{\**}\F^\ltimes$, and so
      $\mathsf{lr}^{\**} \F \subseteq \mathsf{lr}^{\**}i^{\**}\F^\ltimes = \boldsymbol{V}^{\**} \F^\ltimes$.
      
      Thus $\F$ is a pseudo indexing system iff we have an equality
      $\boldsymbol{V}^{\**} \F^\ltimes = \mathsf{lr}^{\**}\F$.
\end{remark}


We can now prove the main result of this section, which is a stronger version of Theorem
\ref{THMII}.


\begin{theorem}\label{SIGMAG_COF_THM}
...
\begin{remark}
	Theorem \ref{SIGMAG_COF_THM} is the colored analogue to {\color{red} \cite[{Remark 6.64 after Theorem 6.59}]{BP_geo}}.
	However, our proof here will be significantly simpler since we have established 
...
\begin{proof}
	It suffices to show that free operad extensions $\O \to \O[u]$ as in \eqref{OURE EQ},
	...
	whenever $\O$ is $\Sigma_\F$-cofibrant and $u \colon X \to Y$ is an $\F$-cofibration in $\Sym^G_{\mathfrak C,\F}(\V)$.
      ...
      First, consider the following commutative cube of groupoids
      \begin{equation}
            \label{FCA_SQ_EQ}
            \begin{tikzcd}
      ...
      \begin{equation}
            ...
            \end{tikzcd}
      \end{equation}
      where $\Omega_{\mathfrak C}^{0,a}$ is the category of alternating trees discussed in \S \ref{OPC_MS_SEC},
      ...
      Since $\F$ is a pseudo indexing system, we have
      \[
      ...
      \]
      where the equalities follow from, in order,
      \eqref{PHIFLTIMES_EQ}, \eqref{FCA_SQ_EQ}, Remark \eqref{PIS_REM}, and \eqref{COLORLR_EQ}.
      Hence the (opposite) left Kan extension
      \[
            \left(\mathsf{lr}_{\mathfrak C}^{a,op}\right)_!
            = \left(\mathsf{lr}_{\mathfrak C}^{op} \pi_{\mathfrak C}^{a,op} \right)_!
      \]
      sends
      $\left(\boldsymbol{V}_{\mathfrak C}^{a}\right)^{\**}\left( (\nabla_{\mathfrak C}^{\**} \F_{\mathfrak C})^\ltimes \right)$-cofibrations in $\V^{G \ltimes \Omega_{\mathfrak C}^{a,op}}$
      to
      ...
      Second, we have % the maps $\varnothing \to \O$ and $u \colon X \to Y$ define
      a $\nabla_{\mathfrak C}^{\**}\F_{\mathfrak C}$-cofibration
      $(\O,u) = (\varnothing \to \O) \amalg (X \xrightarrow{u} Y)$ in $\V^{\amalg_2 G \ltimes \Sigma_{\mathfrak C}^{op}}$ by Remark \ref{SIGMACOF_REM}.
      ...
      this is a $(\boldsymbol{V}_{\mathfrak C}^{a})^{\**}\left( (\nabla_{\mathfrak C}^{\**} \F_{\mathfrak C})^\ltimes \right)$-cofibration.
      
      Combining these two facts with \eqref{OUPUSHRE EQ}, we have that
      the pushout $(\O_{k-1} \to \O_k)$ of $\left(\mathsf{lr}_{\mathfrak C}^{a,op} \right)_! n_k^{\O, X,Y}$
      is an $\F_{\mathfrak C}$-cofibration, as desired.
\end{proof}
...
\begin{proposition}[{c.f. \cite[4.20]{Cav}}]\label{J-CELL_PROP}
      Suppose $\V$ satisfies conditions $(i),(ii),(iii),(iv),(v)$ from Theorem \ref{THMI}.
      \todo[inline]{this is a minimal set of conditions, as the proof uses the properties of the model structures on the fibers}
...
\todo[inline]{hypotheses needed: all of Thm I for (i) and (ii), plus $(vii)$ for (iii). }
...
\todo[inline]{hypotheses needed: all of Thm I plus (vii)}
...
Assume that
{\color{blue} $\V$ is a closed monoidal model category with cofibrant pushout powers such that fixed points in $\V$ send genuine trivial cofibrations to trivial cofibrations, and that $\V$ satisfies the monoid axiom}.
