...
\todo[inline]{Thm 3.1 is non-formal wrt Stephan or BM03, but \textit{is} formal in the sense that the transfer argument from $\Sym^G_{\**}(\Top^G)$ is an easy application of Quillen's path object argument. The difficulty comes in addressing the analogue of THMII.}
...
\begin{remark}\label{SEMI_REM}
	...
	are only required to hold if the map in question % trivial cofibration 
...
\begin{remark}\label{SPNONEX REM}
	...
	More generally,
        ...
        one could ask which analogues of Theorems \ref{THMI},\ref{THMII},\ref{THMIII} hold
        if the choice of the $(G,\Sigma)$-family in Definition \ref{FAM1ST DEF}
        were replaced with
        a choice of model structure on
        $\V^{G \times \Sigma_n^{op}}$
	for each $n \geq 0$.	 
\end{remark}
...
\todo[inline]{I don't think \eqref{FCA_SQ_EQ} is any clearer this way.
  First, it is too wide.
  Second, ``commutativity of the top sections'' is not enough - we are using both the commutativitiy AND the fact that the top rectangles coincide. As a cube, this is simply ``the diagram commutes''.
  Third, the left bottom square is in \eqref{COLORLR_EQ}, which fits nicely in the discussion already from \S 2.
}
\todo[inline]{the splitting of the equations I agree is clearer}
