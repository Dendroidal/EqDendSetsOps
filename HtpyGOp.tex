...
\todo[inline]{talk about families for categories - only $\F_1$-matters, and so we are recovering Caviglia (equivariantly or not - say what the stupid family is for $G = \**$)}
...
However, as we are working with operads with $0$-ary operations, % /units,
...
\begin{proof}
      ...
      so that the claim follows by Proposition \ref{FIBERGLMOD PROP} and Lemma \ref{GOTC_LEM}.
...
% But now both the (TC1) and (TC2) cases follow from the fact that this holds for pushouts of cofibrations in the fiber categories 
% $\mathsf{Op}^{G}_{\mathfrak C_{\O},\F}(\V)$
...
% 	Transfinite compositions of local genuine trivial cofibrations in $\Op^G_\bullet(\V)$ are local genuine weak equivalences. 
% \end{lemma}
...
$\alpha$ as below. % on the bottom below.
\[
