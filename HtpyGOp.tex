...
\begin{remark}
      As $\Cat^{G}_{\mathfrak C}(\V) = \Op^G_{\mathfrak C}(\V) \downarrow \eta_{\mathfrak C}$
      with $\eta_{\mathfrak C}$ the initial $\mathfrak C$-colored category/operad,
...
\end{theorem}
This will be a special case of Theorem \ref{SIGMAG_COF_THM} from \S \ref{SGCOF_SEC}.
...
First, the following is immediate from
...
\begin{equation}
	...
	\end{tikzpicture}%
\end{equation}%
The non-trivial $G$-graph subgroups in
$\F^{\Gamma}_{\vect{C}}$,
$\F^{\Gamma}_{\vect{D}}$
...
% Following Proposition \ref{EQQUILADJ PROP}, any $(\bar{G},\Sigma)$-family $\bar{\F}$ then induces a $(G,\Sigma)$-family $\phi^{\**} \bar{\F}$
...
\begin{remark}\label{COMBADJ REM}
      Parts (i) and (ii) of Corollary \ref{SYMADJ_COR} can be combined: % Remarks \ref{SYMCOCHADJ REM} and \ref{FIXSETCHGR REM}
...
% forgetful functor on sequences, it follows immediately from Corollary \ref{SYMADJ_COR}\ref{SYMCOCHADJ_LBL} % Remark \ref{SYMCOCHADJ REM}
...
\newpage
\section{Weak indexing systems and cofibrancy}
\label{INDSYS SEC}
...
$\Op^G_\F(\V) \to \Sym^G_\F(\V)$ preserves cofibrant objects.

After defining the necessary conditions below, this result will follow as a special case of Theorem \ref{SIGMAG_COF_THM}.
...
However, this condition alone does not suffice.
...
\begin{example}
      All generating cofibrations of $\Op^G(\sSet)$ are $\Sigma_G$-cofibrations,
...
Rephrasing, Theorem \ref{THMIII} says that when $\F$ is a weak indexing system, cofibrant operads are $\Sigma_G$-cofibrant.
...
\begin{remark}
      ...
      \begin{align*}
        \F_T &= \pi_T^{\**}(\F_C) \cap \left( \F_{T_{i_1}}^{\ltimes_G |\lambda_1|} \sqcap_G \dots \sqcap_G \F_{T_{i_r}}^{\ltimes_G |\lambda_r|} \right),
        \\
        \F_{\vect T} &= \pi_{\vect T}^{\**}(\F_{\vect C}) \cap \left( \F_{\vect T_{i_1}}^{\ltimes_G |\lambda_1|} \sqcap_G \dots \sqcap_G \F_{\vect T_{i_s}}^{\ltimes_G |\lambda_s|} \right),
      \end{align*}
      with $\pi_{T} \colon G \times \Aut(T) \to G \times \Aut(C)$ and $\pi_{\vect T} \colon \Aut(\vect T) \to \Aut(\vect C)$.
\end{remark}
...
\begin{proof}
      ...
      \begin{equation}
            ...
            \begin{tikzcd}
                  ...
                  \\
                  \Aut(\vect C) \arrow[r, "\phi_{\vect C}"]
                  &
                  G \times \Aut(C).
            \end{tikzcd}
      \end{equation}
...
By \cite[Remark 6.45]{BP_geo}, $\F$ is a weak indexing system iff
for all $T \in \Omega$ with $\mathsf{lr}(T) = C$,
\begin{equation}
      \label{FT_EQ}
      \F_T \subseteq \pi_{T}^{\**} \F_C.
\end{equation}
In particular, by Proposition \ref{EQQUILADJ PROP} the induction map
      \[
            (\pi_{T})_! \colon \V^{G \times \Aut(T)}_{\F_T} \to \V^{G \times \Aut(C)}_{\F_C},
            % \qquad
...
\begin{lemma}
      Let $\F$ be a $G$-graph family. Then the following are equivalent.
      \begin{enumerate}[label = (\roman*)]
      ...
      \item For all $G$-sets of colors $\mathfrak C$ and $\mathfrak C$-colored trees $\vect T$ with $\mathsf{lr}(\vect T) = \vect C$, 
            $\F_{\vect T} \subseteq \pi_{\vect T}^{\**}\F_{\vect C}$.
      \end{enumerate}
\end{lemma}
\begin{proof}
      (ii) implies (i) by considering $\mathfrak C = \**$.
      Conversely,
      \eqref{FT_EQ} syas $\F_T \subseteq \phi_T^{\**}\F_C$,
      and hence
      $\phi_{\vect T}^{\**}\F_T \subseteq \phi_{\vect T}^{\**}\pi_T^{\**} \F_C$.
      But $\phi_{\vect T}^{\**}\pi_T^{\**} \F_C = \pi_{\vect T}^{\**}\F_{\vect C}$ by \eqref{PHISTAR_EQ} and \eqref{AUTT_EQ},
      and thus the result follows by Lemma \ref{FVECTT_LEM}.
\end{proof}

\begin{corollary}
      \label{PIVECTT_COR}
      The map $(\pi_{\vect T})! \colon \V^{\Aut(\vect T)}_{\F_{\vect T}} \to \V^{\Aut(\vect C)}_{\F_{\vect C}}$ is left Quillen.
\end{corollary}
...
But then since $a_!$ is left Quillen (cf. Corollary \ref{OPADJ_COR}\ref{OPCOCHADJ_LBL}) % Remark \ref{OPCOCHADJ REM}
