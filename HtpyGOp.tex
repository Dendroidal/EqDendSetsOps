...
\todo[inline]{talk about families for categories - only $\F_1$-matters, and so we are recovering Stephan (not Caviglia) (equivariantly or not - say what the trivial family is for $G = \**$)}
\todo[inline]{talk about what conditions we do or do not need for these restrictions}
...
\begin{definition}
      \label{FSQUARE_DEF}
      Given a map $f \in \V^{\mathcal G}$, we write $f^{\square}$ (resp. $f^{\otimes}$) for the maps in $\V^{\Sigma \wr \mathcal G} = \prod_{n \geq 0} \V^{\Sigma_n \wr \mathcal G}$
      given by $f^{\square n}$ (resp. $f^{\otimes n}$) in each component.
      
      Analogously, given a family 
