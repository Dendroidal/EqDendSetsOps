...
Rephrasing, Theorem \ref{THMIII} says that for special $\F$, $\F$-cofibrant operads are $\Sigma_\F$-cofibrant.
...
\begin{corollary}
      \label{PIVECTT_COR}
      The map $(\pi_{\vect T})_! \colon \V^{\Aut(\vect T)}_{\F_{\vect T}} \to \V^{\Aut(\vect C)}_{\F_{\vect C}}$ is left Quillen.
\end{corollary}
...
\end{theorem}
\begin{proof}
      Using the filtration from \eqref{NKOXY EQ} and \eqref{FILTPUSHG EQ}, % from Proposition \ref{FILTPUSHG PROP},
