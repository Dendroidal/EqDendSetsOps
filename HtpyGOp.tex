...
the evaluations $\O(\vect C)$, % $G$-equivariant symmetric sequences, % objects in $\Sym^G_\bullet(\V)$,
...
\begin{proposition}
      The category of categories with the canonical model structure
      is a strongly cofibrantly generated monoidal model category,
      satisfies the global monoid axiom,
      has monoidal weak acyclic cellular fixed points,
      and thus satisfies the hypotheses of Theorem \ref{THMI}.
\end{proposition}
\begin{proof}
      The generating (trivial) cofibrations are
      \[
            \set{ \varnothing \to \**,\ \set{0,1} \to [1],\ \set{0 \rightrightarrows 1} \to [1] },
            \qquad
            \set{ \** \xrightarrow{\varepsilon} \widetilde{[1]} };
      \]
      and cofibrations are functors which are injective on objects.
      Thus all objects are cofibrant, and it is straightforward to check that it satisfies the pushout product axiom,
      so by Lemma \ref{JVG_JG_LEM} it remains to show that $\V$ has weak acyclic cellular fixed points.
      ...
      Finally, $(G/H \cdot f)^H = (G/H)^H \cdot f$, so (iii) follows.
\end{proof}
