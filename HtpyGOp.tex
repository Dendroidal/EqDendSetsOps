...
{\color{red} HERE}
      ...
      and let $\F = \set{\F_n}_{n \geq 0}$ a $(G, \Sigma)$-family which respects units.
      Then there exists a model structure on $\Op^G(\V)$, denoted $\Op^G_\F(\V)$,
      with $\F$-Dwyer Kan weak equivalences.
...
\begin{remark}\label{OP_MAP REM}
      ...
      However, when $f$ is injective, $f_!X$ is simply $X \amalg \varnothing_{\mathfrak D \setminus \mathfrak C}$, and by Corollary \ref{FTF_COR}, $U \check f_! = f_! U$;
      See Appendix \S \ref{FREEOPCOL_SEC} for more details.
\end{remark}
...
\begin{definition}
      ...
      the \textit{$\F$-model structure}, denoted $\V^\G_\F$, if it exists,
...
\begin{remark}
      \label{SIGMACOF_REM}
      We note that because $\G$ is a groupoid, cofibrations in this functor category have an explicit description:
      $f \in \V^\G_\F$ is a cofibration iff $f(g)$ is a cofibratoin in $\V^{\Aut(g)}_{\F_g}$ for all $g \in \G$.
\end{remark}
...
to assume that these equivariant model structures are somewhat well behaved.
\begin{definition}
      ...
      the functor $(-)^H \colon \V^G \to \V$ sends genuine trivial cofibrations to trivial cofibrations.

      Notationally, this may be expressed as $\left(\mathcal J_G\text{-cof}\right)^H \subseteq \mathcal J\text{-cof}$.
\end{definition}
...
(this filtration is induced a $\Sigma_n$-equivariant filtration 
...
we say that \emph{$\vect{C}$ and $\Lambda$ are compatible}.
\end{definition}
...
signatures $\vect{C}$ which are compatible with $\Lambda$
...
Any $(\bar{G},\Sigma)$-family $\bar{\F}$ then induces a $(G,\Sigma)$-family $\phi^{\**} \bar{\F}$
such that, for all $\bar{G}$-set of colors $\bar{\mathfrak{C}}$
the adjunction 
\[
...
Considering \eqref{NKOXY}, we will need the following additional axiom to prove our first main theorem.
...
\begin{remark}
      By adjunction arguments, tnote that for $A \in \V$ and $B \in \V^G$, we have
      \[
      ...
      % We note that
      % \[
      %       G \cdot_H \left( (H \cdot_K A) \otimes X \right) = G \cdot_H \left( H \cdot_K \left(A \otimes X\right) \right)
      %       = G \cdot_K \left(A \otimes X\right).
      % \]
      % Second, the following classes of maps are equal, using the fact that $G \cdot_H (-)$ is a left adjoint and the functoriality of $G \cdot_H (-)$ and $(-) \otimes X$.
      % \begin{align*}
          %           \left( G \cdot_H \left( \left(J^H\text{-cof}\right) \otimes \V^H \right)\text{-cell} \right) \mathbin{\cap} \mathcal W^G
          %           & = 
          %           \left( G \cdot_H \left( \left(J^H \otimes \V^H\right)\text{-cof}\right)\text{-cell} \right) \mathbin{\cap} \mathcal W^G \\
          %           & =
          %           \left( G \cdot_H \left(J^H \otimes \V^H\right)\text{-cof} \right) \mathbin{\cap} \mathcal W^G \\
          %           & =
          %           \left( G \cdot_H \left(J^H \otimes \V^H\right)\text{-cell} \right) \mathbin{\cap} \mathcal W^G \\
          %           & =
          %           \left( G \cdot_H \left(J \otimes \V^H\right)\text{-cell} \right) \mathbin{\cap} \mathcal W^G \\
          %           & =
...
% o that, by Proposition \ref{SIGMAWRGF PROP},
...
% -------------------- GENERAL CASE --------------------
For general $(\V, \otimes)$, the hypotheses and the argument above imply that the rightmost map in \eqref{NKOXY2 EQ} is a genuine trivial cofibration in $\V^{\Aut(\vect U)}$,
and thus
\[
      n_k^{(\O,X,Y)}(\vect U) \cdot_{\Aut_{G \ltimes \Omega^a_{\mathfrak C}[k]}(\vect U)}\Aut_{G \ltimes \Sigma_{\mathfrak C}}(\vect C) \subseteq \left( \mathcal J \otimes \V^{\Aut(\vect C)}\right)\text{-cell}
\]
for all $\vect U \in \Omega^a_{\mathfrak C}[k]$, $k \geq 0$, with $\mathsf{lr}(\vect U) = \vect C$.
...
\begin{definition}
      ...
      \begin{enumerate}[label = (\roman*)]
      \item preserves direct colimits of maps in $\mathcal J_G$ (resp. $\mathcal J \otimes \V^G$).
      \item preserves pushouts of $\mathcal J_G$ (resp. $\mathcal J \otimes \V^G$).
      \item sends $\mathcal J_G$ (resp. $\mathcal J \otimes \V^G$) to $\mathcal J$-cof (resp. $(\mathcal J \otimes \V^G)$-cof).
      \end{enumerate}
\end{definition}
...
%       If for an independent reason we know that each $\mathsf{Op}^{G}_{\mathfrak C}(\V)$ had a full Quillen model structure,
%       then the above proof would show that \textit{all} relative $J_\F$-cells are weak equivalences.

% {\color{blue} Luis: I don't think this is true. Even if knowing the fibers have model strucutres, something along the lines of $\otimes$-perfectness would still be needed to conclude that stuff in (the operadic) $J$-cell are local equivalences.
% This is one of the reasons (possibly the only reason) why \cite{BM13} assume what they do.}
% \end{remark}
...
To motivate the following result, recall that
(cf. Remark \ref{SIGACT REM}),
...
\begin{lemma}
Suppose $\V$ is right proper and consider a diagram 
in $\mathsf{Cat}_{\{0,1\}}(\V)$
\begin{equation}
\begin{tikzcd}[column sep = 45pt]
	\mathbbm{1}
...
\end{tikzcd}
\end{equation}
We now claim that the arched dashed arrows in 
\eqref{BIGISHDIAG EQ}
...
\cite[Props. 2.3 and 2.5]{Ber07b}.

\begin{proposition}
Let $\F$ be a $(G,\Sigma)$-family which respects units.
Moreover, suppose $\V$ is right proper, has cofibrant unit,
and satisfies the coherence axiom,
and let $F: \O \to \P$ in $\Op^G_\F(\V)$
be a local $\F$-fibration.

Then $F$ is $\F$-path lifting iff
$F$ is an $\F$-isofibration.
\end{proposition}


\begin{proof}
Note first that we immediately reduce to 
the case of $F \colon \mathcal{C} \to \mathcal{D}$
a local fibration in $\mathsf{Cat}_{\V}$.

We first address the easier 
``only if'' direction.
Given $a \in \mathcal{C}$
and isomorphism $\alpha \colon \widetilde{\mathbbm{1}} \to \pi_0 \mathcal{D}$
with $\alpha(0)=F(a)$.
Using the coherence axiom we can now find bottom horizontal arrows as on the left diagram below
such that the composite 
$\mathbbm{1} \to \mathcal{D}_f$
represents $\alpha$.
Note that we are free to assume 
$\mathbb{I} \to \mathcal{D}_f$
is a fibration
(if this was not the case, we could perform a factorization
$\mathbb{I} \overset{\sim}{\rightarrowtail} 
\widetilde{\mathbb{I}} \to \mathcal{D}_f$,
...
%
We now obtain the induced lifting problem on the right,
which has a solution since $F$ is an path-lifting by assumption.
Applying $\pi_0\colon \mathsf{Cat}(\V) \to \mathsf{Cat}$ 
we see that there is indeed an isomorphism 
$\tilde{\alpha} \colon \widetilde{\mathbbm{1}} \to \pi_0 \mathcal{C}$
which lifts $\alpha$ and such that 
$\tilde{\alpha}(0) = a$.
...
\begin{remark}\label{FIB_ISOFIB_REM}
