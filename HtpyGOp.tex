...
All of the above axiomatic theories are naive/levelwise, ignoring all the isotropy inherent in symmetric operads,
and equivarinat model structures have only been considered for particular categories, namely spaces and simplicial sets (\cite[Thm. 3.1]{GW18}, \cite[Prop. 3.3.12]{Rez96}).
...
what happens to the middle map \todo{middle map? or first map?}
...
\end{equation}
for the natural composite.
\todo[inline]{come back - figure out the ops}
...
\newpage
\subsection{Pseudo indexing systems and $\Sigma_\F$-cofibrancy}
\label{INDSYS SEC}
