...
and we will observe that
the indued model structure on $\mathsf{sCat}^G_{\bullet}$
...
\begin{remark}\label{SEMI_REM}
	...
	are only required to hold if the map in question 
...
\begin{remark}
	...
	condition out of all the conditions in 
...
\begin{remark}\label{OP_MAP REM}
      ...
      \end{equation}
      In general, we can not give an explicit description of $\check f_!$.
      However, when $f$ is injective, $f_!X$ is simply 
