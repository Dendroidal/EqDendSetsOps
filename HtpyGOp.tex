...
(not present in the non-equivariant context)
which must be considered
when studying equivariant operads,
the importance of which was made clear
by the Hill, Hopkins and Ravenel \cite{HHR16}
solution to the Kervaire invariant one problem.

{\color{red} HERE}

For concreteness, fix a finite group $G$
and consider the category
$\mathsf{sOp}^{G}_{\**} = 
\mathsf{Op}^G_{\**}(\mathsf{sSet}^G)$
of single colored operads 
in the category
$\mathsf{sSet}^G$
of simplicial sets with a $G$-action.
Notably, observe that if $\O in \mathsf{sOp}^{G}_{\**}$
then its $n$-the level has both a $\Sigma_n$-action and $G$-action, commuting with each other, or, equivalently, 
a $G \times \Sigma_n$.
...
The term ``graph'' comes from a neat characterization of the $\Gamma$
in \eqref{GRAPHEQ EQ}:
they are necessarily the graph of a partial homomorphism
$\phi \colon H \to \Sigma_n$ for some subgroup $H$,
i.e.
$\Gamma = \{(h,\phi(h)) | h \in H\}$.

always consists of elements of the form 
...
We now briefly recall what we mean by ``norm maps''
(for a more extensive discussion, 
see the introductions to \cite{Per18},\cite{BP_geo}).
Write $G$ for some fixed finite group.
%
For concreteness, let us 
consider the category 
$\mathsf{sSet}^G$ of
$G$-equivariant simplicial sets
and suppose one has a choice of a
(single colored) operad $\O$ together with an algebra $X$ in the category $\mathsf{sSet}^G$.
One then has maps
\begin{equation}\label{ALGMAP EQ}
	\O(n) \times X^{\times n} \to X
\end{equation}
which must be invariant with respect to the $\Sigma_n$-action on the left side by definition of algebra
and $G$-equivariant with respect to the $G$-actions on both sides
since we are working in the category $\mathsf{sSet}^G$.
Alternatively, note that 
both $\O(n)$ and $X^{\times n}$
must have commuting actions by $G$ and $\Sigma_n$ or, equivalently,
and action by $G \times \Sigma_n$.
Thus, giving $X$ the $G\times \Sigma_n$-action where $\Sigma_n$ acts trivially,
the map \eqref{ALGMAP EQ} is $G\times \Sigma_n$-equivariant.

Given a $G$-set $A$ with $n$ elements, 
one can form a new $G$-object
$N_A X \in \mathsf{sSet}^G$,
called a \emph{norm object},
whose underlying object is 
$X^{\times |A|} \simeq A^{\times n}$
but with a $G$-action that combines the $G$-actions on 
$X$ and $A$.
More explicitly, 
the homomorphism
$\phi \colon G \to \Sigma_n$ encoding the $G$-set $A$
gives rise to a \emph{graph subgroup}
$\Gamma_A \leq G \times \Sigma_n$
via
$\Gamma_A = \{(g,\phi(g))|g\in G\}$
and 
$N_A X$ is then $X^{\times n}$ together with the 
$G$-action induced by $\Gamma_A \leq G \times \Sigma_n$.


By restricting the $G \times \Sigma_n$-action on 
\eqref{ALGMAP EQ} to $\Gamma_A$
and writing $\O(n)^{\Gamma_A}$ for the $\Gamma_A$-fixed points,
one has induced $G$-equivariant maps
\begin{equation}\label{NORMMAP EQ}
\O(n)^{\Gamma_A} \times N_A X \to X.
\end{equation}
Informally, this means that each ``operation''
$\rho \in \O(n)^{\Gamma_A}$
induces a $G$-equivariant map $\rho \colon N_A X \to X$,
and as such we refer to 
$\O(n)^{\Gamma_A}$ as the space of norm maps of $\O$ indexed by $A$.
More generally, if $A$ is a $H$-set for a subgroup $H \leq G$,
one similarly has a graph subgroup
$\Gamma_A \leq G \times \Sigma_n$
given by
$\Gamma_A = \{(h,\phi(h))|h\in H\}$, so that \eqref{NORMMAP EQ}
is then $H$-equivariant.
Notably, such 
subgroups $\Gamma_A \leq G \times \Sigma_n$
has $A$ ranges over all $H$ sets for $H\leq G$
admit a neat characterization: they are precisely those subgroups
$\Gamma \leq G \times \Sigma_n$
such that
$\Gamma \cap \Sigma_n = \**$.

The ``moral'' of the work in \cite{HHR16} (also \cite{BH15})
is then that, in the equivariant context, 
norm maps should be considered one of the key pieces of structure on an algebra $X$.
As such, the preferred notion of equivalence of operads
should be that of \emph{graph equivalences}, i.e. those maps
of operads
$\O \to \mathcal{P}$
such that the maps
\begin{equation}\label{GRAPHEQ EQ}
\O(n)^{\Gamma} \xrightarrow{\sim} \mathcal{P}(n)^{\Gamma}
\end{equation}
are weak equivalences for all graph subgroups 
$\Gamma \leq G \times \Sigma_n$.




Writing $\mathsf{sOp}_{\**}^G$
for the category of such 
$G$-equivariant single colored operads in simplicial sets, the existence of a model structure 
on $\mathsf{sOp}^G_{\**}$
with weak equivalences given by the graph equivalences
in \eqref{GRAPHEQ EQ}
was proven independently in 
\cite[Thm. I]{BP_geo} and \cite[Thm. 3.1]{GW18}.
It seems worth noting that these results are somewhat non-formal.
On the one hand, using the identification
$\mathsf{sOp}_{\**}^G \simeq \mathsf{Op}_{\**}(\mathsf{sSet}^G)$
as single colored operads enriched in 
$\mathsf{sSet}^G$,
one could obtain a model structure
a model structure on $\mathsf{sOp}_{\**}^G$
by applying {\color{red} give cit.}
...
is given by Theorem \ref{THMIII} below, which (in particular)
...
Alternatively, Theorem \ref{THMIII} extends the well known model structures of on all colored operads in
\cite[Thm 1.14]{CM13b} {\color{red} also, add Robertson}
...
We recall that their project is based on the identification of a category
$\Omega$ of trees, 
...
(here, and throughout the paper, we use $\bullet$ to denote a category where the colors/objects are allowed to change)
while $\mathsf{dSet} = \mathsf{Set}^{\Omega^{op}}$
is the category of presheaves on $\Omega$,
together with the model structure for which the fibrant objects are the so called $\infty$-operads.

Similarly, the project this paper belongs is based in the identification by the authors of a category
$\Omega_G$ of \emph{$G$-trees},
the combinatorics of which encode compositions of norm maps in an equivariant operad.
And one of our main goals is to prove that 
\eqref{CMMAINTHM EQ}
...
generalizing the non-equivariant analogue in \cite{BM11},
and in \cite{BP_edss} we likewise generalized \cite{CM13a}
which studies some auxiliary model structures 
used in \cite{CM13b} to establish the Quillen equivalence in 
\eqref{CMMAINTHM EQ}.
...
Colored operads and their algebras have become increasing relevant and utilized by the algebraic topology and related communities.
A desire to understand and control the interaction of algebra and homotopy theory has fueled a wave of research analyzing
model structures and other homotopical features of categories of enriched operads and their algebras.
...
but the model structures above ignore the isotropy of the operations\footnote{We record that this isn't necessarily a mistake, and in fact this allows certain features of these theories to work.
  For example,
  for $\V$ the category of spaces,
  one has that any cofibrant replacement of the commutative operad is always an $E_\infty$-operad in the sense of \cite{May72}.
}
...
\todo[inline]{talk about techinical features needed from Berger-Moerdijk? work similiar things into interval cofibrancy statement}
\todo[inline]{remark about semi-model structures}

\begin{remark}
      \label{SEMI_REM}
      We note that weaker versions of our three main results hold with restricted hypotheses.
      
      A \textit{semi-model structure} is a weakening of Quillen's definition,
      where certain factorization and lifting conditions only hold for maps with a cofibrant source.
      However, they often remain a useful context for conducting homotopy theory.
      In Appendix \ref{SMS_SEC}, we introduce these theories more formally, and show that
      \eqref{THMI_EQ} and \eqref{THMIII1ST EQ},\eqref{THMIII2ND EQ} define semi-model categories
      $\Op_{\mathfrak C, \F}^G(\V)$ and $\Op_{\bullet, \F}^G(\V)$
      under a broader set of hypotheses than given in Theorems \ref{THMI} and \ref{THMIII}:
      \begin{itemize}
      \item $\Op_{\mathfrak C, \F}^G(\V)$ exists without assuming $(v)$ the (global) monoid axiom.
            This in fact implies that Theorem \ref{THMII} always holds without $(v)$.
      \item $\Op_{\bullet, \F}^G(\V)$ exists without the (global) monoid axiom provided that the equivariant model structures on $\V^G$ are mildly better behaved.
      \end{itemize}
\end{remark}
...
\todo[inline]{A remark for the end of the intro: the homotopy theory for equivariant enriched categories}

\begin{remark}
      We note that the theory for equivariant enriched categories is significantly simpler.
      In particular, the desired weak equivalences and fibrations in $\Cat_\bullet(\V)^G$ are detected by the formal theory of \cite{Ste16}:
      As the possible isotropy of $\mathcal C(x,y)$ is simply a subgroup of $G \simeq G \times \Sigma_1$,
      we conclude that a choice of model category is determined by a single family $\F_1$ of subgroups of $G$.
      Given such an $\F_1$, we define a functor $\mathcal C \to \mathcal D$ in $\Cat_\bullet(\V)^G_{\F_1}$ to be an $\F_1$-weak equivalence or $\F_1$-fibration iff
      $\mathcal C^H \to \mathcal D^H$ is so in $\Cat_\bullet(\V)$ for all $H \in \F_1$.

      These theories are recovered by our main results.
      $\Cat_{\bullet}^G(\V)$ is analogously fibered over $\Set^G$,
      and both the fibers and the total category are recoverable as undercategories, with
      \[
            \Cat^{G}_{\mathfrak C}(\V) = \Op^G_{\mathfrak C}(\V) \downarrow \eta_{\mathfrak C},
            \qquad
            \Cat_\bullet^G(\V) = \Op_\bullet^G(\V) \downarrow \eta
      \]
      where $\eta$ (resp. $\eta_{\mathfrak C}$) is the terminal ($\mathfrak C$-colored) $\V$-enriched category (Notation \ref{1_NOT}).
      Thus, if $\V$ satisfies the hypotheses of Theorem \ref{THMIII},
      any $(G,\Sigma)$-family $\F = \set{\F_n}$ induces a $\F$-model structures $\Cat_{\mathfrak C,\F}^G(\V)$, $\Cat_{\bullet,\F}^G(\V)$.
      However, unpacking \eqref{THMI_EQ}, \eqref{THMIII1ST EQ}, and \eqref{THMIII2ND EQ},
      it is clear that the induced model structure only depends on $\F_1$ and is precisely the model structure presented above:
      $\Cat_{\bullet, \F}^G(\V) = \Cat(\V)^G_{\F_1}$.
      
      Finally, we can build these simpler theories directly
      by adapting our tree/operadic analysis in this paper to the linear/categorical setting,
      replacing
      $\Omega_{\mathfrak C}^0$ and $\Sigma_{\mathfrak C}$ with
      $\mathbb N_{\mathfrak C} = \coprod_{n \geq 0} \mathfrak C^{\times n}$ and $\mathfrak C^{\times 2}$, respectively.
      % \begin{itemize}
      % \item $\Omega_{\mathfrak C}^0$ with $\mathbb{N}_{\mathfrak C} = \coprod_{n \geq 0} \mathfrak C^{\times n}$,
      % \item $\Sigma_{\mathfrak C}$ with $\mathfrak C^{\times 2}$,
      % \item $\mathsf{lr}_{\mathfrak C}$ with $G \ltimes \mathbb{N}_{\mathfrak C} \to G \ltimes (\mathfrak C^{\times 2})$,
      % \item $\Sym^G_{\mathfrak C, \F}(\V)$ with $\mathsf{Graph}^G_{\mathfrak C, \F_1}(\V) = \V^{G \ltimes \mathfrak C^{\times 2}}_{\F_1}$,
      % \item $\boldsymbol{V}_{\mathfrak C} \colon \Omega_{\mathfrak C} \to \Sigma \wr \Sigma_{\mathfrak C}$ with
      %       $\boldsymbol{V}_{\mathfrak C} \colon \mathbb{N}_{\mathfrak C} \to \mathbb{N} \wr (\mathfrak C^{\times 2}) = \coprod_{n \geq 0} (\mathfrak C^{\times 2})^{\times n}$.
      % \end{itemize}
      In this case, we see that the proof of the categorical Theorem \ref{THMI} no longer depends upon cofibrant symmetric pushout powers
      (as expected, as we have removed all traces of the symmetric groups),
      and thus this hypothesis can be stricken from both the categorical Theorems \ref{THMI} and \ref{THMIII}.
      
      Moreover, the pseudo indexing system condition in the linear setting becomes trivial,
      as both sides of \eqref{PIS_EQ} are simply the family $\coprod_{n \geq 0} \F_1$ of the groupoid $G \times \mathbb{N}$,
      and thus the categorical Theorem \ref{THMII} follows for any family $\F_1$ of $G$.

      {\color{OliveGreen} % ---------- OLIVE GREEN ----------
        Finally, we note that when our group $G = \**$ is trivial, there are only two posisble families $\F_1$,
        namely the empty family and the ``complete'' family.
        The complete family recovers the canonical model structure on $\Cat_\bullet(\V)$,
        while the empty family recovers one of the trivial model structures, as all maps are trivial fibrations.
      } % ------------------------------
\end{remark}
...
The canonical examples for Theorems \ref{THMI} and \ref{THMIII} are for $\V = \sSet$ and $\V = \sSet_{\**}$.
However, they also hold for (pointed) topological spaces, categories, and sets.
...
\begin{example}
      ...
      \end{enumerate}
      Hence $\Op_\bullet^G(\Top)$ and $\Op_\bullet^G(\Top_{\**})$ have the $\F$-Dwyer-Kan model structure for any $(G, \Sigma)$-family $\F$ with enough units.
\end{example}
...
\begin{example}
      ...
      and the remaining conditions follow by hand and by following \cite[Remark 6.18]{BP_geo}.
...
\begin{proposition}\label{FGC PROP}
      ...
      \begin{equation}\label{FGC_EQ}
            \begin{tikzcd}[column sep = 35pt]
                  G \ltimes \Omega^{0,op}_{\mathfrak{C}}
                  \arrow[d, "\mathsf{lr}_{\mathfrak C}"']
...
\begin{remark}\label{COLOR_SQ_REM}
      ...
      \item the square in the middle is a pushout in $\Op^G(\V)$ iff
            the square on the left is a pushout in the color-fixed fiber. % $\Op^{G}_{\mathfrak C_{\P_1}}(\V)$.
      \end{enumerate}
      \begin{equation}
            \label{COLOR_SQ_EQ}
            \begin{tikzcd}
...
The maps in \eqref{FSQNDEF EQ} and the families from \eqref{FWRNXI EQ} can be assembled for all $n \geq 0$ simultaneously.
\begin{definition}
      \label{FSQUARE_DEF}
      Given a family 
      ...
      Analogously, given a map $f \in \V^{\mathcal G}$, we write $f^{\square}$ (resp. $f^{\otimes}$) for the maps in $\V^{\Sigma \wr \mathcal G}$
      given by $f^{\square n}$ (resp. $f^{\otimes n}$) in each component.
\end{definition}
...
then $f^{\otimes}$ is a (trivial)
$\F^{\ltimes}$-cofibration
between $\F^{\ltimes}$-cofibrant objects in $\V^{\Sigma \wr \G}$.
\end{enumerate}
\end{proposition}
...
\begin{proof}
      It suffices to show this for each $n \geq 0$ individually.
...
discussed in Proposition \ref{FGC PROP}
...
whose proof is postponed to the end of Appendix \S \ref{PUSHOUT_SEC}.
...
We describe this special class now.

\begin{definition}
      For any $G$-set $\mathfrak C$, let $\boldsymbol{V}_{\mathfrak C}$ denote the composite of the first two horizontal maps in \eqref{FGC_EQ}.
      \begin{equation}
            \label{VC_EQ}
            \boldsymbol{V}_{\mathfrak{C}} \colon 
            G \ltimes \Omega_{\mathfrak{C}}^{0,op}
            \xrightarrow{\ G \ltimes \boldsymbol{V}^{op} \ }
            G \ltimes \left(\Sigma \wr \Sigma_{\mathfrak{C}}\right)^{op}
            \xrightarrow{\ \Delta \ }
            \left( \Sigma \wr  \left(G  \ltimes \Sigma_{\mathfrak{C}}^{op} \right)^{op} \right)^{op}
      \end{equation}
...
\begin{definition}
      ...
      \end{equation}
      for $\F^\ltimes$ defined as in \eqref{FWRNXI EQ},
      and $\mathsf{lr} \colon G \times \Omega^{0,op} \to G \times \Sigma^{op}$.
\end{definition}
...
\begin{remark}
      ...
      $\F$ is composed of $G$-graph subgroups (e.g. \cite[Defn. 6.34]{BP_geo}).
...
\begin{remark}
      Theorem \ref{SIGMAG_COF_THM} is the $\mathfrak C$-colored analogue to \cite[{Remark 6.64 with Theorem 6.59}]{BP_geo}.
      ...
      will come almost for free using formal manipulation of families of subgroups of groupoids.
\end{remark}
...
\begin{proof}
      ...
      First, consider the following commuting cube of groupoids
      \begin{equation}
            \label{FCA_SQ_EQ}
            \begin{tikzcd}
                  G \ltimes \Omega_{\mathfrak C}^{0,a,op}
                  \arrow[rr, "\boldsymbol{V}_{\mathfrak C}^{a}"] \arrow[dr] \arrow[dd, "\pi_{\mathfrak C,a}"']
                  &&
                  \Sigma \wr \left( \displaystyle{\coprod_2} G \ltimes \Sigma_{\mathfrak C}^{op} \right)
                  ...
                  &&
                  \Sigma \wr \left( \displaystyle{\coprod_2} G \ltimes \Sigma^{op} \right)
                  \arrow[dd, "\Sigma \wr \nabla_2"]
                  \\
                  G \ltimes \Omega_{\mathfrak C}^{0,op}
                  ...
                  \arrow[uu, leftarrow, crossing over, "\pi_a"', near end]
                  &&
                  \Sigma \wr \left( G \ltimes \Sigma^{op} \right)      
            \end{tikzcd}
      \end{equation}
      where $\Omega_{\mathfrak C}^{0,a}$ is the category of alternating trees from Notation \ref{ALTOMEGA_NOT},
      and $\boldsymbol{V}_{\mathfrak C}^{a}$ is defined analogously to $\boldsymbol{V}_{\mathfrak C}$ from \eqref{VC_EQ} but where active and intert vertices are separated.
      
      Since $\F$ is a pseudo indexing system, \eqref{PHIFLTIMES_EQ}, \eqref{PIS_EQ}, and \eqref{FCA_SQ_EQ} imply
      \[
            \left(\boldsymbol{V}_{\mathfrak C}^{a}\right)^{\**} \left( (\nabla_2^{\**} \F_{\mathfrak C})^\ltimes \right)
            = \left(\boldsymbol{V}_{\mathfrak C}^{a}\right)^{\**}  (\Sigma \wr \nabla_2)^{\**} (\Sigma \wr \pi_{\mathfrak C})^{\**} \F^\ltimes
            = \pi_{\mathfrak C,a}^{\**} \pi_{\mathfrak C}^{\**} \boldsymbol{V}^{\**} \F^{\ltimes}
            \subseteq
            \pi_{\mathfrak C, a}^{\**} \pi_{\mathfrak C}^{\**} \mathsf{lr}^{\**} \F
            = \pi_{\mathfrak C, a}^{\**} \mathsf{lr}_{\mathfrak C}^{\**} \F_{\mathfrak C}
      \]
      and hence the left Kan extension
      \[
            \mathsf{lr}_! = \left(\pi_{\mathfrak C, a}^{\**} \mathsf{lr}_{\mathfrak C}^{\**} \right)_!
      \]
      sends
      $(\boldsymbol{V}_{\mathfrak C}^{a})^{\**}\left( (\nabla_2^{\**} \F_{\mathfrak C})^\ltimes \right)$-cofibrations in $\V^{G \ltimes \Omega_{\mathfrak C}^{a,op}}$
      to
      $\F_{\mathfrak C}$-cofibrations in $\V^{G \ltimes \Sigma_{\mathfrak C}^{op}}_{\F_{\mathfrak C}} = \Sym^G_{\mathfrak C, \F}(\V)$
      by Proposition \ref{EQQUILADJ PROP}.

      Second, the maps $\varnothing \to \O$ and $u \colon X \to Y$ define a $\nabla_2^{\**}\F_{\mathfrak C}$-cofibration $(\O,u)$ in $\V^{\amalg_2 G \ltimes \Sigma_{\mathfrak C}^{op}}$ by Remark \ref{SIGMACOF_REM}.
      Moreover, the functor $n_k^{\O,X,Y} \colon G \ltimes \Omega_{\mathfrak C}^{a}[k]^{op} \to \V$ from \eqref{NKOXY EQ} is given by
      \[
            n_k^{\O,X,Y} = \left( \boldsymbol{V}_{\mathfrak C}^{a}\right)^{\**}
            \left( \left(\O,u\right)^{\square} \right).
      \]
      By Propositions \ref{SIGMAWRGF PROP}(ii) and \ref{EQQUILADJ PROP},
      this is a $(\boldsymbol{V}_{\mathfrak a}^{a})^{\**}\left( (\nabla_2^{\**} \F_{\mathfrak C})^\ltimes \right)$-cofibration.
      
      Combining these two facts, we have that
      $(\O_{k-1} \to \O_k) = \mathsf{lr}_! n_k^{\O, X,Y}$ is an $\F_{\mathfrak C}$-cofibration, as desired.
\end{proof}
...
\begin{remark}
      When $\F$ is a \textit{family of $G$-corollas} \cite[Defn. 4.44]{BP_geo},
      the family $(\boldsymbol{V}^{\**}\F^\ltimes)_T$ agrees the family $\F_T$ from \cite[Prop. 6.44]{BP_geo} for any tree $T$.
\end{remark}
...
\begin{example}
      ...
      where
      $J = N(0 \rightleftarrows 1) = N \I$ is the nerve of the walking isomorphism category and
...
\todo[inline]{hypotheses for this theorem?}
...
\begin{corollary}\label{ALBEETA COR}
      Assume that $\V$ satisfies conditions $(i),(ii),(iii),(iv)$ from Theorem \ref{THMI}, the regular monoid axiom, and $(vii)$ from Theorem \ref{THMIII}.
...
%\end{remark}
% come back here! include this for semi-model structure stuff
