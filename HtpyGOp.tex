...
\begin{theorem}
      ...
      Then there exists a model structure on $\Op^G_{\mathfrak C}(\V)$ such that
      ...
      Finally, analogous model structures $\Op^G_{\mathfrak C}(\V)$, $\Op^G_{\mathfrak C, \F}(\V)$ exist provided that
      $(\V, \otimes)$:
      \begin{enumerate}[label = (\roman*)]
      \item is a cofibrantly generated model category;
      \item is a closed monoidal model category with cofibrant unit;
      \item admits all equivariant model structures;
...
\begin{remark}
      As $\Cat^{G, \mathfrak C}(\V) = \Op^G_{\mathfrak C}(\V) \downarrow \**$,
...
The existence of these model structures will be sufficient for Theorem \ref{THM1_C}.
...
\begin{remark}
      By quick adjunction arguements, we note that for $A \in \V$ and $B \in \V^G$, we have
      \[
...
\begin{theorem}
      Suppose $\V$ satisfies the hypotheses in Theorem \ref{THM1_C},
...
We consider two conditions on $(\V,\otimes)$ necessary for our first main result, Theorem \ref{THM1_C}:
\begin{itemize}
...
\begin{lemma}[{cf. \cite[4.12]{Cav}, \cite[2.10]{BM13}}] \label{RIGHTPROPER_LEM}
...
\begin{convention}
      ...
      In particular, Theorem \ref{THM1_C} and Corollary \ref{CATV_MC_COR} imply that $\Cat^{\mathfrak C}(\V)$ is a (semi)-model category for each set $\mathfrak C$,
...
The main results Theorem \ref{THM1_C} and Theorem \ref{THMII}, adapted to the semi-model structure context, follow with only minimal changes.
