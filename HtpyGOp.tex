...
\subsubsection{Homotopy theory for equivariant enriched categories}

We have an analogous fibered category $\Cat_\bullet^G(\V) \to \Set^G$ of $G$-categories enriched in $\V$.
Both the fibers and the total category are recoverable as undercategories, with
\[
...
Thus any model structure on $\Op^G_{\mathfrak C}(\V)$, $\Op^G_\bullet(\V)$ induces model structures on $\Cat_{\mathfrak C}^G(\V)$, $\Cat_\bullet^G(\V)$.
In particular, if $\V$ satisfies the hypotheses of Theorem \ref{THMIII},
any $(G,\Sigma)$-family $\F = \set{\F_n}$ yields $\F$-model structures $\Cat_{\mathfrak C,\F}^G(\V)$, $\Cat_{\bullet,\F}^G(\V)$.

From \eqref{THMI_EQ}, \eqref{THMIII1ST EQ}, and \eqref{THMIII2ND EQ},
it is straightforward that
these categorical model structures only depend on $\F_1$,
and moreover that
$\Cat_{\bullet, \F}^G(\V) = \Cat_\bullet(\V)^G_{\F_1}$ as in \cite{Ste16};
that is, $\mathcal C \to \mathcal D$ is a weak equivalence or fibration iff
$\mathcal C^H \to \mathcal D^H$ is so in $\Cat_\bullet(\V)$ for all $H \in \F_1$.

We can also recover these model structures directly by adapting our tree/operadic analysis in this paper to the linear/categorical setting, replacing
...
% \end{itemize}
In this case, we see in particular that the proof of the categorical Theorem \ref{THMI} no longer depends upon cofibrant symmetric pushout powers
...
The canonical example for Theorems \ref{THMI} and \ref{THMIII} are for $\V = \sSet$ and $\V = \sSet_{\**}$.
However, they also hold for (pointed) topological spaces.
\begin{example}
...
We explore two other examples: the categories $(\Cat, \times)$ and $(\Set, \times)$ of categories and sets with the cartesian product.

\subsubsection*{The category of categories}

Consider the category of categories $\Cat$ equipped with the cartesian product and the canonical model structure (e.g. \cite{Rez}):
\begin{itemize}
