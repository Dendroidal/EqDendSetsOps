...
\begin{theorem}
      ...
      Then there exists a model structure on $\Op^G(\V)$, denoted $\Op^G_\F(\V)$, such that
...
However, to prove Theomem \ref{THMII}.
To prove Theorem  main results {\color{red} reference}, we will need some control over the equivariant model structures, namely
that $\V$ has \textit{acyclic cofibrant fixed points} for all finite groups $G$:
\begin{equation}
      \label{GTCOFH_COF_EQ}
      \mbox{$(\bigstar)$: fixed points of genuine trivial cofibrations in $\V^G$ are trivial cofibrations.}
\end{equation}
Specifically, $\left(\mathcal J_G\text{-cof}\right)^H \subseteq \mathcal J\text{-cof}$.

This is sufficent to show that $\V$ admits all equivariant model structures.
\begin{lemma}
      Suppose $\V$ is a strongly cofibrantly generated model category which satisfies $(\bigstar)$.
...
\begin{proof}
      ...
      Condition $(ii)$ is exactly $(\bigstar)$.  
\end{proof}
...
and the fact that level genuine trivial cofibrations are closed under transfinite composites and pushouts, % as colimits are computed levelwise,
...
Hence the global monoid axiom implies that each $\O_{k-1}(\vect C) \to \O_k(\vect C)$ and their composite
...
The following definitions synthesize parts of the \textit{cellularity} conditions of \cite{Ste16} with the monoid axiom (Definition \ref{MONAX_DEF}).

\begin{definition}
      We say $\V$ has \textit{weak acyclic cellular fixed points} (resp. \textit{monoidal weak acyclic cellular fixed points}) if for all finite groups $G$ and subgroups $H$,
      \begin{enumerate}[label = (\roman*)]
      \item $(-)^H \colon \V^G \to \V$ preserves direct colimits of maps in $\mathcal J_G$ (resp. $\mathcal J \otimes \V^G$).
      \item $(-)^H \colon \V^G \to \V$ preserves pushouts of $\mathcal J_G$ (resp. $\mathcal J \otimes \V^G$).
      \item $(-)^H$ sends $\mathcal J_G$ (resp. $\mathcal J \otimes \V^G$) to $\mathcal J$-cof (resp. $(\mathcal J \otimes \V^G)$-cof).
      \end{enumerate}
\end{definition}
...
\begin{lemma}
      ...
      \begin{enumerate}[label=(\roman*)]
      \item If $\V$ has weak acyclic cellular fixed points, then $\V$ satifies $(\bigstar)$ from \eqref{GTCOFH_COF_EQ}, and hence
