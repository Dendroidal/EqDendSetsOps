...
\begin{proof}
      ...
      % fixed points in $G$-set commute with directed colimits (as long as $G$ is finitely-generated), so
      % \[
      %       \lim_\alpha \mathcal C^G(G/H \cdot A, B_\alpha) =
      %       \lim_\alpha \mathcal C^H(H/H \cdot A, B_\alpha) =
      %       \left( \lim_\alpha \mathcal C(A, B_\alpha) \right)^H =
      %       \mathcal C^H(A, \colim_\alpha B_\alpha) =
      %       \mathcal C^G(G/H \cdot A, \colim_\alpha B_\alpha),
      % \]
      % as genuine (trivial) cofibrations are in particular underlying cofibrations.
...
\end{lemma}
\begin{proof}
      Unpacking \cite[Equation (6.51)]{BP_geo} we have that $\Gamma \leq G \times \Aut(T)$ is in
      $\left(\F_{T_{i_1}}^{\ltimes_G |\lambda_1|} \sqcap_G \dots \sqcap_G \F_{T_{i_r}}^{\ltimes_G |\lambda_r|} \right)$
      iff for all $l \in \boldsymbol{L}(C)$, % $l \in \lambda_k$,
      \[
      ...
      First, $\Gamma \leq \Aut(\vect T)$ $\in \F_T \cap \Aut(\vect T)$ iff $\pi_{\vect T}(\Gamma) \in \Aut(\vect C)$, as the following diagram commutes.
      \begin{equation}
            \label{AUTT_EQ}
            \begin{tikzcd}
