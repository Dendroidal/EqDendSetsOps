...
% -------Subsection Titles out of line for AMSART -----------------------
\usepackage{etoolbox}
...
%% add spacing between section names
...
\begin{definition}
      Let $\underline{\mathfrak C}$ be a coefficient system of sets.
      Then the category $\underline{\mathfrak C}\Omega_G$ of \textit{$\underline{\mathfrak C}$-coloured trees}
...
\begin{remark}
      ...
      \begin{equation}
            ...
            \end{tikzcd}
      \end{equation}
      We note that the class of morphisms in $\mathsf F^G$ in the image of $E$ are those isomorphic to an adjunction counit $G \cdot_H A|_H \to A$.
      
      Equivalently, $\underline{\mathfrak C}\Omega_G$ is isomorphic to the Grothendieck construction on the functor
      \begin{equation}
            \begin{tikzcd}[row sep = tiny]
                  \Omega_G^{op} \arrow[r]
                  &
...
Objects of $\CO$ are pairs $(T, \mathfrak c)$ of a $G$-tree $T$ with a map $\mathfrak c: E_G(T) \to \underline{\mathfrak C}$ over $O_G$.
...
are given by quotients of trees $q: S \to T$ such that, for every edge orbit $[e]$ of $S$, we have
\begin{equation}
      \mathfrak c([e]) = q_{[e]}^{\**}\mathfrak d([q(e)]),
\end{equation}
where $q_{[e]}: G / G_{[e]} \to G / G_{[q(e)]}$ is the map in $O_G$ induced by $q$.
...
\begin{proof}
      ...
      the result will then follow from \cite[HYPER PROP]{BP_Segal}.
