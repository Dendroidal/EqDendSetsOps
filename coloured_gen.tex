...
\begin{remark}
      For $X\in \dSet_G$ with $X(\eta_{G/H}) = \mathfrak C(G/H)$, we have that
...
Similarly, \cite[Prop 3.90, 4.12, 4.15, 4.26, 4.28, 4.30]{BP17} naturally generalized to the coloured-setting,
...
\todo[inline]{recall adjunctions with $\Lan$ and $\Ran$, canonical op-isos, etc}
...
\begin{definition}
      The \textit{genuine equivariant $\UC$-coloured operad} is the monad
      $\mathbb F_{G,\UC}$ on $\Sym_{G, \UC}(\V) = \Fun(\UC\Sigma_G^{op}, \V)$ given by
      \begin{equation}
      ...
      $\mathsf{Alg}_{\mathbb{F}_{G,\UC}}(\mathsf{Sym}_{G,\UC}(\mathcal{V}))$ of \textit{genuine equivariant $\UC$-coloured operads}.
\end{definition}

\subsection{Comparison with $\mathfrak C$-coloured operads}

For $T = (T_i)_I$, we define
\begin{equation}
      \iota_{\**}\mathfrak c_i: E(T_i) \to \mathfrak C(G/e)
\end{equation}
by $e \mapsto g^{\**}q_e^{\**}(\mathfrak c[f])$, where
$e \in Gf$ (with $f$ minimal in the planar structure on $T$),
$g\in G$ minimal such that $g e = f$,
$q$ the canonical quotient map out of $G$ such that $q^{\**}T$ is $G$-free,
and $q_e$ the induced map $G/G_e \to G / G_{q(e)}$.

We record that $(T_i, \mathfrak c_i) \in \UC\Omega$, and furthermore that
$i \mapsto (T_i, \mathfrak c_i)$ is a well-defined functor $B_{I}G \to \UC\Omega$.
...
\begin{equation}
      \begin{tikzcd}
            ...
            \left(
                  \prod_I Y(T_i, \iota_{\**}\mathfrak c_i)
            \right)^G
            \simeq
            Y(q^{\**}(T,\mathfrak c))^H
      \end{tikzcd}
\end{equation}
for $T \in \Omega_G(G/H)$.
