...
\item[(1)] We believe that using $\pi$ as the name for a generic fibration here does not add confusion, but instead illuminates the symmetry of the definition of a fibered adjunction, such as in the second display equation in Definition 2.22.
        Additionally, in certain proofs (e.g. the proof of Proposition 2.24) giving the second fibration a different name would add clutter to the notation without improving clarity.
        Moreover, in key examples (e.g. Proposition 2.30 and Remark A.21) the maps $\pi$ are in fact the same (up to a forgetful functor).       
\end{enumerate}
