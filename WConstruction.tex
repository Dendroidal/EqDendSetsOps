...
not work with operads directly, but instead with their nerves.
To that end, 
we first recall the category of (colored) trees in \S \ref{TREES_SEC},
then several associated presheaf categories, as well as their connections to (simplicial) operads using the nerve, in \S \ref{NERVE_SEC}.
...
\begin{notation}
        ...
        we let $T_v \to T$ denote the planar outer face consisting of only this vertex.
...
\begin{example}
        ...
        $\Omega(T)$ are either $\**$ or $\emptyset$,
        there is always at most one possible way to compose operations,
        and hence at most one possible operad structure on $\Omega(T)$.
...
we briefly discuss this connection.
...
\begin{remark}
        For any $X$, $T$, $\mathfrak c$ as in Notation \ref{PREOP_NOT}\ref{XUC_LBL},
...
\begin{definition}
        ...
        \item $X$ has the strict right lifting property against
                all inner horn inclusions $\Lambda^e[T] \to \Omega[T]$,
                for all $T \in \Omega$ and $e \in \boldsymbol{E}^i(T)$ an inner edge.
        \item $X$ has the strict right lifting property against
                all Segal core inclusions $Sc[T] \to \Omega[T]$, for all $T \in \Omega$,
        \item The maps
                $X(T) \to X(Sc[T])$ % limit over Segal core poset of $T$
                % induced by $Sc[T] \to \Omega[T]$
