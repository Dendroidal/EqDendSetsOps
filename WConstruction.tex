...
We end this subsection with a discussion of a special type of tree.
...
\begin{definition}
        ...
        Building off this, we denote by $\Sigma_{\mathfrak C}$ the subcategory of $\Omega_{\mathfrak C}$ spanned by $\mathfrak C$-corollas and isomorphisms.        
\end{definition}
...
\begin{notation}
     ...
     Explicitly, the leaf tuple and root edge of $\mathsf{lr}(T)$ are precisely
     the leaf tuple $\underline{l}_T$ and root edge $r_T$ of $T$,
     with a single vertex connecting them.
...
\begin{remark}
        \label{TREECHANGEC_REM}
        For any map of sets $f \colon \mathfrak C \to \mathfrak D$, there are associated functors
        \[
                f \colon \Omega_{\mathfrak C} \to \Omega_{\mathfrak D},
                \qquad
                 f \colon \Sigma_{\mathfrak C} \to \Sigma_{\mathfrak D},
         \]
         given by
         \[
                 \vect T (T, \vect c)
                 \longmapsto
                 f \vect T := (T, \boldsymbol{E}(T) \xrightarrow{\mathfrak c} \mathfrak C \xrightarrow{f} \mathfrak D).
         \]
\end{remark}
...
The goal of this subsection is to setup and prove Theorem \ref{SSC_THM},
which allows us to identify simplicial operads as special presheaves satisfying a strict Segal condition.
...
\todo[inline]{made an active choice to not include $\mathbb F$ or $\Sym_\bullet$}
...
\[
        \O(\vect T) := \prod_{v \in \boldsymbol{V}(T)}\O(\vect T_v) \longto \O(\mathsf{lr}(\vect T)),
\]
natural in $\vect T$.
...
is given by a collection of natural maps
...
\begin{remark}[{cf. \cite[Rem. \ref{OC-OP_MAP REM}]{BP_FCOP}}]
        ...
        \]
        The right adjoint is given by $f^{\**}\O(\vect C) = \O(f \vect C)$, with $f \vect C$ as in Remark \ref{TREECHANGEC_REM};
...
\begin{remark}
        ...
        \cite[Eq. \eqref{TAS-PREOPCOLFIXEQ EQ}]{BP_TAS}.
        % \begin{proposition}[{cf. \cite[Eq. \eqref{TAS-PREOPCOLFIXEQ EQ}]{BP_TAS}}]
        ...
        Henceforth, we will work with the $X_{\mathfrak c}(T)$ notation for the levels of preoperads.
\end{remark}
...
(\cite[\S 2]{CM13a}, \cite[\S 2.3]{BP_edss}).
...
\begin{remark}    
        ...
        (ii) iff (iii) follows by \eqref{SDSET_EQ};
        (iii) iff (iv) follows by \eqref{XUDECALT EQ} and \eqref{COLDEC_EQ}. 
\end{remark}

\begin{remark}
        In this paper, we will only use description (iv).
        However, (i)-(iii) are closer to the original discussions of
...
\begin{proof}
        ...
        $N \colon \Op \to \dSet$
        is fully faithful, with essential image
        those $A \in \dSet$ satisfying the dendroidal sets analogue of Definition \ref{SSC_DEF}(i).
        Thus $N^{\Delta^{op}}$ is fully-faithful, and moreover
...
\begin{remark}
        Given a $\mathfrak C$-colored operad $\O \in \sOp$, then
        \[
