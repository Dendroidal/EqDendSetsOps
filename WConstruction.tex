...
\begin{theorem}
        ...
        it has colors $\boldsymbol{E}(U)$, and
...
\end{theorem}
\begin{proof}
        Combination of Definition \ref{NWTNS DEF}, Remark \ref{NWTNS_REM}, Proposition \ref{NWTNS_NAT_PROP}
\end{proof}

\begin{theorem}
        \begin{enumerate}
        \item Above theorem extends to necklaces (Corollary \ref{NWTNS_NNAT_COR})
        \item $W(X) = \colim_{\Omega[\mathfrak n] \to X \in \mathsf{Nec}_{/ X}} W(\mathfrak n)$ (Theorem \ref{KANEXTCHAR THM})
        \item $W(X)_n(\vect S)$ are equivalence classes of quadruples (Cor. \ref{NWXREPS COR})
        \item these classes have chosen reps, unique up to unique iso (Cor. \ref{NWXREPS2_COR})
        \end{enumerate}
\end{theorem}
...
\begin{definition}\label{CFOREST_DEF}
        ...
        \begin{itemize}
        \item objects pairs $\vect U = (U, \mathfrak c)$ with
                $U \in \Omega$ a tree and
                $\mathfrak c \colon \boldsymbol{E}(U) \to \mathfrak C$ a coloring of its edges;
        \item arrows $\vect U = (U, \mathfrak c) \to (U', \mathfrak c') = \vect{U'}$ maps
                $\varphi \colon U \to U'$ in $\Omega$ such that $\mathfrak c = \mathfrak c' \varphi$.
        \end{itemize}      
        We write
...
A full discussion can be found in \textcolor{red}{CITATION: TAS? OneColor?}

\begin{example}
        One important class of colored operads are the free operads generated by trees.
        Given $U \in \Omega$, we define $\Omega(U)$ to be the $\boldsymbol{E}(U)$-colored operad which,
        when evaluated on an $\boldsymbol{E}(U)$-colored corolla
        $\vect C = (C, \mathfrak c \colon \boldsymbol{E}(C) \to \boldsymbol{E}(U))$, is given by
        ...
        follows from of the observation that,
        for any tree $V \in \Omega$,
        a coloring
        $\mathfrak{c} \colon \boldsymbol{E}(V) \to \boldsymbol{E}(U)$
        defines a map
        $V \to U$ in $\Omega$
        iff
        the restrictions
        $\mathfrak{c}_v \colon \boldsymbol{E}(V_v) \to \boldsymbol{E}(U)$
        define maps
        $V_v \to U$ in $\Omega$
        for all vertices $v \in \boldsymbol{V}(V)$.
\end{example}

(Simplicial) operads can also be identified as presheaves satisfying a strict Segal condition,
under the \textit{nerve} construction.
As this is the perspective we find fruitful for our proofs, 
we briefly discuss this narrative.
...
\begin{equation}
        ...
        N \colon \Op \to \dSet
\end{equation}
sends an operad $\O$ to the dendroidal set defined on $U \in \Omega$ by
$N \O(U) = \Hom(\Omega(U), \O)$,
with $\Omega(U)$ as in \eqref{OMEGADEF_EQ}.

This functor extends by postcomposition to the associated categories of simplicial presheaves,
yielding the leftmost equation below,
\begin{equation}
        \label{SOPN_EQ}
        N \colon \Op^{\Delta^{op}} \to \sdSet,
        \qquad
        N \colon \sOp \to \PreOp,
\end{equation}
where $\sdSet = \Set^{\Delta^{op} \times \Omega^{op}}$ is the category of \textit{simplicial dendroidal sets}.
This extension restricts to the rightmost functor in \eqref{SOPN_EQ},
...
\]
regarding simplicially enriched operads as those simplicial objects with a constant set of functors,
...
\begin{notation}[{cf. \cite[\S 4]{BP_edss}, \cite[\S 3]{BP_TAS}}]
        \begin{itemize}
        \item For $U \in \Omega$, let $\Omega[U] \in \dSet$ denote the representable presheaf $\Omega[U](V) = \Omega(V,U)$.
                Note that $\Omega[U] = N\Omega(U)$.
        \item For $U \in \Omega$, let $Sc[U] \subseteq \Omega[U] \in \dSet$ denote the \textit{Segal core} of $U$
                (e.g. \cite[\S 2]{CM13a}, \cite[\S 2.3]{BP_edss}). % \cite[Eq. \eqref{TAS-eq:SC}]{BP_TAS})
        \item For $X \in \sdSet$, we write $X_n(U)$ for the evaluation at $n \in \Delta$ and $U \in \Omega$,
                and refer to $n$ and $U$ as the \textit{simplicial} and \textit{dendroidal} directions, respectively.
        % \item More generally, we write
        %         \begin{equation}
        %                 \label{SDSET_EQ}
        ...
        \item (cf. \cite[Not. \ref{TAS-XUDECALT NOT}]{BP_TAS})
                Given $X \in \PreOp$, $U \in \Omega$, and a coloring $\mathfrak c \colon \boldsymbol{E}(U) \to X(\eta)$,
                we define $X_{\mathfrak c}(U) \in \sSet$ as the pullback below.
                \begin{equation}
                        \label{XUDECALT EQ}
                        \begin{tikzcd}
                                X_{\mathfrak C}(U) \arrow[r] \arrow[d]
                                &
                                X(U) \arrow[d]
                                \\
                                \** \arrow[r, "\mathfrak c"]
                                &
                                \displaystyle{
                                  \prod\limits_{\boldsymbol{E}(U)} X(\eta)
                                }
                        \end{tikzcd}
                \end{equation}
                ...
                \begin{equation}
                        \label{COLDEC_EQ}
                        X(U) \simeq \coprod_{\mathfrak c \colon \boldsymbol{E}(U) \to X(\eta)} X_{\mathfrak c}(U).
                \end{equation}
        \item Given any $A \in \dSet$, we abusively write $A \in \PreOp \subseteq \sdSet$ for the
                simplicially-constant presheaf $A_n(U) = A(U)$.
        \end{itemize}
\end{notation}
...
\begin{definition}
        ...
        \begin{enumerate}
        \item $X$ has the strict right lifting property against
                all inner horn inclusions $\Lambda^e[U] \to \Omega[U]$,
                for all $U \in \Omega$ and $e \in \boldsymbol{E}^i(U)$ an inner edge.
        \item $X$ has the strict right lifting property against
                all Segal core inclusions $Sc[U] \to \Omega[U]$, for all $U \in \Omega$,
        \item The maps
                $X(U) \to X(Sc[U])$ % limit over Segal core poset of $U$
                % induced by $Sc[U] \to \Omega[U]$
                are isomorphisms of simplicial sets
                for all $U \in \Omega$.
        \item The Segal maps
                $X_{\mathfrak c}(U) \to \prod_{v \in \boldsymbol{V}(U)} X_{\mathfrak c_v}(U_v)$
                are isomorphisms of simplicial sets
                for all $U \in \Omega$ and colorings $\mathfrak c \colon \boldsymbol{E}(U) \to X(\eta)$.
        \end{enumerate}
        % the Segal maps
        % $X_{\mathfrak c}(U) \to \prod_{v \in \boldsymbol{V}(U)} X_{\mathfrak c_v}(U_v)$
        % are isomorphisms of simplicial sets
...
\begin{remark}
        The above conditions can all be checked levelwise on each $X_n \in \dSet$,
        and are in fact equivalent:
        (i) iff (ii) follows by either \cite[Prop. 2.5 and Cor. 2.6]{CM13a} or \cite[Props. 3.22, 3.31]{BP_edss};
        (ii) iff (iii) is clear;
        (iii) iff (iv) follows by \eqref{XUDECALT EQ}. 
\end{remark}

We recall \cite[Prop. 5.3 and Thm. 6.1]{MW09} that the nerve functor
$N \colon \Op \to \dSet$ is a fully-faithful inclusion whose (essential) image is precisely
those $X \in \dSet$ satisfying the strict Segal condition Definition \ref{SSC_DEF}(i).
This immediately extends to simplicial presheaves.
\begin{theorem}
        \label{SSC_THM}
