...
\todo{trees in $\Omega$ will be denoted $T$. This disagrees with TAS, where $T$ is reserved for $G$-trees (except in the appendix)}
...
%      admit a factorization as a non-isomorphism inner face followed by a face map;
        \item a \textit{degeneracy} if it is surjective on edges and preserves leaves
                (and is thus tall).
        \item a \textit{convex map} if we have
                $e <_d e' <_d e''$ in $T$ 
                with $e,e''$ in the image of $\varphi$,
                then $e'$ is as well.
        \end{itemize}
\end{definition}
...
Now, for any map of sets $f \colon \mathfrak D \to \mathfrak C$,
...
\begin{remark}
        Given a $\mathfrak C$-colored operad $\O \in \sOp$, then using the notation from \eqref{XUDECALT EQ} we have
        \[
