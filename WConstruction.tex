...
\end{equation}
% $W_!(X) = \colim_{\Omega[T] \to X} W(T)$.
% \textcolor{OliveGreen}{Given any simplicial operad $\O \in \sOp$, one can also then define the \textit{Boardman-Vogt $W$-construction},
% a natural cofibrant resoltuion of the operad $\O$,
% as the counit
% $W_! hcN \O \to \O$.}
...
\begin{customthm}{A}
        ...
        is defined on an $\boldsymbol{E}(U)$-corolla 
        $\vect{C} = (C,\mathfrak{c})$
        to have $n$-simplices 
...
\end{customthm}
\begin{proof}
        This follows from Remark \ref{NERVEEVAL_REM}, Definition \ref{NWTNS DEF}, Remark \ref{NWTNS_REM}, and Proposition \ref{NWTNS_NAT_PROP}.
\end{proof}

\begin{customthm}{B}
        Let $X \in \dSet$.
        For each $n \in \Delta$ and $X(\eta)$-colored corolla $\vect C$,
        the elements of $W(X)_n(\vect C)$ are given, up to unique isomorphism, by tuples
        \[
	...
	% \end{enumerate}
        such that
        $J_{\bullet}$ is flanked and
        $x$ totally non-degenerate.
\end{customthm}

The category of $\mathfrak C$-colored trees is recalled in Definition \ref{CTREE_DEF}, while
the adjectives \textit{flanked} and \textit{totally non-degenerate} are introduced in Definitions \ref{FLANKED_DEF} and \ref{TOTNONDEG_DEF}, respectively.
...
\begin{proof}[Proof outline]
        ...
        %
        Third, we describe the levels $W(X)_n(\vect C)$ of this colimit as equivalence classes of tuples
...
\begin{notation}
        ...
        \end{equation}
        equivalently the colimit-preserving functor
        such that
        % $X_{\Delta[n]} = X_n$ and 
...
\begin{remark}[{cf. \cite[Eq. \eqref{TAS-STRSEGCON EQ}]{BP_TAS}}]
        \label{NERVEEVAL_REM}
...
\begin{remark}
	...
	(cf. \cite[Cor 3.69]{BP_geo} \todo{not sure what this refers to}).
