...
\todo[inline]{discussion of $\Omega(U)$}
...
% \begin{notation}[cf. \cite[Not. \ref{TAS-XUDECALT NOT}]{BP_TAS}]
%         Given $A \in \dSet$ $U \in \Omega$, and a coloring $\mathfrak c \colon \boldsymbol{E}(U) \to X(\eta)$,
%         define $A_{\mathfrak c}(U) \in \Set$ to be the pullback below.
%         \begin{equation}
%                 \label{AUDECALT_EQ}
%                 \begin{tikzcd}
...
% \begin{definition}
%         \label{SSCDSET_DEF}
%         We say $X \in \dSet$ satisfies the \textit{strict Segal condition} if
%         any of the following equivalent conditions hold:
%         \begin{enumerate}
%         \item $X$ has the strict right lifting property against
%                 all inner horn inclusions $\Lambda^e[U] \to \Omega[U]$.
%         \item $X$ has the strict right lifting property against
%                 all Segal core inclusions $Sc[U] \to \Omega[U]$, $U \in \Omega$.
%         \item The Segal maps
%                 $X(U) \to X(Sc[U]) = \lim_{v \in \boldsymbol{V}(U)}X(U_v)$
%                 are bijections
%                 for all $U \in \Omega$.
%         \item The Segal maps
...
% \begin{remark}
%         The above conditions are in fact equivalent:
%         (i) iff (ii) follows by either \cite[Prop. 2.5 and Cor. 2.6]{CM13a} or \cite[Props. 3.22, 3.31]{BP_edss};
%         (ii) iff (iii) is clear;
%         (iii) iff (iv) follows by \eqref{AUDECALT_EQ}. 
% \end{remark}

% \begin{theorem}[{\cite[Prop. 5.3 and Thm. 6.1]{MW09}}]
%         The nerve $N \colon \Op \to \dSet$
%         is a fully faithful inclusion whose (essential) image is spanned by those $X \in dSet$
%         satisfying the strict Segal condition.
% \end{theorem}


\subsection{Preoperads}

Much of the discussion will be made easier by
considering the \textit{nerves} of (simplicial) operads.



Recall that the nerve functor $N \colon \Cat \to \sSet$ sends a category $\mathcal C$ to the simplicial set $N \mathcal C$
$N \mathcal C_n = \Hom([n], \mathcal C)$,
where $[n] = \set{0 \to 1 \to \dots \to n}$.
Moreover, the (essential) image of the nerve is spanned by those simplicial sets $X$ satisfying a strict Segal condition.
...
\begin{equation}
        ...
        N \colon \Op \to \dSet
\end{equation}
sends an operad $\O$ to the dendroidal set
$N \O(U) = \Hom(\Omega(U), \O)$. 
...
those $X \in \dSet$ satisfying the strict Segal condition.
