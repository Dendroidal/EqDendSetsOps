...
\todo[inline]{Boardman-Vogt construction?}

\todo[inline]{introduce this adjunction}
Once we have a definition of $W(U)$, we can extend this construction to all of $\dSet$ using Yoneda:
%
The adjunction
...
\end{equation}
% $W_!(X) = \colim_{\Omega[U] \to X} W(U)$.
However, this description is opaque/unenlightening.
In the categorical setting,
this left adjoint $W_!$ admits 
a more explicit description, due to Dugger and Spivak \cite{DS11},
...
by providing an identification of $W_!$ applied to an inner horn in Example \ref{WPARTIALT2_EX}
(cf. \cite[Lem. \ref{TAS-WLEFTQPUSH LEM}]{BP_TAS}).
...
\begin{customthm}{A}
        ...
        defined on a tree $U \in \Omega$ as follows.
        The simplicial operad $W(U)$ has colors $\boldsymbol{E}(U)$, and
        is defined on a $\boldsymbol{E}(U)$-corolla 
        $\vect{C} = (C,\mathfrak{c})$
        to have $n$-simplices 
...
\end{customthm}
\begin{proof}
        This follows from Definition \ref{NWTNS DEF}, Remark \ref{NWTNS_REM}, and Proposition \ref{NWTNS_NAT_PROP}.
\end{proof}

\begin{customthm}{B}
        Let $X \in \dSet$.
        For each $n \in \Delta$ and $X(\eta)$-colored corolla $\vect S$,
        the objects in $W(X)_n(\vect S)$ are given, up to unique isomorphism, by tuples
        \[
                (\mathfrak n, S \xrightarrow{\phi} T, \Omega[\mathfrak n] \xrightarrow{x} X, J_\bullet)
        \]
        as in Corollary \ref{NWTNS_NNAT_COR},
        % where
	...
	% \end{enumerate}
        such that
        $J_{\bullet}$ is \textit{flanked} (cf. Definition \ref{FLANKED_DEF}) and
        $x$ is \textit{totally non-degenerate} (cf. Definition \ref{TOTNONDEG_DEF}).
\end{customthm}
\begin{proof}[Proof outline]
        First, we show that Theorem \ref{WU_THM} extends to \textit{necklaces} (Definition \ref{NECKLACE_DEF}) in Corollary \ref{NWTNS_NNAT_COR}.
        %
        Next, in Theorem \ref{KANEXTCHAR THM},
        we show that $W(X)$, as defined in \eqref{WXDEF_EQ}, can be described by
        the colimit
        $W(X) = \colim_{\Omega[\mathfrak n] \to X \in \mathsf{Nec}_{/ X}} W(\mathfrak n)$
        over the category $\mathsf{Nec}_{/ X}$ of necklaces over $X$.
        %
        In Corollary \ref{NWXREPS COR}, we describe the levels $W(X)_n(\vect C)$ of this colimit as equivalence classess of tuples.
        Finally, in Corollary \ref{NWXREPS2_COR} we prove that these classes have chosen representatives, unique up to unique isomorphism.
\end{proof}
...
We will often find it convenient and fruitful to
often work with the nerves of (simplicial) operads.
The that end, in this preliminary section
we first recall the category of (colored) trees in \S \ref{TREES_SEC},
and then in \S \ref{NERVE_SEC} we recall several associated presheaf categories and their connections to (simplicial) operads under the nerve functor.


\subsection{The category of (colored) trees}
\label{TREES_SEC}

\todo[inline]{trees in $\Omega$ will be denoted $T$. This disagrees with TAS, where $T$ is reserved for $G$-trees (except in the appendix)}
...
A map of trees $\phi \colon S \to T$ in $\Omega$ is given by
a map of edge sets $\phi \colon \boldsymbol{E}(S) \to \boldsymbol{E}(T)$ which
preserves ``vertex relations''.
A more rigorous/formal definition using the algebraic notion of \textit{broad posets} is given in \cite[\S \ref{TAS-FORESTS_SEC}]{BP_TAS}.
\todo[inline]{as this paper is taking the propositions \ref{NP_TREEFACT_PROP}, \ref{TREEFACT_PROP} as givens,
  I would be okay not reviewing broad posets.}
% While the tree diagram description above is a helpful heuristic for describing objects in $\Omega$,
...
% (cf. \cite[\S 3]{MW07}, \cite[Rem. 4.4]{Per18}, \cite[Eq. \eqref{TAS-TAUNER EQ}]{BP_TAS}))
...
\begin{remark}[{cf. \cite[Rem. \ref{TAS-CNVXM REM}]{BP_TAS}}]
        \label{CNVXM REM}
	...
	characterized by the following property:
        if an edge $e'$ is pictorally between edges $e$ and $e''$ with are themselves both in the image of $\varphi$,
        then so is $e'$.
	% if $e <_d e' <_d e''$ in $T$ 
	% and $e,e''$ are in the image of $\varphi$
	% then so is $e'$.
	Notably, it follows from the first characterization that convex maps are also closed under composition.
...
\begin{definition}
        ...
        a $\mathfrak C$-tree whose underlying tree is a corolla.
\end{definition}
...
Explicitly, a simplicial operad $\O \in \sOp$ with color set $\mathfrak C$ has levels $\O(\vect C)$ indexed by
$\mathfrak C$-corollas $\vect C = (C, \mathfrak c \colon \boldsymbol{E}(C) \to \mathfrak C)$.
Additionally, $\O$ has composition laws induced by $\mathfrak C$-colored trees.
A full discussion can be found in \textcolor{red}{CITATION}
...
% We now recall from \cite[\S 4]{BP_edss} and \cite[\S 3]{BP_TAS}
% some discussion on simplicial dendroidal sets and preoperads,
% and connect back to the nerve functor in Theorem \ref{SSC_THM}.

\begin{notation}[{cf. \cite[\S 4]{BP_edss}, \cite[\S 3]{BP_TAS}}]
        \label{PREOP_NOT}
        \begin{enumerate}
        \item For $T \in \Omega$, 
                let $\Omega[T]$ denote 
                the representable presheaf $\Omega[T](S) = \Omega(S,T)$.
                Note that $\Omega[T] = N\Omega(T)$ (cf. \cite[Rem. \ref{TAS-NERVESIMDES REM}]{BP_TAS}).
        \item For $T \in \Omega$ and $e \in \boldsymbol{E}^i(T)$,
                let $Sc[T] \subseteq \Lambda^e[T] \subseteq \Omega[T]$ denote
                the \textit{Segal core} of $T$
                and the \textit{inner horn} of $T$ associated to $e$
                (see e.g. \cite[\S 2]{CM13a}, \cite[\S 2.3]{BP_edss}). % \cite[Eq. \eqref{TAS-eq:SC}]{BP_TAS})
        \item For $X \in \sdSet$, we write $X_n(T)$ for the evaluation at $n \in \Delta$ and $T \in \Omega$,
                and refer to $n$ and $T$ as the \textit{simplicial} and \textit{dendroidal} directions, respectively.
        \item \label{XEVAL_LBL} More generally, for $X \in \sdSet$ we write
                \begin{equation}
                        \label{SDSET_EQ}
                ...
                \end{equation}
                for the colimit-preserving functor
                such that
                % $X_{\Delta[n]} = X_n$ and 
                $X\left(G \cdot\Omega[U]\right) = X(U)$ for
                % $n \geq 0$,
                $U \in \Omega$.
        ...
        %         \]
        \item % (cf. \cite[Eq. \eqref{TAS-COLORSET EQ}]{BP_TAS})
                Given $X \in \PreOp$, we call $X(\eta)$ the \textit{color set} of $X$ .
        \item \label{XUC_LBL}
                % (cf. \cite[Not. \ref{TAS-XUDECALT NOT}]{BP_TAS})
                ...
                \begin{equation}
                        ...
                        \end{tikzcd}
                \end{equation}
        \item Given any $A \in \dSet$, we abusively write $A \in \PreOp \subseteq \sdSet$ for the
                simplicially-constant presheaf $A_n(T) = A(T)$.
        \end{enumerate}
\end{notation}
...
\begin{remark}
        ...
        (i) iff (ii) follows by either \cite[Prop. 2.5 and Cor. 2.6]{CM13a} or \cite[Props. 3.22, 3.31]{BP_edss};
        (ii) iff (iii) follows by \eqref{SDSET_EQ}; % Notation \ref{PREOP_NOT}\ref{XEVAL_LBL};
        (iii) iff (iv) follows by \eqref{XUDECALT EQ} and \eqref{COLDEC_EQ}. 
\end{remark}
...
\begin{proof}
        ...
        $N \colon \Op \to \dSet$
        is fully faithful with (essential) image
        those $A \in \dSet$ satisfying the strict Segal condition Definition \ref{SSC_DEF}(i).
        Thus the essentialy image of $N^{\Delta^{op}} \colon \Op^{\Delta^{op}} \to \sdSet$ is precisely
        those $X \in \sdSet$ satisfying the strict Segal condition;
        restricting the source to $\sOp \hookrightarrow \Op^{\Delta^{op}}$ is equivalent to restricting the target to $\PreOp \hookrightarrow \sdSet$.
\end{proof}
