...
\end{scope}
\end{scope}
\begin{scope}[yshift=-0.5em,xshift=2.5em]
...
\begin{notation}
	...
	for the corolla $C_n$,
	which comes together with a unique tall planar map
	$\mathsf{lr}(T) \to T$.
\end{notation}
...
\begin{example}
	...
	$T_v\to S$, 
	as shown by \cite[Prop. 5.11]{Per18}.
\end{example}
...
\begin{remark}
	The strict Segal condition \eqref{SCV1 EQ} admits yet other formulations. For example, one may replace the Segal cores 
	$Sc[T]$ in \eqref{SCV1 EQ} with the inner horns $\Lambda^E[T]$,
	cf. \cite[Props. 3.22, 3.31]{BP_edss},
	resulting in a description 
	closely related to the definition of $\infty$-operad
	\cite[\S 5]{MW09}.	
\end{remark}
...
\end{scope}
	...
	\end{tikzpicture}
	\]
	\caption{Outer face of a dendroidal necklace}
	\label{FIGURE2}
\end{figure}
