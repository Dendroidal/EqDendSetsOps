...
In \S \ref{GENCOF SEC} we will use the following,
...
fibered Yoneda embedding in Notation \eqref{FIBYON NOT}
and of Proposition \ref{FIBYONPUSH PROP}.
...
on $\mathsf{Sym}_{\bullet}(\V)$,
the abstract nonsense argument in 
...
\begin{proof}
Recalling the $C_{\varphi}(-,-)$ notation 
\cite[Not. \ref{OC-MAPSDEC NOT}]{BP_FCOP}
...
and (the dual of) 
\cite[Rem. \ref{OC-CARTCHAR REM}]{BP_FCOP};
\item
the second step uses \cite[\eqref{OC-ADJOVADJ EQ}]{BP_FCOP}
...
note that \cite[\eqref{OC-FREEOP_EQ}]{BP_FCOP}
can be regarded as an arrow in 
the category $\mathsf{Cat} \downarrow^l \V$
from \cite[Rem. \ref{OC-SUBCATDOWNL REM}]{BP_FCOP}
which, being a left Kan extension, is cocartesian over $\mathsf{Cat}$ (cf. \cite[Rem. \ref{OC-SUBCOCART REM}]{BP_FCOP}).
Hence, if $X \in \mathsf{Sym}_{\bullet}^G(\V)$
is $G$-equivariant, 
\cite[\eqref{OC-FREEOP_EQ}]{BP_FCOP} is then a cocartesian arrow in 
$\left(\mathsf{Cat} \downarrow^l \V\right)^G$
over $\mathsf{Cat}^G$.
By Proposition \ref{EQUIVFNCON PROP},
we can hence rewrite 
such a $G$-equivariant \cite[\eqref{OC-FREEOP_EQ}]{BP_FCOP}
...
By \cite[Rem. \ref{OC-CONVER REM}]{BP_FCOP} we now have the following analogue of
\eqref{FROPEXP EQ}.
...
\begin{remark}\label{OP_MAP REM}
      Following \cite[Rem. \ref{OC-ALGPUSHLL REM}]{BP_FCOP},
...
induce Quillen adjunctions, cf. 
\cite[Cor. \ref{OC-SYMADJ_COR}(i)]{BP_FCOP}.
...
\section{Preliminaries for ALL COLORS paper}
...
\subsection{model structures bla}
...
The case $\F_{all}$ of the family of all subgroups
minimizes the classes of weak equivalences and fibrations
and thus, conversely, 
it maximizes the classes of cofibrations and trivial cofibrations.
\end{remark}


\subsection{On model structures on fibers}


Transferring the $\F$-model structures on
$\mathsf{Sym}^G_{\mathfrak{C}}(\V)$
...
\begin{remark}[{\cite[Rem \ref{OC-FVGSIGF REM}]{BP_FCOP}}]
