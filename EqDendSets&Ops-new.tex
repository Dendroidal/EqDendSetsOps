...
Moreover, we say $\langle x \rangle$ is (non-)degenerate if
the dendrex $x$ is (non-)degenerate.
\end{notation}
...
To do so, we will first find it useful to discuss a number of special types of principal presheaves of $N \mathcal{P}$, suggested by \eqref{PUSHOPPR EQ}.

\begin{notation}
For a dendrex $p\colon \Omega[T] \to N \mathcal{P}$
we say the principal subpresheaf 
$\langle p \rangle \subseteq N \mathcal{P}$ is
\begin{itemize}
\item \emph{reduced} if $T \in \Sigma_{\mathfrak{C}}$, i.e. $T$ is a $\mathfrak{C}$-corolla;
...
\item \emph{canonical} if it is non-degenerate elementary and for any proper inner face
...
\begin{notation}
Any tree $T$ has an associated corolla $\mathsf{lr}(T)$, so that we abbreviate
$\partial_r p = \partial_{\mathsf{lr}(T)}p$
and say
$\langle \partial_r p \rangle$
is the \emph{reduction} of  
$\langle p \rangle$.
\end{notation}
...
there exists a \emph{unique} alternating principal subpresheaf
$\langle p^a \rangle$
such that
$\langle \partial_r p^a \rangle = \langle p \rangle$.

More generally, if $\langle p \rangle$
is not reduced, we still write
...
is elementary canonical iff
$\langle p \rangle$ is non-degenerate and
$\langle p^a \rangle$
is a degeneracy of $\langle p \rangle$.
\end{lemma}


\begin{proof}
Consider first the ``only if'' direction. 
$p$ is non-degenerate by definition.
...
For the degeneracy claim,
consider the degeneracy followed by inner face factorization
$T^a \to T' \to T$. 
Then $T'$ inherits a $\{O,B\}$-labeling from $T^a$ and both maps in the factorization are tall planar $B$-inert label maps, 
so that $\langle \partial_{T'} p \rangle$ is again elementary,
showing that 
$\langle p \rangle$
can only be canonical if $T^a \to T$ is a degeneracy,
i.e. if $\langle p^a \rangle$
is a degeneracy of $\langle p\rangle$.

For the ``if'' direction, we consider two cases.
If $\langle p \rangle$
