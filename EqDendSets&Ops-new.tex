...
\begin{example}
      ...
      \item For higher arity operations,
            ...
            Morevover, if $\V = \sSet$ and $C = C_A \in \Sigma_G$ is the $G$-corolla (unique up to isomorphism) corresponding to the graph subgroup $\Gamma$, then
      ...
      \item Similarly, $\Omega(T) \in \sOp^G$ is cofibrant for all $T \in \Omega_G$, as we have a pushout in $\sOp^G$
            \[
                  \begin{tikzcd}
                        \displaystyle{
...
\begin{definition}
      ...
      for any $\mathbf E(U)$-colored sequence $\vect C$, 
      \begin{equation}
            \Omega(U)(\vect C) =
            \begin{cases}
...
\begin{proof}
      ...
      \label{SDEQUIV_LBL} is by construction.
      \label{JTFIB_LBL} is \cite[Prop. 4.1(ii)]{BP_edss}.
      \label{JTCFIB_MAP_LBL} follows from \cite[Lemmas A.27, A.29]{BP_edss}.
      \label{SFIB_JEQ_LBL} follows from \cite[Prop. 4.5(iii), Cor. 4.29(iii)]{BP_edss}.      
\end{proof}
...
\begin{proof}
	...
	in which case it is well known \todo[inline]{well known?}
        \todo{need that $\tau$ is left Quillen (proved below), and cofibrations out of $\Sigma$-cofibrant are $\Sigma$-cofibrations (clear)}
        that
...
showing that \eqref{UNITEQUIV EQ} is in fact a dendroidal equivalence, and thus also a complete equivalence.
	...
	We now turn to the case of $A \to B$ a general cofibration between cofibrant objects \todo{not the hypothesis in the statement}.
...
\begin{proof}
      ...
      \label{JF_VERT_LBL} is \cite[Prop. 4.24(ii)]{BP_edss}.

      \label{DIAG_LBL} is \cite[Prop. 4.5(iv)]{BP_edss}.
\end{proof}
