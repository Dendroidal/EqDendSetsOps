...
\begin{definition}
Given a family of subgroups $\mathcal{F}$ in $G \times \Sigma$,
one defines a family
$\mathcal{F}_{\mathfrak{C}}$
for any $\mathfrak{C} \in \mathsf{F}^G$
via $\mathcal{F}_{\mathfrak{C}} = \pi^{\**}_{\mathfrak{C}}(\mathcal{F})$
for $\pi_{\mathfrak{C}} \colon G \ltimes \Sigma_{\mathfrak{C}} \to G \times \Sigma$
the canonical map.
\end{definition}
...
\begin{remark}\label{LEVEL_COF_REM}
	An immediate consequence of $\V$ having cellular fixed points (cf. the proof of \cite[Prop. 6.3(i)]{BP_geo})
