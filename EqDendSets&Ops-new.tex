...
one has a pointwise simplicial tensoring
...
\begin{example}
      ...
      \item Finally, we observe that for $\Gamma$ a graph subgroup,
            ...
            $\mathbb F_\Gamma(\Gamma/\Gamma \cdot A) = \Omega(C) \cdot A$,
            where $C = C_\Gamma$ is the $G$-corolla corresponding to $\Gamma$, and
            \[
                  \Op(\Set)^G \times \V \xrightarrow{(-) \cdot (-)} \longto \Op^G(\V),
                  \qquad
                  (\O \cdot A)(\vect C) = \O(\vect C) \cdot A.
            \]
      \end{enumerate}
\end{example}
...
\begin{remark}
      ...
      \begin{gather*}
            ...
            \mathbb F_{\Gamma_C} \Big( \Gamma_C / \Gamma_C \cdot (\partial \Delta[n] \to \Delta[n]) \Big) = \Omega(C) \times \big(\partial \Delta[n] \to \Delta[n]\Big)
      \end{gather*}
      where $\Gamma_C$ is the graph subgroup of $G \times \Sigma_n$ such that $C \simeq G \cdot C_n / \Gamma$,
      and $\mathbb F_\Gamma: \sSet^\Gamma \to \sOp^G$ is as in the discussion around \eqref{FGAMMA_EQ}.
