...
so that by Lemma \ref{FORSAKEN LEM} there is an inner face $T^{\bar{c}} \to T^{\bar{e}}$
such that $\bar{c} = \partial_{T^{\bar{c}}} \bar{e}$ is canonical.
...
\\
	...
	\Omega[T^{\bar{e}}] \ar{u}[swap]{\bar{e}} \ar{r} &
	\Omega[T^e] \ar{ul}[swap]{e}
\end{tikzcd}
\]
...
\begin{lemma}
If $e$ is elementary non-degenerate and $E \subseteq \Xi^p$,
then 
$\langle e\rangle$
...
\begin{proof}
Write $T' = T-E$ and $e' = \partial_{T'} e$.
For each vertex 
$T'_v \hookrightarrow T'_v$
we apply the 
``inner face followed by outer face'' factorization to the composite
$T'_v \to T' \to T$
to obtain
$T'_v \to T_v \to T$.
Then $e_v = \partial_{T_v} e$ is still non-degenerate elementary
(since it is an outer face of a non-degenerate elementary)
and all of its inner edges are in $E$, and are thus adjacent to a 
$B$-labeled vertex, 
so that $e_v$ is again canonical (by the observation after Remark \ref{1BADJ REM}).
But thus
$\langle e_v \rangle = 
\langle a^{e'_v} \rangle$
so it follows that
$\langle e_v \rangle = 
\bigcup_{v \in \boldsymbol{V}(T)} 
\langle a^{e'_v}\rangle$,
...
%
%
%let $T_v$ be the corresponding outer face.
...
Write $T' = T=E'$, $T'' = T = E''$.
The assumption
$\langle \partial_{T'} e \rangle
=
\langle \partial_{T''} e \rangle$
says that there exists a solid isomorphism as in the left diagram below.
There is then a bijection
$\boldsymbol{V}(T') \simeq \boldsymbol{V}(T'')$
and we write
$T'_{v'_i} \hookrightarrow T'$,
$T''_{v''_i} \hookrightarrow T''$
to indicate that $v'_i,v''_i$
are identified by this bijection.
Applying the ``inner face followed by outer face factorization''
to the composites
$T'_{v'_i} \hookrightarrow T' \to T$,
$T''_{v''_i} \hookrightarrow T'' \to T$
to get
$T'_{v'_i} \hookrightarrow T_{v'_i} \to T$,
$T''_{v''_i} \hookrightarrow T_{v''_i} \to T$.
Writing
$e_{v_i'} = \partial_{T_{v_i'}}e$,
$e_{v_i''} = \partial_{T_{v_i''}}e$,
one then has the right diagrams
\[
\begin{tikzcd}
	\Omega[T'] \ar{d}[swap]{\simeq} \ar{r} &
	\Omega[T] \ar{r}{e} \ar[dashed]{d}[swap]{\simeq} &
	N \mathcal{P}
&
	\Omega[T'_{v'_i}] \ar{d}[swap]{\simeq} \ar{r} &
	\Omega[T_{v'_i}] \ar{r}{e_{v'_i}} \ar[dashed]{d}[swap]{\simeq} &
	N \mathcal{P}
\\
	\Omega[T''] \ar{r} &
...
&
	\Omega[T''_{v''_i}] \ar{r} &
	\Omega[T_{v''_i}] \ar{ru}[swap]{e_{v''_i}} &
\end{tikzcd}
\]
in which the dashed isomorphisms
exist by Lemma \ref{UNIQINAN LEM}.
But by \cite[Prop. 3.41]{BP17} we can thus also find the
dashed isomorphism on the left diagram
(note that the composites 
$T'_{v'_i} \to T''_{v''_i} \to T_{v''_i}$
form a $T'_{v'_i}$-substitution datum).
But since $N \mathcal{P}$ is $\Sigma$-free,
the only possibility is for the left dashed isomorphism to be the identity, showing that $T'=T''$ and $E'=E''$.
\end{proof}
