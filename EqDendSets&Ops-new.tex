...
\begin{lemma}
      ...
      and $f,g: 1_\V \to \O(c;d)$ are homotopic, then for any $\mathfrak C(\O)$-signature $\ksi$ such that either
      \begin{itemize} %{enumerate}[label = (\arabic*)]
      ...
      \end{equation}
      are homotopic,
      where $\ksi_c^d$ is the signature created from $\ksi$ by replacing $c$ with $d$.
\end{lemma}
\begin{proof}
...
\begin{corollary}[{cf. \cite[Lemma 1.23]{GM17}}]
      ...
      If $f \in \O(c;c)$ is homotopic to the identity on $c$, then for any signature $\ksi$ containing $c$, the map
      $f_{\**}: \O(\ksi) \to \O(\ksi)$
