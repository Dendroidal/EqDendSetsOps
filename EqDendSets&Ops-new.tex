...
is indeed $\Lambda$-equivariant, showing that indeed
$\P(\vect{C})^{\Lambda} \simeq \P(\vect{B})^{\Lambda}$,
as follows from the next remark, which discusses the general case.
\end{example}



\begin{remark}
Consider $\mathfrak{C}-$signatures
$\vect{B}=(\mathfrak{b}_1,\cdots,\mathfrak{b}_n;\mathfrak{c}_0)$
and 
$\vect{C}=(\mathfrak{c}_1,\cdots,\mathfrak{c}_n;\mathfrak{c}_0)$, as depicted below.
We note that, to simplify notation, we assume these have a common target $\mathfrak{b}_0=\mathfrak{c}_0$.
...
\end{tikzpicture}
\]
Now suppose a $(G,\Sigma)$-family $\F$ was chosen and $\Lambda \in \F_n$.
Following Remark \ref{SIGACT REM},
one has that $\Lambda$ is in $\F_{\vect{B}}$ and $\F_{\vect{C}}$
iff
$g \mathfrak{b}_{\sigma(i)} = \mathfrak{b}_i$
and  
$g \mathfrak{c}_{\sigma(i)} = \mathfrak{c}_i$
for all $g\sigma \in \Lambda, 0\leq i \leq n$.

Now let $\mathcal{P} \in \mathsf{Op}^G_{\mathfrak{C}}(\V)$
be an operad.
Given maps $\alpha_i \colon 1_{\mathcal{V}} \to \P(\mathfrak{b}_i;\mathfrak{c}_i)$
from $\mathfrak{b}_i$ to $\mathfrak{c}_i$ for $1\leq i \leq n$
it is then natural to expect precomposition with the $\alpha_i$
to yield a 
$\Lambda$-equivariant map
$\P(\vect{C}) \to \P(\vect{B})$
provided they satisfy the same
$\Lambda$-equivariance condition as the $\mathfrak{b}_i$, $\mathfrak{c}_i$,
i.e. provided that
$g \alpha_{\sigma(i)} = \alpha_i$ for all $g\sigma \in \Lambda, 1\leq i\leq n$.
