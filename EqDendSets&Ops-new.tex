...
\[
      ...
      \to
      \V^{\Aut((\vect U_V)_{ac})}_{gen} \times \V^{\Aut( (\vect U_v)_{in} )}_{gen} \to
...
\begin{remark}
      ...
      \item[$(\bigstar)$] $(J^G)^{\otimes} \subseteq W^G$.
            
            Explicitly, relative $K^G$-cell complexes are genuine weak equivalences in $\V^G$, where
            \[
                  K^G = \sets{X \times G/H \cdot j}{X \in \V^G,\ j \in J}.
            \]
      \end{enumerate}
...
{\color{blue}
\todo[inline]{come back! what weaker assumptions give $\bigstar$?}
To prove $\bigstar$, need a lot:
\begin{enumerate}
\item (i) and (ii) from above
\item strongly cofibrantly generated
\item non-equivariant monoid axiom
\item $(I^G)^{\otimes} = I^G-cof$, which along with monoid axiom imply $J^\otimes = J-cof$ (so we may apply \cite[Prop. 6.3(ii)]{BP_geo})
\item $(X \times G/G \cdot j)^H$-cell $\subseteq W$.
\end{enumerate}
These don't seem ``weaker''...
}
...
\begin{proof}
Since $\mathbb{I},\mathbb{J}$ are cofibrant and $\partial_{i,!}$
...
In particular, we may thus assume there are weak equivalences
$\mathbb{I} \xrightarrow{\sim} \widetilde{\mathbbm{1}}$,
$\mathbb{J} \xrightarrow{\sim} \widetilde{\mathbbm{1}}$.
...
with $\bar{\mathbb F} = \bar{\mathbb F_{\mathfrak C}$ and $\mathbb P = \mathbb P_{\mathfrak C}$ the associated left adjoints.
...
\begin{lemma}
      $ho(X)$ detects isofibrations.
\end{lemma}
\begin{proof}
      \[
            \begin{tikzcd}
                  \sOp^G \arrow[d, "h c N"'] \arrow[r, "\upsilon_{\**}"]
                  &
                  \sOp_G \arrow[r, "\pi_0"]
                  &
                  \Op_G \arrow[d, "N"] \arrow[r, "j^{\**}"] \arrow[dll, Leftrightarrow, shorten >=0.15cm,shorten <=0.15cm]
                  &
                  \Cat^{\mathsf O_G^{op}} \arrow[d, "N"] \arrow[r, equal]
                  &
                  \Cat^{\mathsf O_G^{op}} \arrow[d, equal]
                  \\
                  \Op^G_{\infty} \arrow[rr, "ho"']
                  &&
                  \dSet_G \arrow[r, "j^{\**}"]
                  &
                  \sSet^{\mathsf O_G^{op}} \arrow[r, "\tau"]
                  &
                  \Cat^{\mathsf O_G^{op}}
            \end{tikzcd}
      \]
      Since $ho(X)$ is strict Segal, so is $j^{\**}ho(X)$, and thus $N\tau j^{\**}ho(X) = j^{\**}ho(X)$.
      Moreover, $j^{\**}\upsilon X \to j^{\**}ho(X)$ remains universal.
      Thus the universal property of the unit map implies that $j^{\**}ho(X) \simeq N \tau j^{\**} \upsilon_{\**} X$.
      Finally, $f \colon X \to Y$ in $\dSet^G$ is an isofibration iff $\tau j^{\**}(f^H)$ is,
      i.e. if $\tau j^{\**} \upsilon_{\**} f$ is a level isofibration.
      The result follows.
\end{proof}

\begin{remark}
      \label{W!_LEFTQ_REM}
      This gives us a second proof of Proposition \ref{W!_LEFTQ_PROP}:
      $hcN(f)$ is an $\F$-fibration iff $\tau j^{\**} \upsilon_{\**} h c N(f)$ is,
      but
      \[
            \tau j^{\**} \upsilon_{\**} h c N(f) \simeq
            \tau N \tau j^{\**} \upsilon_{\**} \simeq
            \tau j^{\**} ho(hcN(f)) \simeq
            \tau j^{\**} N \pi_0 \upsilon_{\**} f,
      \]
      and $f \in \sOp^G$ is an isofibration iff
      $\tau j^{\**} N \pi_0 \upsilon_{\**} f$ is a level isofibration.
\end{remark}
