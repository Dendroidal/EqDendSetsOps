...
\section{Operadic model structures on $\mathsf{Op}^G(\mathcal{V})$}
\label{MS_SEC}
...
\begin{definition}
Write $G \times \Sigma \xrightarrow{\pi} 
G \simeq G \times \Sigma_1 \xrightarrow{\iota} G \times \Sigma$
for the natural projection and inclusion.

We say that a family $\F$ of subgroups of $G\times \Sigma$
...
\begin{notation}
We write $\mathbbm{1}$ (resp. $\mathbb{J}$)
...
(resp. $\mathbbm{1}(i,j)= 1_{\V}$ for all $i,j$)
...
\begin{definition}
	...
	which is equivalent to $\mathbb{J}.$
\end{definition}



\begin{remark}
	We note that, since $\mathbb{J}$ is typically not fibrant,
	an arbitrary interval $\mathbb{I}$
	needs not admit a map to $\mathbb{J}$,
	but only a map $\mathbb{I} \to \mathbb{J}_f$,
	where $\mathbb{J}_f$ denotes some fixed chosen fibrant replacement.
\end{remark}
...
\item \textit{path-lifting}
	...
	$\{0\} \to \I$, $\{1\} \to \I$
	where $\I$ is a $\V$-interval; {\color{red} HERE}
