...
\begin{definition}
      ...
      \begin{equation}
            \Omega(T)(C) =
            \begin{cases}
                  \** \qquad \qquad \qquad & \mbox{There is an injective map of trees $C_n \xrightarrow{\phi} T$ such that $\partial \phi = C$}
                  \\
                  \varnothing & \text{else.}
            \end{cases}
      \end{equation}
...
\begin{definition}
      Given $T \in \Omega$, define $W(T)_\bullet \in (\Op^{\mathbf E(T)})^{\Delta^{op}}$ to be the bar construction
      \[
            W(T)_n = \left( \mathbb F^{\mathfrak C}_+ \right)^{n+1} \Omega(T),
      \]
      with the simplicial maps the monadic unit and monadic multiplication.
\end{definition}

To unpack this $\mathbf E(T)$-colored operad, we first consider $\mathbb F^{\mathfrak C} \Omega(T)$ and $\mathbb F^{\mathfrak C}_+ \Omega(T)$.
We observe that $\mathbb F^{\mathfrak C} \Omega(T)(C) = \varnothing$ iff $\Omega(T)(C) = \varnothing$.
...
Unpacking, we see that $W(T)$ has the same objects $\mathbf E(T)$ as $\Omega(T)$, and 
\begin{equation}
