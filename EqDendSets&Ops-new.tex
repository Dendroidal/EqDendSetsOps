...
The following are the key locations where the many-colors require additional care:
\begin{itemize}
\item In Definition \ref{CLPS_DEF}, we only need to color the last tree in the string; this induces a unique choice of coloring such that all maps are colored.
\item Lemma \ref{GWRA_LEM} allows us to recognize wreath products of $G$-action groupoids as a $G$-action groupoid.
\item Need: $\pi^{\**}$ and $|-|$ commutes (Proposition \ref{PIREAL_PROP})
\item Need: $|-|$ commutes with $G \ltimes (-)$ (Proposition \ref{GWRREAL_PROP})
\end{itemize}
...
\begin{proof}
      ...
      The description of the leftmost maps in \eqref{FILT_PUSHG_EQ} and \eqref{FILT_PUSH_EQ} follow by
