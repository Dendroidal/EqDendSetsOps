...
is equivalent to the representability
of the functors 
...
But hence since any $X \in \Alg_T$
with multiplication $m \colon TX \to X$
is canonically described by a coequalizer of free $T$-algebras
$coeq \left( T T X 
\overset{\mu}{\underset{Fm}{\rightrightarrows}}
T X\right)$
it follows that a left adjoint to
$\psi^{\**} \colon \Alg_F \to \Alg_T$
...
F X\right)
$
of $F$-algebras exist for all $X \in \Alg_T$.
\end{remark}
...
\begin{remark}
Proposition \ref{PARTADJP PROP} 
admit a complementary result by interchanging the roles of $P$ and $\bar{F}$ and the roles of Kleisli categories and algebra categories.
