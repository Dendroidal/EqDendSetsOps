...
% \begin{example}
%       \cite[Theorem 3.1]{GW18} shows that $(\Top, \times)$ admits transfers for $G$-operads when $\mathfrak C = \set{*}$.
% \end{example}
...
\begin{proof}
      ...
      following \cite[Theorem 2.1.19]{Hov}, or \cite[Theorem 2.2.2]{WY18} in the semi-model structure case, that:
      \begin{enumerate}[label = (\arabic*)]
...
\begin{lemma}
      ...
      with the second $\otimes_\Fin$ the (fiberwise) simplicial tensoring of $\sOp^G$ defined as in Example \ref{COTENS_EX}.
\end{lemma}
\begin{proof}
...
$\langle \partial_{U^e_v} e\rangle \subseteq \zeta_! B$.
...
the uniqueness up to isomorphism from Remark \ref{PUSHOPPRRST REM}
of the alternating dendrix with reduction $\langle \partial_r a \rangle$.
...
with the middle condition always false, 
hence (Ch2),(Ch3) follow.
...
$e^{-1}(X_{<\langle e\rangle})$
follows directly from $N \mathcal{P}$ being $\Sigma$-free,
