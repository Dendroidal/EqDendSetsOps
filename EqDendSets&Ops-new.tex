...
explicitly, $\mathsf{sk}_{\eta}X(U) = X(\eta)$ if $U \in \Delta$ is a linear tree, and $\varnothing$ otherwise.
\end{remark}
...
\begin{remark}
      ...
      \end{gather*}
      where $\Gamma_C$ is the graph subgroup of $G \times \Sigma_n$ such that $C \simeq G \odot C_n / \Gamma$,
      $\mathbb F_\Gamma: \sSet^\Gamma \to \sOp^G$ is as in the discussion around \eqref{FGAMMA_EQ},
      and $(-) \odot (-)$ indicates the fibered tensoring from Example \ref{COTENS_EX}.
