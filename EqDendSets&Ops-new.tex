...
\begin{lemma}
      ...
      \item \label{SFIB_JEQ_LBL} $X,Y \in \mathsf{sdSet}$ simplicially fibrant, then $X \to Y$ is a joint equivalence iff it is a simplicial equivalence (\cite[Prop. 4.5(iii), Cor. 4.29(iii)]{BP_edss}.
      \item \label{JF_VERT_LBL} $X \in \mathsf{ssSet}$ is joint fibrant iff $X$ is horizontally fibrant and all vertex maps $X(m) \to X(0)$ are weak equivalences in $\mathsf{sSet}$ (\cite[Prop. 4.24(ii)]{BP_edss}).
      \item \label{DIAG_LBL} If $X \in \mathsf{ssSet}$ is horizontal Reedy firant, then $X_0 \to \delta^{\**}X$ is a Kan equivalence in $\mathsf{sSet}$ (\cite[Prop. 4.5(iv)]{BP_edss}).
      \end{enumerate}
\end{lemma}
...
\begin{remark}
      ...
      and hence $W(T)$ is $\mathcal F$-cofibrant in $\sOp^G$ for all $T \in \Omega_{\mathcal F}$.
      \todo[inline]{do we need to worry about the case when $W(T)$ is \textit{not} cofibrant? Then we won't get the simplicial equivalences we want below}
\end{remark}
...
\begin{proof}
	...
	We claim that the following is a zigzag of weak equivalences in $\mathsf{sdSet}^G$.
        ...
        \todo[inline]{I don't see how this follows. I can only show that $X(m) \to X(0)$ is a joint fibration between joint fibrant objects.}
        % 
        ...
        %
        But since $\widetilde{Q}(T) \in \mathsf{sSet}^{\Delta^{op}}$ is itself Reedy fibrant by
        ...
        \[\widetilde{Q}_0(T) = 
              \mathsf{Hom}_{\mathsf{sdSet}}(c_!\Omega[T],\widetilde{Q})=
              \mathsf{Hom}_{\mathsf{PreOp}}(c_!\Omega[T],\gamma_{\**}\widetilde{Q})\]
        \[
        ...
        and that $N\Omega(T) = c_!\Omega[T]$ is tame cofibrant \todo{citation}.
        ...
        $W(T) \to \Omega(T)$ is a weak equivalence of cofibrant operads.
...
\todo[inline]{what changes when we add $\mathcal F$'s?
  (1) $W(T)$ nor $\Omega(T)$ need to be $\mathcal F$-cofibrant,
(2) }
