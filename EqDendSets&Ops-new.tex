...
By the filtration \eqref{OUFILRE EQ}, it thus suffices to check that
...
From the pushouts \eqref{OUPUSHRE EQ}
...
Thus, \eqref{NKOXY EQ} is the image of \eqref{NKOXY3 EQ} under the composite of left Quillen functors
% \[
...
Since \eqref{NKOXY3 EQ} are a genuine cofibration and a genuine trivial cofibration (as $k \geq 1$),
...
\begin{remark}
      The conditions we actually need are: cofibrant symmetric pushout powers plus
      \[
            (\bigstar) \mbox{For each $X \in \V^G$ and genuine trivial cofibration $f \in \V^G$, $X \otimes f$ is a genuine weak equivalence in $\V^G$.}
      \]
      If every object in $\V^G$ is genuine cofibrant, then this follows from Propositions \ref{EQQUILADJ PROP} and \ref{RESGEN PROP}.
\end{remark}
