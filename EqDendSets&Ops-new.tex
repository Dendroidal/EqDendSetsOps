...
follows from Remark \ref{COLOR_SQ_REM}(i)
and the assumption that $F \colon \O \to \P$ is a local $\F$-trivial fibration.
...
As for (ii), Remark \ref{COLOR_SQ_REM}(i)
implies that a fiber map in
$\mathsf{Op}_{\mathfrak{C}}^G(\V)$
has the left lifting property against all local $\F$-trivial fibrations
in $\mathsf{Op}^G(\V)$
iff it is a trivial cofibration in 
$\mathsf{Op}_{\mathfrak{C},\F}^G(\V)$, 
which yields the ``if'' direction.
The ``only if'' direction follows from 
Proposition \ref{FTRIVCHAR PROP}, which implies all trivial fibrations in $\mathsf{Op}_{\mathfrak{C},\F}^G(\V)$
...
guarantees that the analogue of Remark \label{FIBERGLMOD REM}
