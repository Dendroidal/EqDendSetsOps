...
\renewcommand{\1}{\ensuremath{\mathbbm 1}} % {\eta}%{\ensuremath{\mathbb{id}}}
...
{\color{blue} \textbf{A Weaker condition:}
  
  \begin{remark}
        In fact, the proof of \cite[Thm. 2.10]{Ste16} follows from the two condition below:
        \begin{enumerate}[label = (\alph*)]
        \item The domains of $\mathcal I_G$ (resp. $\mathcal J_G$) are small rel $\mathcal I_G$-cell (resp. $\mathcal J_G$-cell).
        \item $\left(\mathcal J_G\text{-cell}\right)^H \subseteq \mathcal J\text{-cell}$
        \end{enumerate}

        In some cases, (a) can be checked by hand (and follows from $(i)$ and $(ii)$ of cellularity above), while (b) follows from a weakening of the cellularity conditions above:
        We say $\V$ has \textit{weak acyclic cellular fixed points} if for all finite groups $G$ and subgroups $H$,
        \begin{enumerate}[label = (\roman*)]
        \item $(-)^H \colon \V^G \to \V$ preserves direct colimits of maps in $\mathcal J^G$;
        \item $(-)^H \colon \V^G \to \V$ preserves pushouts of $\mathcal J^G$
        \item $(\mathcal J_G)^H \subseteq \mathcal J$-cell.
        \end{enumerate}
  \end{remark}
...
By the filtration \eqref{OUFILRE EQ} and {\color{blue} reference}
\todo{{\color{blue}level genuine trivial cofibrations are closed under transfinite composites and pushouts, as colimits are computed levelwise}},
...
% Propositions \ref{EQQUILADJ PROP} and \ref{RESGEN PROP}
% now yields that
\eqref{NKOXY EQ} is indeed a genuine trivial cofibration in $\V^{\Aut(\vect U)}$,
...
\begin{remark}
      ...
      \item \textit{global monoid axiom} (\textbf{USGMA}):
            ...
            We say $\V$ satisfies the \textit{global monoid axiom} if $\mathcal J^{\otimes}_G$ is contained in $\mathcal W^G$ for all finite groups $G$.
      \end{enumerate}
\end{remark}
...
{\color{OliveGreen}
  \begin{remark}
        ...
        \]
        Thus the global monoid axiom implies in particular that $\O_{k-1}(\vect C) \to \O_{k}(\vect C)$ from \eqref{OUPUSHRE EQ}
        is a weak equivalence in $\V^{\Aut(\vect C)}$ for all $k \geq 0$ and $\vect C \in G \ltimes \Sigma_{\mathfrak C}^{op}$. 
        % First, we note that
        % \[
...
We record a particular consequence of the proof of Theorem \ref{THM1_C}.
\begin{corollary}
      \label{LGC_COR}
...
\begin{definition}
      Given a set of colors $\mathfrak C$, let $\1_{\mathfrak C}$ denote the initial $\mathfrak C$-colored operad in $\V$,
      ...
      \[
            \Sym_{\mathfrak C, \1}(\V) := \1_{\mathfrak C} \downarrow \Sym^{\mathfrak C}(\V).
      \]
\end{definition}
The free operad adjunction $\mathbb F_{\mathfrak C}$ factors
\begin{equation}
      \label{F_FAC_EQ}
      \begin{tikzcd}
            \Sym_{\mathfrak C}\V) \arrow[r, shift left, "{(-) \amalg \eta_{\mathfrak C}}"]
            &
            \Sym_{\mathfrak C,\1}(\V) \arrow[l, shift left] \arrow[r, shift left, "\mathbb F_{\mathfrak C, \1}"]
            &
            \Op_{\mathfrak C}(\V) \arrow[l, shift left, "U_\1"]
      \end{tikzcd}
\end{equation}
where
$(-) \amalg \1$ is the (levelwise) coproduct in $\Sym_{\mathfrak C}(\V)$,
the map $\1_{\mathfrak C} \to U_\1(\O)$ selects the identity operations for each color,
and $\mathbb F_{\mathfrak C, \1}$ is the ``free operad with prescribed units'' monad,
defined on $X \in \Sym_{\mathfrak C, \1}(\V)$ to be the pushout below computed in $\Op_{\mathfrak C}(\V)$.
\begin{equation}
      \label{FETA_EQ}
      \begin{tikzcd}
            \mathbb F_{\mathfrak C} \1_{\mathfrak C} \arrow[d] \arrow[r]
            &
            \1_{\mathfrak C} \arrow[d]
            \\
            \mathbb F_{\mathfrak C} X \arrow[r]
            &
            \mathbb F_{\mathfrak C, \1} X
      \end{tikzcd}
\end{equation}
...
\begin{definition}
      Given $T \in \Omega$, define $W(T)_\bullet \in \Op(\sSet) \subseteq (\Op_{\mathbf E(T)}(\Set))^{\Delta^{op}}$ to be the bar construction
      \[
            W(T)_n = \left( \mathbb F_{\mathbf E(T), \1} \right)^{n+1} \left(\Omega(T)\right),
      \]
      with the simplicial maps the monadic unit and monadic multiplication.
\end{definition}

For this definition to be valid, we must verify that
\begin{enumerate}[label = (\roman*)]
\item $\mathbb F_{\mathfrak C,\1}$ is the left adjoint to the forgetful functor,
...
We consider a factorization of the free operad adjunction orthogonal to \eqref{F_FAC_EQ}
\begin{equation}
      \label{F_FAC_NU_EQ}
      \begin{tikzcd}
            \Sym_{\mathfrak C} \arrow[r, shift left, "{\bar{\mathbb F}}"]
            &
            \Op_{\mathfrak C,nu} \arrow[l, shift left, "U_{nu}"] \arrow[r, shift left, "\mathbb P"]
            &
            \Op_{\mathfrak C} \arrow[l, shift left, "\hat U_\1"]
      \end{tikzcd}
\end{equation}
through the category $\Op_{\mathfrak C, nu}$ of ($\mathfrak C$-colored) non-unital operads.


\begin{definition}
      A \textit{non-unital operad} is an operad without units or unitality conditions:
      a $\mathfrak C$-colored symmetric sequence for some set $\mathfrak C$
...
$U_{nu} = U_{\mathfrak C, nu}$ and $\hat U_\1 = \hat U_{\mathfrak C, \1}$ from \eqref{F_FAC_NU_EQ} are forgetful functors,
with $\bar{\mathbb F} = \bar{\mathbb F}_{\mathfrak C}$ and $\mathbb P = \mathbb P_{\mathfrak C}$ the associated left adjoints.
The ``adds units'' functor $\mathbb P$ is easy to describe:
on underlying symmetric sequences, it is the (levelwise) coproduct with $\1_{\mathfrak C}$,
and the composition law may be canonically extended so that the new operations act as units.

To describe the functor $\bar{\mathbb F}$, we realize $\Op_{\mathfrak C, nu}$ as algebras over a monad we will also denote $\bar{\mathbb F}$.
This is built analogously to $\mathbb F$ as in Appendix \ref{MONAD_APDX},
...
\begin{proposition}
      The free non-unital operad monad $\bar{\mathbb F}$ on a $\mathfrak C$-colored symmetric sequence $X \in \Sym_{\mathfrak C}$ is given by the left Kan extension
      \[
            \begin{tikzcd}
            ...
            \end{tikzcd}
      \]
      Moreover, $\bar{\mathbb F}^{\circ n}X$ is given by the left Kan extension      
      \begin{equation}
            \label{FREENUOP_EQ}
            \begin{tikzcd}
...
that $\mathbb P$ and $\bar{\mathbb F}$ can be combined as in Definition \ref{AMALGMON DEF}:
The maps $\mathbb P \Rightarrow \mathbb F$ and $\bar{\mathbb F} \Rightarrow \mathbb F$ are simply
\[
      X \amalg \1 \xrightarrow{\eta \amalg e} \mathbb F X,
      \qquad
      \bar{\mathbb F} X \hookrightarrow \1 \amalg \bar{\mathbb F} X
\]
with $\eta$ the adjunction counit and $e$ the inclusion of the units.
Moreover, as $\mathbb P$ and $F = \mathbb F$ both preserve reflexive coequalizers\footnote{
...
the left adjoint $F_p$ to the forgetful functor $\mathsf{fgt} \colon \Op_{\mathfrak C} \to \Sym^{\mathfrak C}_e$,
i.e. the ``free operad with prescribed units'' functor, exists,
and following
Corollary \ref{ALGLEFTEX_COR} and \eqref{FRMONDES EQ} is given on $Y \in \Sym^{\mathfrak C}_e$ as the coequalizer of the following in $\Sym_{\mathfrak C}$.
\begin{equation}
      \label{FPYCOEQ_EQ}
      \mathbb F P Y = \mathbb F(Y \amalg \1_{\mathfrak C}) \overset{f_1}{\underset{f_2}\rightrightarrows} \mathbb F Y,
      % \begin{tikzcd}
      %       \mathbb F P Y = \mathbb F(Y \amalg \1_{\mathfrak C}) 
      %       \arrow[r, shift left, "{\mu \circ F P \eta_{\bar{\mathbb F}}}"] \arrow[r, shift right, "{Fm}"']
      %       &[30pt]
      %       \mathbb F Y,
      % \end{tikzcd}
      \qquad
      ...
      \quad
      f_2 \colon \mathbb F(Y \amalg \1) \xrightarrow{\eta_{\bar{\mathbb F}} \amalg id} \mathbb F(\bar{\mathbb F} Y \amalg \1) = \mathbb F^2 Y \xrightarrow{\mu} \mathbb FY,
\end{equation}

come back!

This matches the above description of $\mathbb F_\1$ from \eqref{FETA_EQ}:
\begin{lemma}
      \label{FP_FETA_LEM}
...
\begin{proof}
      ...
      (as $P \bar{\mathbb F} \bar{\mathbb F} \to \mathbb F Y$, by Proposition \ref{ALTMULT PROP}, and both $\mathbb F \eta \to \eta$ and the unique map $\eta \to \O$ are all maps of $\mathbb F$-algebras).
...
\begin{corollary}
      ...
      \begin{equation}
            \label{WT_EQ}
            W(T)_n = \mathbb F_\eta^{\circ n+1} \Omega(T) = F_P^{\circ n+1} P \bar{\mathbb F} \Sigma_\bullet[T] = P \bar{\mathbb F}^{\circ n+2}\Sigma_\bullet[T].
      \end{equation}
...
\begin{theorem}
      ...
      (recovered by Theorem \ref{MODEL_THM} and \cite[Thm. 2.1]{Per18} when $G = \**$),
...
Using the simplicial tensoring of $\sOp$ from Example \ref{TENS_EX}, we can formalize the main observations used in the proof of \cite[Prop. 4.5]{CM13b}.
...
as noted in \S \ref{REVMS_SEC} and \cite{BP_GSOP, BP_edss, Per18}.

Moving forward, our first task is to show that \eqref{SOPDSETG_EQ} is a Quillen adjunction.
