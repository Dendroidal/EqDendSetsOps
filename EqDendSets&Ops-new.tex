...
Now, recall the notion of a $(G, \Sigma)$-family from Definition \ref{GGRAPHFAM_DEF}.
...
To that end, for any $(G, \Sigma)$-family $\F = \set{\F_n}$ and for all sequences $C \in G \ltimes \SC^{op}$ of length $n$, we define
$\F_C$ to be
\[
      \F_C = \sets{\Gamma \in \F_n}{\Gamma \leq \Aut(C)}
\]
the set of those subgroups which do act on $\O(C)$.
...
\begin{theorem}
      \label{MODEL_THM}
      Fix an $(G, \Sigma)$-family $\F = \set{\F_n}$. % with units
      Let $(\V, \otimes)$ denote either $(\sSet, \times)$ or $(\sSet_{\**}, \wedge)$.
      Then there exists a cofibrantly generated model structure on the category $\Op^G(\V)$,
...
and consider the $G$-set of colors $\mathfrak C_\Gamma := \Gamma \backslash (G \times \Sigma_n) \cdot_\Gamma \langle n \rangle$;
let $C_0$ denote the signature $([[e,e],1],[[e,e],2],\dots,[[e,e],n];[[e,e],0])$.
...
\end{lemma}
\begin{proof}
      We apply arguments from \cite[Prop. 6.24]{BP_geo} and \cite[Lemma 4.8]{Per16}).
      ...
      In the zeroth stage \footnote{
        In the language of \cite[Lemma 4.8]{Per16}, this is the map associated to the subsets
...
\begin{proof}
      ...
      \[
            \begin{tikzcd}
                  1_\V \amalg 1_\V = \left((1_V \amalg 1_\V)^{\otimes n}\right)^{\Sigma_n}
                  \arrow[r, tail]
                  &
            ...
            \end{tikzcd}
      \]
      where the last map is a genuine $\Sigma_n$-weak equivalence by 2-out-of-3 $1_\V \xrightarrow{\simeq} \mathbb C^{\otimes n} \to 1_\V$.
...
the stacking of $\V$-intervals forms another $\V$-interval \todo{pushout trick and Cav4.14?},
...
Combined, these three lemmas show that with our assumptions, equivalences are virtual equivalences are homotopy equivalences.
%
...
\begin{lemma}
      ...
      Let $H$ denote the projection of $\Stab(C) \leq G \times \Sigma_n$ onto $G$, so in particular $H \leq \Stab(c)$.

      Then, for any $d \in \mathfrak C^H$ $H$-homotopy equivalent to $c$
      % to and $H$-homotopy equivalence $\alpha: 1_\V \to \O^H(c,d)_f$ between $c$ and $d$,
      there exists a zig-zag, natural in functors out of $\O$, of genuine $\Aut(C)$-weak equivalences
      % \footnote{
      %   Recall the notation $\mathrm{Gr}_C$ from Definition \ref{GGRAPHFAM_DEF}.
      % }
      in $\V^{\Aut(C)}_{gen}$ %
...
\begin{proof}
      ...
      \]
      Thus, it suffices to check that $\alpha_{\**}$ is a genuine $\Aut(C)$-weak equivalence when $\O$ is bifibrant.
...
\begin{lemma}
      ...
      For each $0 \leq i \leq n$, let $\Stab_i(C) \leq \Stab(C)$ denote the subgroup of elements $(g, \sigma)$ such that $\sigma(i) = i$ (including $\Stab_0(C) = \Stab(C)$),
      and let $H_i \leq G$ denote the $G$-projection of $\Stab_i(C)$.

      Now, suppose $d_i$ and $c_i$ are $H_i$-homotopy equivalent.
      Then there is a zig-zag, natural in functors out of $\O$,
      of genuine $\Aut(C)$-weak equivalences between $\O(C)$ and $\O(D)$, where $D = (d_1,\dots, d_n;c_0)$ is defined as follows:
      Let $\lambda \leq \underline{n} = \set{1,2,\dots,n}$ denote the set of all $1 \leq j \leq n$ such that
      there exists some $(k_j, \sigma_j) \in \Stab(C)$ such that $\sigma_j(i) = j$ and $c_j = k_j c_i$;
      for each $j \in \lambda$, fix choices of such $(k_j, \sigma_j)$.
      Then for all $1 \leq j \leq n$, we define
      \[
            d_j =
            \begin{cases}
...
\begin{proposition}
      ...
      Then for any $(G, \Sigma)$-family $\F$. %with units,
