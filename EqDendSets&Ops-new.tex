...
\begin{remark}
      ...
      \item equivariant monoid axiom:
            \begin{equation}
                  \label{EQMONAX_EQ}
                  G \cdot_H \left( \left(\mathcal J \otimes \V^H\right)\text{-cell} \right) \subseteq \mathcal W^G
            \end{equation}
            for all homomorphisms of finite groups $H \to G$.
      \end{enumerate}
      If every object in $\V^G$ is genuine cofibrant, then $\bigstar$ follows from Propositions \ref{EQQUILADJ PROP} and \ref{RESGEN PROP}.
\end{remark}
...
{\color{OliveGreen}
  \begin{remark}
        ...
        \]
        from which is follows that \eqref{EQMONAX_EQ} is unchanged if we replace $\mathcal J$ with $\mathcal J^H$.

        Moreover, the following are equivalent, using the fact that $G \cdot_H (-)$ is a left adjoint and functoriality of $G \cdot_H (-)$ and $(-) \otimes X$.
        \begin{enumerate}
        \item $G \cdot_H \left( \left(J^H\text{-cof}\right) \otimes \V^H \right)\text{-cell} \subseteq W^G$
        \item $G \cdot_H \left( \left(J^H \otimes \V^H\right)\text{-cof}\right)\text{-cell} \subseteq W^G$
        \item $G \cdot_H \left(J^H \otimes \V^H\right)\text{-cof} \subseteq W^G$
        \item $G \cdot_H \left(J^H \otimes \V^H\right)\text{-cell} \subseteq W^G$
        \item $G \cdot_H \left( \left(\mathcal J^H \otimes \V^H\right)\text{-cell} \right) \subseteq \mathcal W^G$.
        \item $G \cdot_H \left( \left(\mathcal J \otimes \V^H\right)\text{-cell} \right) \subseteq \mathcal W^G$.                  
        \end{enumerate}
  \end{remark}
}
...
{\color{blue} % ------------------------------ COLOR BLUE ------------------------------
  Assume:
  \begin{enumerate}
  \item the genuine model structures exist
  \item fixed point of genuine trivial cofibrations are trivial cofibrations, so in particular
        \begin{enumerate}[label = (\roman*)]
        \item $(-)^H$ sends direct limits of genuine trivial cofibrations to trivial cofibrations
        \item $(-)^H$ preserves pushouts where one leg is a genuine trivial cofibration (suffices to assume for generators)
        \item $(G/K \cdot (-))^H$ sends genuine trivial cofibrations to trivial cofibrations.
        \end{enumerate}
  \end{enumerate}

  What else would we add to get to the equivariant monoid axiom?

  Any of the following would work:
  \begin{enumerate}[label = (\alph*)]
  \item All objects in $\V^G$ are cofibrant.
  \item $J \otimes \V^G \subseteq J^G\text{-cell}$
  \item $(J \otimes \V^G)\text{-cell}$ are genuine weak equivalences between cofibrant objects
  \end{enumerate}
} % ----------------------------------- COLOR BLUE -----------------------------------
...
\begin{corollary}
      \label{F!LQ_COR}
      $\check f_! \colon \Op_{\mathfrak C} \to \Op_{\mathfrak D}$ is left Quillen.
\end{corollary}
\begin{proof}
      Following Remark \ref{OP_MAP REM}, we observe that $\check f_! \mathbb F_{\mathfrak C} = \mathbb F_{\mathfrak D} f_!$
      (as the right adjoints commute).
      Thus, for any $\mathfrak C$-signature $\vect C$ and $K \in \V$,
      \[
            \check f_! \mathbb F \left( \Sigma_{\mathfrak C}[\vect C] \cdot K \right)
            =
            \mathbb F f_! \left( \Sigma_{\mathfrak C}[\vect C] \cdot K \right)
            =
            \mathbb F \left( \Sigma_{\mathfrak D}[\vect C] \cdot K \right).
      \]
      The result follows as $\check f_!$ commutes with transfinite composites and pushouts.
\end{proof}
