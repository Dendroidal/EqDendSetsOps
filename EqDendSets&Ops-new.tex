...
Since any canonical dendrex $p \colon \Omega[T] \to N \mathcal{P}$
...
$p\colon \Omega[T] \to N \mathcal{P}$,
we write
$\Xi^p \subseteq \boldsymbol{E}^{\mathsf{i}}(T)$
for the subset of inner edges of $T$ which are adjacent to at least one $B$-labeled vertex.
Equivalently, $\Xi^p$ consists of the inner edges which are in the image of the map $T^a \to T$.

In particular, by the previous remark one has that
$p$ is canonical iff $\Xi^p = \boldsymbol{E}^{\mathsf{i}}(T)$.
({\color{blue} flesh this out})
