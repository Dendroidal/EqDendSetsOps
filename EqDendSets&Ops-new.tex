...
\begin{theorem}
      \label{MODEL_THM}
      Fix an $(G, \Sigma)$-family $\F = \set{\F_n}$ such that $H \in \F_1$ for all $H \leq G$. % with units
      Let $(\V, \otimes)$ denote either $(\sSet, \times)$ or $(\sSet_{\**}, \wedge)$.
      Then there exists a cofibrantly generated model structure on the category $\Op^G(\V)$,
...
\todo[inline]{connect with Luis's work}
\begin{lemma}
      \label{LOCALISO_LEM}
      ...
      \[
            ...
            \end{tikzcd}
      \]
      Suppose $A \to B$ is a local isomorphism.
      Then so is $\O \to \P$.
\end{lemma}
\begin{proof}
...
where $(1)$ holds if $\V$ is right proper, and $(2)$ holds if $\V$ satisfies ``coherence condition'' \footnote{To be discussed in \S \ref{DK_SEC}.}
\[
...
Similar results will hold equivariantly. For our group $G$, we define the following:
\begin{definition}
      \label{EQUIVG_DEF}
...
\begin{remark}
      ...
      so $k'_j k_j^{-1} \in \Aut_G(c_i), \Aut_G(d_i)$,
...
\begin{proof}
      ...
      We start with $i=1$; the case $i = 0$, $D = (c_1, \dots, c_n; d_0)$ will follow by an analogous (and simpler) argument.
...
\begin{proof}
      ...
      $W_!J$ is cofibrant in $\sCat^{\set{0,1}}$ as we have a factorization $\set{0,1} \to \Delta[1] \to J$,
      and the counit of the Quillen equivalence $W_!\colon \sSet \rightleftarrows \sCat \ \colon \! h c N$
      yields a weak equivalence $W_!J \xrightarrow{\simeq} [\tilde 1] = \mathbb I = \mathbb I_f$ in $\sCat^{\set{0,1}}$.
...
\begin{definition}
      ...
      \begin{equation}
            ...
            \begin{cases}
                  \** \qquad & \mbox{$e_1e_2\dots e_n \leq e$ is a broad relation \footnote{That is, generates an outer face.} in $T$}T
                  \\
                  \varnothing & \text{else.}
            \end{cases}
      \end{equation}
      ...
      \]
      and {\color{red} people} \todo{find this} show that this adjunction is Quillen.
...
\todo[inline]{the above is garbagio}

To begin, we consider the simplest possible extension.
...
While this definition may seem uninspired, the associated homotopy theories are not quite as simplistic.
\todo[inline]{recall the model structures, only so far as are needed below}
...
\begin{itemize}
\item objects given by a choice of set of colors $\mathfrak{C}$
...
\begin{tikzcd}
	...
	\ar{r}{T_{\mathfrak{D}}} \ar[dashed]{d}[swap]{\widehat{f^{\**}_s}} &
	\mathsf{Alg}_{T_{\mathfrak{D}}}
	\ar{d}{f^{\**}_s}
\\
	\mathsf{WSpan}(\Sigma_{\mathfrak{C}},\mathcal{V}^{op})
	\ar{r}{T_{\mathfrak{C}}} &
	\mathsf{Alg}_{T_{\mathfrak{C}}}
\end{tikzcd}
\end{equation}
...
f^{\**} \circ \mathsf{Lan} \circ T_{\mathfrak{D}} \circ \iota \xleftarrow{\simeq}
\mathsf{Lan} \circ f^{\**}_s \circ T_{\mathfrak{D}} \circ \iota \simeq
\mathsf{Lan} \circ T_{\mathfrak{C}} \circ \widehat{f^{\**}_s} \circ \iota
\xleftarrow{\simeq}
\mathsf{Lan} \circ T_{\mathfrak{C}} \circ \iota \circ \mathsf{Lan}
...
$T_{\mathfrak{C}} T_{\mathfrak{C}} \widehat{f^{\**}_s}
\simeq
T_{\mathfrak{C}} f^{\**}_s T_{\mathfrak{D}}
\to
f^{\**}_s T_{\mathfrak{D}}
$
is encoded by the diagram
