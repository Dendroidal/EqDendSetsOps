...
\begin{example}
      \label{COTENS_EX}
      \todo[inline]{This works for any $\V$, not just $\V = \sSet$}
...
\begin{example}
      ...
      \item When $G = \**$ and $n=1$, $\Gamma = \**$ and $\mathfrak C_\Gamma = \set{0,1}$, and moreover
            \[
                  \mathbb F_{\Gamma}[1_\V] = \mathbb A^{op} = \Omega(C_1),
                  \qquad
                  \mathbb F_{\Gamma}[\varnothing] = \set{0,1} \cdot \Omega(\eta).
            \]
            More generally, for $G$ finite, $\Gamma = H \leq G \leq G \times \Sigma_1$, $\mathfrak C_\Gamma = G/H \cdot \set{0,1}$,
            $\mathbb F_{\Gamma}[1_\V] = \Omega(G/H \cdot C_1)$
            has a single orbit of non-trivial hom objects.
            % \[
            %       \mathbb A_H^{op}((gH,1); (gH,0)) = G/H \cdot 1_\V.
            % \]
...
\begin{remark}
      ...
      \begin{gather*}
            ...
            \qquad
            G/H \cdot (\Omega(\eta) \to J)
      \end{gather*}
      for $C \in \Sigma_G$, $0 \leq i \leq n$, $H \leq G$, and $J$ come back!
      \todo[inline]{this is not quite right}
\end{remark}
...
\subsection{Notes on the model structure on $\mathsf{Op}_G$}
