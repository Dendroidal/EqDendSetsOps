...
\begin{lemma}
      ...
      \begin{enumerate}[label = (\roman*)]
      \item The Reedy generating (trivial) cofibrations in $\mathcal M^{\Delta^{op}}$ and $\mathcal M^{G \times \Omega^{op}}$ are
            \[
      ...
      \item If $\mathbb R$ and $\mathbb S$ are generalized Reedy with
      ...
      \item If $X \in (\mathsf{sdSet}^G)^{\Delta^{op}}$ is fibrant in the Reedy-over-joint model structure,
            then each $X(T)$ is fibrant in the Reedy model structure on $(\mathsf{sSet})^{\Delta^{op}}$.
            \begin{proof}
                  ...
                  \]
                  where $(\sSet)^{\mathbb R}$ is given the $\mathbb R$-Reedy model structure over the Kan-Quillen model structure.
                  and the fact that Reedy fibrations are in particular level fibrations.                  
            \end{proof}
      \item $\left(\mathsf{sdSet}_{jt}^G\right)^{\Delta^{op}} = L_{\Omega^{op} \times G}\left(\left(\mathsf{dSet}^G\right)^{\Delta^{op} \times \Delta^{op}}\right)$.
            \begin{proof}
                  come back!
                  \todo[inline]{I don't think this is true. At least, the set I am localizing against on the RHS is \textbf{not} symmetric in the two $\Delta^{op}$.}
                  % This is a corollary of Lemma \ref{LOCALREEDY_LEM}.
            \end{proof}
      \item If $X \in (\mathsf{sdSet}^G)^{\Delta^{op}}$ is Reedy fibrant, then $X(m) \to X(0)$ is a joint trivial fibration in $\mathsf{sdSet}^G$ for all vertex maps.
            \begin{proof}
                  This follows from two instances of the previous Lemma, combined with \cite[Prop. 4.5(i)]{BP_edss}.
            \end{proof}
      ...
      \item Horizontal and vertical equivalences are joint equivalences (by construction)
      \item weak equivalences in $\mathsf{PreOp}^G$ are detected by $\gamma^{\**}$.
      \item $X,Y \in \mathsf{sdSet}$ simplicially fibrant, then $X \to Y$ is a joint equivalence iff it is a simplicial equivalence (\cite[Prop. 4.5(iii), Cor. 4.29(iii)]{BP_edss}.
      \item $X \in \mathsf{ssSet}$ is joint fibrant iff $X$ is horizontally fibrant and all vertex maps $X(m) \to X(0)$ are weak equivalences in $\mathsf{sSet}$ (\cite[Prop. 4.24(iii)]{BP_edss}).
      \item If $X \in \mathsf{ssSet}$ is horizontal Reedy firant, then $X_0 \to \delta^{\**}X$ is a Kan equivalence in $\mathsf{sSet}$ (\cite[Prop. 4.5(iv)]{BP_edss}).
      \end{enumerate}
\end{lemma}

% \begin{lemma}
%       \label{LOCALREEDY_LEM}
%       $(L_S\mathcal M)^{\mathbb R} = L_{S \square R}(\mathcal M^{\mathbb R})$.
% \end{lemma}
% come back!
...
\begin{proposition}\label{COMUOTOHOM PROP}
      ...
      \[
            \begin{tikzcd}
                  \mathsf{PreOp}^G_{\mathcal F} \ar{d}[swap]{\gamma^{\**}}&
                  \mathsf{sOp}^G_{\mathcal F} \ar{l}[swap]{N} \ar{d}{hcN}
                  \\
                  \mathsf{sdSet}^G_{\mathcal F} &
                  \mathsf{dSet}^G_{\mathcal F} \ar{l}{c_{!}}
            \end{tikzcd}
      \]
\end{proposition}
...
\begin{proof}
	...
	We claim that the following is a zigzag of weak equivalences in $\mathsf{sdSet}^G$.
        ...
        For the second and third maps, note first that
        $\widetilde{Q}$ is homotopically constant, in the sense that
        all vertex maps $\widetilde{Q}(m) \to \widetilde{Q}(0)$ are joint equivalences in $\mathsf{sdSet}$,
        by combining Lemma \ref{COMUOTOHOM_FACTS}(iv),(v).
        \todo[inline]{I don't see how this works. I can only show that $X(m) \to X(0)$ is a joint fibration between joint fibrant objects.}
        % 
        Moreover, since the levels $\widetilde{Q}$ are joint fibrant in $\mathsf{sdSet}$,
        \cite[Prop. 4.5(iii)]{BP_edss} implies that these are in fact simplicial equivalences,
        i.e. that for each tree $T \in \Omega$ the evaluations
        $\widetilde{Q}(T)(m) \to \widetilde{Q}(T)(0)$
        are Kan equivalences in $\mathsf{sSet}$.
        %
        But since $\widetilde{Q}(T) \in \mathsf{sSet}^{\Delta^{op}}$ is itself Reedy fibrant by Lemma \ref{COMUOTOHOM_FACTS}(iii),
        (the proof of) \cite[Prop. 4.24(ii)]{BP_edss} implies that it is in fact joint Reedy fibrant in $\mathsf{sSet}^{\Delta^{op}}$,
        and hence by \cite[Prop. 4.5(iv)]{BP_edss}, one has Kan equivalences 
        $\widetilde{Q}(T)(0) \xrightarrow{\sim}
        \delta^{\**} \widetilde{Q}(T) \xleftarrow{\sim}
        \widetilde{Q}_0(T)$,
        showing that the second and third maps in \eqref{BIGZIG EQ} are indeed simplicial equivalences, and hence joint weak equivalences.
        ...
        is an equivalence (as $\gamma^{\**}\gamma_{\**}\tilde Q \to \tilde Q$ is a joint equivalence)
        between Reedy fibrant objects in $(\mathsf{PreOp}^G_{tame})^{\Delta^{op}}$,
        and that $\Omega(T)$ is tame cofibrant. \todo{citation}
...
Writing $\amalg_{\mathsf{F}}$ and $\mathbin{\hat\amalg}_{\mathsf{F}}$
...
\begin{align*}
  ...
  &
    \simeq \mathrm{coeq}\left(
          \O \mathbin{\hat\amalg_{\mathsf{F}}} \mathbb F X \mathbin{\hat\amalg}_{\mathsf{F}} \mathbb F Y \rightrightarrows \O \mathbin{\hat\amalg}_{\mathsf{F}} \mathbb F Y
          \right)
  \\
  &
...
with respect to $\mathbin{\hat\amalg}_{\mathsf{F}}$,
...
\begin{proof}
Much as in Proposition \ref{ASSOCIDS PROP}, this follows by nothing that all squares in the following diagrams are pullback squares.
...
\\
	...
	&
		\Sigma \wr \Sigma_{\mathfrak C}
\\ % NEW DIAGRAM ------------------------------
		\OC^{n+k+1,s,\lambda} \arrow{r}{\boldsymbol{V}^k} \wr (A_j)
		\ar{r} \ar{d}
		%\bullet \arrow[dashed]{d} \arrow[dashed]{r}
	&
...
\subsection{Filtration of free extensions in the equivariant case}
...
\begin{tikzcd}[ampersand replacement=\&]
	\mathop{\coprod}\limits_{[T] \in \Iso(C \downarrow G \ltimes \Omega_{\mathfrak C}^a[k])}
	\left(
		\mathop{\bigotimes}\limits_{v \in V^{ac}(T)} \O(T_v) \otimes
		Q^{in}_T[u]
...
% Previously: Homotpy thy of ops with fixed g-set of colors, section 2
