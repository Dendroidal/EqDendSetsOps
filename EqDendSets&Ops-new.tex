...
Let $e\colon \Omega[T^e] \to N \mathcal{P}$
be a non-degenerate elementary dendrex. Note the following: 
\begin{itemize}
\item[(a)] if $\Xi^e = \emptyset$ then 
either all vertices of $T^e$ are $\O$-labeled, i.e. $e \in N \mathcal{O}$, or $U^e$ is a $B$-labeled corolla, i.e. $e \in B$.
In other words, $\Xi^e = \emptyset$ iff 
$\langle e \rangle \subseteq A = B \cup N\mathcal{O}$.
\item[(b)] any outer face of $e$ is again elementary,
as is any inner face $\partial_{T^e-f} e$ such that $f \not \in \Xi^e$
...
For (Ch1), by (b) any proper outer face of $e$ is in $A_{<\langle e\rangle}$, so we need only consider the case of
$V=U^e$ with $\Xi^e=\emptyset$, in which case
$\langle e \rangle \subseteq A \subseteq A_{<\langle e\rangle}$ by (a).

For (Ch2),(Ch3), by (a) and the first half of (b) one needs only consider the case of
$V \simeq T^e - E$ where $E \subseteq \Xi^e$ and $\Xi^e \neq \emptyset$.
But then
$V - \Xi^e_V \simeq T^e- \Xi^e$
and by the second half of (b)
one has
$\langle \partial_{T^e-\Xi^e}e \rangle \in  A_{<\langle e\rangle}$
iff
$\langle e \rangle \in  A_{<\langle e\rangle}$
iff
$\langle \partial_{T^e-E}e \rangle \in  A_{<\langle e\rangle}$,
so (Ch2),(Ch3) follow.
...
$e^{-1}(A_{<\langle e\rangle})$
consists of the faces 
isomorphic to
$T^e-E$ for $E \subset \Xi^e$.
