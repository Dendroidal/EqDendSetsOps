...
\begin{definition}
	Recall the category $\mathbbm{1} \in \Cat_{\set{0,1}}(\V)$ which detects arrows.
	A cofibration $\mathbbm{1} \to \mathbb{I}$ in $\Cat_{\set{0,1}}(\V)$ into a $\V$-interval is called \textit{natural} if
	it fits into a commuting diagram of the following form in $\Cat_{\set{0,1}}(\V)$.
\begin{equation}
\begin{tikzcd}
...
\begin{proof}
      ...
      When $|V(T)| \geq 1$, following Proposition \ref{WLEFTQPUSH PROP}
      it suffices to show that
      \[
            i(partial C) = 
            (\partial \Delta[1] \to \Delta[1])^{\square \mathbf E^i(T_e)}
      \]
      is a genuine cofibration in $\sSet^{\Aut(\partial C)}_\Fin$ with $C = \mathsf{lr}(T_e)$.
