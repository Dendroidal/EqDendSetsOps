...
$C \to T^c$ a tall map with $C$ a corolla,
and $\bar{C} \to T^{e}$
any map with $\bar{C}$ a corolla.

Then, given a solid left vertical isomorphism as below such that the diagram commutes, there also exists a dashed map making the diagram commute.
Moreover, if $\bar{C} \to T^{e}$ is tall the dashed map is also tall.
...
\begin{remark}\label{SEGC REM}
By the Segal condition, 
one has 
$\langle p \rangle = 
\bigcup_{v \in \boldsymbol{V}(T)} 
\langle p_v \rangle$,
provided that $\eta$ is not the stick tree.
\end{remark}
