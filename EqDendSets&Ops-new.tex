...
As in \cite{BP_geo}, we will transfer this model structure along the composite free-forgetful adjunction
\begin{equation}
      \label{OPAUTAJD_EQ}
      \begin{tikzcd}
            \displaystyle{\prod_{\vect C \in G \ltimes \Sigma_{\mathfrak C}} \V^{\Aut(\vect C)}}
            \arrow[r, shift left, "{\Sigma_{\mathfrak C}[-]}"]
            &
...
\end{equation}
where
$\prod \V^{\Aut(\vect C)}$, $\Sym^{G, \mathfrak C}(\V)$ are endowed with the $\F$-model strucutre,
and $\mathbb F$ denotes the $\mathfrak C$-fiber $\mathbb F^{G, \mathfrak C}$ of the free operad monad from Definition \ref{COLORMON DEF}, \eqref{FGC_EQ}, in Appendix \ref{MONAD_APDX}.
This transfer will follow much in the same was as in \cite{BP_geo};
we relegate most of the technical algebraic discussion of this monad $\mathbb F$ to Appendix \ref{MONAD_APDX}.
