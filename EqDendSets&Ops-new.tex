...
\begin{example}
      ...
      We warn that the tensoring left adjoint, denoted $(-) \odot_{\Fin} K: \sOp \to \sOp$, is in general \textit{not} levelwise.
\end{example}
...
we relegate the technical discussion of this monad $\mathbb F^G$ to Appendix \ref{MONAD_APDX}.
...
\begin{example}
      ...
      \item For higher arity operations,
            if $\Gamma$ is a graph subgroup, corresponding to a map $G \geq H \xrightarrow{\alpha} \Sigma_n$, then
...
\begin{remark}[{cf. \cite[Remark 6.48]{BP_geo}}]
      ...
      Letting $\lambda$ denote the partition $\boldsymbol{L}(C) = \lambda_1 \amalg \dots \amalg \lambda_k$ such that
      $i$,$j$ are in the same class iff $T_i \simeq T_j$.
      Picking representatives $i_j \in \lambda_j$, we thus have a decomposition
      \[
...
\begin{remark}
      For any $\vect T \in \Omega_{\mathfrak C}$ with underlying tree $T \in \Omega$,
      we have a natural map $\phi: \Aut(\vect T) \to G \times \Aut(T)$.
...
\begin{proposition}[{cf. \cite[Prop. 6.52]{BP_geo}}]
      ...
      \]
      is a (trivial) cofibration in $\V^{\Aut_{\Omega_{\mathfrak C}}(\vect T)}_{\F_{\vect T}}$.
\end{proposition}
\begin{proof}
      ...
      \[
            \V^{\Aut(\vect C)}_{\F_{\vect C}} \times \prod_{i}{}_G \V^{\Sigma_{|\vect \lambda_i|} \wr_G \Aut(\vect T_i)}_{\F_{\vect T_i}^{\ltimes_G |\vect \lambda_i|}} \xrightarrow{\quad \otimes \quad}
            \V^{\Aut(\vect T)}_{\F_{\vect T}}
      \]
      is left Quillen.
      The result now follows from Lemma \ref{641} and the induction hypothesis.
\end{proof}
...
\begin{corollary}
      If $\O \in \sOp^G$ cofibrant, then $\O$ $\Sigma_G$-cofibrant.
\end{corollary}
...
First, we may consider the simplicial Reedy model structure over $\dSet^G$ equipped with the model structure from \cite{Per_eds},
...
\begin{remark}
      \label{TAUCOFIB_REM}
      We note that $\tau \colon \mathsf{PreOp}^G \to \sOp^G$
      sends the collection of arrows in (TC1) and (TC2) surjectively onto the collection of generating cofibrations, namely
      \begin{gather*}
            ...
            \mathbb F_{\Gamma_C} \Big( \Gamma_C / \Gamma_C \cdot (\partial \Delta[n] \to \Delta[n]) \Big) = \Omega(C) \cdot \big(\partial \Delta[n] \to \Delta[n]\Big)
      \end{gather*}
      where $\Gamma_C$ is the graph subgroup of $G \times \Sigma_n$ such that $C \simeq G \cdot C_n / \Gamma$,
      $\mathbb F_\Gamma: \sSet^\Gamma \to \sOp^G$ is as in the discussion around \eqref{FGAMMA_EQ},
      and $(-) \cdot (-)$ indicates the fibered tensoring from {\color{red} source}.
...
\begin{proof}
	...
	in which case it is straightforward that $\mathcal{O} \to \mathcal{P}$ is a $\Sigma$-cofibration
        \todo[inline]{cofibrations with $\Sigma$-cofibrant source are $\Sigma$-cofibrations - prove this.}
	and each of the levels
...
\begin{definition}
A dendrex $p\colon \Omega[T] \to N \mathcal{P}$ 
is called:
\begin{itemize}
\item \emph{elementary} if for each vertex $T_v \hookrightarrow T$
...
Moreover, we write $\chi(r)$ for an arbitrarily chosen canonical representative of $\langle a \rangle$.
...
If $\langle e \rangle \subseteq N \mathcal{P}$
is elementary, the tree $T$ can be naturally regarded as a
$\{\O,B\}$-labeled tree by labeling $T_v$ according to whether
it is $\langle \partial_{T_v} e\rangle \subseteq \O $ or
$\langle \partial_{T_v} e\rangle \subseteq B$.
...
%$\langle \partial_{T^a} e \rangle = \langle \chi(\partial_r e) \rangle$.

%Crucially, note that we have just shown that if 
%$\langle e \rangle$ is elementary then
%$\langle \chi({\partial_r e}) \rangle \subseteq \langle e \rangle$.
%%Moreover, the uniqueness in Remark \ref{PUSHOPPRRST REM}
%%implies that $\langle p \rangle$ is canonical iff
%%$\langle p^a \rangle = \langle p \rangle$.
\end{remark}


\begin{lemma}\label{FORSAKEN LEM}
Suppose $e \colon \Omega[T^e] \to N \mathcal{P}$ is elementary
and $\langle \partial_r e\rangle$ is not unital.
Then there exists an inner face map
$T^c \to T^e$ such that $\partial_{T^c} e$ is canonical.

In particular,
$\langle \partial_{T^c} e \rangle = \langle \chi(\partial_r e) \rangle$,
and hence 
$\langle \chi({\partial_r e}) \rangle \subseteq \langle e \rangle$.
\end{lemma}

\begin{proof}
By Remark \ref{ELEMLABEL REM} we can regard $T^e$
as a $\{\O,B\}$-labeled tree so that, 
by ({\color{blue} the alternating tree analog of}) \cite[Prop. 5.49]{BP_geo},
there is hence a unique alternating tree $T^a$ together with a tall planar $B$-inert label map $T^a \to T$.
Writing
$a = \partial_{T^a} e$,
$r = \partial_r e$, 
one has $\langle \partial_r a\rangle = \langle r\rangle$,
and hence by Remark \ref{PUSHOPPRRST REM}
one has that $a$ is a degeneracy of $\chi(r)$,
i.e. there is a degeneracy map $T^a \to T^c$
such that $\partial_{T^a} \chi(r) = a$.
Since $\langle r \rangle = \langle \partial_r e\rangle$ is not unital
by assumption,
neither is $\langle \chi(r) \rangle$, 
so that $T^c$ can not be the stick tree $\eta$,
and thus $T^a \to T^c$ has a section which is an inner face 
(this follows from \cite[Cor. 5.38]{Per_eds} since no edge of $T^c$ is both a root and a leaf).
But then the composite 
$T^c \to T^a \to T$
must be a face (or else $\chi(r) = \partial_{T^c} e$ would be degenerate) and is tall, hence an inner face, finishing the proof.
\end{proof}
...
\begin{proof}
First note that by applying  
``tall map followed by outer face'' factorization to
$C \to T^{e}$
to obtain
$C \to T^{\bar{e}} \to T^{e}$
the dendrex $\partial_{T^{\bar{e}}} e$
is still elementary (being an outer face of an elementary dendrex),
so we are free to assume in the remainder of the proof that
$C \to T^e$ is a tall map.

Next, note that commutativity of \eqref{UNIQINAN EQ} implies
$\langle \partial_r e \rangle =
\langle \partial_r c \rangle$,
which is not unital by assumption, 
so that by Lemma \ref{FORSAKEN LEM} there is an inner face $T^{\bar{c}} \to T^{e}$
such that $\bar{c} = \partial_{T^{\bar{c}}} e$ is canonical.
...
$\Omega[T^a] \to \Omega[T^c] \xrightarrow{c} N \mathcal{P}$,
$\Omega[T^{\bar{a}}] \to \Omega[T^{\bar{c}}] \xrightarrow{\bar{c}} N \mathcal{P}$  
are alternating dendrices. Since $\mathsf{lr}$ sends tall maps to isomorphisms, we can form the diagram
\[
\begin{tikzcd}
	\Omega[C] \ar{r}{\simeq} \ar{rd}[swap]{\simeq} &
	\Omega\left[\mathsf{lr}(T^a)\right] \ar[dashed]{d}[swap]{\simeq} \ar{r} &
	\Omega[T^a] \ar[dashed]{d}[swap]{\simeq} \ar{r} &
	\Omega[T^c] \ar{r}{c} \ar[dashed]{d}[swap]{\simeq} &
	N \mathcal{P}
\\
	 &
	\Omega\left[\mathsf{lr}(T^{\bar{a}})\right] \ar{r} &
	\Omega[T^{\bar{a}}] \ar{r} &
	\Omega[T^{\bar{c}}] \ar{ru}[swap]{\bar c} \ar{r} &
	\Omega[T^{e}] \ar{u}[swap]{e} &
\end{tikzcd}
\]
and we will argue that all dashed vertical isomorphisms exist.
...
from the fact that the factorization of degenerate dendrices through non-degenerate dendrices is unique up to (unique) isomorphism (cf. \cite[Prop. 5.62]{Per_eds}).
%
The moreover claim is clear.
%
%The ``moreover'' claim follows since,
%if $e$ is canonical,
%the identity
%$\langle \partial_r c \rangle =
%\langle \partial_r e \rangle$
%and Remark \ref{PUSHOPPRRST REM} implies
%$\langle c \rangle =
%\langle e \rangle$,
%so that $c,e$ are non-degenerate representatives, and a map relating them must be an isomorphism.
\end{proof}
...
%\begin{lemma}\label{UNIQINAN LEM}
%Let 
%$c \colon \Omega[T^c] \to N \mathcal{P}$ and
%$\bar{c} \colon \Omega[T^{\bar{c}}] \to N \mathcal{P}$
%be canonical dendrices,
%and $C \to T^c$ and $\bar{C} \to T^{\bar{c}}$
%be tall maps where $C,\bar{C} \in \Sigma$ are corollas.
%Then, given a solid left vertical isomorphism as below such that the %diagram commutes, there also exists a dashed isomorphism making the diagram commute. 
%\[
%\begin{tikzcd}
%	\Omega[C] \ar{d}[swap]{\simeq} \ar{r} &
%	\Omega[T^c] \ar{r}{c} \ar[dashed]{d}[swap]{\simeq} &
%	N \mathcal{P}
%\\
%	\Omega[\bar{C}] \ar{r} &
%	\Omega[T^{\bar{c}}] \ar{ru}[swap]{\bar c} 
%\end{tikzcd}
%\]
%\end{lemma}
...
%\[
%\begin{tikzcd}
%	\Omega[C] \ar{d}[swap]{\simeq} \ar{r}{\simeq} &
%	\Omega\left[\mathsf{lr}(T^a)\right] \ar[dashed]{d}[swap]{\simeq} \ar{r} &
%	\Omega[T^a] \ar[dashed]{d}[swap]{\simeq} \ar{r} &
%	\Omega[T^c] \ar{r}{c} \ar[dashed]{d}[swap]{\simeq} &
%	N \mathcal{P}
%\\
%	\Omega[\bar{C}] \ar{r}{\simeq} &
%	\Omega\left[\mathsf{lr}(T^a)\right] \ar{r} &
%	\Omega[T^{\bar{a}}] \ar{r} &
%	\Omega[T^{\bar{c}}] \ar{ru}[swap]{\bar c} 
%\end{tikzcd}
%\]
%and we will argue that all dashed vertical isomorphisms exist.
%That the second vertical isomorphism exists is trivial.
%The existence of the third vertical isomorphism follows from 
%\eqref{PUSHOPPR EQ} which implies that, for $a,\bar{a}$ alternating dendrices, all isomorphisms 
%$\partial_r a \simeq \partial_r \bar{a}$
%are induced from an isomorphism $a \simeq \bar{a}$.
%Lastly, the existence of the fourth isomorphism follows 
%from the fact that the factorization of degenerate dendrices through non-degenerate dendrices is unique up to (unique) isomorphism (cf. \cite[Prop. 5.62]{Per_eds}).
%\end{proof}
...
%Crucially, note that we have just shown that if 
%$\langle p \rangle$ is elementary then
%$\langle p^a \rangle \subseteq \langle p \rangle$.
%%Moreover, the uniqueness in Remark \ref{PUSHOPPRRST REM}
%%implies that $\langle p \rangle$ is canonical iff
%%$\langle p^a \rangle = \langle p \rangle$.
%\end{remark}
...
An elementary subpresheaf $\langle c \rangle$
is canonical iff
for any elementary $\langle e \rangle$
such that $\langle \partial_r c \rangle \subseteq \langle e \rangle$
it is $\langle c \rangle \subseteq \langle e \rangle$.
\end{lemma}


\begin{proof}
If $\langle \partial_r c \rangle$ is unital it is 
clear that both conditions imply
$\langle c \rangle = \langle \partial_r c \rangle$, 
so in the remainder we assume that 
$\langle \partial_r c \rangle$ is not unital.

Consider first the ``only if'' direction.
Writing 
$c \colon \Omega[T^c] \to N \mathcal{P}$,
$e \colon \Omega[T^e] \to N \mathcal{P}$
for canonical and elementary representatives, 
the condition 
$\langle \partial_r c \rangle \subseteq \langle e \rangle$
amounts to the existence of the solid diagram in \eqref{UNIQINAN EQ}, and hence the existence of the dashed arrow implies
$\langle c \rangle \subseteq \langle e \rangle$,
as desired.

For the ``if'' direction,
the assumption implies
$\langle c \rangle \subseteq \langle \chi(\partial_r c) \rangle$
while the ``if'' direction for $\chi(\partial_r c) \rangle$ implies 
$\langle \chi(\partial_r c) \rangle \subseteq \langle c \rangle$,
showing 
$\langle c \rangle = \langle \chi(\partial_r c) \rangle$ so that
$\langle c \rangle$ is indeed canonical.
\end{proof}
...
%\begin{proof}
%Fix representatives
%$c \colon \Omega[T] \to N \mathcal{P}$,
%$e \colon \Omega[S] \to N \mathcal{P}$,
%$\partial_r c \colon \Omega[C] \to N \mathcal{P}$.


%For the ``only if'' direction, the assumption
%$\langle \partial_r c \rangle \subseteq \langle e \rangle$
%says that there is a map
%$C \to S$ such that $\partial_C e = \partial_r c$.
%Consider now the 
%``tall map followed by outer face'' factorization $C \to \bar{S} \to S$.
%Then $\bar{e}=\partial_{\bar{S}}e$ is again elementary since outer faces of elementary dendrices are elementary.
%Moreover, since $C \simeq \mathsf{lr}(\bar{S})$,
%it is hence 
%$\langle \partial_r c \rangle = 
%\langle \partial_C e \rangle =
%\langle \partial_r \bar{e} \rangle$.
%Since we are assuming $\langle e \rangle$ canonical,
%it is tautological from Remark \ref{PUSHOPPRRST REM} that
%$\langle c \rangle = 
%\langle a^{\partial_r c} \rangle = 
%\langle a^{\partial_r \bar{e} } \rangle 
%%= \langle q^a \rangle
%$,
%and thus Remark \ref{ELEMLABEL REM}
%implies that 
%$\langle c \rangle = 
%\langle a^{\partial_r \bar{e} } \rangle 
%\subseteq \langle \bar{e} \rangle
%\subseteq \langle e \rangle$, as desired.


%For the ``if'' direction,
%suppose the representative $c$ is non-degenerate and let
%$T^a \to T$ be as in Remark \ref{ELEMLABEL REM}.
...
%so that $c' = \partial_{T'} c$ is again elementary
%and 
%$\langle \partial_{r} c' \rangle =
%\langle \partial_{r} c \rangle $. 
%The assumption then implies it is 
%$\langle c \rangle \subseteq \langle c' \rangle$
%and thus 
%$\langle c \rangle = \langle c \rangle$.
%But since $c$ was assumed non-degenerate, this is only possible is the face map $T' \to T$ is an isomorphism, i.e. if
%$T_a \to T$ is a degeneracy, i.e. if
%$\langle c \rangle = \langle a^{\partial_r c} \rangle$.
%\end{proof}
...
%\begin{lemma}\label{CANCHAR LEM}
%An elementary subpresheaf $\langle p \rangle$
%is canonical iff
%for any elementary $\langle q \rangle$
%such that $\langle \partial_r p \rangle \subseteq \langle q \rangle$
%it is $\langle p \rangle \subseteq \langle q \rangle$.
%\end{lemma}
...
%Fix representatives
%$p \colon \Omega[T] \to N \mathcal{P}$,
%$q \colon \Omega[S] \to N \mathcal{P}$,
%$\partial_r p \colon \Omega[C] \to N \mathcal{P}$.

%For the ``only if'' direction, the assumption
%$\langle \partial_r p \rangle \subseteq \langle q \rangle$
%says that there is a map
%$C \to S$ such that $\partial_C q = \partial_r p$.
%Consider now the 
%``tall map followed by outer face'' factorization $C \to \bar{S} \to S$.
%Then $\bar{q}=\partial_{\bar{S}}q$ is again elementary since outer faces of elementary dendrices are elementary.
%Moreover, since $C \simeq \mathsf{lr}(\bar{S})$,
%it is hence 
%$\langle \partial_r p \rangle = 
%\langle \partial_C q \rangle =
%\langle \partial_r \bar{q} \rangle$.
%Since we are assuming $\langle p \rangle$ canonical,
%it is tautological from Remark \ref{PUSHOPPRRST REM} that
%$\langle p \rangle = 
%\langle \left(\partial_r p\right)^a \rangle = 
%\langle \left(\partial_r q\right)^a \rangle =
%\langle q^a \rangle$,
%and thus Remark \ref{ELEMLABEL REM}
%implies that 
%$\langle p \rangle = 
%\langle q^a \rangle \subseteq \langle q \rangle$, as desired.


%For the ``if'' direction,
%suppose the representative $p$ is non-degenerate and let
%$T^a \to T$ be as in Remark \ref{ELEMLABEL REM}.
%Writing $T^a \to T' \to T$ for the 
%``degeneracy followed by inner face'' factorization, 
%$T'$ inherits a $\{\O,B\}$-labeling from $T^a$ and both maps in the factorization are tall planar $B$-inert label maps, 
%so that $p' = \partial_{T'} p$ is again elementary
%and 
%$\langle \partial_{r} p' \rangle =
%\langle \partial_{r} p \rangle $. 
%The assumption then implies it is 
%$\langle p \rangle \subseteq \langle p' \rangle$
%and thus 
%$\langle p \rangle = \langle q \rangle$.
%But since $p$ was assumed non-degenerate, this is only possible is the face map $T' \to T$ is an isomorphism, i.e. if
%$T_a \to T$ is a degeneracy, i.e. if
...
\begin{remark}\label{1BADJ REM}
Since any canonical dendrex $c \colon \Omega[T] \to N \mathcal{P}$
has an alternating degeneracy, 
it follows that, giving $T$ the $\{\O,B\}$-labeling as in 
Remark \ref{ELEMLABEL REM},
all inner edges of $T$ are adjacent to at least one $B$-labeled vertex.
\end{remark}


More generally, given a non-degenerate elementary dendrex
$e \colon \Omega[T] \to N \mathcal{P}$,
we write
$\Xi^e \subseteq \boldsymbol{E}^{\mathsf{i}}(T)$
for the subset of inner edges of $T$ which are adjacent to at least one $B$-labeled vertex.
Equivalently, $\Xi^e$ consists of the inner edges which are in the image of the map $T^a \to T$.

In particular, by the previous remark one has that
$e$ is canonical iff $\Xi^e = \boldsymbol{E}^{\mathsf{i}}(T)$.
({\color{blue} flesh this out - something about how tall maps are degeneracies iff they are surjective})
...
$\bar{e} \colon \Omega[T^{\bar{e}}] \to N \mathcal{P}$
an elementary dendrex,
and 
$T^e-E \to T^{\bar{e}}$ a map where $E \subseteq \Xi^e$.

Then, 
if the solid diagram below commutes, there exists a dashed map making the diagram commute.
\begin{equation}\label{UNIQINAN2 EQ}
\begin{tikzcd}
	\Omega[T^e-E] \ar{r} \ar{rd}&
	\Omega[T^e] \ar{r}{e} \ar[dashed]{d} &
	N \mathcal{P}
\\
	 &
	\Omega[T^{\bar{e}}] \ar{ru}[swap]{\bar{e}} 
\end{tikzcd}
\end{equation}
\end{lemma}
...
\begin{proof}
We will abbreviate $T' = T^e -E$. 
Just as in the proof of Lemma \ref{UNIQINAN LEM} we are free to assume $T' \to T^{\bar{e}}$ is a tall map.

For each vertex $T'_v$ we apply the 
`'inner face followed by outer face factorization''
to the composites
$T'_{v} \hookrightarrow T' \to T^{e}$,
$T'_{v} \hookrightarrow T' \to T^{\bar{e}}$
to get
$T'_{v} \hookrightarrow T_{v}^e \to T^e$,
$T'_{v} \hookrightarrow T_{v}^{\bar{e}} \to T^{\bar{e}}$
and, further writing
$e_v = \partial_{T^e_v} e$,
$\bar{e}_v = \partial_{T^{\bar{e}}_v} \bar{e}$, 
we obtain solid diagrams
\begin{equation}\label{UNIQINAN2A EQ}
\begin{tikzcd}
	\Omega[T'_v] \ar{r} \ar{rd}&
	\Omega[T^e_v] \ar{r}{e_v} \ar[dashed]{d} &
	N \mathcal{P}
\\
	 &
	\Omega[T^{\bar{e}}_v] \ar{ru}[swap]{\bar{e}_v} 
\end{tikzcd}
\end{equation}
We now claim that $e_v$ is canonical. Indeed, $e_v$ is non-degenerate elementary since it is a face of $e$, which is also non-degenerate elementary. And since all inner edges of $T^e_v$ are in 
$E \subseteq \Xi^e$, they are all adjacent to $B$-labeled vertices, 
so that $e_v$ is indeed canonical by the observation after Remark \ref{1BADJ REM}.


By Lemma \ref{UNIQINAN LEM} we can thus find the dashed arrow in \eqref{UNIQINAN2A EQ} for each $v$ and thus, 
by Lemma \ref{UNIQINAN LEM},
we obtain the desired dashed arrow in \eqref{UNIQINAN2 EQ}.
\end{proof}


\begin{lemma}
Let $e \colon \Omega[T] \to N \mathcal{P}$
be elementary non-degenerate and
$E,E' \subseteq \Xi^e$.

Then if 
$\langle \partial_{T-E} e \rangle
=
\langle \partial_{T-E'} e \rangle$
it must be $E = E'$.
\end{lemma}
...
\[
\begin{tikzcd}
	\Omega[T-E] \ar{d}[swap]{\simeq} \ar{r} &
	\Omega[T] \ar{r}{e} \ar[dashed]{d} &
	N \mathcal{P}
\\
	\Omega[T-E'] \ar{r} &
	\Omega[T] \ar{ru}[swap]{e} &
\end{tikzcd}
\]
...
so that $T-E=T-E'$ and $E=E'$.
\end{proof}



{\color{purple} HERE-HERE}
...
%\item
 $\partial_{T^e-E} e$ is non-degenerate and 
$\langle p \rangle = \langle \partial_{T^e-E} e \rangle$,
%,
so that in particular 
$\langle p \rangle \subseteq \langle e \rangle$.
%\item for any elementary $\langle q \rangle$ such that
%$\langle p \rangle \subseteq \langle q \rangle$ it is
%$\langle p^e \rangle \subseteq \langle q \rangle$.
%
%In fact, letting $q \colon \Omega[S] \to N \mathcal{P}$
%be a non-degenerate representative,
%any map
%$T^e - E \to S$
%such that 
%$\langle \partial_{T^e-E} q \rangle =
%\langle p \rangle $
%has a factorization
%$T^e - E \hookrightarrow T^e \to S$
%such that 
%$\langle \partial_{T^e} q \rangle =
%\langle p^e \rangle $.
%
%there is a unique face map
%$T_e \to S$ such that $p^e = \partial_{T^e} q$. 
%\end{enumerate} 

Moreover, 
$\langle e \rangle$ is the smallest elementary containing 
$\langle p \rangle$.
\end{lemma}
...
For each vertex $T_v \hookrightarrow T$, write
$p_v = \partial_{T_v} p$
and,
noting that $\langle p_v \rangle$ is reduced,
we write 
$e_v \colon \Omega[T^e_v] \to N \mathcal{P}$
...
implies that
$T_v \simeq \mathsf{lr}(T^e_v)$,
so that by choosing tall maps $T_v \to T^e_v$ 
one obtains a $T$-substitution datum
\cite[Def. 3.38]{BP_geo}
which by 
\cite[Prop. 3.41]{BP_geo}
can be assembled into a tree $T^e$
together with a tall map
$T \to T^e$ such that for every vertex $T_v$
the ``tall map followed by outer face'' factorization of
the composite
$T_v \to T \to T^e$
is given by
$T_v \to T^e_v \to T^e$.
Since each vertex of $T^e$ is in exactly one of the outer trees $T^e_v$,
we define $e \colon \Omega[T^e] \to N \mathcal{P}$
as the unique dendrex such that
$\partial_{T^e_v} e = e_v$.
Note that since the $e_v$ are non-degenerate then so is $e$.

To finish the proof, it remains to show that the canonical map
$T \to T^e$ induces an isomorphism $T \simeq T^e - E$
for some $E \subseteq \Xi^{p^e}$.
That $T \to T^e$ is an inner face map follows
from the fact that all maps 
$T_v \to T^e_v$ are inner face maps
(this could only fail if some
$\langle p_v \rangle$ was unital,
which would contradict the assumption that $p$ is non-degenerate)
and hence
$T \simeq T^e-E$ where 
$E = \amalg_{v \in \boldsymbol{V}(T)} \boldsymbol{E}^{\mathsf{i}}(T^e_v)$. The claim that
$E \subseteq \Xi^{e}$, i.e. that all edges in $E$ are adjacent to a $B$-labeled vertex,
follows from Remark \ref{1BADJ REM}.

For the ``moreover'' claim, 
the formula
$\langle e \rangle = 
\bigcup_{v \in \boldsymbol{V}(T)} 
\langle a^{p_v}\rangle$
is immediate by the construction, 
so that the claim that $\langle e \rangle $
is the smallest elementary containing
$\langle p \rangle $
follows from Remark \ref{SEGC REM} and Lemma \ref{CANCHAR LEM}.
\end{proof}
...
The $G$-poset of principal subpresheaves is the poset formed by the 
...
%and a subset $E \subseteq \Xi^{p^e}$
%such that:
%\begin{enumerate}[label=(\alph*)]
%\item $\partial_{T^e-E} p^e$ is non-degenerate and 
%$\langle p \rangle = \langle \partial_{T^e-E} p^e \rangle$,
%so that in particular 
...
%\begin{proof}
%Let $p\colon \Omega[T] \to N \mathcal{P}$
%be a non-degenerate representative. We first build $e_p$.
...
%noting that $\langle p_v \rangle$ is reduced,
%we write 
%$p_v^e \colon \Omega[T^e_v] \to N \mathcal{P}$
%for a canonical representative of 
%$\langle p_v^a \rangle$
%(cf. Remark \ref{PUSHOPPRRST REM}).
%The identity
%$\langle p_v \rangle = \langle \partial_r p^e_v \rangle$
%implies that
%$T_v \simeq \mathsf{lr}(T^e_v)$,
%so that by choosing tall maps $T_v \to T^e_v$ 
%one obtains a $T$-substitution datum
...
%$T_v \to T^e_v \to T^e$.
%Since each vertex of $T^e$ is in exactly one of the outer trees $T^e_v$,
%we define $p^e \colon \Omega[T^e] \to N \mathcal{P}$
%as the unique dendrex such that
%$\partial_{T^e_v} p^e = p^e_v$.
%Note that since the $p^e_v$ are non-degenerate then so is $p^e$.

%To finish the proof of (a), it remains to show that the canonical map
%$T \to T^e$ induces an isomorphism $T \simeq T^e - E$
%for some $E \subseteq \Xi^{p^e}$.
%That $T \to T^e$ is an inner face map follows
%from the fact that all maps 
...
%$T \simeq T^e-E$ where 
%$E = \amalg_{v \in \boldsymbol{V}(T)} \boldsymbol{E}^{\mathsf{i}}(T^e_v)$. The claim that
%$E \subseteq \Xi^{p^e}$, i.e. that all edges in $E$ are adjacent to a $B$-labeled vertex,
%follows from Remark \ref{1BADJ REM}.
...
\begin{lemma}
Let $p \colon \Omega[T] \to N \mathcal{P}$
be a non-degenerate elementary dendrex and 
$E \subseteq \boldsymbol{E}^{\mathsf{i}}(T)$
be a subset.
Then for any dendrex 
$q \colon \Omega[S] \to N \mathcal{P}$ and map
$T^e - E \to S$
such that 
$\langle \partial_{T^e-E} p\rangle
=
\langle \partial_{T^e-E} q\rangle$
...
there exists an elementary subpresheaf
$\langle p^e \rangle$, 
with non-degenerate representative 
$p^e \colon \Omega[T^e] \to N \mathcal{P}$,
such that:
\begin{enumerate}[label=(\alph*)]
\item $\langle p \rangle = \langle \partial_{T^e-E} p^e \rangle$ for some unique $E \subseteq \Xi^{p^e}$,
so that in particular 
$\langle p \rangle \subseteq \langle p^e \rangle$;
\item for any elementary $\langle q \rangle$ such that
$\langle p \rangle \subseteq \langle q \rangle$ it is
...
we write 
$p_v^e \colon \Omega[T^e_v] \to N \mathcal{P}$
for a non-degenerate representative of 
$\langle p_v^a \rangle$
(cf. Remark \ref{PUSHOPPRRST REM}).
The identity
$\langle p_v \rangle = \langle \partial_r p^a_v \rangle$
implies that
$T_v \simeq \mathsf{lr}(T^e_v)$,
so that by choosing tall maps $T_v \to T^e_v$ 
one obtains a $T$-substitution datum
...
which by 
\cite[Prop. 3.41]{BP_geo}
can be assembled into a tree $T_e$
together with a tall map
$T \to T^e$ such that for every vertex $T_v$
the ``tall map followed by outer face'' factorization of
the composite
...
We now address (a) expect for its uniqueness claim, which 

{\color{green} HERE}

Since $p_v$ can't be degenerate
...
there exists an elementary subpresheaf
$\langle e_p \rangle$
which is the smallest elementary subpresheaf such that
$\langle p \rangle \subseteq \langle e_p \rangle$.

In addition, given non-degenerate representatives
$p \colon \Omega[T] \to N \mathcal{P}$,
$e_p \colon \Omega[T^e] \to N \mathcal{P}$,
there is a face map
$T \hookrightarrow T^e$, unique up to isomorphism, such that
$\langle p \rangle = \langle \partial_{T} e_p \rangle$.
Lastly, this face map is necessarily an inner face of the form
$T \simeq T^e - E$ for some $E \subseteq \Xi^{e_p}$.
\end{lemma}

{\color{red} HERE}

\begin{proof}
Consider first the case where $\langle p \rangle$ is reduced. Then, 
by Remark \ref{PUSHOPPRRST REM} and Lemma \ref{CANCHAR LEM},
%if $\langle p \rangle$ is reduced
one can set $\langle e_p \rangle = \langle p^a \rangle$.
For the additional claims, recall that the identity
$\langle p \rangle = \langle \partial_r e_p \rangle$ 
can be rewritten as
$\langle p \rangle = \langle \partial_{\mathsf{lr}(T)} e_p \rangle$ 
...
\begin{remark}
\eqref{PUSHOPPR EQ} 
can be restated as saying as giving the following two conditions:
\begin{enumerate}[label=(\roman*)]
\item
for each reduced 
$r \colon \Omega[C] \to N \mathcal{P}$
there exists alternating
$a \colon \Omega[T^a] \to N \mathcal{P}$
such that
$\langle r \rangle = \langle \partial_r a \rangle$;
\item
given alternating
$a \colon \Omega[T^a] \to N \mathcal{P}$ and
$\bar{a} \colon \Omega[T^{\bar{a}}] \to N \mathcal{P}$
such that 
$\langle\partial_r a \rangle = 
\langle \partial_r \bar{a} \rangle$,
i.e. such that a solid
left vertical isomorphism an in the digram below exists,
\[
\begin{tikzcd}
	\Omega\left[C\right] \ar{d}[swap]{\simeq} \ar{r} &
	\Omega[T^a] \ar{r}{a} \ar[dashed]{d}[swap]{\simeq} &
	N \mathcal{P}
\\
	\Omega\left[\bar{C}\right] \ar{r} &
	\Omega[T^{\bar{a}}] \ar{ru}[swap]{\bar a} 
\end{tikzcd}
\]
\end{enumerate}
\end{remark}
...
\langle p_v^c\rangle$.
In particular, 
$\langle p^e \rangle$ is the smallest elementary containing 
$\langle p^e \rangle$.
\end{lemma}


\begin{lemma}
If $p$ is elementary non-degenerate and $E \subseteq \Xi^p$,
then 
$\langle p\rangle$
is the smallest elementary presheaf containing 
$\langle \partial_{T-E} p\rangle$.
\end{lemma}

\begin{proof}
Write $T' = T-E$ and for each vertex 
$T'_v$ let $T_v$ be the corresponding outer face.
Then $p_v$ is still canonical (by observation after Remark \ref{1BADJ REM}), and so it equals 
$\left(p'_v\right)^c$.
\end{proof}
...
Let $e \colon \Omega[T] \to N \mathcal{P}$
be elementary non-degenerate and
$E^1,E^2 \subseteq \Xi^e$.

Then if 
$\langle \partial_{T-E^1} e \rangle
=
\langle \partial_{T-E^2} e \rangle$
it must be $E^1 = E^2$.
\end{lemma}
...
=
\langle \partial_{T-E^2} e \rangle$
says that there exists a solid isomorphism as in the left diagram below.
Then considering the right diagram for each
$v \in \boldsymbol{V}(T^1)$, 
the dashed isomorphisms therein exist
by Lemma \ref{UNIQINAN LEM} so that, after assembly, one obtains the dashed isomorphism on the left diagram.
\[
\begin{tikzcd}
	\Omega[T^1] \ar{d}[swap]{\simeq} \ar{r} &
	\Omega[T] \ar{r}{e} \ar[dashed]{d}[swap]{\simeq} &
	N \mathcal{P}
&
	\Omega[T^1_v] \ar{d}[swap]{\simeq} \ar{r} &
	\Omega[T_v] \ar{r}{e_v} \ar[dashed]{d}[swap]{\simeq} &
	N \mathcal{P}
\\
	\Omega[T^2] \ar{r} &
	\Omega[T] \ar{ru}[swap]{e} &
&
	\Omega[T^2_v] \ar{r} &
	\Omega[T_v] \ar{ru}[swap]{e_v} &
\end{tikzcd}
\]
But since $N \mathcal{P}$ is $\Sigma$-free,
no such isomorphism can exist whenever $E^1 \neq E^2$.
\end{proof}
...
To enrich $N: \dSet \rightleftarrows \Op$ from Definition \ref{OT_DEF} to a Quillen adjunction out of $\sOp$,
we need a topological enrichment of the inclusion $\Omega \into \Op$.
We do this by constructing the free simplicial resolution \footnote{Also called the \textit{Godemont} resolution, c.f. \cite[\S 8.3]{BM06}.}
...
\begin{proof}
      ...
      The result then follows from the description of (equivariant) simplicial equivalences before Proposition \ref{JB_PROP}.
...
\begin{proof}[Proof of Lemma \ref{COMUOTOHOM PROP}]
      ...
      Proposition \ref{JB_PROP} \ref{SFIB_JEQ_LBL} % \cite[Prop. 4.5(iii)]{BP_edss}
...
We end this section by building the first step in our understanding of the homotopical nature of this filtration.
...
Recall that a map $f: X \to Y$ in $\Sym^{G, \mathfrak C}(\V)$ is a \textit{level genuine (trivial) cofibration} if
$f(\vect C)$ is a genuine trivial cofibration in $\V^{\Aut(\vect C)}_{gen}$ for all $\mathfrak C$-signatures $\vect C \in G \ltimes \Sigma_{\mathfrak C}$. 
\begin{lemma}[{cf. \cite[5.73]{BP_geo}}] % EXMAINLEM LEM
      \label{EXMAINLEM LEM}
      Suppose $\mathcal{V}$ is a cofibrantly generated closed monoidal model category
      with cellular fixed points and
