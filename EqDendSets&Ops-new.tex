...
\begin{definition}
      ...
      We say $\F$ \textit{has units} if $H \in \F_1$ for all $H \leq G$. \footnote{
        As in \cite{BP_geo} and \cite{BH15}, one often restricts to \textit{$G$-graph families}; see Definition \ref{GRAPHSUB_DEF}.}. %\ref{COR_GGRAPH_DEF}
...
\begin{corollary}[{cf. \cite[Remark 4.6]{Cav}, \cite[{\color{red}Lemma }1.16]{CM13b}}]
...
\todo[inline]{was this condition hidden in \cite{BP_edss}?}
...
it is $\langle \partial_{U_v} p\rangle \subseteq N \mathcal{O} \amalg_{\zeta_!B (\eta)} \zeta_!B$; % \O \amalg_{\mathfrak{C}}B$;
...
is an \todo{non-degen?} elementary dendrex, 
...
Let $e \colon \Omega[U^e] \to N \mathcal{P}$ be a \todo{non-degen?} non-degenerate elementary dendrex. 
We write
...
In particular,  
$\langle p \rangle \subseteq \langle e \rangle$ \todo{don't we already know this from Remark \ref{ELEMLABEL REM}?}.
\end{lemma}
...
Note that since the $e_v$ are non-degenerate and canonical, $e$ is non-degenerate and elementary. % then so is $e$.
...
\todo{non-degen?} elementary subpresheaves 
$\langle e \rangle$
...
$N \mathcal{O} \amalg_{\zeta_!B (\eta)} \zeta_!B \to N \mathcal{P}$,
and moreover $\P$ being $\Sigma$-cofibrant implies that $N \mathcal P$ is $\Sigma$-free. \todo{elaborate}
...
so (Ch2),(Ch3) follow.
\todo[inline]{how does Ch3 follow?
  $\langle \partial_{U^e-\Xi^e}e \rangle \subseteq \langle \bar e \rangle$ doesn't imply 
  $\langle \partial_{U^e-\Xi^e}e \rangle \subseteq \langle X_{\langle e \rangle}$, does it?}
\todo[inline]{how is $\langle e \rangle \subseteq  X_{<\langle e\rangle}$ possible, cf. (b)}
...
$e^{-1}(X_{<\langle e\rangle})$
follows from $N \mathcal{P}$ being $\Sigma$-cofibrant \todo{elaborate}
...
a rephrasing of Lemma \ref{MODSTRCOMRE LEM}(i) 
(included for the sake of Remark {\color{red} add}).
...
\begin{proof}
Note first that, for $X\in \mathcal{C}$ and $Y \in \mathsf{Alg}_F$,
the natural isomorphisms
$\mathsf{Alg}_{\bar{F}}(\bar{F}X,Y) 
\simeq \mathcal{C}(X,Y)
 \simeq \mathsf{Alg}_{F}(FX,Y)$
show that the left adjoint sends free $\bar{F}$-algebras
$\bar{F} X$ to $FX = P \bar{F}X$,
and similarly for free maps.
Moreover, the maps 
$\mathsf{Alg}_{\bar{F}}(\bar{F}X,Y)
\to
\mathsf{Alg}_{\bar{F}}(\bar{F}\bar{F}X,Y)$
and
$\mathsf{Alg}_{F}(FX,Y)
\to
\mathsf{Alg}_{F}(F\bar{F}X,Y)$
induced by the multiplications
$\mu \bar{F} \bar{F} \Rightarrow \bar{F}$,
$\mu F \bar{F} \Rightarrow F$
are adjoint to the two composites below.
Here the left maps are induced by the functors $\bar{F}$ and $F$, the middle maps are induced by $\bar{F} \Rightarrow F$ (so that the square commutes by naturality) and the right map by the algebra action $FY \to Y$.
\[
\begin{tikzcd}[row sep=0]
	&
	\C(\bar{F}X,\bar{F}Y) \ar{rd}
\\
	\C(X,Y) \ar{ru} \ar{rd}
&&
	\C(\bar{F}X,FY) \ar{r}
...
\end{tikzcd}
\]
In other words, the left adjoint sends
$\bar{F}\bar{F}X \xrightarrow{\mu_X}\bar{F}X$
to 
$F\bar{F}X \xrightarrow{\mu_X} FX$.

Since any $\bar{F}$-algebra $X$ with multiplication
$\bar{F}X \xrightarrow{m} X$
is canonically described as the coequalizer
$X = coeq \left( \bar{F} \bar{F} X \overset{\mu}{\underset{\bar{F}m}{\rightrightarrows}} \bar{F} X\right)$
we have that the left adjoint applied to $X$ gives
\[
coeq 
 \left(
...
\left(
 \begin{tikzcd}[column sep=14pt]
	\bar{F} \bar{F} X
	\ar[shift left=.3em]{r}{\mu}
	\ar[shift right=.3em]{r}[swap]{\bar{F}m} &
	\bar{F} X
 \end{tikzcd}
\right)
\right)
...
To check (ii),(iii),(iv), we consider the diagrams
\[
\begin{tikzcd}
	FF\bar{F} X
	\ar[shift left=.3em]{d}{F\mu}
	\ar[shift right=.3em]{d}[swap]{FFm}
	\ar{r}{\mu\bar{F}}
&
	F\bar{F}X
	\ar[shift left=.3em]{d}{\mu}
	\ar[shift right=.3em]{d}[swap]{Fm}
&%
	\bar{F} \bar{F} X
	\ar[shift left=.3em]{d}{\mu}
	\ar[shift right=.3em]{d}[swap]{\bar{F}m}
	\ar{r}
&
	F \bar{F} X
	\ar[shift left=.3em]{d}{\mu}
...
\begin{enumerate}[label=(\roman*)]
\item on a $T$-algebra $X$ with multiplication $TX \rightarrow{m} X$,
...
shows that the multiplication 

{\color{red} HERE}


Proposition \ref{ALTMULT PROP}
says that the monoidal structure on $F = P \bar{F}$
is completely determined by the monoidal structures on $P$, $\bar{F}$ and the twist map.
In other words, given monads $P$, $\bar{F}$ and a twist map as in 
Remark \ref{TWISTMAP REM}, one should be able to build a monad structure on $F = P \bar{F}$
with multiplication given as in \eqref{ALTMULT EQ}.
\end{remark}
...
\begin{remark}
The commutativity of the diagrams in \eqref{DOUBLETWIST EQ}
are the only requirements needed for
the multiplication prescribed by \eqref{ALTMULT EQ}
to be associative.
\end{remark}
...
\]
\item 
if $P,\bar{F}$ are as in Definition \ref{AMALGMON DEF}
and $F=P\bar{F}$, $T=\bar{F}$,
then \eqref{ALGMONABS EQ} becomes simply
\[
PPX \xrightarrow{\mu} PX
\]
for the monad structure on $P$, regardless of whether the functors are known to preserve reflexive coequalizers 
(this is because 
$
\check{coeq} \left( F T X 
\overset{\mu}{\underset{Fm}{\rightrightarrows}}
F X\right)
$
then becomes split after forgetting to $\mathcal{C}$)

\item 
if $P,\bar{F}$ are as in Definition \ref{AMALGMON DEF}
and $F=P\bar{F}$, $T=P$,
then the monad \eqref{ALGMONABS EQ}
restricts to the Klesli category 
$\mathsf{Kl}_P$
where it is given by $\bar{F}$ on objects
and on a map
$A \xrightarrow{f} P B$ it is given by
$\bar{F} A \xrightarrow{\bar{F}f} \bar{F} P B \xrightarrow{\tau} P \bar{F} B$, so that in particular the first half
Proposition \ref{ALTMULT PROP} (iii) say $\bar{F}$ preserves free arrows, and is described by $\bar{F}$ on them (which in particular gives unitality of $\bar{F}$).
Similarly, the first half of Proposition \ref{ALTMULT PROP}(ii)
show that $\bar{F}$ is compatible with composition in
$\mathsf{Kl}_P$.
Lastly, the second halves of Proposition \ref{ALTMULT PROP}(ii)(iii) show that the unit and multiplication of $\bar{F}$, regarded as free maps in
$\mathsf{Kl}_P$, do define natural transformations in the Kleisli category.
\end{itemize}
...
\check{coeq}
\left(
P \bar{F} \bar{F} P X \rightrightarrows P \bar{F} \bar{F} X
\right)
\]
...
\begin{remark}
Let $T$ be a monad, recall that the Kleisli category
$\mathsf{Kl}_T$ has as objects the objects of $\mathcal{C}$
and an arrow
$A \rightsquigarrow B$
...
\]
There is then a canonical functor
\[
\begin{tikzcd}[row sep = 0pt]
	\mathsf{Kl}_T \ar[hookrightarrow]{r}{\iota} &
	\mathsf{Alg}_T
\\
	A \ar[mapsto]{r} &
	FA
...
\begin{remark}
Let $\psi \colon T \to F$ be a map of monads.

One then has a natural forgetul functor 
...
$TY \xrightarrow{\psi Y} FY \xrightarrow{m} Y$.
...
$A \xrightarrow{f} TB \xrightarrow{\psi B} FB$.

One then has a (non-commutative) square
...
\end{tikzcd}
\end{equation}
where the vertical maps $\iota$ are the standard fully-faithful inclusions.
The notation $\psi_!$ is motivated by the fact that 
$\psi_!$ provides a partial left adjoint to $\psi^{\**}$
along the subcategory
$\mathsf{Kl}_T \overset{\iota}{\hookrightarrow} \mathsf{Alg}_T$,
i.e.
one has isomorphisms
\[
	\mathsf{Alg}_T(\iota A, \psi^{\**} Y) =
	\mathsf{Alg}_T(T A, \psi^{\**} Y) \simeq
	\mathcal{C}(A,Y) \simeq
	\mathsf{Alg}_F(FA,Y) =
	\mathsf{Alg}_F(\iota \psi_! A,Y)
\]
which are natural in $A \in \mathsf{Kl}_T$, $Y \in \mathsf{Alg}_F$
(note that the interesting claim is that this is natural on all maps in $\mathsf{Kl}_T$).
\end{remark}
...
Definition \ref{AMALGMON DEF}. Then:
\begin{enumerate}[label=(\roman*)]
\item Writing $\psi \colon P \to F$ for the standard map 
in Definition \ref{AMALGMON DEF}(i)
then, in addition to the standard maps in 
\eqref{PARTADJ EQ},
there is a functor 
\[
\begin{tikzcd}[row sep=0]
...
\\	
	...
	\ar[mapsto]{r}
&
	 (\bar{F}A \xrightarrow{f} \bar{F}FB \xrightarrow{\mu}
	 FB = P\bar{F} B)
\end{tikzcd}
\]
...
\end{tikzcd}
\end{equation}
the top horizontal maps are adjoint, with adjunction unit and counit given by the composites
\[
	A \xrightarrow{\eta} 
	\bar{F} A \xrightarrow{\eta \bar{F}}
	P \bar{F} A
\qquad
	\bar{F} B \xrightarrow{\psi}
	F B
\]
and the composites below coincide.
\[
...
\qquad
	...
	\mathsf{Alg}_P
\]
\item Writing $\psi \colon \bar{F} \to F$ for the standard map 
in Definition \ref{AMALGMON DEF}(i) then,
in addition to the standard maps in \eqref{PARTADJ EQ},
there is a functor
\[
\begin{tikzcd}[row sep=0]
	\mathsf{Alg}_{\bar{F}}
...
&
	\mathsf{Alg}_F
\\
	(X,\bar{F}X\xrightarrow{m}X) \ar[mapsto]{r} 
&
	(PX,
	FPX \xrightarrow{\mu}
	F X = P \bar{F} X \xrightarrow{Pm} PX)
\\	
	(A \xrightarrow{f} B)
	\ar[mapsto]{r}
&
	 (P A \xrightarrow{Pf} PB)
\end{tikzcd}
\]
such that in the diagram
...
\end{tikzcd}
\end{equation}
the top horizontal maps are adjoint, with adjunction unit and counit given by
\[
	X \xrightarrow{\eta} 
	PX 
\qquad
...
and the composites below coincide.
\[
	\mathsf{Kl}_{\bar{F}} \xrightarrow{\psi_!} 
	\mathsf{Kl}_F \xrightarrow{\iota} 
	\mathsf{Alg}_F
\qquad
	\mathsf{Kl}_{\bar{F}} \xrightarrow{\iota}
	\mathsf{Alg}_{\bar{F}}  \xrightarrow{\psi_!} 
	\mathsf{Alg}_F
\]
\end{enumerate}
\end{proposition}
...
\begin{remark}
Slightly abusing notation by also writing 
$\bar{F}$ for the monad on $\mathsf{Kl}_{P}$,
one has that 
$\mathsf{Kl}_{\bar{F}}(\mathsf{Kl}_{P}) = 
\mathsf{Kl}_F$.
\end{remark}
...
The usefulness of Definition \ref{AMALGMON DEF}
comes from the observation that, using the twist map in Remark \ref{TWISTMAP REM},
$P$ can also be regarded as a monad on 
$\mathsf{Alg}_{\bar{F}}(\mathcal{C})$.
Indeed, given a $\bar{F}$-algebra $X$, 
$PX$ is again a $\bar{F}$-algebra with multiplication given by
$\bar{F} P X \xrightarrow{\tau_X} P \bar{F} X \xrightarrow{P m_X} PX$.

One thus has the following diagram of left adjoints between algebra categories.
\begin{equation}\label{COMULEFTADJ EQ}
\begin{tikzcd}
	\mathcal{C} \ar{r}{\bar{F}} \ar{d}[swap]{P} &
	\mathsf{Alg}_{\bar{F}}(\mathcal{C}) \ar{d}{P}
\\
	\mathsf{Alg}_{P}(\mathcal{C}) \ar{r}{F^r}
&
	\mathsf{Alg}_{F}(\mathcal{C})
\end{tikzcd}
\end{equation}
The functor $F^r$ (in fact monad), which has not yet been described, is computed by the coequalizer
$F^r(Y) = coeq \left(FP(Y) \rightrightarrows F(Y) \right)
= coeq \left(P\bar{F}P(Y) \rightrightarrows P\bar{F}(Y) \right)
$

\begin{proposition}
The square \eqref{COMULEFTADJ EQ} commutes, yielding
$F^r P \simeq P \bar{F}$.

Moreover, the induced identifications
$(F^r)^{\circ n} P \simeq P \bar{F}^{\circ n}$
are compatible with the monoidal structure. 
For instance, the square
\begin{equation}
\begin{tikzcd}
	F^rF^r P \ar{r}{\simeq} \ar{d}[swap]{\mu} &
	P \bar{F} \bar{F} \ar{d}{\mu}
\\
	F^r P \ar{r}{\simeq}
&
	P \bar{F}
...
\begin{remark}
By definition the left functor $P$, top functor $\bar{F}$, 
and diagonal functor $F=P\bar{F}$
in \eqref{COMULEFTADJ EQ} are monadic 
(i.e. they identify their target as the category of algebras over the source for the adjunction monad).
But then by Beck's monadicity theorem \cite[VI.7 Thm. 1]{McL},
it is immediate that the right functor $P$ and bottom functor $F^r$ in \eqref{COMULEFTADJ EQ} are likewise monadic.
\end{remark}
...
the mild generalization of \textit{weak indexing systems} of the authors \cite[\S 9]{Per18}, \cite[\S4.4]{BP_geo} (and independently identified by Gutierrez-White \cite{GW}).
