...
\begin{enumerate}
\item Reedy generating (trivial) cofibrations in $\mathcal M^{\Delta^{op}}$ are
      \[
            (\partial \Delta[n] \to \Delta[n]) \square i
      \]
      where $i$ is a generating (trivial) cofibration in $\mathcal M$.
\item if $X \in \mathcal M^{\Delta^{op}}$ is Reedy fibrant, then all vertex maps $X_m \to X_0$ are weak equivalences in $\mathcal M$
      (as $(\Delta[0] \to \Delta[n]) \box i$ is a trivial cofibration in $\mathcal M^{\Delta^{op}}$)
      here!
\end{enumerate}
