...
If $e \colon \Omega[T^e] \to N \mathcal{P}$
is an elementary dendrex,
the tree $T$ can be naturally regarded as an
...
\cite[Prop. 5.49]{BP_geo}, there is hence an unique alternating tree $T^a$ together with a tall planar $B$-inert label map $T^a \to T^e$,
and it then follows that
$\partial_{T^a} e$ is an alternating dendrex so that
$\langle \partial_{T^a} e \rangle = \langle \partial_r e \rangle_{\chi}$.
%
...
\begin{definition}
Let $e \colon \Omega[T^e] \to N \mathcal{P}$ be a non-degenerate elementary dendrex.
We write
$\Xi^e \subseteq \boldsymbol{E}^{\mathsf{i}}(T^e)$
for the subset of inner edges of $T^e$ which are adjacent to at least one $B$-labeled vertex.
\end{definition}
...
Let $e \colon \Omega[T^e] \to N \mathcal{P}$ be a non-degenerate elementary dendrex, 
$T^a \to T^e$ be as in Remark \ref{ELEMLABEL REM}, 
and write $a = \partial_{T^a} e$.
Since $T^a$ is alternating, all of its inner edges are adjacent to a 
$B$-labeled vertex. 
Therefore, the fact that $T^a \to T^e$ is a tall $B$-inert label map
implies that $\Xi^e$ consists of the inner edges in the image of $T^a$.
\end{remark}
...
is canonical iff $\Xi^e = \boldsymbol{E}^{\mathsf{i}}(T)$.
\end{proposition}

\begin{proof}
We use the notation in Remark \ref{XIEREDEF REM}.
Since $\langle a\rangle = \langle \partial_r e \rangle$, 
$\langle e \rangle$
is canonical 
iff
 $\langle a\rangle = \langle e \rangle$, i.e.
iff
$T^a \to T^e$ is a degeneracy. 
But since a map is a degeneracy iff it is tall and surjective,
it follows that $T^a \to T^e$ is a degeneracy
iff its image includes all inner edges, i.e. iff $\Xi^e = \boldsymbol{E}^{\mathsf{i}}(T)$.
\end{proof}
...
In particular,
$\langle \partial_{T^c} e \rangle = \langle \partial_r e \rangle_{\chi}$,
and hence 
...
$T^c \to T^a \to T$
must be a face (or else $c = \partial_{T^c} e$ would be degenerate) and is tall, hence an inner face, finishing the proof.
\end{proof}
...
and $\mathsf{lr}(T^c) \to T^{e}$ a tall map.
...
\begin{tikzcd}
	\Omega\left[\mathsf{lr}(T^c)\right] \ar{r} \ar{rd}&
...
\begin{proof}
Note first that commutativity of \eqref{UNIQINAN EQ} implies
...
\[
\begin{tikzcd}
	\Omega[\mathsf{lr}(T^c)] \ar{r}{\simeq} \ar{rd}[swap]{\simeq} &
...
\begin{proof}
We abbreviate $T' = T^e -E$. 
Note first that, 
by applying  
``tall map followed by outer face'' factorization to
$T' \to T^{\bar{e}}$
to obtain
$T' \to \tilde{T} \to T^{\bar{e}}$,
the dendrex $\partial_{\tilde{T}} \bar{e}$
is still elementary (being an outer face of an elementary dendrex),
so we reduce to the case where $T' \to T^{\bar{e}}$ is a tall map.
...
By Lemma \ref{UNIQINAN LEM} we can thus find the dashed arrow in \eqref{UNIQINAN2A EQ} for each $v \in \boldsymbol{V}(T')$ and thus, 
by 
\cite[Prop. 3.41]{BP_geo},
...
which is elementary, so the claim is clear.
...
one has
$\Xi^c = \boldsymbol{E}^{\mathsf{i}}(T^c)$
and
$\langle \partial_r e \rangle = \partial_{T^c-\Xi^c} c$,
