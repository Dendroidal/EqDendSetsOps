...
then $\langle \partial_V^{\**} b_i\rangle \subseteq A_{<i}$
...
implies that $\Xi^e$ consists of those inner edges which are in the image of $T^a$.
\end{remark}
...
Since $\partial_{T}e = p$ was chosen to be degenerate,
...
\begin{proof}
Let $T^a \to T^e$ be as in Remark \ref{ELEMLABEL REM}.
...
$T^c \to T^a \to T$
must be a face (or else $c = \partial_{T^c} e$ would be degenerate) and is tall, and is hence an inner face.%, finishing the proof.
\end{proof}
...
by Lemma \ref{FORSAKEN LEM} there is an inner face $T^{\bar{c}} \to T^{e}$
...
from the fact that the factorization of degenerate dendrices through non-degenerate dendrices is unique up to (unique) isomorphism \cite[Prop. 5.62]{Per_eds}.
%
...
For each vertex $T'_v \hookrightarrow T'$ we apply the 
``inner face followed by outer face factorization''
to the composites
$T'_{v} \hookrightarrow T' \to T^{e}$,
$T'_{v} \hookrightarrow T' \to T^{\bar{e}}$
to get
$T'_{v} \to T_{v}^e \hookrightarrow T^e$,
$T'_{v} \to T_{v}^{\bar{e}} \hookrightarrow T^{\bar{e}}$
and, further writing
...
\end{tikzcd}
\end{equation}
We now claim that $e_v$ is canonical. Indeed, $e_v$ is non-degenerate elementary since it is an outer face of $e$, which is also non-degenerate elementary. And since all inner edges of $T^e_v$ are in 
...
(or $T'_v \to T_v$ would be a degeneracy),
by Lemma \ref{UNIQINAN LEM} there is a dashed arrow in \eqref{UNIQINAN2A EQ} for each $v \in \boldsymbol{V}(T')$ and thus, 
by 
\cite[Prop. 3.41]{BP_geo},
...
If $e \colon \Omega[T^e] \to N \mathcal{P}$ is a non-degenerate elementary dendrex
and $E \subseteq \Xi^e$,
then 
...
and
$\langle \partial_r e \rangle = 
\langle \partial_{T^c-\Xi^c} c \rangle$
...
and let $e \colon \Omega[T] \to N \mathcal{P}$
...
$\Xi^e$ are as given in Definition \ref{XIEDGES DEF}.

We now check the characteristic edge conditions. (Ch0.2) is clear.


For (Ch1), since outer faces of elementary dendrices are again elementary, we reduce to the case 
$\bar{V}=U^e$, or else it would be $\partial_{V} e \in \langle \bar{e} \rangle$
for some
$\langle \bar{e} \rangle \subsetneq \langle e \rangle$.
But if $\bar{V}=U^e$, the assumption in (Ch1) is that
$\Xi^e = \emptyset$, so that $e$ must either be a dendrex where all vertices map to $\mathcal{O}$, i.e. $e \in N \mathcal{O}$,
or $U^e$ is a corolla and its vertex maps to $B$, i.e. 
$e \in B$.
In either case, one has
$\langle e \rangle \subseteq A = B \cup N\mathcal{O}$, and (Ch1) follows.
