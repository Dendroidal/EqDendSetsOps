...
$\mathsf{Fun}(\Sigma_{C}^{op},\mathcal{V})$.
\end{remark}
...
If $\mathcal{V}$ is a closed symmetric monoidal category, the free algebra monad $\mathbb{F}$ on $\mathsf{Sym}(\mathcal{V})$ 
...
The main result of this section applies to a large class of $G$-graph families, with only the following minor condition.
...
\begin{theorem}
      \label{MODEL_THM}
      Fix an $G$-graph family $\F = \set{\F_n}$ with units.
      Let $(\V, \otimes)$, denote either $(\sSet, \times)$ or $(\sSet_{\**}, \wedge)$.
      Then there exists a cofibrantly generated model structure on the category $\Op^G(\V)$,
...
\begin{proof}
      ...
      $(iii)$ $\sSet$ and $\sSet_{\**}$ have cofibrant symmetric pushout powers by \cite[Remark 6.18]{BP_geo},
      ...
      and
      \ref{GENSET_LBL} $\sSet$ and $\sSet_{\**}$ has a generating set of intervals
...
\begin{equation}
      ...
      % \end{cases}
\end{equation}
where the right $H$-action on the disjoint union is defined as follows:
...
Before proving 2-out-of-3 for weak equivalences, we make a brief digression about ``homotopies'' in any $\V$-category $\mathcal C$.
...
\begin{definition}
      ...
      For an operad $\O \in \Op(\V)$, we say operations $f,g \in \O(c;d)$ are \textit{homotopic} if they are homotopic in $j^{\**}\O$. 
\end{definition}
...
To do so, we will need to translate between
...
\todo[inline]{should ``homotopy in general model cat'' be moved to preliminaries? part of this section?}
\begin{definition}
      \label{EQUIV_DEF}
...
\begin{remark}
      We note that \eqref{J11_CYL_EQ} in fact states the fact that any arrow in $\mathcal C$ which factors through an interval $\mathbb J$
      \[
            \mathbb A \longto \mathbb J \longto \mathcal C
      \]
...
\begin{lemma}
      ...
      \label{REF_VIRT_LEM}
      Suppose the $G$-graph family $\F$ has units.
...
\begin{proposition}
      ...
      Then, for any $G$-graph family $\F$ with units,
...
\begin{corollary}
      Suppose $\F$ contains all trivial subgroups $H \leq G \times \Sigma_m$.
      If $F: \O \to \P$ is a weak $\F$-equivalence, then $\pi_0(F^H)$ is an equiavlence of operads for all $H \leq G$.
\end{corollary}
...
It remains to show that $\sSet_{\**}$ satisfies the coherence condition.
\todo[inline]{options: just reference \cite[Prop. 2.24]{BM13} - though I'm not sure everything there works - or use these assumption-heavy results.}
\begin{corollary}
      \label{PTSSETCOH_COR}
...
We now have all of the pieces to prove Theorem \ref{INTRO_MODEL_THM}.
...
\begin{proof}
      ...
      imply that where $\V$ satisfies the coherence axiom, 
      the (semi)-model structures from Theorem \ref{MODEL_THM} are Dwyer-Kan.
\end{proof}
...
\begin{remark}
      ...
      replacing $\mathcal H$ with an arbitrary $\V$-interval $\mathbb J$ and $\mathscr F$ with $\mathbb A$.
\end{remark}
...
% \begin{corollary}
%       \label{PTSSET_COR}
%       $\sSet_{\**}$ has cellular fixed points, and moreover for any $G$-graph family $\F$ with units,
%       $\Op^G(\sSet_{\**})$ has the $\F$-Dwyer-Kan model structure.
% \end{corollary}
% % \begin{proof}
% %       Combine Lemmas \ref{PT_PROP_LEM}, \ref{PT_CELL_LEM}, and \ref{P_P_WE_LEM} with Proposition \ref{PT_COH_PROP}.
% % \end{proof}
...
\begin{example}
      ...
      \item $(\sSet, \times)$ satisfues this by an argument of Joyal (see Proposition \ref{SSET_COH_PROP} below),
            as does $(\sSet_{\**}, \wedge)$ (see Corollary \ref{PTSSETCOH_COR} below). 
...
\todo[inline]{clean this up. What do we actually need?}
{\color{red} % ------------------------------ COLOR RED ------------------------------
  we never actually proved that $\sSet_{\**}$
  \begin{enumerate}
  \item is cellular (follows from below)
  \item is a monoidal model cat (follows from work of Hovey)
  \item has cofibrant symmetric pushout powers (probably?)
  \end{enumerate}
  how much of that do we want to say?
} % ------------------------------ COLOR RED ------------------------------
come back here!
...
is also monoidal via the smash product.
...
We recall the following (e.g. \cite{Hov99})
...
\begin{lemma}
      ...
      \item a monoidal model category {\color{red} (needs that $1_\V = \**$ and is cofibrant)}
      \item a cofibrant unit {\color{red} (needs that $\**$ is cofibrant)},
      \item cofibrant symmetric pushout powers {\color{red} (no idea)},
      \item left or right proper {\color{red} (maybe?)}
      \end{enumerate*}
\end{lemma}
