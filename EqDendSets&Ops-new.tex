...
$\mathsf{Sym}^\bullet(\mathcal{V})$ which is suitably fibered as in \S \ref{FIBCAT_SEC} over $\mathsf{F}$
...
Additionally, by fibering over a fixed $G$-set $\mathfrak{C} \in \mathbb{F}^G$,
...
In \S \ref{SSEQ_SEC}, we build the category $\Sym(\V)$, % Definition \ref{SYMV_DEF}
describe the fibered monad $\mathbb F$, % \eqref{FREEOP_EQ}, elaborated in Appendix \ref{MONAD_APDX}.
and determine the fibers of the equivariant monad $\mathbb F^G$. % \eqref{FGC_EQ}.

In \S \ref{EHT_SEC}, we identify several pieces of equivariant homotopy theory needed in
\S \ref{SSEQC_MS_SEC} and \S \ref{OPC_MS_SEC} to build model structures on the color-fixed categories $\mathsf{Sym}^{G,\mathfrak{C}}(\mathcal{V})$ and $\mathsf{Op}^{G,\mathfrak{C}}(\mathcal{V})$.
...
In this short subsection, we provide a description of the left adjoint $\check f_! \colon \Op^{\mathfrak D} \to \Op^{\mathfrak C}$
from Remark \ref{OP_MAP REM}
for an inclusion $f \colon \mathfrak C \to \mathfrak D$ of colors,
