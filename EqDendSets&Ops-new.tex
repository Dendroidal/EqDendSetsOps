...
such that each $\O(C)$ has an action by the subgroup $\Aut(C) \leq G \times \Sigma_n$.
...
This result formally extends to diagrams $\V^{\mathcal G}$ where $\mathcal G$ is a groupoid.
\begin{definition}
      For any groupoid $\mathcal G$, a \textit{family} of subgroups of $\mathcal G$ is
      a collection $\F = \set{\F_d}$ of families of subgroups $\F_d$ of $\Aut(d)$ for all $d \in \mathcal G$,
      such that for any map $\alpha: d \to d'$, $\alpha^{\**}\F_{d'} \subseteq \F_d$ (or equivalently, equality).
\end{definition}

\begin{proposition}
      \label{TRANS_MODEL_PROP}
      Let $\V$ be a cofibrantly generated model category with cellular fixed points, $\mathcal G$ a groupoid, and $\F$ a family of subgroups of $\mathcal G$.
      Then the category of diagrams $\V^{\mathcal G}$ has an \textit{$\F$-model structure} $\V^\G_\F$, where
      a map $f: X \to Y$ is a
      weak equivalence (resp. fibration) iff $f(d): X(d) \to Y(d)$ is so in $\V^{\mathsf{Aut}(d)}_{\F_d}$ for each $d \in \mathcal G$.

      Moreover, if $X \in \V^{\mathcal G}$ is cofibrant, it is also levelwise cofibrant.
\end{proposition}
\begin{proof}
      This is the model structure transferred along the adjunction
      \begin{equation}
            \begin{tikzcd}
                  \V^{\mathcal G} \leftrightarrows
                  \mathop{\prod}\limits_{d \in \mathcal G}\V^{\mathsf{Aut}(d)}_{\F_d}
            \end{tikzcd}
      \end{equation}
      ...
      \begin{equation}
            I^{\mathcal G}_\F = \sets{\mathcal G(d,-) \cdot_{\mathcal G(d,d)} \mathcal G(d,d)/H \cdot i}{d \in \mathcal G, i \in I, H \in \F_d}
      \end{equation}
...
any $G$-graph system automatically extends to a family for the groupoid $\mathcal G = G \ltimes \Sigma_{\mathfrak C}^{op}$,
by defining for any $\mathfrak C$-signature $\ksi = (c_1,\dots,c_n;c_0)$ of length $n+1$,
\[
      \F_\ksi = \F_n \cap \Aut_{G \times \Sigma_n}(\ksi)
\]
the graph subgroups $\Gamma \in \F_n$ such that $\Lambda \leq \Aut_{G \times \Sigma_n}(\ksi)$.
...
\begin{definition}
      ...
      $\mathfrak C$ a $G$-set, and $\F$ a $G$-graph system.
...
by constructing an appropriate filtration (Proposition \ref{FILT_PUSH_PROP}) and
showing that the levels are appropriately well-behaved (Lemma \ref{EXMAINLEM LEM}).
We relegate this discussion to Appendix \ref{MONAD_APDX}.
...
\begin{convention}
      ...
      and thus has cofibrant replacements.
\end{convention}
...
\todo[inline]{integrate this example better}
...
Fix a graph subgroup $\Gamma \in \F_n$ of $G \times \Sigma_n$,
and consider the $G$-set of colors $\mathfrak C_\Gamma := G \cdot_\Gamma \langle n \rangle$;
let $C_0$ denote the signature $([e,1],[e,2],\dots,[e,n];[e,0])$.
...
\todo[inline]{can use \cite[5.37]{BP_geo} to show that $1_\V^{\otimes n} \to \mathbb C^{\otimes n}$ is a genuine $\Sigma_n$-cofibration;
  then it is clear that $\mathbb C^{\otimes n}$ is genuine cofibrant,
  then 2-out-of-3 makes it so $\mathbb C^{\otimes n} \to 1_\V$ is a genuine equivalence.
  Hence $(\mathbb C^{\otimes n})^\Gamma$ for any $\Gamma \leq \Sigma_n$ is a cylinder object}
\begin{lemma}
      \label{ASSEM_HOM_LEM}
...
\todo[inline]{what does this mean? And if this htpy isn't genuine equivariant, this is not relevant to our story}
\begin{remark}
      \label{ASSEM_HOM_REM}
...
\begin{remark}
      We note that virtual equivalence does not depend on the choice of fibrant replacement or the choice of $\V$-interval:
      \todo[inline]{see diagram in Cav about generating set of interval stuff - do we need to worry about that here?}
      as there are always weak equivalences connecting any two options.
\end{remark}
...
\todo[inline]{instead: say that ANY fibrant replacement of $\mathcal C$ give you the same notion. then shoulnd't need this remark}
\begin{remark}
      \label{HVIRT_REM}
...
\begin{lemma}
      [{cf. \cite[4.10]{Cav}}]
      \label{CAV_4.10_LEM}
      $H$-equivalence and virtual $H$-equivalence define equivalence relations on $\mathcal C_\O^H$.
\end{lemma}
The key feature used here is that a stacking of intervals again forms an interval.
\textit{pushout trick and Cav4.14}
% \begin{proof}
%       {\color{OliveGreen}
...
\end{lemma}
The key idea here is that for any $\V$-interval $\mathbb J$, the two objects are isomorphic in $\pi_0(\mathbb J)$.
% \begin{proof}
...
\begin{lemma}
      ...
      \label{REF_VIRT_LEM}
      Let $\F$ be a $G$-graph system. %Suppose the $G$-graph system $\F$ has units.
...
\begin{lemma}
      ...
      and $C = (c_1, \dots, c_n; c)$ a $\mathfrak C$-signature.
      Let $H$ denote the projection of$\Stab(C) \leq G \times \Sigma_n$ onto $G$.

      Then, for any $d \in \mathfrak C^H$ and $H$-homotopy equivalence $\alpha: 1_\V \to \O^H(c,d)_f$ between $c$ and $d$,
      there exists a natural zig-zag of weak equivalences in $\V^{\Aut(C)}_{\mathrm{Gr}_C}$ %
      \footnote{Recall the notation $\mathrm{Gr}_C$ from Definition \ref{GGRAPHFAM_DEF}.}
      between $\O(C)$ and $\O(D)$, where $D = (c_1, \dots, c_n; d)$,
      %
      % $c,d \in \mathfrak C_\O$, 
      % $H = \Stab(c)$, and suppose $c$ and $d$ are homotopy $H$-equivalent (so in particular $H \subseteq \Stab(d)$).
      %
      % Then, for any $\mathfrak C_\O$-signature $C = (c_1,\dots, c_n;c) \in G \ltimes \Sigma_{\mathfrak C_\O}$,
      % there exists a zig-zag of weak equivalences in $\V^{\Aut(C)}_{\mathrm{Gr}_C}$
      % between $\O(C)$ and $\O(D)$,
      % where $D = (c_1,\dots, c_n;d)$.
      % %
      % Additionally, any functor $F: \O \to \P$ induces a natural zig-zag of weak equivalences in $\V^{\Stab(C)}_{\mathrm{Gr}_C}$
      % between $\P(F(C))$ and $\P(\F(D))$.
\end{lemma}
\begin{proof}
      Using lifting, we can show that if $c$ and $d$ and homotopy $H$-equivalent in $\O$, they are
      certainly homotopy $H$-equivalent in the functorial bifibrant replacement $\O_{fc}$. 
      Thus we may assume we have representing maps
      $\alpha: 1_\V \to \O^H(c;d)_{fc}$ and $1_\V \to \O^H(d;c)_{fc}$.
      ...
      \begin{equation}
            ...
            \end{tikzcd}
      \end{equation}
      where the $\simeq$ denote weak equivalences in $\V^{\Aut(C)}_{\mathrm{Gr}_C}$, and $\alpha_{\**}$ is the composite
      \[
            \O(C) \xrightarrow{\alpha} \O(C) \otimes \O(c,d) \longto \O(D).
      \]
      Thus, it suffices to check that $\alpha_{|**}$ is an $\mathrm{Gr}_C$-weak equivalence when $\O$ is bifibrant.

      Let $\beta: 1_V \to \O^H(d,c)$ denote the $H$-homotopy inverse of $\alpha$,
      ...
      \[
            \O(C)^\Gamma \otimes \O(c,d)^H \longto \left( \O(C) \otimes \O(c,d) \right)^\Gamma \xrightarrow{\circ} \O(D)^\Gamma
      \]
      and so in particular the maps $\alpha_{\**}$ and $\beta_{|**}$ are $\Aut(C)$-equivariant.
      Thus, for all $\Gamma \leq \Aut(C)$,  we have homotopies
      \[
      ...
      \begin{equation}
            ...
            \end{tikzcd}
      \end{equation}
      Hence, as both $\O(C)^\Gamma$ and $\O(D)^\Gamma$ are bifibrant by Remark \ref{LEVEL_COF_REM},
      \[
            \alpha_{\**}: \O(C)^\Gamma \leftrightarrows \O(D)^\Gamma : \beta_{\**}
      \]
      are inverse isomorphisms in $\Ho(\V)$ for all $\Gamma \leq \Aut(C)$,
      and thus $\alpha_{\**}$ is a weak equivalence in $\V^{\Aut(C)}_{\mathrm{Gr}_C}$, as desired.
      \todo[inline]{why graph subgroups?}
...
\begin{lemma}
      ...
      Then, for any $\mathfrak C_\O$-signature $C = (c_1,\dots,c_n;c) \in G \ltimes \Sigma_{\mathfrak C_\O}^{op}$,
...
\todo[inline]{how much do we need to say here?}
...
\end{definition}
\todo[inline]{the above definition is garbage}

\todo[inline]{go over again how the non-equivariant $W_!$ intersects with the equivariant one}
...
\todo[inline]{how much do we need to recall here? I guess most of it is to be recalled in ``tame time''...}
