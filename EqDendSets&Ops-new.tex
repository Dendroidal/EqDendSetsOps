...
\todo[inline]{come back: replace $\ksi$ with $C$}
...
\begin{theorem}[{\cite[Thm. 2.10]{Ste16}}]
      Let $\V$ be cofibrantly generated model category, and $\F$ any family of subgroups of $G$ containing the trivial subgroup.
      Then
      \begin{itemize}
      \item the projective model structure on $\V^{\mathsf O_G^{op}}$;
      \item the $\F$-model structure on $\V^G$ exists, where an arrow $f \in \V$ is a weak equivalence or fibration iff each $f^H$ is in $\V$; and
      \item the inclusion $\V^G \xrightarrow{i_{\**}} \V^{\mathsf O_G^{op}}$, $i_{\**}X(G/H) = X^H$ is a right Quillen equivalence.
      \end{itemize}
\end{theorem}      
...
In the homotopy theory of $G$-operads, as indicated in the main result of \cite{Bon_geo}, it is
...
\begin{definition}
      ...
      A \textit{$G$-graph family} is a collection $\F = \set{\F_n}_{n \geq 0}$ of families $\F_n$ of graph subgroups of $G \times \Sigma_n$, one family for each $n$.
\end{definition}
...
In the previous section, we built model structures on the category operads with a single fixed color $G$-set $\mathfrak C$.
We will now generalize and synthesize \cite{BM13}, \cite{Cav}, and \cite{CM13b} to assemble these
into a single model structure on the category of $G$-operads $\Op^G(\V)$ with varying colors.
...
We begin with some mild enriched category theory.
...
Heuristically, we think of an interval as detecting some notion of equivalence, using the model structure on $\V$;
...
Now, recall the notion of a $G$-graph family from Definition \ref{GGRAPHFAM_DEF}.
...
In the many-colored setting, the levels of our operad do not have a full action of $G \times \Sigma_n$, and the $G$-graph family needs to reflect this.
To that end, for any $G$-graph family $\F = \set{\F_n}$ and for all sequences $C \in G \ltimes \SC^{op}$ of length $n$, we define
$\F_C$ to be
\[
...
The main result of this section applies to a large class of $G$-graph families. We only require the following.
but not all of the.Before

]\begin{definition}
      We say a $G$-graph family $\F$ \textit{has units} if
...
We can now state and prove the main result of this section (see the disucssion after Theorem \ref{INTRO_MODEL_THM}).
...
\begin{corollary}[{cf. \cite[\S 4.2]{Cav}, \cite[1.16]{CM13b}}]
...
\begin{example}
      ...
      \item In \cite[Prop. 2.24]{BM13}, Berger-Moerdijk show that if $\V$ satisfies transfer for operads, is right proper, and has cofibrant unit,
            ...
            \todo[inline]{come back, this is unsatisfing}
...
\begin{example}
      ...
      Let $(\V, \otimes) = (\sSet, \times)$.
      Then  by Propositions \ref{SSET_COH_PROP} and \ref{COH_DK_ARE_WE_PROP}, we have that weak equivalences in $\sOp^G_\F$ are
      Dwyer-Kan $\F$-equivalences.
      For fibrations, if $W_!$ denotes the left adjoint to the homotopy coherent nerve
      $N_{hc}: \sCat \to \sSet$,
      we note that $W_! J$ is a $\sSet$-interval, where $J$ is the nerve of the walking isomorphism $J = N \tilde{[1]}$, by Example \ref{WJ EX}.
...
\end{corollary}
% \begin{proof}
%       Combine Lemmas \ref{PT_PROP_LEM}, \ref{PT_CELL_LEM}, and \ref{P_P_WE_LEM} with Proposition \ref{PT_COH_PROP}.
% \end{proof}
...
\begin{example}
      ...
      \todo[inline]{Warning: I don't know that $\Top_{\**}$ satisfies condtion $(iii)$ of cellularity.
        The two spaces clearly agree as \textit{sets}, but do they agree as \textit{spaces} if the basepoint isn't closed?}
\end{example}



% \begin{example}
%       $(\sSet_{J}, \times)$, simplicial sets with the Joyal model structure:
%       \begin{enumerate}
%       \item is cofibrantly generated (Joyal);
%       \item is a closed monoidal model category with cofibrnat unit {\color{red} SOURCE};
%       \item has cellular fixed points by \cite[Example 2.14]{Ste16}, \cite[Thm. 3.3]{Ber17};
%       \item cspp?
%       \item right proper? NOOOOOOOOPE.
% \end{example}
...
{\color{red} Need to add demand that $TT \Rightarrow T$ and $I \to T$ are fiber natural transformations}.

\begin{definition}
Given a split Grothendieck fibration $\mathcal{C} \to \mathcal{D}$,
a \textit{Grothendieck monad} is a monad $T\colon \mathcal{C} \to \mathcal{C}$ such that the diagram below commutes.
\[
\begin{tikzcd}
\mathcal{C} \ar{rr}{T} \ar{rd} && \mathcal{C} \ar{dl}
\\
& \mathcal{D}
\end{tikzcd}
\]
...
Our first goal is to show Corollary \ref{ALGAREGROTH COR}, stating that 
if $T$ is a Grothendieck monad then 
$\mathsf{Alg}_T(\mathcal{C})$
is again a Grothendieck construction over the base $\mathcal{D}$.
We start by unpacking the data of a Grothendieck monad.


\begin{proposition}
A Grothendieck monad $T$ on $\mathcal{C} \to \mathcal{D}$
is equivalent to the data of
\begin{itemize}
\item for each $d \in \mathcal{D}$, a monad $T_d\colon \mathcal{C}_d \to \mathcal{C}_d$
...
\begin{proof}
The monads $\mathcal{C}_d$ are the restrictions to $T$ to $\mathcal{C}_d$ while the natural transformations 
$T_d f^{\**} \Rightarrow f^{\**} T_{d'}$
follow by applying
$T$ to the pullback arrows to obtain arrows 
$T f^{\**}c \to Tc$,
which are canonically factored to obtain 
$T f^{\**}c \to f^{\**}Tc \to Tc$ (one can readily check that naturality is then equivalent to $T$ being compatible with the composition of a pullback arrow followed by a fiber arrows).
(a) is then equivalent to $T$ being compatible with the composition of pullback arrows while
(b) is equivalent to the naturality of the natural transformations
$TT\Rightarrow T$, $I \Rightarrow T$
with respect to pullback arrows.
\end{proof}
...
\begin{lemma}
Suppose $T$ is a Grothendieck monad for 
...
\begin{lemma}
Given a Grothendieck monad $T$ on $\mathcal{C} \to \mathcal{D}$, the pullback functors $f^{\**}$
...
{\color{red} Note(Luis): assuming split here is unnecessary. one can easily provide a direct universal property proof}
...
The claim that this is a $T_d$-algebra structure is a consequence of \eqref{GROTHCART EQ}.
\end{proof}



\begin{corollary}\label{ALGAREGROTH COR}
Given a Grothendieck monad $T$ on $\mathcal{C} \to \mathcal{D}$, the algebra category 
$\mathsf{Alg}_T(\mathcal{C})$
...
Suppose now that $I$ is a fixed diagram category.
Given a split Grothendieck fibration
$\mathcal{C} \to \mathcal{D}$,
one has that $\mathcal{C}^I \to \mathcal{D}^I$
is again a split Grothendieck fibration, with pullbacks defined levelwise.
Similarly, given a monad $T$ on $\mathcal{C}$, one has a levelwise monad
$T^I$ on $\mathcal{C}^I$, and a natural identification
$\mathsf{Alg}_{T^I}(\mathcal{C}^I)\simeq
\left(\mathsf{Alg}_T(\mathcal{C})\right)^I$.
Combining these two observations, it is moreover clear that if
$T$ is a Grothendieck monad on $\mathcal{C} \to \mathcal{D}$
then $T^I$ is again a Grothendieck monad on 
$\mathcal{C}^I \to \mathcal{D}^I$.
