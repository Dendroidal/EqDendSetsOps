...
the implication in Remark \label{FIBERGLMOD REM}(iii) 
...
To work with this definition, we must ensure that $\mathbb F_\eta$ is in fact a monad, and moreover that is is the left adjoint to the forgetful functor.
...
\subsubsection{Non-unital operads}
...
\begin{proposition}
      ...
      \[
            \begin{tikzcd}
                  \left(\Omega_{\mathfrak C}^{\textrm{nu},0}\right)^{op} \arrow[r,"V"] \arrow[d, "\mathsf{lr}"]
                  &
                  \left(\Sigma \wr \Sigma_{\mathfrak C}\right)^{op} \arrow[r, "X"] \arrow[dl, Rightarrow]
                  &
                  \left(\Sigma \wr \V^{op}\right)^{op} \arrow[r, "\otimes"]
                  &
                  \V
                  \\
                  \Sigma_{\mathfrak C}^{op} \arrow[urrr, "\Lan = \bar F X"']
            \end{tikzcd}
      \]
      ...
      \begin{equation}
            \label{FREENUOP_EQ}
            \begin{tikzcd}
                  \left(\Omega_{\mathfrak C}^{\textrm{nu},n}\right)^{op} \arrow[r,"{(V^0)^{\circ n+1}}"] \arrow[d, "\mathsf{lr}"]
                  &[20pt]
                  \left(\Sigma ^{\wr n+1} \wr \Sigma_{\mathfrak C}\right)^{op} \arrow[r, "{(\sigma^0)^{\circ n+1}}"] \arrow[dl, Rightarrow]
                  &[20pt]
                  ...
                  \V
                  \\
                  \Sigma_{\mathfrak C}^{op} \arrow[urrrr, end anchor = south west, "\Lan = \bar F^{\circ n} X"']
            \end{tikzcd}
      \end{equation}
\end{proposition}

\subsubsection{Amalgamating monads}

\begin{lemma}
      The conditions from Definition \ref{AMALGMON DEF} hold for $P$ and $\bar F$.
      Moreover, $P$ and $F$ preserve reflexive coequalizers.
\end{lemma}
...
\begin{corollary}
      ...
      \begin{equation}
            \label{WT_EQ}
            W(T)_n = \mathbb F_\eta^{\circ n+1} \Omega(T) \cong F_P^{\circ n+1} P \bar F \Sigma_\bullet[T] \cong P \bar F^{\circ n+2}\Sigma_\bullet[T].
      \end{equation}
...
As discussed in \S \ref{SSEQ_SEC},
...
\eqref{2NDRED EQ} 
is also compatible with $G$-equivariance, i.e. it is in fact $\Lan^G$.
% $
% \mathsf{Lan}^G \colon 
% \left(
% \mathsf{WSpan}_l
...
the the right adjoint $f^{\**}$ preserves certain pushouts.
...
This finishes the argument establishing \eqref{HATSFT EQ}.
...
\begin{proof}
      ...
      For (ii), the first half is immediate, and using
      \begin{equation}
            \label{CFS_EQ}
            \check{f_!} \O \simeq \mathop{coeq}(\mathbb F_{\mathfrak D} f_! \mathbb F_{\mathfrak C}\O \rightrightarrows \mathbb F_{\mathfrak D} f_! \O)
      \end{equation}
      from Remark \ref{ALGPUSHLL REM}
      and the fact that $f^{\**}$ commutes with reflexive colimits\footnote{
        This follows as $\otimes$ commuting with colimits in both variables implies it commutes with reflexive colimits; see e.g. \cite[Lemma 2.3.2]{Rez96}.},
