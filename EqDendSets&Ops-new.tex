...
\begin{lemma}
      ...
      \item $G$ is a Reedy category, with $\set{\mathcal F}$ a Reedy-admissible collection.
            Thus we will say $X \in \left(\mathsf{sSet}^{\Delta^{op}}\right)^G_\F$ is \textit{horizontal Reedy fibrant} iff
            $X \in (\sSet)^{\Delta^{op} \times G}$ is $\set{{e} \times \set{\mathcal F}}$-Reedy fibrant iff
            $X^H \in \mathsf{sSet}^{\Delta^{op}}$ is horizontally Reedy fibrant for all $H \in \mathcal F$.
            
            Moreover, the joint model structures on
            $\mathsf{sSet}^{\Delta^{op} \times G}$ and $\mathsf{s}\left(\mathsf{Set}^{\Delta^{op}}\right)^G$
            agree with each other and the $\F$-model structure over the joint model structure on
            $\left(\mathsf{sSet}^{\Delta^{op}}\right)^G$.
            % 
      \item \label{RJOINTT_LBL} If $X \in (\mathsf{sdSet}^G)^{\Delta^{op}}$ is fibrant in the Reedy-over-joint model structure,
            then for all $U \in \Omega$, $X(U)$ is horizontal Reedy fibrant in
      ...
      \item \label{JF_VERT_LBL}
            ...
            % 
      \item \label{DIAG_LBL} If $X \in \mathsf{sSet}^{\Delta^{op} \times G}$ is horizontal Reedy firant, then
      ...
      \item weak equivalences in $\mathsf{PreOp}^G$ are detected by $\gamma^{\**}$.
            % 
      \item \label{SFIB_JEQ_LBL} $X,Y \in \mathsf{sdSet}$ simplicially fibrant, then $X \to Y$ is a joint equivalence iff it is a simplicial equivalence
...
as the \emph{cyclic subpresheaf generated by $x$}.
...
To do so, we will first find it useful to discuss a number of special types of cyclic presheaves of $N \mathcal{P}$, suggested by \eqref{PUSHOPPR EQ}.

\begin{notation}
For a drendex $p\colon \Omega[T] \to N \mathcal{P}$
we say the cyclic presheaf 
$\langle x \rangle \subseteq N \mathcal{P}$ is
\begin{itemize}
\item \emph{elementary} if for each vertex $T_v \hookrightarrow T$,
it is $\langle \partial_{T_v}^{\**} x\rangle \subseteq \O \amalg_{\mathfrak{C}}B$
\end{itemize}
\end{notation}
...
\begin{proof}
      ...
      implies that these are in fact $G$-graph simplicial equivalences,
      i.e. that for each tree $U \in \Omega$ the evaluations
      $\widetilde{Q}(U)(m) \to \widetilde{Q}(U)(0)$
      are $G$-graph Kan equivalences in $\mathsf{sSet}^{G \times \Aut(U)}$.
      % 
      But since $\widetilde{Q}(U) \in \mathsf{sSet}^{\Delta^{op} \times (G \times \Aut(U))}$ is itself Reedy fibrant by
      ...
      implies that it is in fact $G$-graph joint Reedy fibrant in $\left(\mathsf{sSet}^{\Delta^{op}}\right)^{G \times \Aut(U)}$,
      and hence by Lemma \ref{COMUOTOHOM_FACTS} \ref{DIAG_LBL}, %\cite[Prop. 4.5(iv)]{BP_edss}
      one has $G$-graph Kan equivalences 
      $\widetilde{Q}(U)(0) \xrightarrow{\sim}
      \delta^{\**} \widetilde{Q}(U) \xleftarrow{\sim}
      \widetilde{Q}_0(U)$,
      showing that the second and third maps in \eqref{BIGZIG EQ} are indeed $G$-graph simplicial equivalences, and hence joint weak equivalences.
      
      For the fourth equivalence, note that one can write
      \[
            \widetilde{Q}_0(U) = 
            \mathsf{Hom}_{\mathsf{sdSet}^G}(c_!\Omega[U],\widetilde{Q})=
            \mathsf{Hom}_{\mathsf{PreOp}^G}(c_!\Omega[U],\gamma_{\**}\widetilde{Q}),
      \] 
      \[
      ...
      \]
      in $\sSet^{G \times \Aut(U)}$ for each $U \in \Omega$.
      ...
      and that $\Omega[U]/\Gamma = \Omega[G \cdot U/\Gamma]$ is tame cofibrant for all $G$-graph subgroups $\Gamma \leq G \times \Aut(U)$ by Lemma \ref{OMEGATTAME_LEM}.
      
      For the fifth equivalence, note that
      \[
            \left(\gamma^{\**} N \widetilde{\mathcal{O}}\right)_0(U) = 
            \mathsf{Hom}_{\mathsf{PreOp}^G}(\Omega[U], N \widetilde{\mathcal{O}}) =
            \mathsf{Hom}_{\mathsf{sOp}^G}(\Omega(U),  \widetilde{\mathcal{O}}),
      \]
      \[
            \left(hcN \widetilde{\mathcal{O}} \right)(U) = 
            \mathsf{Hom}_{\mathsf{sOp}^G}(W(U),  \widetilde{\mathcal{O}}),
      \]
      in $\sSet^{G \times \Aut(U)}$ for each $U \in \Omega$,
      so that the required claim follows since 
      $\widetilde{\mathcal{O}}$ is Reedy fibrant and
      $W(U) \to \Omega(U)$ is a $G$-graph weak equivalence of cofibrant operads in $\sOp^{G \times \Aut(U)}$.
...
For COMUOTOHOM PROP: replace all instances of ``$G$-graph'' with ``$\mathcal F$'' in the proof (and the model structures).
