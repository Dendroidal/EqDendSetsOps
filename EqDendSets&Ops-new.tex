...
\begin{remark}
      \label{GRAPHF_REM}
      If $\F$ is the $(G, \Sigma)$-family composed of all \textit{graph subgroups} $\Gamma \leq G \times \Sigma_n$,
...
\begin{theorem}[{cf. \cite[Thm 2.1, Thm 8.22]{Per_eds}}]
...
This will be applied in Proposition \ref{W!_COF_PROP}, the main result necessary to prove that $W_!$ is left Quillen.
...
\begin{example}
      ...
      \[
            \begin{tikzpicture}
                  [grow=up,auto,level distance=1em,every node/.style = {font=\footnotesize},dummy/.style={circle,draw,inner sep=0pt,minimum size=1.75mm}]                  
                  \node {$S$}
                  child{node [dummy] {}
                    child{node [dummy] {$\vdots$}
                      child{node [dummy] {}
                        child{node [dummy] {}
                          child{node [dummy] {$\vdots$}
...
\todo[inline]{$\F$-model structures for $\dSet^G$ - write them down above!}
here!
...
\begin{proof}
      ...
      Then Proposition \ref{W!_COF_PROP} also implies that $h c N (f)$ is a graph inner-fibration between $G$-$\infty$-operads.
...
\begin{align*}
  ...
  &
    \simeq \colim_{[l] \in \Delta^{op},[n] \in \Delta^{op}} 
    \mathsf{Lan} N \circ \left( N^{\circ n} \iota \O \amalg \iota X^{\amalg 2l +1} \amalg \iota Y \right),
...
Noting that $N^{\circ n} \iota \O \amalg \iota X^{\amalg 2l +1} \amalg \iota Y$ denotes a certain span
...
Our first step is to describe 
$\Omega^{n,\lambda_l}_{\mathfrak{C}}$.
Much like Proposition \ref{ASSOCIDS PROP} implies that
