...
{\color{OliveGreen} % ---------------------------------------- OLIVE GREEN ----------------------------------------
  We end by proving a technical step in the above proof, is greater... something than Corollary \ref{LOCALISO_COR}.
  \begin{lemma}
        \label{LOCALISO_LEM}
...
\section{Equivariant dendroidal Segal spaces and Segal preoperads}

In this expository section, we recall the features of the model structures discussed in \cite{BP_edss}
...
In this section, we build an auxiliary model structure on preoperads,
as well as prove the technical heart of this paper in Proposition \ref{KEYPR PROP},
...
of $\Omega(T)$ as an algebra in \textit{pointed} $\mathbf E(T)$-colored symmetric sequences $\Sym^{\mathbf E(T)}_{\**}(\Set)$.
...
\begin{definition}
      ...
      \]
\end{definition}
We should think of pointed symmetric sequences as already having identified the "identity" operations in any potential operadic structure.
...
Otherwise, if $C_n \xrightarrow{\phi} T$ is injective such that $\partial \phi = C$, then
\begin{equation}
      \label{FET_EQ}
...
Now, $\mathbb F^{\mathbf E(T)}\eta_{\mathbf E(T)}(C) = \varnothing$ unless $C$ is a 1-corolla with both edges colored by the same edge $e \in \mathbf E(T)$;
in this case, $\mathbb F^{\mathbf E(T)}\eta(C) = \mathbb Z_{\geq 0}$, as in \eqref{FET_EE_EQ}.
% 
If we also denote the unit map by $\eta$, we see that the image of
\[
...
\begin{definition}
      ...
      \end{equation}
\end{definition}
The culmination of extnesive work by Cisinski-Moerdijk-Weiss \cite{CM13a,CM13b,CM11,MW09,MW07} is the following.
...
On the other hand, the homotopy theories involved are much more involved, as noted in \S \ref{MSC_SEC}, \S \ref{MS_SEC}, and \cite{Per_eds}.
