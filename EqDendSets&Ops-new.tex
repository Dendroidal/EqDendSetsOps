...
\begin{remark}
      ...
      This also follows from Remark \ref{VE_CHOCIE_REM}, as both $(\mathcal C^H)_f$ and $(\mathcal C_f)^H$ are fibrant replacements for $\mathcal C^H$.
\end{remark}
...
\begin{lemma}
      ...
      there exists some $(k_j, \sigma_j) \in \Stab(C)$ such that $\sigma_j(i) = j$ (and hence $c_j = k_j c_i$);
...
\end{lemma}
\begin{proof}
      As in Proposition \ref{CAV_4.14_PROP1} above, it is sufficient to show a weak equivalence
      $\O(C) \to \O(D)$ in $\V^{\Aut(C)}_{gen}$
      for $\O$ bifibrant in $\Op^{G,\mathfrak C(\O)}_{gen}(\V)$.

      First, we claim that $\Stab(C) \leq \Stab(D)$, so fix $(h, \pi) \in \Stab(C)$.
      %-----
      We note that $j \in \lambda$ iff $\pi(j) \in \lambda$, as $\pi(j) = (\pi \sigma_j)(1)$.
      %
      Moreover, if $j \in \lambda$, then $(k_{\pi(j)}^{-1} h k_j, \sigma_{\pi(j)}^{-1} \pi \sigma_j) \in \Stab_1(C)$,
      since
      \[
