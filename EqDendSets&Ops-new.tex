...
is weakly equivalent to $\widetilde{\mathbbm{2}}$.
...
in our formulation of Theorem \ref{INTCOF THM}.
...
\begin{proof}
      ...
      By Corollary \ref{F!LQ_COR}, the relevant span is a trivial cofibration in one leg, while the other leg has a locally cofibrant target.
      ...
      Thus $a_! (G/H \cdot \phi)$ is a trivial $\F$-cofibration in $\Op^{G, \mathfrak C_\O}(\V)$ by Corollary \ref{F!LQ_COR}.
...
For our group $G$, we define the following:
\begin{definition}
      \label{EQUIVG_DEF}
