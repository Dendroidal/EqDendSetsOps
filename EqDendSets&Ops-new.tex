...
We need a colored (but non-$G$-tree) generlization of a result from \cite{BP_geo}.
...
Let $G$ be a chosen fixed group. 
Our next goal is to show that, given a sequence
$\mathsf{Alg}_T(\mathcal{C}) \to \mathcal{C} \to \mathcal{D}$
where $\mathcal{C} \to \mathcal{D}$
is a split Grothendieck fibration and
$T$ is a Grothendick monad, the $G$-object sequence
\[\mathsf{Alg}_T(\mathcal{C})^G \to \mathcal{C}^G \to \mathcal{D}^G\]
has the same properties, i.e. that 
$\mathcal{C}^G \to \mathcal{D}^G$ is again a split Grothendieck fibration and that 
$\mathsf{Alg}_T(\mathcal{C})^G \simeq \mathsf{Alg}_{T^G}(\mathcal{C}^G)$
is the category of algebras on $\mathcal{C}^G$ for some Grothendieck monad $T^G$. 

To do so, we first introduce some notation.
Given $d \in \mathcal{D}^G$, we write 
$\left(\mathcal{C}^G\right)_d$
for the fiber of $\mathcal{C}^G$ over $d$
and, slightly abusing notation, we write $\mathcal{C}_d$ for the fiber of $\mathcal{C}$ over the underlying object $d \in \mathcal{D}$.
Crucially, we note that 
$\left(\mathcal{C}^G\right)_d$
is in general rather different from the $G$-object category
$\left(\mathcal{C}_d\right)^G$.
Instead, one has that pullbacks equip $\mathcal{C}_d$ with a $G^{op}$ action and one has the following.


\begin{proposition}
Let $\mathcal{C} \to \mathcal{D}$ be a split Grothendieck fibration and $G$ a chosen group.

Then $\mathcal{C}^G \to \mathcal{D}^G$ is  
 naturally identified with the Grothendieck construction of
\begin{equation}
\begin{tikzcd}[row sep = tiny]
    \left(\mathcal{D}^G\right)^{op} \arrow[r] &
\mathsf{Cat}
\\
	d \arrow[r, mapsto] &
	\left(\mathcal{C}^G \right)_d
...
Moreover, there are natural identifications
\[
\left(\mathcal{C}^G\right)_d \simeq
\mathsf{Fun}^{G^{op}}(EG,\mathcal{C}_d)
\]
where $EG$ is the category with objects the elements of $G$ and arrows
$g \xrightarrow{h} hg$, together with the obvious $G^{op}$-action.
\end{proposition}
...
\begin{proof}
The first claim is a straightforward consequence of the observation that, if $f$ is an arrow in $\mathcal{D}^{G}$, then $f^{\**}$ commutes with the action pullbacks, i.e. that 
$f^{\**} g^{\**} = g^{\**} f^{\**}$ for all $g \in G$.
