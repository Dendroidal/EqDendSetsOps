...
\begin{notation}
      ...
      \begin{equation}
            ...
            \V^{\Aut(\vect D)} \longto \prod_{\vect C \in G \ltimes \Sigma_{\mathfrak C}} \V^{\Aut(\vect C)}_{gen} \longto \Sym^{G, \mathfrak C}(\V),
            \qquad
...
the constructed \eqref{CHAREDGE EQ} is indeed
$N \mathcal{O} \amalg_{\zeta_!B (\eta)} \zeta_!B \to N \mathcal{P}$.
...
hence (Ch2),(Ch3) follow as the middle condition is always false.
...
$f \colon X \to Y$ is
\begin{itemize}
\item cofibrations are the maps which are injective on objects, i.e. 
the maps 
