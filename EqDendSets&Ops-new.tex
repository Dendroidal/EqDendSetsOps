...
\begin{description}
\item [$F$ and $LF$ implies $L$:] This step requires Proposition \ref{CAV_4.14_PROP2}, which shows that a homotopy equivalence of colors induces a weak equivalence between certain associated mapping objects.
...
\todo[inline]{add a diagram HERE which describes the situation, a visual aid}
\todo[inline]{also move discussion about the general path here}
...
\begin{lemma}
      \label{AUTC_LEM}
      For $C$, $D$, $\lambda$, and $H_i$ defined as above, we have the following:
      \begin{enumerate}[label = (\roman*)]
...
Moreover, pointed simplicial sets also satisfy the coherence axiom.
\begin{corollary}
      \label{PTSSETCOH_COR}
