...
\subsection{Fiber monads}


\begin{definition}
Given a split Grothendieck fibration $p\colon \mathcal{C} \to \mathcal{D}$,
a \textit{fiber monad} is a monad $T\colon \mathcal{C} \to \mathcal{C}$ such that the diagram below commutes
\[
\begin{tikzcd}
...
We start by unpacking the data of a fiber monad.


\begin{proposition}
A fiber monad $T$ on $\mathcal{C} \to \mathcal{D}$
...
\begin{lemma}
Given a fiber monad $T$ on $\mathcal{C} \to \mathcal{D}$, the pullback functors $f^{\**}$
...
Given a fiber monad $T$ on $\mathcal{C} \to \mathcal{D}$, the algebra category 
$\mathsf{Alg}_T(\mathcal{C})$
