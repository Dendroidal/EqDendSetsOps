...
one has that $T \colon \mathcal{C} \to \mathsf{Alg}_F^p(\mathcal{C})$ preserves cocartesian arrows so that the canonical map
...
\todo[inline]{add citations/comments that the proofs from \cite{BP_geo} easily generalize to this context}
...
one has a pull back $\mathfrak C$-symmetric sequence $f^{\**}X$
given by
$ f^{\**}X(\xi') = X(f(\xi'))$
which is an operad if $X$ itself is an operad.
Moreover, the two $f^{\**}$ functors fit into a pair if adjunctions 
...
\end{tikzcd}
\end{equation}
where, just as in Remark \ref{ALGPUSHLL REM}, we highlight that
...
Moreover, when $\mathcal{F}=\mathcal{F}_{all}$ is the family of all subgroups we simply refer to this model structure as the genuine model structure on $\mathsf{Sym}^G_{\mathfrak{C}}$.
\end{definition}
...
which are built in Appendix \ref{MONAD_APDX}, most notably Proposition \ref{FILTPUSHG PROP}.
...
\subsection{Symmetric sequences and colored operads (remainders)}
...
\begin{notation}
      \label{FD_NOT}
      For all $\mathfrak C$-signatures $\vect D \in G \ltimes \Sigma_{\mathfrak C}$, we let $\Sigma_{\mathfrak C}[-,\vect D]$ denote the composite
      \begin{equation}
            \label{FREESIGMA_EQ}
            \Sigma_{\mathfrak C}(-, \vect D)[-] \colon
            \V^{\Aut(\vect D)} \longto \prod_{\vect C \in G \ltimes \Sigma_{\mathfrak C}} \V^{\Aut(\vect C)} \longto \Sym^{G, \mathfrak C}(\V),
            \qquad
            \Sigma_{\mathfrak C}(-, \vect D)[K] = \Hom_{\Sigma_{\mathfrak C}}(-, \vect D) \cdot_{\Aut(\vect D)}K.
      \end{equation}
      Let $\mathbb F_{\vect D} = \mathbb F \circ \Sigma_{\mathfrak C}(-,\vect D)$ denote the composite of left adjoints.
\end{notation}

\begin{proposition}
      \label{GCOF_SYMMS_PROP}
      The generating (trivial) cofibrations of $\Sym^{G,\mathfrak C}(\V)$ are given by
      $\Sigma_{\mathfrak C}(-, \vect D)[i]$
      for $i$ a generating (trivial) cofibration in $\V^{\Aut(\vect D)}_{\F_{\vect D}}$.
\end{proposition}

More generally: (after Theorem \ref{FMODEL THM}):
\begin{notation}
      Given a groupoid $\mathcal G$ and an element $x \in \mathcal G$,
      we let $\mathcal G(-,x)$ denote the composite
      \[
            \mathcal G(-,x) \colon \V^{\Aut(x)} \longto \prod_{y \in \mathcal G} \V^{\Aut(y)} \longto \V^{\mathcal G},
            \qquad
            \mathcal G(y,x)[K] = \Hom_{\mathcal G}(y,x) \cdot_{\Aut(x)} K.
      \]
\end{notation}

\begin{proposition}
      \label{GCOF_GPD_PROP}
      For any groupoid $\mathcal G$ and family of subgroups $\F$ of $\mathcal G$,
      the generating (trivial) cofibrations of $\V^{\mathcal G}_{\mathcal F}$ are given by
      $\mathcal G[i,x]$
      for $x \in \mathcal G$ and $i = \Aut(x)/H \cdot i'$ for $H \in \F_x$ and $i'$ a generating (trivial) cofibration in $\V$.
\end{proposition}
...
\section{Operadic model structures on $\mathsf{Op}^G$}
\label{MS_SEC}
\renewcommand{\C}{\mathfrak C}

{\color{red} HERE}

\subsection{Types of maps in $\mathsf{Op}^G$}

We start our discussion of types of maps in $\mathsf{Op}^G$
...
cases (g),(gf) in fact reduce to the work in \cite{Cav,BM13}, 
so that case (f) is the true novelty of our work in this section.
...
\item Let $\1 = \Omega(\eta)$ be the $\V$-category that detects objects: it has a single object $\set{\**}$, with $\1(\**, \**) = 1_\V$.
      % \footnote{This notation matches the fact that this category is equal to the image of the ``stick'' tree $\eta \in \Omega$.}.
\end{itemize}
%\end{definition}
...
\begin{definition}
      ...
      A set $\mathscr{G}$ of $\V$-intervals is \textit{generating} if all $\V$-intervals $\J$ can be obtained
      as a retract of a trivial extension of an element in $\mathscr{G}$ in $\Cat^{\set{0,1}}(\V)$:
      \begin{equation}
            \begin{tikzcd}
...
We may now extend the model category notions to the equivariant context.
...
\begin{itemize}
	...
	\item a {\em local trivial $\F$-fibration} if it is both a local $\F$-fibration and a local $\F$-weak equivalence.
	\item {\em $\F$-essentially surjective} (resp. {\em $\F$-path lifting}) if $j^*F^H$ is essentially surjective (resp. path lifting) in $\Cat(\V)$ for all $H = H \times \set{e} \in \F_1$.
	\item a {\em $\F$-fibration} if both $\F$-path lifting and a local $\F$-fibration.
	\item a {\em $\F$-weak equivalence} if both $\F$-essentially surjective and a local weak $\F$-equivalence.
...
\begin{example}
      ...
      \item For higher arity operations,
            ...
            \]
            with $K \in \V$ and $\otimes_\Fin$ denotes the $\V$-tensoring from Example \ref{COTENS_EX};
...
% \begin{remark}
%       In the case where $\V = \sSet$ equipped with the Kan model structure and $\F = \F^\Gamma$ is the $(G, \Sigma)$-family of graph subgroups,
%       generating cofibrations and trivial cofibrations are given by
%       \begin{gather*}
%             \Omega(C) \otimes_\Fin(\partial \Delta[n] \to \Delta[n]),
%             \qquad
%             G/H \cdot (\varnothing \to \Omega(\eta)
%             \\
%             \Omega(C) \otimes_\Fin(\Lambda^i[n] \to \Delta[n]),
%             \qquad
%             G/H \cdot (\Omega(\eta) \to \mathbb J)
%       \end{gather*}
%       for $C \in \Sigma_G$, $0 \leq i \leq n$, $H \leq G$, and $\mathbb J$ is a $\sSet$-interval.
% \end{remark}
...
\begin{tikzcd}
	...
	\partial \Delta[1]^{\times \boldsymbol{E}^{\mathsf{i}}(T)}
	\ar{r} \ar{d}
&
...
is the iterated pushout product
\[
\left(
...
The category $\Omega_{\mathfrak{C}}^n$ of \textit{planar strings} has as objects strings $T_0 \to T_1 \to \cdots \to T_n$ of maps that are planar and tall, and as arrows a tuple of compatible isomorphisms.
\end{definition}
...
\[
      ...
      \V^{op}.
\]
Corollary \ref{LABIDEN_COR} implies that
$N \left(N^{\circ n} \iota \O \amalg_{\mathsf{F}} \iota X^{\amalg_{\mathsf{F}} 2l +1}\amalg_{\mathsf{F}} \iota Y \right)$
is the span
$\Sigma_{\mathfrak C}^{op} \xleftarrow{\mathsf{lr}} (\Omega_{\mathfrak C}^{n,0,\lambda_l})^{op} \xrightarrow{N_{n,l}^{(\O,X,Y)}} \V$,
...
\todo[inline]{equivariant monad? explicitly?}
...
\begin{example}
      We elaborate on this construction for the particular case where
\end{example}

\begin{corollary}
      For any $(G, \Sigma)$-family $\F$ and injective map of colors $f: \mathfrak C \to \mathfrak D$,
      the map $f^{\**}: \Op^{\mathfrak D}_\F(\V) \to \Op^{\mathfrak C}_\F(\V)$ preserves (trivial) cofibrations.
\end{corollary}
\begin{proof}
      We first show this for generating (trivial) cofibrations, as in Proposition \ref{GCOF_SYMMS_PROP}.
      Let $X$ be a free symmetric sequence (cf. \eqref{FREESIGMA_EQ})
      \[
            X = \Sigma_{\mathfrak D}[K, \ksi] = \Hom_{\Sigma_{\mathfrak D}}(-,\ksi) \cdot_{\Aut(\ksi)} K
      \]
      ...
      it is straightforward that the only case where $\widehat{f^{\**}}(X)$ is not the initial symmetric sequence (i.e. the constant to the initial object in $\V$)
      is when $\ksi$ is in fact a $\mathfrak C$-signature, and then
      \begin{equation}
            \label{RFREE_EQ}
            \widehat{f^{\**}}\left(\Sigma_{\mathfrak D}[K,\ksi]\right) = \Sigma_{\mathfrak D}[K,\ksi].
      \end{equation}
      The claim follows.

      In the general case, we see that since $\otimes$ commutes with colimits in each variable, it commutes with filtered colimits, in particular transfinite compositions, by for example \cite[Lemma 2.3.3]{Rez96}.
