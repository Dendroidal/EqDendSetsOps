...
\begin{theorem}[{\cite[Thm. 2.10]{Ste16}}]
      ...
      Then
      \begin{itemize}
      \item the projective model structure on $\V^{\mathsf O_\F^{op}}$;
...
\begin{proof}
      ...
      $(iii)$ $\sSet$ and $\sSet_{\**}$ have cellular fixed-point functors by e.g. \cite[Example 2.14]{Ste16} and Lemma \ref{PT_CELL_LEM},
...
We split the proof of 2-out-of-3 into the three possible cases. For the first, preservation of local weak equivalences is non-trival, while for the second and third, we must control preservation of essential surjectivity.
...
We now introduce these three variations \cite{Cav, BM13}.
\begin{definition}
      \label{EQUIV_DEF}
...
%      Now, if $\mathbb C$ is any cylinder in $\V$, we have a lift in the following diagram, for $i \in \set{0,1}$.
      We note that any arrow in a fibrant $\V$-category $\mathcal C_f$ which factors through an interval $\mathbb J$
      \[
            \mathbb A \longto \mathbb J \longto \mathcal C_f
      \]
      encodes a homotopy equivalence:
      Since $\mathcal C_f$ is fibrant, we may assume $\mathbb J = \mathbb J_f \xrightarrow{\simeq} (1_\V)_f$,
      and then we have teh following factorization through a cylinder object $\mathbb C$ of $\V$.
      \begin{equation}
            \label{J11_CYL_EQ}
            \begin{tikzcd}
                  1_\V \amalg 1_\V \arrow[r, "{(id, f)}"] \arrow[d, tail]
                  &
                  \J(0,1) \arrow[r, tail, "\simeq"]]
                  &
                  \J_f(0,1) \arrow[d, "\sim", two heads]
                  \\
...
We can now prove the results given in the introduction to this subsection.
The following facts about these definitions follow from the proofs of their non-equivariant counterparts:
...
Adding Remark \ref{AJC_REM}, the proof of the following result is completed, and again works in the equivariant setting.
\begin{lemma}
