...
\begin{corollary}
If $e \colon \Omega[T^e] \to N \mathcal{P}$ is a non-degenerate elementary
and $E \subseteq \Xi^e$,
then 
$\langle e\rangle$ is the smallest elementary subpresheaf
containing $\langle \partial_{T^e - E} e\rangle$,
i.e. if 
$\langle \partial_{T^e - E} e\rangle
\subseteq \langle \bar{e} \rangle$
...
then $\mathsf{lr}(T) \simeq T - \boldsymbol{E}^{\mathsf{i}}(T)$
...
$T \to T^e$ identifies $T \simeq T^e - E$
for some $E \subseteq \Xi^{e}$.
But since $p$ is non-degenerate,
the $\langle p_v \rangle$ are non-unital,
so that the $T^e_v$ are not stick trees,
and thus Remark \ref{WHENLRINN REM} implies
$T \simeq T^e-E$ where 
$E = \amalg_{v \in \boldsymbol{V}(T)} \boldsymbol{E}^{\mathsf{i}}(T^e_v)$.
%
That
