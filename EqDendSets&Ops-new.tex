...
\begin{remark}
Definition \ref{MODEL_DEFN} implies
a fixed color map $\O \to \P$ in 
$\mathsf{Op}^G_{\mathfrak{C},\F}(\V)$
is a weak equivalence/cofibration in 
the fiber (semi-)model structure 
$\mathsf{Op}^G_{\mathfrak{C},\F}(\V)$
iff it is a weak equivalence/cofibration in 
the global model structure 
$\mathsf{Op}^G_{\F}(\V)$

{\color{red} finish argument, note that this fails for fibrations}
\end{remark}
...
Thus we have established \eqref{HATSFT EQ}.
...
\end{corollary}
\begin{proof}
      For (i), it suffices to show that $\widehat{f^{\**}} f_!$ is the 
...
\begin{example}
      ...
      - it must preserve cofibrant objects (i.e. by BM13a), but Lemma \ref{BASICPUSH LEMMA} only takes care of the easy case from \cite[Thm. 1.15]{BP13a}.}
\end{example}
