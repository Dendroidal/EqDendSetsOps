...
\todo[inline]{aren't these just $\Omega(C) \times X$? MAKE THIS CLEAR}
...
\section{Review of revelent categories and model structures}
...
\begin{remark}
      ...
      sends the collection (TC1) and (TC2) surjectively to the collection of generating cofibrations, namely
      \begin{gather*}
...
\begin{proof}
	...
	in which case it is straightforward that $\mathcal{O} \to \mathcal{P}$ is a $\Sigma$-cofibration
        \todo[inline]{using the fact that $\tau$ preserves cofibrations and that cofibrations with $\Sigma$-cofibrant source are $\Sigma$-cofibrations - how hard is this last statement?}
...
\begin{corollary}
      Cofibrations are tame cofibrations.
\end{corollary}
\begin{proof}
      By Remark \ref{TAUCOFIB}, all generating cofibrations are of the form $\tau(A\to B)$ for $A \to B$ a tame cofibration,
      and thus all
      \todo[inline]{come back!}
\end{proof}
...
\item \emph{canonical} 
if there is an alternating representative
$p\colon \Omega[T] \to N \mathcal{P}$.
\end{itemize}
\end{definition}
