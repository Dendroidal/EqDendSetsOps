...
and hence these are weak equivalences between $\O_f(\ksi)$ and $\O_f(\theta)$ in $\V^{\Stab(\ksi)}_{\F_\ksi}$.
 ...
 Iterating this procedure, we have the following.
 \todo[inline]{come back: what is the statement I actually need so that I can prove the real proposition?}
 \begin{lemma}
       [{c.f. \cite[4.14]{Cav14}}]
       \label{CAV_4.14_PROP}
       \todo[inline]{come back: write up remark for what happens when $c$ changes, formally write up this statemewnt}
       % Let $\O$ be any operad in $\mathsf{Op}^G(\V)$,
       % $\ksi = (c_1,\ldots,c_n;c)$ a signature in $\C(\O)$,
       % $K = \Stab_G(c)$,
       % and suppose $(c_i,c_i')$ and $(c,c')$ are pairs of homotopy equivalent colors.
       % Then there exists a zig-zag of weak equivalences in $\V^{\Stab(\ksi)}_{\F_\ksi}$ between $\O(\ksi)$ and $\O(\ksi')$,
...
\begin{theorem}
      ...
      the unit is cofibrant \todo{come back: where do I use this?},
      the model structure is right proper \todo{come back: where do I use this?},
      there exists a set $\mathbb{G}$ of generating $\V$-intervals,
      is cartesian closed \todo{come back: this can be weakened},
      has cellular fixed-point functors. %which commute with a symmetric monoidal fibrant replacement functor in $\V$.
