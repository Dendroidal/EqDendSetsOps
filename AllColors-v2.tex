...
(vi) is well-known,
(vii) is straightforward from ``cellularity'' considerations (\cite[Prop. 4.10(i)]{BP_FCOP},\cite{Ste16}),
and
(viii) follows from either 
\cite[Lemma 1.12]{BM13}, which says that any combinatorial monoidal model category has a generating set of $\V$-intervals,
or \cite[Lemma 2.1]{BM13}, which says that any $\V$-interval is generating if all objects of $\V$ are fibrant.
...
for the map adjoint to $\mathfrak c$, 
$G \cdot C  \in \Phi^G$
for the $G$-free forest determined by $C$,
we define $G \cdot_{\mathfrak{C}} \vect{C} \in \Phi^G_{\mathfrak{C}}$, using the functor \eqref{PHIF_EQ}, to be
...
is the $G$-set of colors for $(G \cdot C)^\tau$.
\end{definition}
...
\begin{remark}
	...
	in \cite[Def. 1.2]{BM13}.
        ...
        More broadly, the global monoid axiom, combined with the cofibrant symmetric pushout powers condition
        (i.e. the technical conditions (iv),(v) of Theorem \ref{THMA}),
        play an analoguos role to the hypotheses of \textit{adequacy} in \cite[Prop. 1.4]{BM13},
        or, for example,
        \textit{$h$-monoidality} in \cite{BB17}.
        However, these conditions are generally uncomparable, as the end goals are distinct:
        ours serve to build fixed-point model structures as in Theorem \ref{THMA},
        while adequacy, $h$-monoidality, and its variants serve to build projective model structures
        (cf. Remark \ref{GTRIV REM}, \cite[Remark 6.18]{BP_geo}, \cite[Remark 4.8]{BP_FCOP}).
\end{remark}
...
such that the induced set map $\mathfrak C_\O^H \to \mathfrak C_\P^H$ on $H$-fixed colors is surjective
for all $H \in \F_1$.
\end{proposition}
...
\begin{proof}
	...
	$\F$-fibrations in $\mathsf{Op}_{\bullet}^G(\V)$ must be local $\F$-fibrations.
        Proposition \ref{LOCALTCHAR PROP} implies (iii) and the ``if'' direction of (ii),
...
with our conclusions in Propositions \ref{ISOFIBEASY PROP} and \ref{ISOFIBHARD PROP}.
...
\begin{lemma}[{cf. \cite[Lemma 2.6]{Ber07b}}]
	...
	Let $\V$ be a model category.
        ...
        \item Consider the diagram below, 
                ...
                exists.
        \item Dually, given the diagram below following the same notations,
                \begin{equation}
                        \label{TRANSLIFTDUAL EQ}
                        \begin{tikzcd}[column sep = 45pt]
                                A \arrow[r] \arrow[d, rightarrowtail]
                                &
                                X \arrow[r, "\simeq"] \arrow[d, twoheadrightarrow]
                                &
                                X' \arrow[d]
                                \\
                                B \arrow[r]
