...
\begin{notation}
      ...
      we write $(A \to B) \square (K \to L)$ for the pushout product map, with target $B \times L$ (see, for example, \cite[11.1.7]{Ri14}).
\end{notation}
...
\begin{remark}\label{HOISNERVENON REM}
	...
	$X \in \mathsf{PreCat} \subset \mathsf{PreOp}$ is a \textit{precategory},
        that is $X \in \mathsf{ssSet} \subset \mathsf{sdSet}$ with $X(\eta)$ a discrete simplicial set,
...
\end{tikzcd}
\]
Finally, we have a fibered pushout product map $(X \to Y) \square_{\mathfrak C_\bullet} (K \to L)$ defined analogously to \cite[11.1.7]{Ri14}.
\end{definition}
...
\begin{remark}\label{PUSHOPPRRST REM}
        ...
        is simply the data of a single operation, say $\phi \in \mathcal P(\vect D)$,
