...
the category of $G$-operads with fied $G$-set $\mathfrak C$ of colors,
is again non formal.
The many colored case additionally brings a few novel and notable nuances.
% Firstly, since
First, in the single colored cased, we have
$\sOp^G_{\**} = \Op_{\**}(\sSet^G)$,
so equivariant operads can be recovered as reuglar operads enriched in a $G$-equivariant setting.
However, as
the group $G$ typically acts non-trivially
on the set of colors $\mathfrak{C}$,
no such result holds for $\mathsf{sOp}^G_{\mathfrak C}$.
Moreover, for a fixed $G$-set $\mathfrak C$,
$\sOp^G_{\mathfrak C}$ cannot be realized as the category of
$G$-objects in some category\footnote{
...
in any case, such approaches would not produce
