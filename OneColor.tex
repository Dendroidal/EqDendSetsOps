...
\title{The homotopy theory of equivariant colored operads with fixed colors}
...
we synthesize these into a single
Dwyer-Kan style model structures
on the category
...
This paper, along with the two sequels \cite{BP_ACOP,BP_TAS},
completes a program by the authors to 
generalize the Cisinski-Moerdijk project in
\cite{CM11,CM13a,CM13b} to the equivariant context,
culminating in the existence of a Quillen equivalence
\begin{equation}\label{CMMAINTHM EQ}
        W_!\colon \mathsf{dSet}^G \rightleftarrows \mathsf{sOp}_{\bullet}^G \colon hcN
\end{equation}
between
equivariant simplicial operads and
...
on the category $\mathsf{sOp}^G_{\mathfrak C} = \Op^G_{\mathfrak C}(\sSet)$
of $G$-operads with fixed $G$-set $\mathfrak C$ of colors,
is again non formal.
The many-colored case brings a few additional and novel nuances.

First, as we noted above,
equivariant operads with a single color can be defined as regular (non-equivariant) operads enriched in a $G$-equivariant setting.
...
$\sOp^G_{\mathfrak C}$ cannot be realized as the category of $G$-objects in some non-equivariant category\footnote{
...
where we note that the right-hand side includes all possible $G$-set structures on $|\mathfrak C|$.
...
Together, these observations imply that we cannot apply the formal techniques of e.g. \cite{Cav,Ste16};
...
\begin{remark}\label{RESTTOCATS REM}
	...
	where $\**_{\mathfrak C}$ denotes the terminal object in $\Cat_{\mathfrak C}^G(\V)$
        (with all hom-objects the terminal object of $\V$, and all structure maps uniquely determined),
...
Below we list the known examples of categories satisfying all the conditions in Theorem \ref{THMI}. % that we are aware of.
...
all follow from Proposition \ref{WEAKCELL PROP} below,
...
In \S \ref{EHT_SEC},
we first discuss the global monoid axiom mentioned in Theorem \ref{THMI},
and second extend much of the work 
...
This result concerns the properties of pushout powers $f^{\square n}$,
