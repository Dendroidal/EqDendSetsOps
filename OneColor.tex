...
\title{The homotopy theory of equivariant operads, color by color}
...
\begin{abstract}
	...
	on the category of equivariant simplicial operads with a fixed $G$-set of colors,
	...
	we obtain model structures on the category of equivariant simplicial operads with a fixed $G$-set of colors
...
Theorems \ref{THMI} and \ref{THMII}, %  \ref{THMIII}, 
...
to the context of operads with any fixed $G$-set of colors.

In the main result of the sequel \cite[Thm A]{BP_ACOP},
we synthesize these into a single model structure on the category
$\sOp^G_\bullet = \Op_\bullet(\sSet)^G$
of equivariant operads with varying $G$-sets of colors
(here and throughout the paper, we use $\bullet$ to indicate that colors/objects are allowed to change),
extending the well known Dwyer-Kan model structures on all colored operads (and maps that change colors) in
\cite[Thm 1.14]{CM13b}, \cite[Thm. 2]{Rob11}
from the non-equivariant context to the 
equivariant context.

This paper is part of a program \cite{Per18,BP_geo,BP_edss,BP_ACOP,BP_TAS} of the authors to generalized the Cisinski-Moerdijk project of \cite{CM11,CM13a,CM13b} to the equivariant setting,
which builds a Quillen equivalence between simplicial operads and a homotopy coherent combinatorial version akin to the quasicategories of Joyal and Lurie.

Further discussion and context can be found in the introductions to e.g. 
\cite{BP_geo,BP_ACOP,BP_TAS}.
...
on $\mathsf{sOp}^G_{\mathfrak C} = \Op^G_{\mathfrak C}(\sSet)$,
the category of $G$-operads with fixed $G$-set $\mathfrak C$ of colors,
is again non formal,
and additionally the many-colored case brings a few new notable nuances.

First, in the single-colored case, we have
$\sOp^G_{\**} = \Op_{\**}(\sSet^G)$
that equivariant operads can be recovered as regular operads enriched in a $G$-equivariant setting.
However, as the group $G$ may act non-trivially on the set $\mathfrak C$ of colors,
no such result holds.
Moreover, for fixed $G$-set $\mathfrak C$,
$\sOp^G_{\mathfrak C}$ cannot be realized as $G$-objects in some category\footnote{
There in an inclusion
$\sOp^G_{\mathfrak C} \hookrightarrow \left(\sOp_{|\mathfrak C|}\right)^G$,
where $|\mathfrak C|$ is the underlying set of $\mathfrak C$,
and we note that the right-hand side includes all possible $G$-set structures on $|\mathfrak C|$.
However, the sophistication needed to deal with this category is similar to what is needed in the context of
all $G$-sets of colors simultaneously,
and thus we do not pursue this observation further.}.
Together, these imply that we cannot apply the formal techniques of e.g. \cite{Cav,Ste16};
in any case, such approaches would not yield the notion of equivalences sugggested by graph subgroups.
...
\begin{remark}\label{RESTTOCATS REM}
	...
	$\Cat_\bullet^G(\V) \simeq \Op_\bullet^G(\V) \downarrow \**$ and
        $\Cat_{\mathfrak C}^G(\V) \simeq \Op_{\mathfrak C}^G(\V) \downarrow \**$,
	where $\**$ denotes the terminal $\V$-category,
	so the $\F$-model structure on $\Op_{\mathfrak C}^G(\V)$
	also induces a model structure on $\Cat_{\mathfrak C}^G(\V)$.
	Since $\Cat_{\mathfrak C}^G(\V)$ contains only unary operations,
	...
	in Theorem \ref{THMI}
	(i.e. no analogues of the pseudo indexing system condition is needed).
	For details, see Remark \ref{CSPNTHI REM}. % \cite[Remark \ref{ALBEETA REM}]{BP_ACOP}
\end{remark}
...
\begin{remark}\label{GTRIV REM}
	...
	$G=\**$ and the categorical case $\Cat_{\mathfrak C}(\V)$,
	...
	$\Cat_{\mathfrak c}(\V) \subseteq \Cat_\bullet(\V)$ discussed in \cite{BM13}.
...
\end{enumerate}
In all these cases, conditions (i) and (iii) % and (vi)
are well known.
% and
...
\begin{remark}
        These examples also satisfy the axioms necessary for \cite[Thm A]{BP_ACOP},
        yielding a Dwyer-Kan model structure on $\Op_\bullet^G(\V)$.
        See \cite[\S 1.2]{BP_ACOP} for details.
\end{remark}
...
\begin{remark}\label{CPPWHY REM}
	...
	Theorem \ref{THMI}. % and \ref{THMIII}.
...
\begin{remark}\label{SPNONEX REM}
	...
	\ref{THMI} (and \cite[Thm A]{BP_ACOP}), 
	...
	\ref{THMI}, \ref{THMII} (and \cite[Thm A]{BP_ACOP}).
	...
	Theorems \ref{THMI}, \ref{THMII} (and \cite[Thm A]{BP_ACOP})
...
in \cite[Thm. \ref{AC-THMIII}]{BP_ACOP},
...
However, the equivariant setting preesents some
technical challenges that will
require us to somewhat repackage the non-equivarinat narrative of \cite{CM13b,Cav}.
...
Lastly, \S \ref{INDSYS SEC} proves 
...
% \begin{remark}
%       Pseudo indexing systems are a generalization of the notion of \textit{indexing systems} introduced by Blumberg-Hill \cite[Def. 3.22]{BH15}.
%       Moreover, they encompase the notion of \textit{weak indexing system} given by the authors
%       (\cite[Def. 9.5]{Per18}, \cite[Def. 4.49]{BP_geo}, \cite[Def. 6.2]{BP_edss});
...
which will be used 
during the proof of \cite[Prop. \ref{AC-J_CELL_PROP}]{BP_ACOP},
