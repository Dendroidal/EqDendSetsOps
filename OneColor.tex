...
\hypersetup{
  final,
  pdftitle={The homotopy theory of equivariant operads with a fixed color},
...
Our first main result in this paper, 
Theorem \ref{THMI}, %  \ref{THMIII}, 
...
In the main result of the sequel \cite[Thm. \ref{AC-THMA}]{BP_ACOP},
we synthesize these into a single Dwyer-Kan style model structure on the category
...
% equivariant context.
This paper, along with the two sequels \cite{BP_ACOP,BP_TAS}, completes a program by the authors to
generalize the Cisinski-Moerdijk project of \cite{CM11,CM13a,CM13b} to the equivariant setting,
culminating in a Quillen equivalence
\begin{equation}\label{CMMAINTHM EQ}
	W_!\colon \mathsf{dSet}^G \rightleftarrows \mathsf{sOp}_{\bullet}^G \colon hcN
\end{equation}
between
simplicial operads and
a homotopy coherent combinatorial presentation akin to the quasicategories of Joyal and Lurie.

Further discussion and context can be found in the introductions to \cite{BP_ACOP,BP_TAS}.
...
is again non formal.
The many-colored case additionally brings a few novel and notable nuances.

First, in the single-colored case, we have
$\sOp^G_{\**} = \Op_{\**}(\sSet^G)$,
i.e. that equivariant operads can be recovered as regular operads enriched in a $G$-equivariant setting.
However, as the group $G$ may act non-trivially on the set $\mathfrak C$ of colors,
no such result holds for $\mathsf{sOp}^G_{\mathfrak C}$.
...
in \cite[Thm. \ref{AC-THMA}]{BP_ACOP},
...
& 
	...
	&
	\arrow[Rightarrow, from=D, to=F,shorten <=0.10cm]%,"\epsilon"]
...
\begin{proposition}\label{SPANPIECEJ PROP}
	...
	%\Omega^{n,\bullet,\lambda}_{\mathfrak{C}} \wr (A_j)
	%\ar{rrrrr}[name=toE]{\boldsymbol{V}^k} \ar{rd}[swap]{d_i} \ar{dd}[swap]{f_{\**}}
	%&&&
	%&&
	%\Sigma \wr \Omega^{n-k-1,\bullet,\lambda}_{\mathfrak{C}} \wr (A_j) \ar{dd}{f_{\**}}
	%&
	%\Omega^{n,\bullet,\lambda}_{\mathfrak{C}} \wr (A_j)
	%\ar{rrrrr}[name=toE2]{\boldsymbol{V}^k} \ar{rd}[swap]{d_i} \ar{dd}[swap]{f_{\**}}
	%&&&
	%&&
	%\Sigma \wr \Omega^{n-k-1,\bullet,\lambda}_{\mathfrak{C}} \wr (A_j) \ar{dd}{f_{\**}}
	%\\
	...
	%\Omega^{n-1,\bullet,\lambda}_{\mathfrak{C}} \wr (A_j) \ar{rrrru}[swap]{\boldsymbol{V}^{k-1}}
	%\\
	%\Omega^{n,\bullet,\lambda}_{\mathfrak{D}} \wr (B_j) \ar{rrrrr}[name=toB]{\boldsymbol{V}^k} \ar{rd}[swap]{d_i}
	%&&&
	%&&
	%\Sigma \wr \Omega^{n-k-1,\bullet,\lambda}_{\mathfrak{D}} \wr (B_j)
	%&
	%\Omega^{n,\bullet,\lambda'}_{\mathfrak{C}} \wr (B_{j'}) \ar{rrrrr}[name=toB2]{\boldsymbol{V}^k} \ar{rd}[swap]{d_i}
	%&&&
	%&&
...
We note that the proof of \cite[Prop. 5.42]{BP_geo} % RANTRANS PROP
...
which will be used 
in the proof of \cite[Prop. \ref{AC-J_CELL_PROP}]{BP_ACOP} in the sequel,
...
\begin{proof}
	...
	But this is immediate from Corollary \ref{FGTPUSH COR}.      
\end{proof}
