...
\begin{definition}
      ...
      A \textit{$\mathfrak C$-coloured operad} $\mathcal{O}$ in $\V$ consists of the following data:
...
\begin{remark}
      (Coloured) operads are also known as \textit{symmetric multicategories}.
\end{remark}
...
\begin{remark}
	Unlike in the single-coloured case, $\Op^G(\V)$ does \textit{not} coincide with the category of colored operads in $\V^G$.
...
Main observation:
\begin{remark}
      Given a map $\phi: T \to S$ of $G$-trees such that $S$ is $\UC$-coloured, there is a unique colouring on $T$ such that
      $\phi$ is a map of coloured trees.
      Moreover, any factorization $T \to S' \to S$ in $\Omega_G$ of a map of coloured trees $T \to S$
      comes equipped with a canonical colouring so that the factorization is in fact in $\UC\Omega_G$.

      As an upshot, the vast majority of the operations and constructions in \cite[\S 3-5]{BP17}
      follow through immediately to the coloured setting,
...
\footnote{We do this as opposed to the original definition: unless we somehow force the ``correct isotropy'' of colour on the non-equivariant trees, we just see $\Phi\mathfrak C(G/e)$, and not the whole coefficient system.}
\begin{definition}
...
\begin{equation}
      \begin{tikzcd}[row sep = tiny]
            ...
            &
            (G \cdot U, \mathfrak c: \Phi(G \cdot U) \to \UC)
      \end{tikzcd}
\end{equation}
sending maps $(\phi, g)$ to $(h,e) \mapsto (hg^{-1}, \phi(e))$. 
...
\todo[inline]{come back: make sure that $i \mapsto (T_i, \mathfrak c_i)$ is in fact a functor: in particular, does the ``minimal'' thing hold up?}
...
Category of labeled planar coloured strings, as before.
As most of the construction/category theory is on the level of strings (or formally due to pullback constructions),
``colouring'' does not affect the proofs of \cite[Prop 5.30,5.41]{BP17}.
...
\begin{proposition}
      ...
      equipped with a unique planar (hence colour-fixed) label map in $\UC\Omega_G^e$
...
\begin{proof}
      ...
      the edges of this tree are a $G$-subset of $E(T)$, the colouring descends uniquely.       
\end{proof}
...
\begin{remark}
      ...
      as there is a canonical colouring on $T'$, namely $\Phi E(T') \to \Phi E(S) \to \UC$. 
\end{remark}
...
\begin{proposition}
      ...
      \begin{equation}\label{FILTRATION_LAN_LEVEL}
            \begin{tikzcd}
                  ...
                  \\
                  \coprod\limits_{[T, \mathfrak d] \in \mathsf{Iso}
                    \left(C \downarrow_{\mathsf{r}} \UC\Omega_G^a[k]\right)}
                  \left(
                        \bigotimes\limits_{v \in V_{G}^{ac}(T)}\P(T_v, \mathfrak d_v) \otimes
...
\begin{example}
      ...
      \end{equation}
      where
      $(\F_n)_{(c_1,\dots,c_n;c_0)}$ is all $\Gamma \in \F_n$ such that $\Gamma \leq \mathsf{Aut}(c_1,\dots,c_n;c_0)$. 
...
equipped with a $\UC$-colouring on their associated $G$-corollas $C_\Gamma$.
...
\begin{proof}
      ...
      Unless $\UC$ is a $G$-set, we don't even know if we have a map of colourings on levels other than $G/e$,
