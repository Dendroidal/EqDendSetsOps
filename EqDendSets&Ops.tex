...
{\color{cyan}
  \begin{remark}
        ...
        sending $(A, (c_a))$ to the obvious map $A \to \mathcal C$ (thinking of $A$ as a discrete category).Furthermore, we have a pair of maps
        \begin{equation}
...
\begin{definition}
      ...
      \end{equation}
      A \textit{$\mathfrak C$-symmetric sequence} is a functor $X: \Sigma_{\mathfrak C} \to \V$.
...
\begin{definition}
      ...
      We let $\Sym(\V) = \mathsf F \ltimes \Sym^{(-)}(\V)$ and $\Op(\V) = \mathsf F \ltimes \Op^{(-)}(\V)$
...
We apply this result to both $\Sym(\V) = \mathsf F \ltimes \Sym^{(-)}(\V)$ and $\Op(\V) = \mathsf F \ltimes \Op^{(-)}(\V)$.

For the category $E = \Sym^G(\V) = (\Sym(\V))^G$ of \textit{$G$-symmetric sequences},
we note that, for any $G$-set $\mathfrak C$, we have
\begin{equation}
      (E G \times \Sigma_{\mathfrak C})/G \simeq G \ltimes \Sigma_{\mathfrak C},
\end{equation}
where $\Sigma_{\mathfrak C}$ inherits a $G$-action from the action on $\mathfrak C$;
we refer to this later category
\begin{equation}
      G \ltimes \Sigma_{\mathfrak C} = \coprod_{n \geq 0}(G \times \Sigma_n) \ltimes \mathfrak C^{\times n+1}
\end{equation}
as the \textit{$(G, \mathfrak C)$-symmetric category},
and the category $\Sym^{G, \mathfrak C}(\V)$ of functors $X: G \ltimes \Sigma_{\mathfrak C} \to \V$
...
\begin{equation}
      \mathsf F^G \longrightarrow \Cat,
      \qquad
      \qquad
      \mathfrak C \longmapsto \Sym^{G, \mathfrak C}(\V).
\end{equation}
\todo[inline]{formal definition of $\mathcal C/G$?}


Now, for $E = \Op(\V) = \mathsf F \ltimes \Op^{(-)}(\V)$,
...
(satisfying $(0) - (2)$ below)
by composition with the forgetful functor $\Op^{\mathfrak C}(\V) \to \Sym^{\mathfrak C}(\V)$.
%
Explicitly, an object $\O \in \Fun^{G^{op}}(E G^{op}, \Op^{\mathfrak C}(\V)) =: \Op^{G, \mathfrak C}(\V)$
...
We will now investigate $G \ltimes \Sigma_{\mathfrak C}$ and related categories,
...
\begin{remark}
      We further observe that $G \ltimes \Sigma_{\mathfrak C}$ is equivalent to the pullback on the left below,
...
Unpacking definitions (cf. Remark \ref{G_GR_REM}), we see that a map $(U, \mathfrak c) \to (V, \mathfrak d)$ is given by
...
\subsubsection{Single-Colored Operads}
...
\begin{proof}
      ...
      every object in $\sSet^G$ is genuine cofibrant by \cite[Remark 5.71]{BP17},
...
We will need a result about the model structures $\Op^{G,\mathfrak C}_\F$.
\begin{lemma}
      [{cf. \cite[Prop. 5.67 and Lemma 5.73]{BP17}}]
      \label{LGC_LEM}
      ...
      If $f$ is a (trivial) $\F$-cofibration in $\Op^{G, \mathfrak C}(\V)$ with $\O$ locally genuine cofibrant, then
...
\begin{proof}
      ...
      We have/will show {\color{red} COME BACK} that we have a filtration
      \begin{equation}
      ...
      \begin{gather*}
            \left(\varnothing \to \bigotimes_{v \in V^{ac}(T)}\O(T_v,\mathfrak d)\right)
            =
            \mathop{\square}\limits_{v \in V^{ac}(T)}(\varnothing \to \O)(T_v, \mathfrak d),
            \qquad
            \mathop{\square}\limits_{v \in V^{in}(T)}u(T_v, \mathfrak d)
      \end{gather*}
...
\begin{proof}
      ...
      this is again a trivial cofibration, and hence by Lemma \ref{LGC_LEM} a local genuine trivial cofibration.
      ...
      and thus a local genuine trivial cofibration by Lemma \ref{LGC_LEM}.
...
Our goal in this section is to show that the string categories $\Omega^n$ in \cite{BP17} and the natural functors connecting them can naturally be extended to objects and functors in 
the $2$-category
...
\subsection{Grothendieck monads}

\begin{definition}
Given a Grothendieck fibration $\mathcal{C} \to \mathcal{D}$,
a \textit{Grothendieck monad} is given by the data of
\begin{itemize}
\item for each $d \in \mathcal{D}$, a monad $T_d$ on the fiber $\mathcal{C}_d$
\item for each arrow $d \to d'$ in $\mathcal{D}$, a natural transformation 
$T_d f^{\**} \Rightarrow f^{\**} T_{d'}$
\end{itemize}
subject to the following conditions
\begin{itemize}
\item the composite 
$T_d f^{\**} g^{\**} \Rightarrow f^{\**} T_{d'} g^{\**} \Rightarrow f^{\**} g^{\**} T_{d''}$
coincides with the natural transformation 
$T_d (gf)^{\**} \Rightarrow (gf)^{\**} T_{d''}$
induced by $gf$.
\item the squares
...
A Grothendieck monad then defines a monad $T$ on $\mathcal{D}$
by acting via the fiberwise monads on objects and fiber arrows, 
and on pullback arrows by setting 
$T(f^{\**}c' \to c')$ to be the natural composite
\[
T_df^{\**}c' \to f^{\**}T_{d'}c' \to T_{d'}c'
\]
The monad structure maps $TT \Rightarrow T$, $I \Rightarrow T$ are then given fiberwise, with their naturality with respect to pullback arrows corresponding to the commutativity of the diagrams in 
\eqref{GROTHCART EQ}.


\begin{proposition}
Given a Grothendieck monad $\{T_d\}$ on $\mathcal{C} \to \mathcal{D}$, the pullback functors $f^{\**}$
lift to the algebra categories to give functors
\[
f^{\**} \colon
\mathsf{Alg}_{T_d}(\mathcal{C}_d)
\to 
\mathsf{Alg}_{T_d'}(\mathcal{C}_d')
\]
\end{proposition}
...
\begin{proof}
Given a $T_{d'}$-algebra $c'$, 
the composite
$T_d f^{\**} c' \to f^{\**}T_{d'} c' \to f^{\**} c'$
provides the required $T_d$-action on $f^{\**}c'$.

The claim that this is indeed a $T_d$-algebra structure is a  consequence of \eqref{GROTHCART EQ}.
\end{proof}

\begin{remark}
Should the pullback functors $f^{\**}$ admit left adjoints $f_{!}$,
...
\begin{corollary}
Given a Grothendieck monad $\{T_d\}$ on $\mathcal{C} \to \mathcal{D}$, the algebra category 
$\mathsf{Alg}_T(\mathcal{C})$
is naturally identified with the Grothendieck construction of
\begin{equation}
\begin{tikzcd}[row sep = tiny]
    \mathcal{D}^{op} \arrow[r] &
\mathsf{Cat}
...
\begin{lemma}
Suppose $\{T_d\}$ is a Grothendieck monad for 
