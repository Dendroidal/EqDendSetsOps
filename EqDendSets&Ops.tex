...
\begin{example}
      ...
      %
      These are both isomorphic to the standard Grothendieck construction $(BG \ltimes \mathcal C)$
      on the composite
      \begin{equation}
...
We will now investigate $G \ltimes \Sigma_{\mathfrak C}^{op}$ and related categories,
...
\begin{definition}
      We observe that $G \ltimes \Sigma_{\mathfrak C}^{op}$ is equivalent to the pullback on the left below,
      where $E: \Sigma \to \mathsf F$ sends $\underline{n}$ to $\underline{n+1}$,
      and we are implicitly using the diagonal functor from Remark \ref{WR_DIAG_REM}.
      \begin{equation}
            \label{COMEGA_B_EQ}
            \begin{tikzcd}
                  G \ltimes \Sigma_{\mathfrak C}^{op} \arrow[d] \arrow[r, "E"]
                  &
                  ...
                  & %
                  G \ltimes \Omega_{\mathfrak C}^{op} \arrow[d] \arrow[r, "E"]
                  &
                  \mathsf F \wr (G \ltimes {\mathfrak C}) \arrow[d]
                  \\
                  G \times \Sigma^{op} \arrow[r, "E"]
                  &
                  \mathsf F \wr G
                  & %
                  \Omega \arrow[r, "E"]
      ...
      We have a natural inclusion of categories $G \ltimes \Sigma_{\mathfrak C}^{op} \into G \ltimes \Omega_{\mathfrak C}^{op}$,
...
\begin{definition}
      ...
      $V: G \ltimes \Omega_{\mathfrak C}^{op} \to (\Sigma \wr (G \ltimes \Sigma_{\mathfrak C}^{op}))^{op}$,
      as colorings of a tree restrict to colorings of each vertex corolla
      (equivalently, this is $\pi^{\**}$ applied to $V: \Omega^{op} \to (\mathsf F \wr \Sigma)^{op}$ for
      $\pi: (\mathsf F \wr (G \ltimes \mathfrak C))^{op} \to \mathsf F^{op}$;
...
\subsection{Monad and Filtration}

\begin{proposition}
      We have a monad $N$ on $\mathsf{WSpan}^r_{\Cat \downarrow^r \mathsf F \wr G \ltimes \mathfrak C}(\Sigma^{op}, \V)$.
\end{proposition}
\begin{proof}
      ...
      \begin{equation}
            ...
            \end{tikzcd}
      \end{equation}
      Then $N\mathcal A$ is the span given by the strict 2-pullback
      \begin{equation}
            \begin{tikzcd}
      ...
      \end{equation}
      on the objects $(T, (a_v)) \in \Omega \wr \mathcal A$; we note that $\lambda(a_v) + 1 = E(T_v)$.
      The maps between these functors are given by the following diagram.
      \begin{equation}
            \begin{tikzcd}
...
\begin{definition}
      ...
      \begin{equation}
            \begin{tikzcd}
                  \mathbb F^{\mathfrak C} X \arrow[d, "u"'] \arrow[r]
                  &
                  \O \arrow[d]
                  \\
...
\begin{proposition}
      ...
      \end{equation}
      such that, for each $\mathfrak C$-profile $(C, \mathfrak c) \in G \ltimes \Sigma_{\mathfrak C}$,
      the diagram below is a pushout in $\V^{\Aut(C, \mathfrak c)}$.
      \begin{equation}
            \label{FILT_PUSH_EQ}
            \begin{tikzcd}
                  \coprod_{[T] \in \Iso(C \downarrow_r G \ltimes \Omega_{\mathfrak C}^a[k])}
                  \left(
                        \mathop{\bigotimes}\limits_{v \in V^{ac}(T)} \O(T_v, \mathfrak d) \otimes
                        \left(
                              Q^{in}_T[u]
                        \right)
                  \right) \cdot_{\Aut(T, \mathfrak d)} \Aut(C, \mathfrak c)
                  \arrow[r]
                  \arrow[d]
                  &
                  \O_{n-1}(C, \mathfrak c) \arrow[d]
                  \\                  
                  \coprod_{[T] \in \Iso(C \downarrow_r G \ltimes \Omega_{\mathfrak C}^a[k])}
                  \left(
                        \mathop{\bigotimes}\limits_{v \in V^{ac}(T)} \O(T_v, \mathfrak d) \otimes
                        \left(
                              \mathop{\bigotimes}\limits_{v \in V^{in}(T)} Y(T_v, \mathfrak d)
                        \right)
                  \right) \cdot_{\Aut(T, \mathfrak d)} \Aut(C, \mathfrak c)
                  \arrow[r]
                  &
      ...
      where we define
      \begin{equation}
            \square_{v \in V^{in}(T)} u(T_v, \mathfrak d): Q^{in}_T[u] \to \otimes Y(T_v, \mathfrak d).
      \end{equation}
\end{proposition}
\begin{proof}
...
\end{theorem}
\begin{proof}
      This follows from the above results completley analogously to the proof of Theorem I from \cite{BP17}.
\end{proof}
