...
\todo[inline]{come back: replace with $G \ltimes (-)$ version}
\begin{proposition}
...
\todo[inline]{come back: what happens here to the punctured cube subcategories?}
\begin{remark}
      \label{OEFIB_REM}
...
We need colored (but non-$G$-tree) generlization of a result from \cite{BP17}.
\todo[inline]{come back}
\begin{lemma}[{cf. \cite[5.73]{BP17},\textit{EXMAINLEM LEM}}]
      \label{EXMAINLEM LEM}
...
\begin{proof}
      ...
      and its existance and properties follow from Proposition \ref{FILT_PUSH_PROP}
      and Lemma \ref{EXMAINLEM LEM} above,
