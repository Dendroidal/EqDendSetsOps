...
\usepackage{tikz}%
\usetikzlibrary{matrix,arrows,decorations.pathmorphing, cd, patterns, calc, decorations.pathreplacing}
\tikzset{%
...
\begin{definition}
      ...
      \item For any signature $\ksi \in \mathfrak C^{\times n+1}$ and $\sigma \in \Sigma_n$, a map $\O(\ksi) \to \O(\sigma \cdot \ksi)$,
            ...
            \end{equation}
      \item For any compatible signatures $\ksi = (c_1, \ldots, c_n; c_0)$, $\ksi_i = (c_{1}^i, \ldots, c_{m_i}^i; c_i)$, a \textit{composition} map
            \begin{equation}
                  \O(\ksi) \times \O(\ksi_1) \times \ldots \times \O(\ksi_n) \to \O(\ksi \circ (\ksi_i))
            \end{equation}
      \end{enumerate}
      where $\ksi \circ (\ksi_i) = (c_1^1,c_2^1,\dots,c_{m_n}^{n}; c_0)$,
...
\begin{definition}
      Given a map $f: \mathfrak C' \to \mathfrak C$ and a $\mathfrak C$-colored operad $\O$,
...
\item For compatible signatures $\ksi$, $\ksi_1$, $\ldots$, $\ksi_n$, \textit{composition maps}
      \begin{equation}
            \O(\ksi) \otimes \O(\ksi_1) \otimes \ldots \otimes \O(\ksi_n) \to \O(\ksi \circ (\ksi_i)),
      \end{equation}
\end{enumerate}
...
\begin{remark}
	...
	Indeed, objects in $\Op(\V^G)$ have a fixed $G$-set of colors,
        and each level $\O(\ksi)$ has an action by the full group $G$
	(though only a partial action by $\Sigma_{|\ksi|}$).
\end{remark}
...
\begin{definition}
      ...
      where $E: \Omega \to \mathsf F$ sends a tree $T$ to its set $E(T)$ of edges.
...
\begin{remark}
      ...
      \begin{equation}
            ...
            \begin{tikzcd}[row sep = tiny]
                  ...
                  T \arrow[r, mapsto]
                  &
                  \Hom_{\mathsf{Cat}}(E(T), B_{\mathfrak C}G)
                  \\
                  (\phi: T \to S, g) \arrow[r, mapsto]
                  &
                  (E(T) \xrightarrow{\phi} E(S) \xrightarrow{\mathfrak c} \mathfrak C \xrightarrow{g^{-1}} \mathfrak C)
            \end{tikzcd}
      \end{equation}
      \todo[inline]{compare with genuine case: RHS equals
        $\Hom(\Phi(E(T)), \Phi(\mathfrak C))$
        equals
        $\Hom(\Phi(E(G \cdot T)), \mathfrak C_{fr}))$}
...
\begin{remark}
      ...
      \begin{equation}
            ...
            \begin{tikzcd}[row sep = tiny]
                  ...
                  T \arrow[r, mapsto]
                  &
                  \Hom_{\mathsf{Cat} \downarrow O_G}((G \cdot E(T))/G, \UC).
            \end{tikzcd}
      \end{equation}
      ...
      \begin{equation}
            ...
            \begin{tikzcd}
                  ...
                  &
                  F \wr B_{\mathfrak C(G/e)}G \arrow[r] \arrow[d]
                  &
      ...
      % bottom arrow sends $T$ to $G \cdot E(T)$, and
      % on arrows sends $(\phi,g)$ to
      % \begin{equation}
      %       \begin{tikzcd}[row sep = tiny]
      %             G \cdot E(S) \arrow[r, "\phi \times {g^{-1}}"]
      %             &
      %             G \cdot E(T)
      %             \\
      %             (h,s) \arrow[r, mapsto]
      ...
      \begin{equation}
            ...
            \begin{tikzcd}[row sep = tiny]
                  ...
                  T \arrow[r, mapsto]
                  &
                  \Hom_{\mathsf{Set}^{O_G^{op}}}(\Phi(G \cdot E(T)), \UC)
            \end{tikzcd}
      \end{equation}
      In this context, a map $\phi: (T,\mathfrak c) \to (S, \mathfrak d)$ of colored $G$-trees is itself colored iff
      ...
      \begin{equation}
            ...
            \begin{tikzcd}[row sep = tiny]
                  ...
                  &&
                  \Phi(G \cdot E(S)) \arrow[dl, "\mathfrak d"]
                  \\
                  &
                  \UC
            \end{tikzcd}
...
\begin{definition}
      The category of \textit{symmetric $(G,\mathfrak C)$-sequences}, denoted $\Sym^{G,\mathfrak C}$, is
...
\begin{definition}
      \label{EG_DEFN}
      The \textit{edge orbit} functor $E_G: \Omega_G \to \mathsf F \wr O_G$ sends a $G$-tree $T$ to
...
\begin{remark}
      ...
      {\color{OliveGreen}
        One may be tempted to say that $E_G$ gives a functor $\Omega_G \to \Set \downarrow O_G$,
        with $E_G(T) \to O_G$ given by $G t \mapsto G/G_t$ for $t \in G t$ minimal.
        However, this fails to be a functor, as it does not allow for the action of \textit{quotient} maps.
        It is however a functor when restricted to the category $\Omega_G^{fr}$ of \textit{free} $G$-trees
        (cf. \eqref{COMEGA_FG_GR_EQ}).

        Instead, $E_G$ defines a functor to $\Cat \downarrow_w O_G$ of functors and ``weakly-commuting'' triangles,
...
a $G$-tree $T$ and
a functor $\mathfrak c: E_G(T) \to \underline{\mathfrak C}$ over $O_G$.
Explicitly, each orbit of edges $G t$ (with $t$ minimal)
is assigned a color $\mathfrak c(G t) \in \underline{\mathfrak C}(G/G_{t})$.
...
\begin{remark}
      {\color{OliveGreen}
        \todo[inline]{How do we define a Grothendieck construction that works, analogous to \eqref{COMEGA_OG_GR_EQ}?}
        It is not
        \begin{equation}
              \begin{tikzcd}[row sep = tiny]
                    \Omega_G^{op} \arrow[r]
                    ...
                    T \arrow[r, mapsto]
                    &
                    \Hom_{\Cat \downarrow_w O_G}(E_G(T), \UC)
              \end{tikzcd}
        \end{equation}
        as we want the coloring to be strictly equivariant.
        Instead, it uses the fact the $E_G$ is weak, and that weakness can be exploited since $\UC$ is a root fibration.
      }
\end{remark}
...
\begin{remark}
      ...
      \begin{equation}
            \begin{tikzcd}[row sep = tiny]
                  ...
                  T \arrow[r, mapsto]
                  &
                  \Hom_{\Set^{O_G^{op}}}(\Phi(E(T)), \underline{\mathfrak C}),
            \end{tikzcd}
      \end{equation}
...
\begin{remark}
      ...
      with the coloring of $q^{\**}T$ defined as follows:
...
%       $\underline{\mathfrak C}\Omega^G$ of ``$\underline{\mathfrak C}$-colored trees with $G$-action''.
%       \todo[inline]{leaf-root for later?}
%       Any colored $G$-tree $(T, \mathfrak c)$ with $T = (T_a)_{a \in A}$ has that each $T_a$ is a
%       ``$\underline{\mathfrak C|_{G_a}}$-colored tree with $G_a$-action.''
% \end{remark}
...
\begin{proof}
      ...
      such that
      the existence of $U_T$ and $\mathfrak c_{U_T}$ follow from the existence of the pushout below in $\underline{\mathfrak C}\Omega_G^{pt,cf}$.
      \begin{equation}
            \begin{tikzcd}
      ...
      (with unique choices such that $(U_{v_{Ge}}, \mathfrak c_{U_{v_{Ge}}}) \into(U_R, \mathfrak c_{U_R})$ is planar [and color-fixed]).
...
\begin{example}
      ...
      \end{equation}
      where
      $(\F_n)_{\ksi}$ is all $\Gamma \in \F_n$ such that $\Gamma \leq \mathsf{Aut}(\ksi)$.
...
\begin{theorem}
      ...
      \begin{equation}
            \O(\ksi)^\Gamma \to \O'(\ksi)^\Gamma
      \end{equation}
      are weak equivalences (resp. fibrations) in $\V$ for all
      $\mathfrak C$-signatures $\ksi$ and
      graph subgroups $\Gamma \leq \Stab(\ksi)$.
...
\begin{proof}[{proof of Theorems \ref{THM1_C} and \ref{THM2_C}}]
      ...
      \begin{equation}
            ...
            \begin{tikzcd}[column sep =5em]
                  ...
                  \ar[shift left=1.5]{l}
                  \\ % ---------- NEXT ROW ----------
                  \mathop{\prod}\limits_{\ksi \in \UC\Sigma}
                  \mathcal{V}^{\mathsf{Aut}(\ksi)}_{\F_\ksi}
                  \ar[shift left=1.5]{r}
                  &
                  \mathsf{Sym}^{G, \mathfrak C}_\F(\mathcal{V}) 
                  \arrow[l, shift left=1.5, "\left(\text{ev}_{\ksi}(\minus)\right)"] 
...
\begin{definition}
      ...
      \begin{equation}
            \begin{tikzcd}[row sep = tiny]
                  ...
                  &
                  \Hom_{\Set^{O_G^{op}}}(\Phi(G \cdot_\Gamma \underline{n+1}), \mathfrak C)
            \end{tikzcd}
      \end{equation}
...
\begin{proof}
      ...
      \todo[inline]{or just $Y(q^{\**}(-))$, with $\UC O_{\F_n}$ a fibration over $O_G$}
...
We follow \cite{BM13} and \cite{Cav14} to assemble the collection of "color-fixed" ($\F$)-model structures into
...
It is straightforward that the operad $C_\Gamma[X]$ has the universal property
\begin{equation}
      \Hom_{\Op^G(\V)}(C_\Gamma[X], \O) = \mathop\prod\limits_{\ksi \in (\mathfrak C(\O)^{\times n+1})^\Gamma}\Hom_{\V^\Gamma}(X, \O(\ksi)).
\end{equation}

Define $I_{\F,loc}$ and $J_{\F, loc}$ to be the sets
\begin{align*}
  \set{C_{\Gamma}[\Gamma/\Gamma \cdot i]}, \qquad \qquad \set{C_{\Gamma}[\Gamma/\Gamma \cdot j]},
\end{align*}
...
\end{lemma}
\begin{proof}
      By definition, $F$ is a trivial $\F$-fibration iff
...
\begin{proof}
      ...
      Hence, since $G/H \cdot (\1 \to \1 \amalg \1)$ is in $I_{\F}\mbox{-cof}$, the composite
      \begin{equation}
            \begin{tikzcd}
...
\begin{lemma}
      ...
      If weak $\F$-equivalences are closed under transfinite compositions, then relative $J_{\F}$-cells are weak equivalences.
\end{lemma}
\begin{proof}
      ...
      We know that $\psi$ is injective on colors and fully-faithful
      ...
      and hence is a local weak $\F$-equivalence in $\mathsf{Op}^G(\V)$. 
      ...
      \begin{equation}
            ...
            \end{tikzcd}
      \end{equation}
      Hence $\psi'$ is essentially surjective and a local weak $\F$-equivalence, hence a weak $\F$-equivalence in $\mathsf{Op}^G(\V)$. 

      Now, consider the top pushout. Similarly, \cite[4.20]{Cav14}says that pushouts of this form are created in $\Op(\V)_{\C(X)}$
      as the pushout
      \begin{equation}
            \begin{tikzcd}
      ...
      Moreover, by \cite[Thm. 1.15]{BM13} and the assumption that $1_\V$ is cofibrant,
      $\phi$ is a trivial cofibration in $\Op(\V)$,
      and hence $p_! (G/H \cdot \phi)$ is a trivial cofibration in $\Op^{G,\C(X)}(\V)$.
...
However, the relevant notion of weak $\F$-equivalence on colours for this paper lives in $\V^{\Aut(\ksi)}_{\F_\ksi}$ as opposed to $\V$,
...
\begin{proof}
      ...
      $\O_1(\ksi)^\Gamma \simeq \O_2(F(\ksi))^\Gamma \simeq \O_3(LF(\ksi))^\Gamma$.
...
$\mathsf{sSet}$ is right proper.
	...
	yielding a countable contractible subcomplex whose image in $\mathcal{O}$ contains $g$. Lastly, performing a cofibrant replacement if that complex finishes the proof.
