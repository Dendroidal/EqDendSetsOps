...
\begin{definition}
      ...
      \item For any compatible signatures $\ksi = (c_1, \ldots, c_n; c_0)$, $\ksi_i = (c_{i,1}, \ldots, c_{i,m_i}; c_i)$, a \textit{composition} map
            ...
            \end{equation}
      \end{enumerate}
      subject to the compatibilities one would expect.
...
\begin{remark}
	...
	Indeed, objects in $\Op(\V^G)$ have a $G$-fixed set of colors, and each level $\O(\ksi)$ is a full $G$-set
...
Many of the natural functors around $\Omega$ and $\Sigma$ have generalizations to the colored setting,
...
\begin{remark}
      { \color{blue} % -------------------- ALTERNATIVELY: ----------------------------------------
        ...
        Then $\CO$ is also isomorphic to the pullback below\footnote{We note that the class of morphisms in $\mathsf F^G$ in the image of $E$ (restricted to $\Omega_G^0$)
...
\begin{proof}
      ...
      \begin{enumerate*}
      \item[(iv)] $\sSet$ is right proper,
      \item[(v)] $\sSet$ has a generating set of intervals, and
      \item[(vi)] the class of genuine weak equivalences in $\mathsf{Op}^G(\sSet)$ is closed under transfinite compositions.
      \end{enumerate*}
      $(iv)$ follows by e.g. \cite[Prop 2.1.5]{Cis06},,
      $(v)$ follows by e.g. \cite[Lemma 1.12]{BM13}, and
      $(vi)$ follows by an argument analogous to \cite[Lemma 1.24]{CM13b}.

      Now, we note that condition $(vi)$ proves the analogous statement for any $\F$,
