...
\begin{example}
      ...
      %
      These are both isomorphic to the standard Grothendieck construction $(BG \ltimes \bar{\mathcal C})$
      on the composite
      \begin{equation}
            BG^{op} \xrightarrow{(-)^{-1}} BG \longto \Cat,
            \qquad \qquad
            \** \longmapsto \mathcal C,
            \qquad
            (\** \xrightarrow{g} \**) \longmapsto (\mathcal C \xleftarrow{g^{-1}} \mathcal C);
      \end{equation}
      here the bar is referring to the conjugate \textit{right} $G$-object (i.e. $G^{op}$-object) associated to the given left $G$-object.
      % These are both isomorphic to the type-(iii) Grothendieck construction $(BG \ltimes \mathcal C)^{op}$ from Remark \ref{GRTH_RMK} on the functor
      % \begin{equation}
      %       (BG^{op})^{op} \to \Cat,
      %       \qquad \qquad
      %       \** \longmapsto \mathcal C.
      ...
      \todo[inline]{come back: is this really the convention I want?}
...
\begin{notation}
      \label{F_WR_NOT}
      We let $\mathsf F$ denote a full subcategory of \textit{ordered finite sets} and set maps,
      such that the only ordered isomorphisms are the identity.
      
      Given a category $\mathcal C$, let $\mathsf F \wr \mathcal C$ denote the (standard) Grothendieck construction on the functor
      \begin{equation}
            \mathsf F \longto \Cat,
            \qquad \qquad
...
\begin{proof}
      ...
      Fix $(e, \epsilon(e) \xrightarrow{t} \mathfrak C)$ in $\mathcal E_{\mathfrak C}$,
      and $\ki = \begin{cases}
...
\begin{definition}
      Given a category $\mathcal C$ over $\Sigma$, let $\mathcal C_{\mathfrak C}$ denote the pullback
      \begin{equation}
            \begin{tikzcd}
...
\begin{remark}
      ...
      where $E: \Sigma \to \Sigma$ sends $\underline{n}$ to $\underline{n+1}$.
      ...
      \begin{equation}
            ...
            \begin{tikzcd}
                  ...
                  &
                  G^{op} \ltimes (\Sigma \wr \mathfrak C) \arrow[d] \arrow[r]
                  &
                  \Sigma \wr (G^{op} \ltimes \mathfrak C) \arrow[d]
                  \\
                  \Sigma \arrow[r, "E"]
                  &
                  \Sigma
      ...
      We note that $G \ltimes \OC$ is given by the right pullback,
      as $G \ltimes (-)$ preserves pullbacks and the rightmost square is a pullback
      ...
      given by $\pi_{\mathfrak C}^{\**}$ of the normal inclusion of corollas,
...
Unpacking definitions (cf. Remark \ref{G_GR_REM}), we see that a map $(U, \mathfrak c) \to (V, \mathfrak d)$ in $G \ltimes \Omega_{\mathfrak C}^{op}$
is given by
a map $f: U \to V$ in $\Omega^{op}$ and an element $g\in G$,
such that $\mathfrak c(f(e)) = g.\mathfrak d(e)$ for all $e \in E(V)$.
\begin{equation}
      \begin{tikzcd}
            E(U) \arrow[d, "\mathfrak c"']
            &
            E(V) \arrow[d, "\mathfrak d"] \arrow[l, "f"']
            \\
...
\end{proposition}
\begin{proof}
      We have that $|\OC^{\bullet, \lambda_l}| \simeq \OC^{t,\lambda_l}$ and $|\OC^{t,\lambda_\bullet}| \simeq \OC^e$
      by natural generalizations of Remark 5.36 and Proposition 5.41 in \cite{BP17}.
...
As our desired filtration is just on the \textit{objects}, we can use Lemma \ref{GD_PULL_LEM} to ignore the $G \ltimes (-)$ parts of this presentation.
Let $U^G: \V^{G \ltimes \mathcal C} \to \V^{\mathcal C}$ denote the forgetful functor.

\begin{corollary}
      $U^G \O[u] \simeq \Lan_{(\OC^e \to \SC)^{op}}\tilde N$.
\end{corollary}
...
\begin{lemma}
      $\hat\Omega_{\mathfrak C}^e \into \OC^e$ is $\Ran$-initial.
\end{lemma}
\begin{proof}
      As $\hat\Omega^e \into \OC^e$ is $\Ran$-initial over $\Sigma$, this follows from Lemma \ref{RANINIT_PULL_LEM}.
\end{proof}
\begin{proof}
      We have a right retraction $\hat \Omega^e \into \Omega^e \to \hat \Omega^e$
      by \cite[Prop. 5.49]{BP17},
      and functoriality of $\pi_{\mathfrak C}^{\**}$ implies
      we have a right retraction
      $\hat \Omega_{\mathfrak C}^e \into \OC^e \to \hat \Omega_{\mathfrak C}^e$.
      As before, this implies we have right retractions on all undercategories, showing $\Ran$-initiality.
\end{proof}

\begin{proposition}
      \label{OU_RED3_PROP}
      $U^G \O[u] \simeq \Lan_{(\hat\Omega_{\mathfrak C}^{e} \to \SC)^{op}} \tilde N^{(\O,X,Y)}$. 
\end{proposition}

We will now filter the category $\hat\Omega_{\mathfrak C}^e$ to produce a levelwise filtration of $\O \to \O[u]$.
...
\begin{remark}
      ...
      In particular, applying Corollary \ref{PI_GFIB_COR} to \cite[Remark 5.57]{BP17}, we have a map of Grothendieck fibrations
      \begin{equation}
            \begin{tikzcd}
                  \hat\Omega_{\mathfrak C}^e[k \setminus Y] \arrow[rr, hookrightarrow] \arrow[dr]
                  &&
                  \hat\Omega_{\mathfrak C}^e[k] \arrow[dl]
                  \\
                  &
...
\begin{lemma}
      \label{LANINT_LEM}
      $\hat\Omega_{\mathfrak C}^e[\leq k-1] \into \hat\Omega_{\mathfrak C}^e[\leq k \setminus Y]$ is $\Ran$-initial over $\SC$.
\end{lemma}
\begin{proof}
      This follows from Lemma \ref{RANINIT_PULL_LEM} and the non-colored case \cite[Lemma 5.58]{BP17}.
\end{proof}
\begin{remark}
...
\begin{definition}
      ...
      \begin{equation}
            \begin{tikzcd}
                  |[alias = A]| \hat\Omega_{\mathfrak C}^e[\leq k]^{op} \arrow[r, "\tilde N"] \arrow[d, "\mathsf{lr}"']
                  &
                  \V
                  \\
                  \SC^{op} \arrow[ur, "\O_k"', ""{name = B}]
                  \arrow[Rightarrow, from = A, to = B]
            \end{tikzcd}
      \end{equation}
\end{definition}

Since $\hat\Omega_{\mathfrak C}^e[\leq 0] \simeq \SC$, and
$\hat\Omega_{\mathfrak C}^e$ is the union of (the nerves of) the $\hat\Omega_{\mathfrak C}^e[\leq k]$,
...
\begin{equation}
      ...
      \begin{tikzcd}
            \hat\Omega_{\mathfrak C}^e[k \setminus Y] \arrow[r] \arrow[d]
            &
            \hat\Omega_{\mathfrak C}^e[\leq k \setminus Y] \arrow[d]
            \\
            \hat\Omega_{\mathfrak C}^e[k] \arrow[r]
            &
            \hat\Omega_{\mathfrak C}^e[\leq k].
      \end{tikzcd}
\end{equation}
In fact, this is a pushout at the level of nerves, since
\begin{equation}
      \hat\Omega_{\mathfrak C}^e[k]
      \cap
      \hat\Omega_{\mathfrak C}^e[\leq k \setminus Y]
      =
      \hat\Omega_{\mathfrak C}^e[k \setminus Y],
      \qquad
      \hat\Omega_{\mathfrak C}^e[k]
      \cup
      \hat\Omega_{\mathfrak C}^e[\leq k \setminus Y]
      ...
      \hat\Omega_{\mathfrak C}^e[\leq k],
\end{equation}
and a map $T \to S$ in $\hat\Omega_{\mathfrak C}^e[\leq k]$ is in one of the subcategories named in \eqref{FILT_PRELAN_EQ} iff $T$ is.

Applying left Kan extension over $\SC^{op}$ to \eqref{FILT_PRELAN_EQ} and reducing via Lemma \ref{LANINT_LEM}
yields the pushout diagram
\begin{equation}
      \begin{tikzcd}
            \label{FILT_LAN_EQ}
            \Lan_{\hat\Omega_{\mathfrak C}^e[k \setminus Y]^{op}} \tilde N \arrow[r] \arrow[d]
            &
            \O_{k-1} \arrow[d]
            \\
            \Lan_{\hat\Omega_{\mathfrak C}^e[k]^{op}} \tilde N \arrow[r]
            &
            \O_k
      \end{tikzcd}
\end{equation}
...
\begin{proof}
      ...
      This follows from Remark \ref{OEFIB_REM}.
      By first left Kan extending to $\OC^a[k]$, we can rewrite the leftmost map in \eqref{FILT_LAN_EQ} as
      \begin{equation}
            \Lan_{(\OC^a[k] \to \SC)^{op}}\left(
      ...
      Finally, the description of the leftmost map in \eqref{FILT_PUSH_EQ} follows by noting that
      the undercategories $C \downarrow \OC^a[k]$ are groupoids.
\end{proof}

\begin{remark}
      We also have a description of $\O_{k-1}(C) \to \O_k(C)$ as the pushout below,
      as the analysis after Proposition \ref{OU_RED2_PROP} basically follows even after applying $G \ltimes (-)$.
      \begin{equation}
            \label{FILT_PUSHG_EQ}
            \begin{tikzcd}
...
\begin{theorem}
      ...
      More generally, for $\F = \set{\F_n}_{n \geq 0}$ with $\F_n$ an arbitrary collection of subgroups of $G \times \Sigma_n$,
...
\begin{proof}
      ...
      and its existance and properties follow from Proposition \ref{FILT_PUSH_PROP} above
      and the colored (and non-$G$-tree) generalization of \cite[Lemma 5.72]{BP17}, which again follows from \cite[Prop. 6.24]{BP17},
      completely analogously to the proof of Theorem I from \cite{BP17}.
\end{proof}

\begin{remark}
      The category $\Op^{G,\mathfrak C}(\Top)$ actually has full model structures for all indexing families $\F$ lifted from $\Sym^{G, \mathfrak C}_\F(\Top)$,
...
\begin{proof}
      ...
      Recalling the filtration in \eqref{FILT_PUSH_EQ}, % We have/will show {\color{red} COME BACK} that we have a filtration
      % \begin{equation}
