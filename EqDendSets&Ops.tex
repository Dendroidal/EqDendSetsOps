...
{\color{OliveGreen} In this section, assume that $\V$ admits (semi)-transfer for categories and operads}.

As in the previous section, we fix an \textit{indexing collection} $\F = \set{\F_n}$ of families $\F_n$ of graph subgroupos of $G \times \Sigma_n$

\begin{definition}
      We say $\V$ \textit{admits (semi)-transfers for $G$-categories/operads} if
      $\V$ has cellular fixed points and
...
\begin{equation}
      \Hom_{\Op^G(\V)}(\mathbb F_\Gamma[X], \O) = \mathop\prod\limits_{\ksi \in (\mathfrak C(\O)^{\times n+1})^\Gamma}\Hom_{\V^\Gamma}(X, \O(\ksi)).
\end{equation}
...
\begin{proof}
      ...
      If $j \in J_{\F, loc}$, then
      the existance of the model structure on $\Op^{G, \mathfrak C(\O)}(\V)$
      combined with \eqref{COLOR_SQ_EQ}
      implies that the pushout is again a local weak $\F$-equivalence which is the identity on colors,
      and hence is a weak $\F$-equivalence. 
...
\todo[inline]{If $\J$ is a $\V$-interval, is $\J(0,0)$ a cylinder object?}
...
\begin{definition}
      ...
      the same objects as $\mathcal C$, and $\pi_0(\mathcal C)(x,y) = \Ho(\V)(1_\V, \mathcal C(x,y))$.
\end{definition}
...
\begin{definition}
      ...
      \item \textit{virtually $H$-equivalent } if
            ...
            \item they are virtually equivalent in $\mathcal C^H$
            \item they are $H$-equivalent in some fibrant replacement $\mathcal C_f \to \mathcal C$
                  in $\Op^{G, \mathfrak C(\O)}(\V)$.
            \end{enumerate}
...
\begin{proof}
      ...
      Reflexivity follows from the composition $\I_c \to \I \to \C^H, \C_f^H$
      ...
      $(h \** h') \circ i_0 = h \circ i_0$ and $(h \** h') \circ i_1 = h' \circ i_1$.
\end{proof}

\begin{lemma}
      [{cf. \cite[4.12]{Cav14}, \cite[2.10]{BM13}}]
      \label{RIGHTPROPER_LEM}
      If $\V$ is right proper, then all virtual $H$-equivalent colors are $H$-equivalent. 
\end{lemma}
\begin{proof}
      {\color{OliveGreen}
        Need: virtual $H$-equivalent (i). Then it follows identically as in \textit{loc cite}.
        
        Also, if ``interval'' means there is a direct weak equivalence $\J \to \I$, then
        we also need that $1_\V$ is cofibrant;
        the argument in \cite{BM13} just gives a zig-zag of weak equivalences.
      }
\end{proof}
...
\begin{proof}
      {\color{OliveGreen}
        Need: virtual $H$-equivalent (i). Then it follows exactly as in \textit{loc cite}.
      }
\end{proof}

\begin{lemma}
      [{cf. \cite[4.11]{Cav14}, \cite[2.9]{BM13}}]
      Any local weak $\F$-equivalence $F: \O\to \P$ in $\mathsf{Op}^G(\V)$
      reflects virtual $H$-equivalences.
\end{lemma}
\begin{proof}
      {\color{OliveGreen}
      ...
      Since each $j^{\**}: \Op^{G, \mathfrak C}(\V) \to \Cat^{G, \mathfrak C}(\V)$ is right Quillen,
      it commutes with fibrant replacement.
      we may choose a fibrant replacement $j^{\**}F_f$ of $j^{\**}F$ which is a local trivial $\F$-fibration:
      \begin{equation}
            \begin{tikzcd}
                  \O \arrow[d, dashed, "\simeq"'] \arrow[r, "\simeq"]
                  &
                  F^{\**} \O' \arrow[r, "\cong", two heads] \arrow[d, "\simeq"']
                  &
                  \O' \arrow[d, "\simeq"]
                  \\
                  \O_f \arrow[r, dashed, two heads]
                  &
                  F^{\**} \O'_f \arrow[r, "\cong", two heads]
                  &
                  \O'_f
            \end{tikzcd}
      \end{equation}
      (where all designations here are ``local'').
      
      This implies each $(F_f)^H$ is path-lifting,
      and hence any virtual $H$-equivalence $\J \to (\O'_f)^H$ between colors  
      $F'(a) = F(a)$ and $F'(b) = F(b)$
      ...
      \todo[inline]{CHECK THE STABILIZERS}
\end{proof}
