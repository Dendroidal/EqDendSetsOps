...
{\color{cyan}
  \begin{remark}
        ...
        % 
        Moreover, for any fibration $E$ over $\mathcal C$ and weak map $(F, \Phi)$ of spans
        \begin{equation}
              \begin{tikzcd}[row sep = tiny]
                    \mathcal D \arrow[dr, "p", ""'{name=U}] \arrow[dd, "F"']
                    \\
                    & \mathcal C
                    \\
                    |[alias=V]| \mathcal E \arrow[ur, "q"']
                    \arrow[Rightarrow, from = U, to = V]
              \end{tikzcd}
        \end{equation}
...
%            \end{tikzcd}
      ...
      and the second arrow is a genuine $\Sigma_n$-cofibration since $\V$ has cofibrant pushout powers.
...
\begin{convention}
      ...
      In general, we will be interested in \textit{left} $G$-objects.
      When considering groups as a category, however, we will let $G$ denote the category which encodes
      \textit{right} $G$-objects, with composition given by
      $\left( \** \xrightarrow{g} \** \xrightarrow{h} \** \right)
      =
      \left( \** \xrightarrow{gh} \** \right)$. 
      Counter to this, we will abuse notation and still use $\V^G$ to denote the category of left $G$-objects in $\V$.
\end{convention}
...
\begin{notation}
      [{cf. \cite{BP17}}]
      We let $\mathsf F$ denote a full subcategory of \textit{finite sets} equipped with a total order and set maps,
...
We will now move into the world of $G$-equivariant colored operads.
...
$\underline{c}_d$ is a $G/e$-indexed diagram\footnote{
  We recall that $(G \ltimes (G/e))^{op} = B_{G/e} G$.}
$E G := (G \ltimes (G/e))^{op} \to \mathcal C_d$,
...
Thus, for $E = \Op(\V) = \mathsf F \ltimes \Op^{(-)}(\V)$,
...
Explicitly, $\O$ is given by the following data:
...
\subsubsection{Colored trees and a monadic description of equivariant operads}
\label{COMEGA_SEC}

We now categorify the set of $\mathfrak C$-signatures, generalizing the categories $\Sigma$ and $G \times \Sigma$.
...
\begin{definition}
      ...
      \end{equation}
      and call the iterated Grothendieck construction
      \begin{equation}
            G \ltimes \Sigma_{\mathfrak C} = \coprod\limits_{n\geq 0} (G \times \Sigma_n) \ltimes \mathfrak C^{\times n+1}.
      \end{equation}
      the \textit{$(G,\mathfrak C)$-symmetric category}.
...
such that $\mathfrak c(e) = g.\mathfrak d(f(e))$ for all $e \in E(U)$.
\begin{equation}
      \begin{tikzcd}
            ...
            \mathfrak C 
            &
            \mathfrak C \arrow[l, "g"]
      \end{tikzcd}
\end{equation}

In particular, we have maps of the form
\begin{equation}
      g = (id, g): (U, E(U) \xrightarrow{\mathfrak c} \mathfrak C \xrightarrow{g \cdot} \mathfrak C)
      \to
...
\begin{remark}
      ...
      \begin{equation}
            ...
            \begin{tikzcd}[row sep = tiny]
                  ...
                  &
                  (E(U) \xrightarrow{\phi} E(V) \xrightarrow{\mathfrak c} \mathfrak C \xrightarrow{g} \mathfrak C)
            \end{tikzcd}
      \end{equation}
...
\todo[inline]{Is this remark needed/useful?}
{\color{red}
  \begin{remark}
        Note that we can replace the $G$-set $\mathfrak C$ with a
        \textit{coefficient system} $\underline{\mathfrak C}$,
        defining categories $\Omega_{\UC}$ generalizing the $G \ltimes \Omega_{\mathfrak C}$.
        % --------------------------
        ...
        We note that the $G$-set definitions are recovered by replacing $\UC$ with $O_G \ltimes \Phi(\mathfrak C)$,
        and, in any case, the resulting categories are always equal to $G \ltimes \Omega_{\mathfrak C(G/e)}$.
  \end{remark}
}
      % ------------------------------------------------------------
...
\begin{remark}
      ...
      We note that the set of objects in $G \ltimes \Sigma_{\mathfrak C}$ is precisely the set of $\mathfrak C$-signatures,
      and every $\mathfrak C$-colored operad has an underlying symmetric $(G,\mathfrak C)$-sequence.
...
\begin{definition}
      ...
      \begin{equation} 
            \begin{tikzcd}
                  ...
                  \arrow[r, "V"]
                  &
                  (\Sigma \wr (G \ltimes \Sigma_{\mathfrak C}))^{op} \arrow[r, "\Sigma \wr X"]
                  \arrow[dl, Rightarrow]
                  &
...
{\color{cyan}
  \begin{remark}      
        ...
        $(d, c \xrightarrow{\alpha} p(d)) \mapsto c$ from Remark \ref{FIB_WSPAN_REM}.
...
\begin{remark}
      $\underline{\mathfrak C}\Omega_G$ is also naturally a \textit{root fibration}, 
...
\begin{equation}
      ...
      \begin{tikzcd}[row sep = tiny]
            \iota:
            &
            G \ltimes \Omega_{\UC} \arrow[r, hookrightarrow]
            &
            \Omega_{G, \UC}
            \\
      ...
      \end{tikzcd}
\end{equation}
sending maps $(\phi, g): (U, \mathfrak c) \to (V, \mathfrak d)$ to $(-) \cdot g \circ \phi$.
{\color{blue} Alternatively, sending $(\phi,g)$ to $((h,t) \mapsto (hg^{-1}, \phi(t)))$.}
In particular, for $u \in E(U)$ and $G(g,u) = (G,u)$, we have maps
\begin{equation}
  \phi_{G,u}: (G,u) \to (G, \phi(u), \qquad \qquad (g,u) \mapsto (gh, \phi(u))
  \\
  \phi_{G,u}^{\**}: \mathfrak C \to \mathfrak C, \qquad \qquad c \mapsto h c
\end{equation}
such that
\begin{equation}
      \mathfrak c(G,u) = \mathfrak c(u) = h \mathfrak d(\phi(u)) = \phi_{G,u}^{\**}\mathfrak d(G, \phi(u))
\end{equation}

This defines a pair of adjunctions
\begin{equation}
      \label{ISTAR_ADJ_EQ}
      \begin{tikzcd}[column sep =5em]
            \V^{\Omega_{G,\UC}^{op}} \arrow[r, "\iota^{\**}"']
            &
            \V^{(G \ltimes \Omega_{\UC})^{op}}
            \arrow[l, bend right, "\iota_{!}"']
            \arrow[l, bend left, "\iota_{\**}"]
      \end{tikzcd}
\end{equation}
The adjoints can be described explicitly.
%
We begin by constructing ``component'' maps in the opposite direction of $\iota$.
Given $(T = (T_i)_I, \mathfrak c) \in \UC\Omega_G$, we define
$(T_i, \mathfrak c_i) \in G \ltimes \Omega_{\UC})$ by defining
\begin{equation}
      \mathfrak c_i: E(T_i) \to \mathfrak C(G),
      \qquad \qquad
      \mathfrak c_i(t) = q_t^{\**}(\mathfrak c (G t)),
\end{equation}
where
$q_t: G \to Gt$ sends $e \mapsto t$
(equivalently, $(G \cdot T_i,\mathfrak c_i) = (G \cdot T_i, q^{\**} \mathfrak c_i) = q^{\**}(T, \mathfrak c)$, where
$q: G \cdot T_i \to T$ is the unique quotient map sending $(e,r_i)$ to $r_i$). 
...
Then the map $G \ltimes I \to \UC\Omega$, $i \mapsto (T_i, \mathfrak c_i)$ is a functor
...
\begin{equation}
      ...
      \iota_!Y(T, \mathfrak c) =
      \begin{cases}
            Y(T_1, \mathfrak c_1) \quad & \mbox{$T$ $G$-free}
            \\
...
\begin{remark}[{cf. \cite[Rem 4.35]{BP17}}]
      ...
      the canonical map
      \begin{equation}
            X(T,\mathfrak c) \xrightarrow{\simeq} X(q^{\**}(T, \mathfrak c))^\Gamma
      \end{equation}
...
\begin{proposition}
      ...
      \begin{equation}\label{FILTRATION_LAN_EQ}
            \begin{tikzcd}
...
Let $\F = \set{\F_n}$ be an indexing family containing all trivial $e$-sets $\set{e} \leq G \times \Sigma_n$.

Recall the inclusion of the orbit category $\O_{\F_n} \into \Sigma_G$ as those $G$-trees $C_\Gamma$
with underlying $G$-set of the form $G \cdot_\Gamma \underline{n+1}$, $\Gamma$ a graph subgroup of $G \times \Sigma_n$
(the planar structure is determined entirely by the planar structure on $\underline{n}$ and $G$).

\begin{definition}
      Given a coefficient system $\UC$, we define the category $\O_{\F_n, \UC}$ to be
      the Grothendieck construction on the functor
      \begin{equation}
            \O_{\F_n}^{op} \longrightarrow \Set
            \qquad \qquad
            G \times \Sigma_n / \Gamma \longmapsto \Hom_{\Cat \downarrow O_G}(E_G(C_\Gamma), \UC)
            {\color{blue} \simeq \Hom_{\Set^{O_G^{op}}}(\Phi \ltimes (G \cdot_\Gamma \underline{n+1}), \UC)},
      \end{equation}
      as in \eqref{COMEGAG_OG_GR_EQ}.
...
Explicitly, objects in $O_{\F_n, \mathfrak C}$ are given by $G \times \Sigma_n/ \Gamma$ along with a $\mathfrak C$-coloring of $C_\Gamma$,
and arrows are maps $\phi = (h,\tau)\Lambda: G \times \Sigma_n / \Gamma \to G \times \Sigma_n / \Lambda$ in $O_{\F_n}$
such that the induced map $C_\Gamma \to C_\Lambda$ is a map of $\mathfrak C$-colored trees $G$-trees.

{\color{blue}
  \begin{remark}
        Alternatively, we require that
        $\mathfrak d(G/H, [g,i]) = \mathfrak c(G/H, [g h, \tau^{-1}(i)]$
        for all $(G/H, [g,i]) \in \Phi(G \cdot_{\Lambda} \underline{n+1})$.
  \end{remark}
}

\begin{lemma}
      The natural inclusion $\coprod_{n \geq 0} \O_{\F_n, \UC} \xrightarrow{\simeq} \Sigma_{G, \UC}$
      is an equivalence of categories,
      with strict image (on level $n$)
      those colored $G$-trees with underlying tree in the standardized planar form $C_\Gamma = G \cdot_\Gamma C_n$.
\end{lemma}

If $\UC$ is actually a $G$-set $\Phi \mathfrak C$, then
objects are just pairs $(\Gamma, \ksi \in (\mathfrak C^{\times n+1})^\Gamma)$.
as maps $E_G(C_\Gamma) \to \Phi(\mathfrak C)$ over $O_G$ are equivalently $G$-maps of sets $C_\Gamma \to \mathfrak C$,
and arrows $(\Gamma,\ksi) \to (\Lambda,\theta)$ are maps $\phi = (h,\tau)\Lambda \in \O_{\F_n}$ such that
$\ksi = (h, \tau)\theta$.

The inclusion \eqref{CTREES_INC_EQ} of $\mathfrak C$-colored trees into $\mathfrak C$-color $G$-trees
restricts along the natural subcategories
\begin{equation}
      \begin{tikzcd}
            B_n(\UC) := (G \times \Sigma_n) \ltimes \mathfrak C(G/e)^{\times n+1} \arrow[r, hookrightarrow] \arrow[d, hookrightarrow]
            &
            G \ltimes \Sigma_{\UC} \arrow[r, hookrightarrow] \arrow[d, hookrightarrow]
            &
            G \ltimes \Omega_{\UC} \arrow[d, hookrightarrow]
            \\
            O_{\F_n, \UC} \arrow[r, hookrightarrow]
            &
            \Sigma_{G,\UC} \arrow[r, hookrightarrow]
            &
            \Omega_{G, \UC}
      \end{tikzcd}
\end{equation}
where in the above notation we see $\iota(\ksi) = (G \times \Sigma_n / e, (g,i) \mapsto g.c_i)$.
% We have natural maps
% \begin{equation}
%       \begin{tikzcd}[row sep = tiny]
%             B_n(\UC) := (G \times \Sigma_n) \ltimes \mathfrak C(G/e)^{\times n+1} \arrow[r, "\iota"]
%             &
%             O_{\F_n, \UC} \arrow[r, "\gamma"]
%             &
%             O_{\F_n, \Phi\mathfrak C(G/e)} =: O_{\F_n, \mathfrak C}
%             \\
%             \ksi \arrow[r, mapsto]
%             &
...
%             \\
%             &
%             (G \times \Sigma_n / \Gamma, \mathfrak c) \arrow[r, mapsto]
%             &
%             (\Gamma, \ksi_{\mathfrak c})
%       \end{tikzcd}
% \end{equation}
This yields a restricted adjunction
\begin{equation}
      \begin{tikzcd}[column sep =5em]
            \V^{O_{\F_n, \mathfrak C}^{op}} \arrow[r, "\iota^{\**}"']
            &
            \V^{B_n(\UC)^{op}}
            \arrow[l, bend right, "\iota_{!}"']
            \arrow[l, bend left, "\iota_{\**}"]
      \end{tikzcd}
\end{equation}
with explicit formula for the adjoints (cf. \eqref{ISTAR_EQ_EQ}) given by
\begin{align*}
  (\iota_{\**} Y)(G \times \Sigma_n/\Gamma, \mathfrak c) & = X(\ksi_{\mathfrak c})^\Gamma
  \\
  (\iota_! X)(G \times \Sigma_n/\Gamma, \mathfrak c) &=
                                                       {\begin{cases}
                                                               X(\ksi_{\mathfrak c}) \qquad \qquad & G \cdot_\Gamma \underline{n+1} \mbox{ is $G$-free} \\
                                                               \varnothing & \mbox{otherwise,}
                                                         \end{cases}}
\end{align*}
where $\ksi_{\mathfrak c} = (c_1,\dots, c_n;c_0)$ with $c_i := q_{G,i}^{\**} \mathfrak c(G q(i))$,
when $q: G \cdot C_n \to C_\Gamma$ is the unique quotient map preserving minimal elements
{\color{blue} (alternatively, $c_i = \mathfrak c(G/e, [e,i])$)}.

\begin{remark}
      We note that the adjoints actually factor through the natural map
      \begin{equation}
            \gamma^{\**}: \V^{O_{\F_n, \Phi\UC(G/e)}^{op}} \longrightarrow \V^{O_{\F_n, \UC}^{op}}
      \end{equation}
      with $\gamma(G \times \Sigma_n / \Gamma, \mathfrak c) = (\Gamma, \ksi_{\mathfrak c})$. 
\end{remark}

We equip $\V^{O_{\F_n, \UC}^{op}}$ with the projective model structure, and
$\V^{B_n(\UC)}$ with the model structure as in Proposition \ref{TRANS_MODEL_PROP} with $\F_{\ksi} = \F_n \cap \Stab(\ksi)$:
$f$ is a weak equivalence (resp. fibration) iff $f(\ksi)^\Gamma$ is so in $\V$ for all $\ksi \in \mathfrak C^{\times n+1}$ and graph subgroups $\Gamma \leq \Stab(\ksi)$. 

\begin{lemma}
      The pair $(\iota^{\**}, \iota_{\**})$ is a Quillen adjunction.
\end{lemma}
\begin{proof}
      We verify that $\iota_{\**}$ is the right adjoint of $\iota^{\**}$.
      In particular, it is clear that $\epsilon$ is a natural isomorphsim
      \begin{equation}
      ...
      \begin{equation}
            ...
            =
            Y(q^{\**}(G \times \Sigma_n/\Gamma, \mathfrak c))^\Gamma
      \end{equation}
      where $q: G \cdot C_n \to C_\Gamma$ is the pullback map (ie. the unique quotient map preserving minimal elements),
      and $\eta$ is the natural induced transformation.
      It is straightforward to check that the pair $(\epsilon, \eta)$ give an adjoint.
      
      Finally, $\iota_{\**}$ is right Quillen by the definition of the above model structures.
...
\begin{proposition}
      The above adjunction is an equivalence if $\mathfrak C$ is a $G$-set.
\end{proposition}
\begin{proof}
      Following the proof in \cite{Ste16}, it suffices to compare the following evaluations:
      \begin{align*}
        \Hom_{\UC O_{\F_n}^{op}}((G \times \Sigma_n/\Gamma, \mathfrak c), (G \times \Sigma_n / \Lambda, \mathfrak d)) \cdot A
        \\
        \left(
        \iota_{\**}\iota^{\**}\Hom_{\UC O_{\F_n}^{op}}((G \times \Sigma_n/\Gamma, \mathfrak c), - ) \cdot A
        \right)
        (G \times \Sigma_n / \Lambda, \mathfrak d)        
      \end{align*}

      The elements in the top are those $(h,\tau)\Gamma$ such that $\mathfrak d([G,i]) = h \mathfrak c([G, \tau^{-1}(i)])$,
      while the bottom are those $(h,\tau)\Gamma$ with 
      $q_{(G,i)}^{**}\mathfrak d([G,i]) = h q_{(G, \tau^{-1}i)}^{\**}\mathfrak c([G, \tau^{-1}(i)])$.
      If $\mathfrak C$ is a $G$-set, then these coincide, as the $q^{\**}$ are just compatible inclusions.
...
\begin{remark}
      Is is easy to construct examples of $\UC$ where the two conditions in the proof are not equivalent.
      For example, if $G/e$ is a point, then the second condition is always satisfied.
\end{remark}
...
\begin{theorem}
      ...
      \item \label{CSPP_LBL} has cofibrant symmetric pushout powers,
            \footnote{Also needed for Lemmas \ref{CAV_4.14_PROP2}}, % \ref{LOCAL_COF_LEM}            
            % --------------------
      \item \label{RP_LBL} is right proper
            \footnote{Needed for Lemma \ref{RIGHTPROPER_LEM} and Lemma \ref{2OUTOF3_PROP}.},
      \item \label{WE_LBL} has that weak equivalences are closed under transfinite composition
            \footnote{Needed for Proposition \ref{J-CELL_PROP}.},
...
\begin{proposition}
      ...
      % \begin{itemize}
      % \item[$\star$]
      the class of weak equivalences is closed under transfinite composition.
      % \item[$\dagger$] as a model category, $\V$ is left proper and either cellular or combinatorial.
      % \end{itemize}
      % and weak $\F$-equivalences are closed under transfinite compositions.
      Then relative $J_{\F}$-cells (with cofibrant source) are weak equivalences.
\end{proposition}
\begin{proof}
      Essentially surjective maps are closed under transfinite composition by Lemma \ref{TRANSCOMP_ES_LEM}.

      Now, it suffices by our assumptions show that the pushout of a map $j \in J_{\F}$ is a local weak $\F$-equivalence. 
      ...
      %       \end{tikzcd}
      % \end{equation}
      Hence $\psi'$ is essentially surjective and a local weak equivalence %trival $\F$-cofibration.
      ...
      and is thus in particular a weak equivalence in $\Op^G(\V)$ %a (local) trivial $\F$-cofibration in $\Op^G(\V)$.
      
      Since both $\phi'$ and $\psi'$ are essentially surjective and local weak equiavlences,
      % local trivial $\F$-cofibrations in $\mathsf{Op}^G(\V)$,
...
\begin{remark}
      The (strong) assumption that weak equivalences in $\V$ are closed under transfinite composition can be replaced with
      ``trivial $\F$-cofibrations in $\Op^G(\V)$ are local $\F$-cofibrations''.
      This should follow from a more complete analysis of the model structures $\Op^{G, \mathfrak C}(\V)$ and the change-of-color maps.
\end{remark}
\todo[inline]{come back: make sense of the argument commented out below, see what is truly needed.}


% %%%%%%%%%%%%% ---------- THIS IS ALL BAD - want something to replace "weak equivalences are preserved by transfinite comp as an assumption --------------------

% % If $\V$ is a \textit{cellular} or \textit{combinatorial} model catgory (see e.g. \cite{Hir03}),
% % then so is $\V^G_\F$ for any group $G$ and family of subgroups $\F$ (it is easy to modify \cite[Thm. 12.1.5]{Hir03}).
% % In addition, if $\V$ is left proper, then all Bousfield localizations $L_S\V^G_{\F}$ of $\V^G_\F$ exist \cite[Thm. 4.1.1]{Hir03}.
% % Specifically, there exists a mixed model structure $\V^G_{\F,mix}$
% % where cofibrations are genuine (i.e. as in $\V^G_{gen}$) and weak equivalences contain the $\F$-weak equivalences.

% % \begin{definition}
% %       Suppose $\V$ is either a cellular or combinatorial model category.
% %       A map $f: \O \to \P$ in $\Op^G(\V)$ is called a \textit{local $\F$-mixed (trivial) cofibration} if
% % each $f(\ksi)$ is a (trivial) cofibration in $\V^{\Stab(\ksi)}_{\F,mix}$. 
% % \end{definition}

% \begin{lemma}
%       \label{COF_TC_LEM}
%       Suppose $\V$ is either cellular or combinatorial. 
%       Local $\F$-mixed (trivial) cofibrations in $\Op^G(\V)$
%       % and trivial genuine cofibrations (with cofibrant source)
%       are closed under transfinite composition.
% \end{lemma}
% \begin{proof}
%       Locally, any string of maps
%       \begin{equation}
...
%       looks like      
%       \begin{equation}
%             \O_0(\ksi) \longrightarrow \O_1(F_1(\ksi)) \longrightarrow \O_2(F_2(\ksi)) \longrightarrow \dots
%             \longrightarrow \O_\alpha(F_\alpha(\ksi)) \longrightarrow \dots
%       \end{equation}
%       where $\bar f_\alpha$ denotes the compoite $\O_0 \to \O_\alpha$.
%       Thus, as the restriction map $\V^{\Stab(f(\ksi))}_{\F_{f(\ksi)},mix} \to \V^{\Stab(\ksi)}_{\F_\ksi, mix}$
%       preserves (trivial) cofibrations by the proof of \cite[Prop 6.6]{BP17}
%       \todo[inline]{NOPE! That Prop is not applicable},
%       % if
%       % \begin{equation}
%       % \O_\alpha(\theta) \xrightarrow{f_\alpha(\theta)} \O_{\alpha+1}(f_\alpha(\theta))
%       % \end{equation}
%       % is a (trivial) cofibrations in $\V^{\Stab(\theta)}_{gen}$
%       % for all $\mathfrak C(\O_\alpha)$-signatures $\theta$,
%       % $f_\alpha(f_{\alpha-1}(\ksi))$ in one $\V^{\Stab(\ksi)}_{gen}$
%       % for any $\mathfrak C(\O_{\alpha-1})$-signature $\ksi$.
%       % %
%       and (trivial) cofibrations in $\V^{\Stab(\ksi)}_{gen}$ are closed under transfinite composition,
%       the result follows.
% \end{proof}
...
% \begin{lemma}
%       \label{LOCAL_COF_LEM}
%       %Suppose $\V$ is a cofibrantly generated closed monoidal model category with cellular fixed points
%       Suppose $\V$ has cofibrant symmetric pushout powers.
%       If $f: \O \to \P$ in $\Op^{G, \mathfrak C}(\V)$ is a (trivial) $\mathrm{Gr}$-cofibration,
%       then each $f(\ksi)$ is a (trivial) cofibration in $\V^{\Stab(\ksi)}_{\F_\ksi, mix}$.
% \end{lemma}
% \begin{proof}
%       By cellularity of cofibrations, it suffices to show that, for
%       any (trivial) cofibration $i: A \to B$ in $\V$,
%       $\mathfrak C$-signature $\theta$, and $H \in \F_\theta$,
%       the map
%       \begin{equation}
%             \mathbb F^{G, \mathfrak C}
%             \left(
%                   \Hom_{G \ltimes \Sigma_{\mathfrak C}}(-, \theta)/H \cdot 
%                   \left( A \xrightarrow{i} B \right)
%             \right)
%             % \mathbb F^{G, \mathfrak C}\left(\Hom_{G \ltimes \Sigma_{\mathfrak C}}(-, \theta)/H \cdot B \right)
%       \end{equation}
%       is a level genuine (trivial) cofibration.
%       As $\Stab(\ksi) \cdot_{\Aut(U, \mathfrak d)}(-): \V^{\Aut(U,\mathfrak d)}_{gen} \to \V^{\Stab(\ksi)}_{gen}$
%       is left-Quillen, it suffices by the filtration \eqref{FGC_EXPLICIT_EQ} to show that
%       \begin{equation}
%             \bigotimes_{v \in V(U)}\left( \Hom_{G \ltimes \Sigma_{\mathfrak C}}(\ksi_v, \theta) \cdot i) \right)
%       \end{equation}
%       is a (trivial) cofibration in $\V^{\Aut(U,\mathfrak d)}_{gen}$
%       for all $(U, \mathfrak d) \in \Omega^{G, \mathfrak C}$ with $\mathsf{lr}(U, \mathfrak d) = C_\ksi$.

%       By our assumption on $\V$ and \cite[Prop. 6.24]{BP17},
%       it thus suffices to show that each arrow in the composite
%       \begin{equation}
%             \prod_{|\lambda|}\V_{gen}^{\Sigma_{|\lambda_i|} \wr \Stab(\ksi_{v_i})}
%             \longto
%             \V^{\Aut((\ksi_{v_i}))}_{gen}
%             \xrightarrow{\ q^{\**} \ }
%             \V_{gen}^{\Stab(U,\mathfrak d)}
%       \end{equation}
%       is left Quillen,
%       where
%       $q: \Aut(U, \mathfrak d) \to \Aut((\ksi_v))$ is the natural homomorphism
%       (with automorphism groups in the categories $G \ltimes \Omega_{\mathfrak C}$ and $F \wr (G \ltimes \Sigma_{\mathfrak C})$ respectively),
%       and $\lambda = \amalg_i \lambda_i$ is the partition of $V(U,\mathfrak d)$
%       with vertices in the same class iff they are isomorphic in $G \ltimes \Sigma_{\mathfrak C}$.      
%       This holds by \cite[Remark 5.71]{BP17}.
%       %%%%%% DOING THIS WRONG - CONSIDERED WRONG SHIT DAMMIT ------------------------------
%       % For the right map, for any $\Gamma \in \mathrm{Gr}_{(U,\mathfrak d)}$, let $\Gamma_k = \pi_k^{\**}(\Gamma)$, where
%       % \begin{equation}
%       %       \pi_i: \Aut(U,\mathfrak d)
%       %       \longto
%       %       \Aut((\ksi_{v}))
%       %       \simeq
%       %       \prod_{|\lambda|}\Sigma_{|\lambda_i|} \wr \Aut(\ksi_{v_i})
%       %       \longto
%       %       \Sigma_{|\lambda_k|} \wr \Aut(\ksi_{v_k}).
%       % \end{equation}
%       % Now, given $v_{kj} \in \lambda_k$, the group
%       % \begin{equation}
%       %       \pi_{v_{kj}}
%       %       \left(
%       %             \Gamma_k \cap \Sigma_{{\lambda_k}_{v_{kj}}} \wr \Aut(\ksi_{v_{kj}})
%       %       \right)
%       % \end{equation}
%       % is given by those $(g,\sigma)$ such that there exists $(g, \phi(g)) \in \Gamma$ such that
%       % $g.v_{ki} = v_{ki}$, $\phi(g)$ and $\phi(g)$ acts by $\sigma$ on $v_{ki}$.
%       % Letting $G_{ki} \leq G$ denote the subgroup which fixes $v_{kj}$, this is the graph subgroup on the natural map
%       % $G_{jk} \to \Aut(\ksi_{kj})$.

%       % THIS FOLLOWS FROM THE EQUIVALENT VERSION OF 6.59 FOR $\mathfrak C$-COLORED (NON-GENUINE) OPERADS. WHICH IS HARD.
% \end{proof}
