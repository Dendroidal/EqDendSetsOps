...
Further, if $A$ is a $G \times \Sigma$-set, then
$G \ltimes (\Sigma \ltimes A) \simeq \Sigma \ltimes (G \ltimes A) \simeq (G \times \Sigma) \ltimes A$.
...
\begin{theorem}
      ...
      \item \label{CSPP_LBL} has cofibrant symmetric pushout powers,
            \footnote{Also needed for Lemmas \ref{CAV_4.14_PROP2, LOCAL_COF_LEM}.},            
            % --------------------
...
\begin{proof}
      ...
      \end{equation}
      in $\Op^G(\V)$, letting $\bar f_\alpha$ denote the compoite $\O_0 \to \O_\alpha$,
      such that the maps
      \begin{equation}
            \O_\alpha(\theta) \xrightarrow{f_\alpha(\theta)} \O_{\alpha+1}(f_\alpha(\theta))
      \end{equation}
      are (trivial) cofibrations in $\V^{\Stab(\theta)}_{\mathrm{Gr}_\theta}$
      for all $\mathfrak C(\O_\alpha)$-signatures $\theta$.

      Now, suppose $\theta = f_{\alpha-1}(\ksi)$, so
      $\Stab(\ksi) \leq \Stab(\theta)$.
      Then the restriction map $\V^{\Stab(\theta)}_{\mathrm{Gr}_\theta} \to \V^{\Stab(\ksi)}_{\mathrm{Gr}_\ksi}$
      preserves (trivial) cofibrations (cf. \cite[Prop 6.6]{BP17}),
      and thus $f_\alpha(\theta)$ 
      restricts to the (trivial) cofibration $f_\alpha(f_{\alpha-1}(\ksi))$ in $\V^{\Stab(\ksi)}_{\mathrm{Gr}_\ksi}$.
      %
      Thus, when evaluated at any $\mathfrak C(\O_0)$-signature $\ksi$,
      the above string of maps yields a string of (trivial) cofibrations in $\V^{\Stab(\ksi)}_{\mathrm{Gr}_\ksi}$
      \begin{equation}
...
\todo[inline]{Using a big hammer,
  the following would be a consequence of a full discussion of $\mathfrak C$-operads, in particular
  the version of \cite[Lemma 6.59]{BP17} for $\mathfrak C$-colored (non-genuine) operads.
  Though the following might also work:}
\begin{lemma}
      \label{LOCAL_COF_LEM}
      Suppose $\V$ % is a cofibrantly generated closed monoidal model category with cellular fixed points
      additionally has cofibrant symmetric pushout powers,  
      % Suppose $\V$ admits transfer for operads, and suppose
      and let
      $f: \O \to \P$ in $\Op^{G, \mathfrak C}(\V)$ be any (trivial) $\mathrm{Gr}$-cofibration.
...
\begin{proof}
      ...
      $\mathfrak$-signature $\theta$, and $H \in \F_\theta$,
      the map
      \begin{equation}
      ...
      \end{equation}
      is a level (trivial) $\mathrm{Gr}$-cofibration.
      As $\Stab(\ksi) \cdot_{\Aut(U, \mathfrak d)}(-): \V^{\Aut(U,\mathfrak d}_{\mathrm{Gr}} \to \V^{\Stab(\ksi)}_{\mathrm{Gr}_\ksi}$
      is left-Quillen, it suffices by the filtration \eqref{FILTRATION_LAN_LEMMA} to show that
      \begin{equation}
            \bigotimes_{v \in V(U)}\left( \Hom_{G \ltimes \Sigma_{\mathfrak C}}(\ksi_v, \theta) \cdot i) \right)
      \end{equation}
      is a (trivial) cofibration in $\V^{\Aut(U,\mathfrak d}_{\mathrm{Gr}}$
      for all $(U, \mathfrak d) \in \Omega^{G, \mathfrak C}$ with $\mathsf{lr}(U, \mathfrak d) = C_\ksi$.

      Stated generally, it thus suffices to show that each arrow in the composite
      \begin{equation}
            \prod_{|lambda|}\V_{\mathrm{Gr}_{\ksi_{v_i}}}^{\Sigma_{|\lambda_i|} \wr \Stab(\ksi_{v_i})}
            \longto
            \V^{\Aut((\ksi_{v_i}))}_{\sqcap \mathrm{Gr}_{\ksi_{v_i}}^{\ltimes |\lambda_i|}}
            \xrightarrow{\ q^{\**} \ }
            \V_{\mathrm{Gr}_{U}}^{\Stab(U,\mathfrak d)}
      \end{equation}
      is left Quillen,
      where $\lambda = \amalg_i \lambda_i$ is a partition of $V(U,\mathfrak d)$
      where vertices are in the same class iff they are isomorphic in $G \ltimes \Sigma_{\mathfrak C}$,
      and $\mathrm{Gr}_{(U,\mathfrak d)}$ is the family of all graph subgroups of $G \times \Sigma_U$ contained in $\Aut(U, \mathfrak d)$.
      
      The left map is left Quillen by \cite[(6.15)]{BP17}.
