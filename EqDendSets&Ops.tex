...
\begin{definition}
      ...
      \item \textit{virtually $H$-equivalent } if
            \todo[inline]{come back: which one of these allows for the proof in \S 5.3.1 ?
              (or is it both, cf. the next remark)}
            \begin{enumerate}[label = (\roman*)]
      ...
      \item \textit{(virtually, homotopy) equivalent}
            if there exists an $H$ such that they (virtually, homotopy) $H$-equivalent.
      \end{itemize}
\end{definition}

\begin{remark}
      \label{HK_EQUIV_REM}
      If $K \leq H \leq G$, then $H$-equivalent implies $K$-equivalent.

      If $\V$ has a fibrant replacement functor that commutes with taking fixed points
...
\begin{definition}
      ...
      {\em $(H)$-equivalent} (resp. {\em virtually $(H)$-equivalent}, {\em homotopy $(H)$-equivalent}) if they are so in $j^*\O$. 
\end{definition}
...
\begin{proof}
      {\color{OliveGreen}
        ...
        The ``if'' direction is immediate using postcomposition by fibrant replacement.
        The ``only if'' direction follows as an immediate consequence of \textit{loc cite},
        once we complete their argument.
      }
      
      Let $\C_f$ denote $(\O^H)_f$.
...
The following two lemmas show that in good cases, a homotopy equivalence of colors induces a homotopy equivalence of objects.
...
\begin{proof}
      ...
      % The above sentence is where we need that $(-)^H$ and $(-)_f$ commute.
...
\begin{lemma}
      ...
      % \begin{enumerate}[label=(\roman*)]
      % \item $c$ and $d$ are homotopy $H$-equivalent with $H = \Stab(c)$, or
      % \item
...
\begin{proof}
      ...
      representing the homotopy $H$-equivalence,
      with $H_{\beta\alpha,1}$ and $H_{\alpha\beta,1}$ the associated
      ...
      % That $k_i interchnages $\O^H_f$ and $\O^{H_i}_f$ requires that $(-)^H$ and $(-)_f$ commute.
      ...
      We firstly claim that $\Stab_{G \times \Sigma_n}(\ksi) \subseteq _{G \times \Sigma_n} \Stab(\theta)$.
      ...
      \begin{equation}
            \begin{tikzcd}
                  ...
                  \V(\O(\ksi)^\Gamma, \O(\ksi)^\Gamma)
                  % &
                  % 1_\V \amalg 1_\V \arrow[rr, "{((\beta_i)_{\**}, id)}"] \arrow[d]
                  % &&
                  % \V(\O(\ksi_c^d)^\Gamma, \O(\ksi_c^d)^\Gamma)
                  \\
                  ...
                  % &
                  % \O(d_1;d_1)^{H_\Gamma} \arrow[r, "\Delta"]
                  % &
                  % \left(
                  %       \bigotimes_\lambda \O(d_i;d_i)
                  % \right)^\Gamma
                  % \arrow[u, "{(-)_{\**}}"]
            \end{tikzcd}
      \end{equation}
      ...
      Hence
      $(\alpha_i)_{\**}: \O(\ksi)^\Gamma \to \O(\ksi_c^d): (\beta_i)_{\**}$
