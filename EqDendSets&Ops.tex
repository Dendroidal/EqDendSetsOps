...
\begin{remark}
      ...
      \begin{equation}
            ...
            \begin{tikzcd}[row sep = tiny]
                  ...
                  \\
                  (\phi: S \to T,g) \arrow[r, mapsto]
                  &
                  (E(S) \xrightarrow{\phi} E(T) \xrightarrow{\mathfrak c} \mathfrak C \xrightarrow{g^{-1}} \mathfrak C)
            \end{tikzcd}
      \end{equation}
      \todo[inline]{compare with genuine case: RHS equals
        $\Hom(\Phi(E(T)), \mathfrak C_{const})$ equals
...
\begin{remark}
      ...
      %       \end{tikzcd}
      % \end{equation}
      Further, this is equal to the Grothendieck construction 
      \begin{equation}
            ...
            \end{tikzcd}
      \end{equation}
      In this context, a map $\phi: (S,\mathfrak c) \to (T, \mathfrak d)$ of colored $G$-trees is itself colored iff
      the triangle below commutes.
      \begin{equation}
            \label{COLORED_MAP_FG_EQ}
            \begin{tikzcd}[row sep = tiny]
                  \Phi(G \cdot E(S)) \arrow[rr, "\phi"] \arrow[dr, "\mathfrak c"']
                  &&
                  \Phi(G \cdot E(T)) \arrow[dl, "\mathfrak d"]
                  \\
                  &
                  \UC
            \end{tikzcd}
...
\begin{definition}
      ...
      Then the category $\underline{\mathfrak C}\Omega_G$ of \textit{$\underline{\mathfrak C}$-colored $G$-trees}
      ...
      \begin{equation}
            ...
            \end{tikzcd}
      \end{equation}
      The category $\CS$ of $\underline{\mathfrak C}$-colored corollas is the subcategory defined similarly,
...
is assigned a ``color'' $\mathfrak c(Ge) \in \underline{\mathfrak C}(G/G_{e})$.
...
\begin{remark}
      { \color{blue} % -------------------- ALTERNATIVELY: ----------------------------------------
        Alternatively,
        then $\UC\Omega_G$ is also isomorphic to the pullback below\footnote{We note that the class of morphisms in $\mathsf F^G$ in the image of $E$ (restricted to $\Omega_G^0$)
        ...
        \begin{equation}
              ...
              \end{tikzcd}
        \end{equation}
        In this case, a coloring is a map $\mathfrak c: \Phi E(T) \to \mathfrak C$ of coefficient systems,
...
{\color{red} In this section, assume that the canonical model structures on $\Cat(\V)$, $\Op(\V)$, and $\Op^{G,\C}(\V)$ exist.}
