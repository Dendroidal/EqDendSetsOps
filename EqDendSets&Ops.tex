...
\begin{remark}
      (Coloured) operads are also known as \textit{symmetric multicategories} \todo{cite stuff}.
\end{remark}
...
As most of the construction/category theory is on the level of strings (or formal due to pullback constructions),
...
The same proof as before gives this too.
\begin{proposition}
...
\begin{proposition}
      ...
      Then the category of diagrams $\V^{\mathcal D}$ has a model structure where
      a map $f: X \to Y$ is a
      weak equivalence (resp. fibration) iff $f(d): X(d) \to Y(d)$ is so in $\V^{\mathsf{Aut}(d)}_{\F_d}$ for each $d \in \mathcal D$.
\end{proposition}
\begin{proof}
      This is the model structure transfered along the adjunction
      \begin{equation}
            \begin{tikzcd}
                  \V^\D \leftrightarrows
                  \prod_{d \in \D}\V^{\mathsf{Aut}(d)}_{\F_d}
            \end{tikzcd}
      \end{equation}
      which exists by adapting and combining the proofs of \cite[Thm 11.6.1]{Hir03} and \cite[Prop 2.6]{Ste16}.
\end{proof}
...
\begin{example}
      ...
      Then $\Sym^{G,\mathfrak C}(\V) = \Fun(\UC\Sigma, \V)$ has an $\F$-model structure
      \begin{equation}
            \Sym^{G,\mathfrak C}_\F(\V) = \prod_n \V^{B_{\mathfrak C^{\times n+1}}(G \times \Sigma_n)}_{\F_n},
      \end{equation}
      where $\V^{B_{\mathfrak C^{\times n+1}}(G \times \Sigma_n)}_{\F_n}$ has the model structure lifted from the adjunction to
...
equipped with $\UC$-colourings of the associated $G$-corollas $C_\Gamma$.
...
$B_{\UC^{\times n+1}} \to \UC O_{\F_n}$
sending the tuple $(c_1,\dots,c_n;c_0)$ to
...
\begin{lemma}
      ...
      where $c_i := \mathfrak c(G/e, [1, i])$.
\end{lemma}
\begin{lemma}
      This and $\iota^{\**}$ form a Quillen adjunction.
\end{lemma}
\begin{proof}
      \todo[inline]{make sure to type out $\epsilon$ and $\eta$}
      $\iota_{\**}$ again fully-faithful since $\epsilon$ is an iso.

      Again clear that $\iota_{\**}$ is right Quillen.
\end{proof}
...
\end{proposition}
\begin{proof}
      \todo[inline]{come back: make sure to type up explicitly}
\end{proof}
