...
\begin{proof}
      ...
      By the existance of the $\F$-model structure on $\Op^{G, \mathfrak C_{\P}}(\V)$ by Theorem \ref{THM1_C},
      ...
      Hence $\phi'$ is a trivial $\F$-cofibration in $\mathsf{Op}^{G,\C_\O}(\V)$,
...
\begin{example}
      ...
      \item If $\V$ satisfies Lurie's \textit{invertibility hypothesis} \cite[A.3.2.12]{Lur09}, then $\V$ satisfies the coherence axiom.
...
\todo[inline]{come back: make sure I'm done here?}
...
\begin{example}
      ...
      coherent by \cite[Lem. 4.16]{BV73}.
\end{example}
