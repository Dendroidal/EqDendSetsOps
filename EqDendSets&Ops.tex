...
with perhaps the most useful being the ``graph subgroup equivalences'' (see, e.g. \cite{BP17}, \cite{BH15}),
...
\section{Homotopy theory of operads with a fixed $G$-set of colors}
...
\begin{definition}[{cf. \cite[Defn. 5.38]{BP17}}] 
...
\begin{proof}
      ...
      by Remark 5.36 and Proposition 5.41 of \cite{BP17}.
...
\begin{definition}[{\cite[Defn. 5.44,5.48]{BP17}}]
...
\begin{remark}
      \label{LEVEL_COF_REM}
      An immediate consequence of cellularity (see \cite[Prop. 6.3(i)]{BP17})
...
\todo[inline]{come back: move to appendix}
% appendix 
...
We will now generalize and synthesize \cite{BM13}, \cite{Cav14}, and \cite{CM13b} to assemble the collection of
...
\begin{proof}
      ...
      $(iii)$ $\sSet$ has cofibrant symmetric pushout powers by \cite[Remark 6.18]{BP17},
...
\begin{remark}
      \label{WEAKCELL_REM}
      Conveniently, at no place in our proof of Theorem \ref{MODEL_THM} (including importantly Theorem I and II from \cite{BP17})
      ...
      The reason we (and \cite{Ste16}) use cellular fixed points is that the full condition (iii) is what really allows us to say anything meaningful about the \textit{cofibrant} objects in our category (see, e.g. \cite[Prop. 6.56 and Lemma 6.59]{BP17}).
\end{remark}
...
\begin{example}
      ...
      $\V$ the category of simplicial modules over a ring? \cite[\S 3.1.15]{Rez96}  (from \cite[Example 4.23]{Cav14}).
\end{example}
...
\begin{lemma}
      All cofibrations power cofibrations implies the commutative monoid axiom \cite[Lemma 4.5.4.11]{Lur17}.

      All cofibrations power cofibrations implies the cofibrant symmetric pushout powers axiom (obvious).
\end{lemma}

\todo[inline]{how much of \cite{WY15} extends via this arrangement?}
...
\begin{corollary}
      [{\cite[Prop. 6.15]{Per17}, cf. \cite[Cor. 4.6]{CM11}}]
...
\begin{proof}
      ...
      By \cite[Thm. 8.22]{Per17}, this is a fibration in $\dSet^G$ iff $\tau (h c N_d(f)^H) = \tau (h c N_d(f^H))$ is a categorical fibration for all $H \leq G$.
