...
\begin{remark}
      ...
      Consequently, for any fibration $f \in \sOp^G$, $\pi_0(f^H)$ is an isofibration.
\end{remark}
...
\begin{lemma}
      \label{PT_CELL_LEM}
      Suppose $\V$ cofibrantly generated monoidal model category with cellular fixed points,
      and further that every object in $\V$ is cofibrant.
...
\begin{proof}
      ...
      where both the front and back panels are pushouts.
      \begin{equation}
            \begin{tikzcd}[row sep = tiny]
...
\begin{remark}
      We record that the additional assumption, that all objects in $\V$ are cofibrant, was only used to prove condition $(iii)$.
\end{remark}
...
\begin{proposition}
      \label{VPT_INT_PROP}
      Suppose $\V$ is a cofibrantly generated monoidal model category with cofibrant unit and cofibrant symmetric pushout powers,
      and moreover $\V$ is left proper and $\**$ is cofibrant in $\V$.
...
\begin{proof}
      ...
      \begin{equation}
            ...
            \end{tikzcd}
      \end{equation}
      Since $\V$ is left proper and $\**$ is cofibrant, $(-)_+: \V \to \V_{\**}$ (and hence $\Cat^{\mathfrak C}(\V) \to \Cat^{\mathfrak C}(\V_{\**})$)
      preserves \textit{all} weak equivalences, as indicated by the following pushout square.
      \begin{equation}
            \begin{tikzcd}
            ...
            \end{tikzcd}
      \end{equation}
      Thus $\J_+$ is a $\V_{\**}$-interval, as desired.
\end{proof}
...
\begin{proof}
      ...
      This is adjoint to $U\alpha$, which thus factors through a natural cofibration $\mathbb A \rightarrowtail \J$.
      Applying $(-)_+$ yields the diagram below, for which the top arrow is again $\alpha$ by adjointness.
      \begin{equation}
            \begin{tikzcd}
                  \mathbb A_+ \arrow[d, rightarrowtail] \arrow[r, "\alpha_+"]
                  &
                  (U \C_f)_+ \arrow[r]
                  &
                  \C_f
      ...
      This too holds by the following diagram.
      \begin{equation}
            \begin{tikzcd}
...
\end{corollary}
\begin{proof}
      Combine Lemmas \ref{PT_PROP_LEM} and \ref{PT_CELL_LEM} with Proposition \ref{PT_COH_PROP}.
\end{proof}
...
\begin{example}
      ...
      \item has cellular fixed points by \cite{Pia91};
      ...
      Moreover, we can manually check that $\Top_{\**}$ satisfies condition $(iii)$ of cellularity,
      and thus Lemma \ref{PT_PROP_LEM} and Proposition \ref{PT_COH_PROP}
      imply that $\Op^G(\Top_{\**})$ has the Dwyer-Kan model structure.
\end{example}
...
\begin{proof}
      ...
      Pick a decomposition $T \simeq G \cdot_H U$ and consider $G/H \cdot i: G \cdot_H \left( W_! \partial \Omega[U] \to W(U) \right)$.      
      ...
      Again, $j$ is bijective on objects and $j_C$ is the identity unless $C = h \cdot \mathsf{lr}(U)$.
      When $C = \mathsf{lr}(U)$, $j_C$ is the pushout product
      \begin{equation}
            j_C =
            \left(
                  \begin{tikzcd}
...
\begin{proof}
      ...
      Then Proposition \ref{W!_COF_PROP} also implies that $f$ is a $G$-inner-fibration between $G$-$\infty$-operads.
      By \cite[Cor. 8.21]{Per17}, this is a fibration in $\dSet^G$ iff $\tau(h c N(f^H))$ is a categorical fibration for all $H \leq G$.
...
\begin{remark}
      ...
      This story, along with colored genuine equivariant operads, will be further explored in a sequel. 
\end{remark}
