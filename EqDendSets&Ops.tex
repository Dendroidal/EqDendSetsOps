...
\begin{definition}
      ...
      \item For all signatures $\ksi \in \mathfrak C^{\times n+1}$ and $\sigma \in \Sigma_n$,
            ...
            \end{equation}
      \item For all compatible signatures $\xi = (c_1, \ldots, c_n; c_0)$, $\xi_i = (c_{1}^i, \ldots, c_{m_i}^i; c_i)$,
...
\begin{convention}
      ...
      On top of this, though, we will abuse notation and denote the category of left $G$-objects in $\V$ as $\V^G$
      (as opposed to the more accurate $\V^{G^{op}}$).
\end{convention}
...
$\underline{c}_d$ is a $G/e$-indexed diagram $E G := (G \ltimes (G/e))^{op} \to \mathcal C_d$\footnote{
...
\begin{equation}
      ...
      \end{tikzcd}
\end{equation}
sending maps $(\phi, g)$ to $(-) \cdot g \circ \phi$.
...
$(T_i, \mathfrak c_i) \in G \ltimes \Omega_{\UC})$ by choosing $\mathfrak c_i: E(T_i) \to \mathfrak C(G)$ to be
\begin{equation}
...
\end{remark}
\todo[inline]{come back: make sense of the argument in the comments below, see what is truly needed.}
