...
% We recall that a (very good) \textit{cylinder object} in a model category $\V$ is an object $\mathbb C \in \V$ equipped with a factorization
% \begin{equation}
%       \begin{tikzcd}
%             1_\V \amalg 1_\V \arrow[r, tail]
%             &
%             \mathbb C \arrow[r, two heads, "\sim"]
%             &
%             1_\V
%       \end{tikzcd}
% \end{equation}
% of the fold map\footnote{
%   These are cylinder objects of the unit.}.
% These are used to define homotopies in our model category.
% \begin{definition}
%       Given $X \in \V$, we say two maps $f,g: 1_\V \to X$ are \textit{homotopic} if we have a factorization as below.
%       \begin{equation}
%             \begin{tikzcd}
...
%                   \\
%                   &
%                   \mathbb C \arrow[ur, dashed]
%             \end{tikzcd}
%       \end{equation}
%       In particular, for any $\mathcal C \in \Cat(\V)$, this gives a notion of a homotopy between maps in
%       any hom-object $\mathcal C(x,y)$.
% \end{definition}
...
\end{theorem}
\todo[inline]{compare with Theorem \ref{MODEL_THM}.}
...
\begin{lemma}
      ...
      Then levelwise (trival) cofibrations are closed under transfinite composition.
\end{lemma}
\begin{proof}
      ...
      \end{equation}
      As (trivial) cofibrations in $\V^{\Stab(\ksi)}_{\F_\ksi}$ are closed under transfinite composition, the result is proven.
\end{proof}

\begin{lemma}
      \label{LEVEL_COF_LEM}
      If $f: \O \to \P$ in $\Op^{G, \mathfrak C}(\V)$ is a (trivial) $\F$-cofibration, it is a levelwise (trivial) $\F$-cofibration.
\end{lemma}
\begin{proof}
...
\begin{proof}
      ...
      by Lemma \ref{COF_TC_LEM}, it suffices to prove that the pushout of a map $j \in J_{\F}$ is a levelwise $\F$-cofibration.
      ...
      and hence a level trivial $\F$-cofibration by Lemma \ref{LEVEL_COF_LEM}. 
      ...
      If $\V$ only admits semi-transfer, then the extra assumption that $\O$ is cofibrant is sufficient to run an identical argument
      (as the utilized pushouts remain weak equivalences in the underlying semi-model structure).
      \todo[inline]{come back: make sure $\F$'s appear where they should in the above}
\end{proof}
...
\begin{remark}
      \label{HK_EQUIV_REM}
      We note that if $K \leq H \leq G$, then (virtual, homotopy) $H$-equivalent implies (virtual, homotopy) $K$-equivalent
...
\begin{proof}
      ...
      Now, we may consider $\O_f(a;b) \in \V^{\Stab(\ksi)}$ via pullback on the projection $\Stab(\ksi) \to H$,
...
\begin{proof}
      ...
      Chose a set of lifts (cf. \eqref{FIBFIX_LIFT_EQ})
      \begin{equation}
...
\begin{proof}
      ...
      Moreover, if $b \in \P^H$, then by Remark \ref{ESS_SUR_REM}, there exists $a \in \O^H$ such that
...
\begin{theorem}
      ...
      Fix a weak indexing system $\F$, and
      ...
      $\F$-fibrations, weak $\F$-equivalences, generating $\F$-cofibrations, and generating trivial $\F$-cofibrations
      as defined in Definition \ref{MODEL_DEFN}.
\end{theorem}
...
\begin{theorem}
      ...
      %       \footnote{Needed for \ref{J-CELL_LEMMA}}.
      \end{enumerate}
      Then there exists a cofibrantly generated semi-model structure $\Op^G_\F(\V)$ on $\mathsf{Op}^G(\V)$ with
      $\F$-fibrations, weak $\F$-equivalences, generating $\F$-cofibrations, and generating trivial $\F$-cofibrations
      as defined in Definition \ref{MODEL_DEFN}.
\end{theorem}
\begin{proof}
      ...
      After this difference, the proofs are identical, as
...
\begin{proof}
Firstly, we show that this is a Quillen adjunction.
