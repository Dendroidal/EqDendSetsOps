...
\begin{definition}
      ...
      \item \textit{virtually $H$-equivalent } if
            ...
            % \item they are $H$-equivalent in some fibrant replacement $\mathcal C_f$ of $\mathcal C$
...
Next goal: Dwyer-Kan equivalences.

\begin{definition}
      $F: \O \to \P$ in $\Op^G(\V)$ is a \textit{Dwyer-Kan} (or \textit{DK}) \textit{$\F$-equivalence} if
      $F$ is a local weak $\F$-equivalence and
      $\pi_0F^H$ is an equivalence of operads for all $H \leq G$, ie.
      $\pi_0F^H$ is a local bijection and $j^{\**}\pi_0 F^H$ is essentially surjective.
\end{definition}

\begin{lemma}
      If the trivial subgroups $H \leq G \times \Sigma_n$ are $\F$-admissible for all $H$ and $n$
      (equivalently, if $\F$ is a true indexing system) then
      local weak $\F$-equivalences are local $\pi_0$-bijections.
\end{lemma}
\begin{proof}
      $\O(\ksi) \to \P(F(\ksi)$ an $\F_\ksi$-equivalences implies that
      $\O^H(\ksi) \to \P^H(F(\ksi))$ is a weak equivalence in $\V$,
      and hence the $\O^H(\ksi)$ and $\P^H(F(\ksi))$ are isomorphic in $\Ho(\V)$,
      implying the result.      
\end{proof}
...
\begin{itemize}
	...
	and $\Omega[\eta] \to I$ a weak equivalence in $\mathsf{PreOp}$ such that $I(\eta) = \{0,1\}$ and $\Omega[\eta] \amalg \Omega[\eta] \to I$ is a tame cofibration;
